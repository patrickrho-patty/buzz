...
//! cargo test -p crew-test-client --test conformance_multitenant -- --ignored
//! ```
//!
...
mod row_zero_host_binding {
    ...
    /// properties of it (see channel: `crew-relay-rewrite`, 2026-06-27):
    ///
    ///   * [`super::channels_membership::same_channel_uuid_in_two_communities_is_isolated`]
    ///     (Mari, `crew-db`) asserts **coexistence**: a channel UUID that exists
...
mod nip11_relay_info {
    ...
    /// (`crates/crew-relay/src/nip11.rs::_RELAY_INFO_BUILD_STATIC_INPUT_FENCE`):
...
// ---------------------------------------------------------------------------
// API tokens and NIP-98 replay (Sami — crew-auth)
// ---------------------------------------------------------------------------
mod api_tokens_nip98_replay {
    ...
    /// in `crew-relay`** — there is no route by which a client can bind a token
    /// to a community over HTTP, so the "mint in A, present to B" precondition
    /// has no entry point.
    ///
    /// Verified on PR head:
    ///   * `crates/crew-relay/src/router.rs:52-79` — full route list is `/`,
    ...
    ///   * `crates/crew-relay/src/api/` directory is `{bridge, events, media,
    ...
    /// contract (its deps are `crew-ws-client`/`reqwest`/`tokio-tungstenite`/
    /// `s3` — no `sqlx`, no `crew-db`).
    ///
    ...
    ///
    ///   1. `crates/crew-db/src/api_token.rs:425
    ...
    ///   2. `crates/crew-db/src/api_token.rs:488
    ...
    fn token_minted_in_a_does_not_authorize_in_b() {
        ...
        // If anyone adds a `/tokens` route to `crew-relay`, this row's shape
    ...
    /// in `crates/crew-relay/src/api/bridge.rs:79` (return `Ok(())` before
    ...
    /// (`crates/crew-auth/src/nip98_replay.rs:163
    ...
    /// (relay-membership is open under `CREW_REQUIRE_AUTH_TOKEN=false`).
    #[tokio::test]
    #[ignore]
    ...
    /// the pattern in `crates/crew-auth/src/nip98.rs`; kept local to this
...
// ---------------------------------------------------------------------------
// Membership / allowlist / archived identities (Sami — crew-auth)
// ---------------------------------------------------------------------------
...
mod users_profiles_nip05 {
    ...
    /// bridge endpoint; with `CREW_REQUIRE_AUTH_TOKEN=false` the dev-mode
    ...
    /// `crates/crew-relay/src/api/nip05.rs::extract_domain` — strips the
    ...
    /// `crates/crew-db/src/user.rs:140` (`UPDATE users SET ... WHERE
    /// community_id = $N AND pubkey = $M`) AND the event-storage fence at
    /// `crates/crew-db/src/event.rs::get_events_by_ids` (or the equivalent
    ...
    /// `crates/crew-relay/src/handlers/side_effects.rs::handle_kind0_profile`
    /// validates the handle via `crates/crew-relay/src/api/nip05.rs::canonicalize_nip05`
    ...
    /// from `crates/crew-db/src/user.rs::get_user_by_nip05` (line 185 form
...
// ---------------------------------------------------------------------------
// Feed read-side isolation (Perci — crew-db/relay bridge)
// ---------------------------------------------------------------------------
mod feed_read_side_isolation {
    ...
    /// `crates/crew-db/src/feed.rs`; this row is the wire-level conformance
...
// ---------------------------------------------------------------------------
// Channels and channel membership (Mari — crew-db)
// ---------------------------------------------------------------------------
mod channels_membership {
    ...
    /// `crates/crew-relay/src/handlers/side_effects.rs::handle_create_group`.
    ...
    /// `CREW_REQUIRE_AUTH_TOKEN=false` recipe). Returns the events as their
    ...
    /// `crates/crew-db/src/event.rs::query_events` (the non-p-tag branch,
...
mod workflows {
    ...
    /// the REST bridge (`POST /events`). In dev mode (`CREW_REQUIRE_AUTH_TOKEN
    ...
    /// `crates/crew-relay/src/handlers/command_executor.rs:703`
    ...
    /// (`crates/crew-workflow/src/lib.rs` — "approval gates not yet implemented,
    ...
    /// minting), **not** crew-db — the scoping fence it will exercise is
    /// already landed.
    #[tokio::test]
    #[ignore]
...
mod search_fts {
    ...
    ///   - `crates/crew-search/src/query.rs::search` lines 160-161 (the FTS
    ///     `WHERE community_id = $ctx` predicate); and
    ///   - `crates/crew-db/src/event.rs::get_events_by_ids` lines 870-872
...
// ---------------------------------------------------------------------------
// Redis pub/sub, presence, typing, cache invalidation (Max — crew-pubsub)
// ---------------------------------------------------------------------------
mod pubsub_presence_typing {
    ...
    ///     `crew-pubsub/src/presence.rs::presence_key` (shared Redis key). The B
...
// ---------------------------------------------------------------------------
// Media / Blossom / S3 (Perci — crew-media)
// ---------------------------------------------------------------------------
mod media_blossom {
    use super::*;
...
// ---------------------------------------------------------------------------
// Git hosting / NIP-34 / object storage (Perci — crew-media/git)
// ---------------------------------------------------------------------------
mod git_hosting {
    use super::*;
...
// ---------------------------------------------------------------------------
// Audit log and observability (Dawn — crew-audit)
// ---------------------------------------------------------------------------
mod audit_log {
    ...
    //! `crates/crew-relay/src/router.rs` (the route list is `/`, `/info`,
    ...
    //! (its deps are `crew-ws-client`/`reqwest`/`tokio-tungstenite`/`s3` — no
    //! `sqlx`, no `crew-audit`), and a DB-direct read can never catch a
    ...
    //!
    //! Substrate on PR head: `crates/crew-audit/src/entry.rs` keys `AuditEntry`
