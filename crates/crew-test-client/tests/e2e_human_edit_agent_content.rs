...
async fn connect_agent_with_owner(agent_keys: &Keys, owner_keys: &Keys) -> CrewTestClient {
    let url = relay_url();
...
async fn test_owner_can_edit_agent_message() {
    ...
    let mut owner_client = CrewTestClient::connect(&relay_url(), &owner_keys)
        .await
...
async fn test_third_party_cannot_edit_agent_message() {
    ...
    let mut third_party_client = CrewTestClient::connect(&relay_url(), &third_party_keys)
        .await
...
async fn test_removed_author_cannot_edit_own_message_in_private_channel() {
    ...
    let mut victim_client = CrewTestClient::connect(&relay_url(), &victim_keys)
        .await
