...
//!
//! ```text
//! RELAY_URL=ws://localhost:3001 cargo test -p crew-test-client --test e2e_event_reminder -- --ignored
//! ```
...
async fn test_other_user_subscription_closed_for_author_only_kind_ws() {
    ...
    let mut ws = CrewTestClient::connect(&url, &other_keys)
        .await
        .expect("connect");
...
async fn test_mixed_kind_filter_omits_other_authors_reminders_ws() {
    ...
    let mut ws_reader = CrewTestClient::connect(&url, &reader_keys)
        .await
...
async fn test_fanout_isolation_other_user_does_not_receive_reminder() {
    ...
    let mut ws_other = CrewTestClient::connect(&url, &other_keys)
        .await
    ...
    match result {
        Err(crew_test_client::TestClientError::Timeout) => {
