...
//!   cargo test -p crew-test-client --test e2e_git -- --ignored --nocapture
//! ```
...
use crew_media::S3AddressingStyle;
...
/// Run `git` with the Crew credential helper and isolated config.
...
/// Run `git` with the Crew credential helper and isolated config. Asserts the
...
fn git_s3_probe_builds_both_addressing_styles() {
    ...
    assert_eq!(path.url(), "https://storage.example/crew-media");
    ...
    assert_eq!(virtual_hosted.url(), "https://crew-media.storage.example");
}
