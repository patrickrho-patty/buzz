...
//!
//! The ingest unit tests in `crew-relay` pin the envelope contract in isolation.
...
//!
//! ```text
//! RELAY_URL=ws://localhost:3000 cargo test -p crew-test-client --test e2e_project -- --ignored
//! ```
    ...
    let cases: Vec<(&str, nostr::Event, &str)> = vec![
        ...
        (
            ...
            project_event(
                ...
                "Bad kind",
                &[format!("30618:{}:crew", owner.public_key().to_hex())],
                None,
            ),
        ...
        (
            ...
            project_event(
                ...
                "Uppercase",
                &[format!("{REPO_ANNOUNCEMENT_KIND}:{}:crew", "A".repeat(64))],
                None,
            ),
            "member `a` tag must be",
        ),
