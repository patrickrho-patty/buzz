...
fn valid_input() -> String {
    "capability[]=authtype\n\
     ...
     wwwauth[]=Nostr realm=\"crew\", method=\"GET\"\n\
     \n"
    .to_string()
}
...
/// A Crew-managed agent must carry its NIP-OA owner attestation inside the
...
fn includes_nip_oa_auth_tag_in_signed_event() {
    ...
    let out = run_helper(
        &valid_input(),
        &[("NOSTR_PRIVATE_KEY", &nsec), ("CREW_AUTH_TAG", &auth_tag)],
    );
    assert!(
        out.status.success(),
        "helper failed: {}",
...
fn malformed_nip_oa_auth_tag_fails_closed() {
    ...
    let out = run_helper(
        &valid_input(),
        &[("NOSTR_PRIVATE_KEY", &nsec), ("CREW_AUTH_TAG", "not-json")],
    );
