...
NIP-98 credential helper for git — signs HTTP auth events with your Nostr key so git can push/pull from Crew's git server without passwords.
...
When a Crew git server returns `HTTP 401` with a
...
| `method hint` | Server's `WWW-Authenticate` header is missing `method="..."` | Upgrade the Crew server |
