//! Harness-side Crew relay client.
//!
//! Connects to the Crew relay via NIP-01 WebSocket, authenticates via NIP-42,
...
///
/// Connects to the Crew relay, authenticates via NIP-42, and streams
...
impl HarnessRelay {
    ...
    pub async fn discover_channels(&self) -> Result<HashMap<Uuid, ChannelInfo>, RelayError> {
        ...
        let member_filter = nostr::Filter::new()
            .kind(Kind::Custom(
                crew_core::kind::KIND_NIP29_GROUP_MEMBERS as u16,
            ))
            ...
            match relay_msg {
                RelayMessage::Event {
                    ...
                    if subscription_id == OBSERVER_CONTROL_SUB_ID {
                        ...
                        match event_tx.try_send(Some(crew_event)) {
                            Ok(()) => {
...
/// sends (`crates/crew-relay/src/handlers/auth.rs`).
///
...
mod tests {
    ...
    fn make_observer_frame(keys: &Keys) -> Event {
        ...
        let encrypted = crew_core::observer::encrypt_observer_payload(
            keys,
            &recipient.public_key(),
            &json!({"type": "test"}),
        )
        .expect("encrypt test observer payload");
        crew_sdk::build_agent_observer_frame(
