            ...
            if let Some(sid) = agent.state.sessions.get(cid) {
                ...
                .await
                {
                    Ok(sid) => {
                        ...
                        // Seed a zero usage baseline: crew-acp spawned this session
                ...
                mode = rx => {
                    ...
                    if agent.acp.has_in_flight_prompt() {
                            ...
                            .await
                        {
                            Ok(stop_reason) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(crew_core::agent_turn_metric::StopReason::Cancelled),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                            ...
                            Err(error) => {
                                ...
                                publish_agent_turn_metric(
                                    ...
                                    &session_id,
                                    &turn_id,
                                    Some(crew_core::agent_turn_metric::StopReason::Error),
                                )
                                .await;
                                send_prompt_result(
                                    &result_tx,
                        ...
                        publish_agent_turn_metric(
                            ...
                            &session_id,
                            &turn_id,
                            Some(crew_core::agent_turn_metric::StopReason::EndTurn),
                        )
                        .await;
                        send_prompt_result(
                            &result_tx,
...
pub(crate) async fn fetch_channel_info(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            crew_core::kind::KIND_NIP29_GROUP_METADATA as u16,
        ))
...
async fn fetch_huddle_instructions(
    ...
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(
            crew_core::kind::KIND_HUDDLE_GUIDELINES as u16,
        ))
        .author(*owner)
...
fn huddle_instructions_from_query_response(
    ...
    if event.pubkey != *owner
        || event.kind.as_u16() as u32 != crew_core::kind::KIND_HUDDLE_GUIDELINES
        || !event
            .tags
            .iter()
...
async fn fetch_canvas_section(channel_id: Uuid, rest: &RestClient) -> Option<String> {
    ...
    let h_tag = SingleLetterTag::lowercase(Alphabet::H);
    let filter = nostr::Filter::new()
        .kind(nostr::Kind::Custom(crew_core::kind::KIND_CANVAS as u16))
        .custom_tags(h_tag, [channel_id.to_string()])
        .limit(1);
...
pub(crate) fn canvas_section_from_query_response(
    ...
    if event.kind != nostr::Kind::Custom(crew_core::kind::KIND_CANVAS as u16) {
        tracing::warn!(
            target: "canvas::fetch",
            channel = %channel_uuid,
    ...
    CountFut: std::future::Future<Output = Result<serde_json::Value, crate::relay::RelayError>>,
{
    ...
    let replies_filter = nostr::Filter::new()
        .kinds([
            nostr::Kind::Custom(crew_core::kind::KIND_STREAM_MESSAGE as u16),
            nostr::Kind::Custom(crew_core::kind::KIND_STREAM_MESSAGE_V2 as u16),
        ])
...
fn acp_stop_to_core(r: &StopReason) -> crew_core::agent_turn_metric::StopReason {
    use crew_core::agent_turn_metric::StopReason as CoreStop;
    match r {
...
pub(crate) fn build_turn_metric_counts(
    ...
    Option<crew_core::agent_turn_metric::TokenCounts>,
    Option<crew_core::agent_turn_metric::TokenCounts>,
) {
    use crew_core::agent_turn_metric::TokenCounts;
    ...
    let cumulative_counts = Some(TokenCounts {
        ...
        // reported this field (e.g. goose or older crew-agent sessions).
...
async fn publish_agent_turn_metric(
    ...
    session_id: &str,
    turn_id: &str,
    stop_reason: Option<crew_core::agent_turn_metric::StopReason>,
) {
    use crew_core::agent_turn_metric::AgentTurnMetricPayload;
    ...
    let ciphertext = match crew_core::agent_turn_metric::encrypt_agent_turn_metric(
        &ctx.agent_keys,
        owner_pk,
        &payload,
    ) {
    ...
    let event = match EventBuilder::new(
        Kind::Custom(crew_core::kind::KIND_AGENT_TURN_METRIC as u16),
        ciphertext,
    )
    .tags([
...
///
/// Builds a reaction event with `crew_sdk::build_reaction`, signs it with
...
pub(crate) async fn reaction_add(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match crew_sdk::build_reaction(target_id, emoji) {
        Ok(b) => b,
        Err(e) => {
...
pub(crate) async fn post_failure_notice(
    ...
    let thread_ref = thread_tags.root_event_id.as_deref().and_then(|root| {
        ...
        let parent_id = thread_tags
            ...
            .unwrap_or(root_id);
        Some(crew_sdk::ThreadRef {
            root_event_id: root_id,
            parent_event_id: parent_id,
        })
    });
    let builder =
        match crew_sdk::build_message(channel_id, content, thread_ref.as_ref(), &[], false, &[]) {
            Ok(b) => b,
            Err(e) => {
...
pub(crate) async fn reaction_remove(rest: &crate::relay::RestClient, event_id: &str, emoji: &str) {
    ...
    let builder = match crew_sdk::build_remove_reaction(target_id) {
        Ok(b) => b,
        Err(e) => {
...
mod tests {
    ...
    fn goose_uses_system_prompt_only_after_custom_method_succeeds() {
        ...
        assert!(has_system_prompt_support(2, "crew-agent", None));
        ...
        assert_eq!(
            session_new_system_prompt(false, 2, "crew-agent", Some("instructions")),
    ...
    async fn run_prompt_task_commits_standing_context_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "crew-acp-standing-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn channel_prompt_commits_delivery_state_only_after_acp_success() {
        let capture = std::env::temp_dir().join(format!(
            "crew-acp-channel-delivery-lifecycle-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  if [ "$count" -eq 1 ]; then
    ...
    async fn merged_cancel_prompt_commits_and_deduplicates_all_rendered_event_ids() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "crew-acp-merged-delivery-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
while IFS= read -r line; do
  ...
  count=$((count + 1))
    ...
    async fn late_successful_steer_ack_excludes_event_from_next_channel_wire_prompt() {
        ...
        let capture = std::env::temp_dir().join(format!(
            "crew-acp-late-steer-wire-{}.ndjson",
            Uuid::new_v4()
        ));
        let quoted_capture = capture.to_string_lossy().replace('\'', "'\\''");
        let script = format!(
...
printf '%s\n' '{{"jsonrpc":"2.0","id":0,"result":{{"stopReason":"end_turn"}}}}'"#
    ...
    async fn test_publish_agent_turn_metric_buzz_agent_harness_name() {
        ...
        ctx.harness_name = "crew-agent".to_string();
        let usage = crate::usage::TurnUsage {
            session_id: "sess-ba".to_string(),
            turn_seq: 1,
            delta_reliable: false, // first turn from crew-agent
    ...
    fn test_build_turn_metric_counts_cache_read_tokens_thread_through() {
        // Wire-parse a crew-agent payload with cache, run it through the tracker,
    ...
    fn huddle_instructions_require_owner_signature_and_channel() {
        ...
        let event = |keys: &Keys, channel_id: Uuid| {
            ...
            serde_json::to_value(
                EventBuilder::new(
                    Kind::Custom(crew_core::kind::KIND_HUDDLE_GUIDELINES as u16),
                    "reply immediately",
                )
                .tags([h_tag])
                .sign_with_keys(keys)
    ...
    fn make_canvas_event_value(content: &str) -> serde_json::Value {
        ...
        let event = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_CANVAS as u16), content)
            .tags([h_tag])
            .sign_with_keys(&keys)
            .expect("sign");
    ...
    fn test_canvas_section_from_query_response_string_timestamp_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(Kind::Custom(crew_core::kind::KIND_CANVAS as u16), "content")
                .tags([h_tag])
                .sign_with_keys(&keys)
                .expect("sign"),
        )
    ...
    fn test_canvas_section_from_query_response_tampered_event_returns_none() {
        ...
        let mut ev = serde_json::to_value(
            EventBuilder::new(
                Kind::Custom(crew_core::kind::KIND_CANVAS as u16),
                "original",
            )
            .tags([h_tag])
            .sign_with_keys(&keys)
