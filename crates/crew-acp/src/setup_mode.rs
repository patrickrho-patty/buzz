...
//! model, or credentials), it spawns `crew-acp` with a setup-mode payload
...
///
/// crew-acp is a separate crate and must NOT depend on desktop types —
...
/// (`managed_agents/readiness.rs`). Desktop serializes it; `crew-acp`
...
impl SetupPayload {
    ...
    /// 2. A fenced `crew:config-nudge` sentinel block containing the structured
    ...
    fn nudge_body(&self) -> String {
        let prose = if self.requirements.is_empty() {
            ...
            let footer = if has_doctor_requirement {
                ...
                // Mixed: some Crew-managed fields, some external config.
                "Open Edit Agent in the Crew app for the Crew-managed fields; fix the external CLI config files manually and restart the agent.".to_string()
            } else {
                // All Crew-managed — original footer unchanged.
                "Open Edit Agent in the Crew app to set these.".to_string()
            };
...
pub(crate) async fn run_setup_listener(config: Config, payload: SetupPayload) -> Result<()> {
    ...
    loop {
        let Some(crew_event) = relay.next_event().await else {
        ...
        {
            handle_setup_membership(&mut relay, &crew_event, &config, &rules, &channel_ids).await;
            continue;
        }
        ...
        let is_dm = crate::is_dm_channel(crew_event.channel_id, &channel_info).await;
...
async fn handle_setup_membership(
    ...
    let kind_u32 = crew_event.event.kind.as_u16() as u32;
    let channel_id = crew_event.channel_id;
...
async fn publish_setup_nudge(
    ...
    let event_builder = crew_sdk::build_message(
        channel_id,
        &body,
        thread_ref.as_ref(),
...
mod tests {
    ...
    fn setup_payload_deserializes_git_bash_requirement() {
        let payload: SetupPayload = serde_json::from_str(
            r#"{"agent_name":"Crew Agent","agent_pubkey":"test","requirements":[{"surface":"git_bash"}]}"#,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn nudge_body_git_bash_copy_points_to_agent_runtimes() {
        let payload = SetupPayload {
            agent_name: "Crew Agent".to_string(),
            agent_pubkey: "test".to_string(),
    ...
    fn nudge_body_mixed_requirements_uses_split_footer() {
        // Mixed list: one Crew-managed env key + one external config invalid.
    ...
    fn nudge_body_all_buzz_managed_retains_original_footer() {
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("Open Edit Agent in the Crew app to set these."),
    ...
    fn nudge_body_contains_sentinel_block() {
        // The body must end with a ```crew:config-nudge fence so the desktop
        ...
        let body = payload.nudge_body();
        assert!(
            body.contains("```crew:config-nudge\n"),
    ...
    fn nudge_body_sentinel_round_trips_payload() {
        ...
        let fence_open = "```crew:config-nudge\n";
        let fence_close = "\n```";
        let start = body
            .rfind(fence_open)
    ...
    //
    // These tests prove the desktop→crew-acp→sentinel path preserves the
    // `availability` field. They simulate what actually happens at runtime:
    // desktop serializes a `cli_login` JSON blob → crew-acp parses it via
