...
//! turn.  Both goose and crew-agent use this same wire format.  The payload
...
/// ```
///
/// `used` and `contextLimit` are optional because crew-agent does not track a
...
pub(crate) struct UsageUpdatePayload {
    /// Total tokens used (context-usage proxy). Optional — crew-agent omits
    /// this field or sends 0 because it does not track a context window limit.
    #[serde(default)]
    #[allow(dead_code)]
    pub used: u64,
    /// Context window size. Optional — crew-agent omits this field.
    #[serde(default)]
    #[allow(dead_code)]
    pub context_limit: u64,
    /// Session-cumulative inclusive input tokens.
    ///
    /// `None` when crew-agent omitted the field — this happens when the
    ...
    /// emitted by crew-agent when every turn in the session so far supplied a
    ...
    #[serde(default)]
    pub pricing_identity: Option<crew_core::agent_turn_metric::PricingIdentity>,
}
...
pub struct TurnUsage {
    ...
    /// Session-cumulative genuine provider total tokens as reported by crew-agent;
    ...
    /// Session-cumulative cache-read input tokens as reported by crew-agent.
    ...
    /// Session-cumulative cache-write tokens as reported by crew-agent.
...
pub(crate) struct UsageTracker {
    ...
    pending_identity: Option<Option<crew_core::agent_turn_metric::PricingIdentity>>,
...
impl UsageTracker {
    ...
    /// Seed a zero baseline for a session that crew-acp just spawned.
    ///
    /// When crew-acp creates a session itself via `session/new`, the session's
...
mod tests {
    ...
    /// The camelCase key crew-agent actually puts on the wire must land on the
    ...
    #[test]
    fn crew_agent_notification_flows_through_tracker() {
    ...
    #[test]
    fn crew_agent_payload_no_context_fields_processes_correctly() {
    ...
    fn pool_omitted_cache_field_publishes_no_cache_read_tokens_in_kind44200() {
        // End-to-end: a crew-agent or goose payload that omits the cache field
    ...
    fn pool_reported_cache_field_publishes_nonzero_cache_read_tokens_in_kind44200() {
        // End-to-end: a crew-agent payload with a nonzero cache count must
    ...
    fn make_pricing_identity_payload(
        ...
        UsageUpdatePayload {
            ...
            pricing_identity: Some(crew_core::agent_turn_metric::PricingIdentity {
    ...
    /// Old harnesses (goose, older crew-agent) that do not emit `pricingIdentity`
    ...
    // The ACP tracker has a DISTINCT multi-notification path: crew-agent sends
