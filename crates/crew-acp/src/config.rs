//! Configuration for the crew-acp harness.
//!
...
/// while running long sub-tools (e.g. a crew-agent running another agent, or
...
/// Override via `--idle-timeout` / `CREW_ACP_IDLE_TIMEOUT`.
pub(crate) const DEFAULT_IDLE_TIMEOUT_SECS: u64 = 900;

/// Default absolute wall-clock cap per agent turn (2 hours).
/// Override via `--max-turn-duration` / `CREW_ACP_MAX_TURN_DURATION`.
...
/// CLI args for `crew-acp models` — query available models from an agent.
///
...
pub struct AuthAgentArgs {
    /// Agent binary to spawn (e.g. "goose", "claude-agent-acp", "codex-acp").
    #[arg(long, env = "CREW_ACP_AGENT_COMMAND", default_value = "goose")]
    pub agent_command: String,

    /// Arguments passed to the agent binary.
    #[arg(
        long,
        env = "CREW_ACP_AGENT_ARGS",
        default_value = "acp",
        value_delimiter = ','
    )]
...
/// CLI args for `crew-acp auth-methods` — query adapter-advertised login methods.
#[derive(Debug, Parser)]
#[command(
    name = "crew-acp auth-methods",
...
/// CLI args for `crew-acp authenticate` — start an adapter-owned login flow.
#[derive(Debug, Parser)]
#[command(
    name = "crew-acp authenticate",
...
pub struct CliArgs {
    ...
    #[arg(long, env = "CREW_ACP_AGENT_OWNER")]
    ...
    #[arg(long, env = "CREW_ACP_MCP_COMMAND", default_value = "")]
    pub mcp_command: String,

    /// Idle timeout: max seconds of silence before killing a turn.
    /// Resets on any agent stdout activity.
    #[arg(long, env = "CREW_ACP_IDLE_TIMEOUT")]
    pub idle_timeout: Option<u64>,

    /// Absolute wall-clock cap per turn (safety valve).
    #[arg(long, env = "CREW_ACP_MAX_TURN_DURATION", default_value_t = DEFAULT_MAX_TURN_DURATION_SECS)]
    pub max_turn_duration: u64,

    /// Deprecated: alias for --idle-timeout. If both set, --idle-timeout wins.
    #[arg(long, env = "CREW_ACP_TURN_TIMEOUT", hide = true)]
    pub turn_timeout: Option<u64>,

    #[arg(
        long,
        env = "CREW_ACP_SYSTEM_PROMPT",
    ...
    #[arg(
        long,
        env = "CREW_ACP_HEARTBEAT_PROMPT_FILE",
        conflicts_with = "heartbeat_prompt"
    )]
    pub heartbeat_prompt_file: Option<PathBuf>,

    #[arg(long, env = "CREW_ACP_INITIAL_MESSAGE")]
    pub initial_message: Option<String>,

    #[arg(
        long,
        env = "CREW_ACP_SUBSCRIBE",
        default_value = "mentions",
        value_enum
    )]
    pub subscribe: SubscribeMode,

    #[arg(long, env = "CREW_ACP_KINDS", value_delimiter = ',')]
    pub kinds: Option<Vec<u32>>,

    #[arg(long, env = "CREW_ACP_CHANNELS", value_delimiter = ',')]
    pub channels: Option<Vec<String>>,

    #[arg(long, env = "CREW_ACP_NO_MENTION_FILTER")]
    pub no_mention_filter: bool,

    #[arg(long, env = "CREW_ACP_CONFIG", default_value = "./crew-acp.toml")]
    pub config: PathBuf,

    #[arg(long, env = "CREW_ACP_DEDUP", default_value = "queue", value_enum)]
    pub dedup: DedupMode,
    ...
    #[arg(
        long,
        env = "CREW_ACP_MULTIPLE_EVENT_HANDLING",
        default_value = "steer",
        value_enum
    )]
    pub multiple_event_handling: MultipleEventHandling,

    #[arg(long, env = "CREW_ACP_NO_IGNORE_SELF")]
    ...
    #[arg(long, env = "CREW_ACP_NO_MEMORY", conflicts_with = "memory")]
    pub no_memory: bool,
    ...
    /// Use `crew-acp models` to discover available model IDs.
    #[arg(long, env = "CREW_ACP_MODEL")]
    ...
    #[arg(long, env = "CREW_ACP_EFFORT_LEVEL")]
    ...
    #[arg(long, env = "CREW_ACP_SESSION_TITLE")]
    ...
    #[arg(long, env = "CREW_ACP_RESPOND_TO_ALLOWLIST", value_delimiter = ',')]
    ...
    #[arg(long, env = "CREW_ACP_IDLE_POOL_SLEEP", default_value_t = 0)]
...
pub struct Config {
    ...
    /// `--no-memory` / `CREW_ACP_NO_MEMORY` opt-out.
    ...
    /// Per-persona env vars to inject at agent spawn time (e.g., GOOSE_PROVIDER, GOOSE_MODEL, CREW_AGENT_MODEL).
...
/// startup budget (see block/crew#3355). Skip that unrelated global startup
...
///
/// Codex sandboxes MCP subprocesses (including `crew-cli`) behind a Seatbelt sandbox
/// that blocks all outbound network by default. Without this env var, `crew-cli`
...
pub fn propagate_legacy_env_vars() {
    for (legacy, canonical) in [
        ("CREW_ACP_PRIVATE_KEY", "CREW_PRIVATE_KEY"),
        ("CREW_ACP_API_TOKEN", "CREW_API_TOKEN"),
    ] {
...
impl Config {
    ...
    pub fn from_args(mut args: CliArgs) -> Result<Self, ConfigError> {
        ...
        let idle_timeout_secs = {
            let raw = match (args.idle_timeout, args.turn_timeout) {
                (Some(idle), Some(_turn)) => {
                    tracing::warn!(
                        "--turn-timeout / CREW_ACP_TURN_TIMEOUT is deprecated and ignored \
                         when --idle-timeout / CREW_ACP_IDLE_TIMEOUT is also set"
                    );
                    idle
                }
                (Some(idle), None) => idle,
                (None, Some(turn)) => {
                    tracing::warn!(
                        "--turn-timeout / CREW_ACP_TURN_TIMEOUT is deprecated; \
                         use --idle-timeout / CREW_ACP_IDLE_TIMEOUT instead"
                    );
                    turn
                }
            ...
            for s in &raw {
                RespondTo::from_str(s.trim(), true).map_err(|_| {
                    ConfigError::ConfigFile(format!(
                        "invalid value in CREW_ACP_ALLOWED_RESPOND_TO: '{s}' \
        ...
        // opens the Seatbelt network sandbox for crew-cli (an MCP subprocess). No-op
...
mod tests {
    ...
    fn test_config(mode: SubscribeMode) -> Config {
        Config {
            ...
            config_path: PathBuf::from("./crew-acp.toml"),
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&crew_core::kind::KIND_STREAM_MESSAGE));
            assert!(kinds.contains(&crew_core::kind::KIND_WORKFLOW_APPROVAL_REQUESTED));
            assert!(kinds.contains(&crew_core::kind::KIND_STREAM_REMINDER));
        }
    }
    ...
    fn default_agent_env_recognizes_hermes_identities() {
        ...
        for command in ["goose", "codex-acp", "claude-agent-acp", "crew-agent", ""] {
            assert!(
    ...
    fn codex_network_env_non_codex_agent_returns_none() {
        ...
        assert!(codex_network_env("crew-agent", "wss://relay.example.com").is_none());
    }
    ...
    fn test_load_rules_valid_toml() {
        let dir = std::env::temp_dir().join("crew-acp-test-valid");
...
"#,
    ...
    fn test_load_rules_empty_name_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-empty-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_duplicate_name_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-dup-name");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_invalid_filter_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-bad-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    fn test_load_rules_channel_scope_typo_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-scope-typo");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(
            &path,
...
"#,
    ...
    fn test_load_rules_too_many_rules_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-too-many");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn test_load_rules_filter_too_long_rejected() {
        let dir = std::env::temp_dir().join("crew-acp-test-long-filter");
        let path = dir.join("rules.toml");
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn inactivity_exit_defaults_disabled_and_accepts_cli_value() {
        let key = "0".repeat(64);
        let default = CliArgs::parse_from(["crew-acp", "--private-key", &key]);
    ...
    fn lazy_pool_defaults_off() {
        let key = "0".repeat(64);
        assert!(!CliArgs::parse_from(["crew-acp", "--private-key", &key]).lazy_pool);
    }
    ...
    fn lazy_pool_cli_flag_enables_deferred_startup() {
        let key = "0".repeat(64);
        let args = CliArgs::try_parse_from(["crew-acp", "--private-key", &key, "--lazy-pool=true"]);
        assert!(args.is_err(), "bool flags do not take an explicit value");
        assert!(CliArgs::parse_from(["crew-acp", "--private-key", &key, "--lazy-pool"]).lazy_pool);
    }
    ...
    fn test_permission_mode_value_enum_camel_case_aliases() {
        ...
        // (e.g. CREW_ACP_PERMISSION_MODE=bypassPermissions). The #[value(alias)]
    ...
    fn test_multiple_event_handling_default_is_steer() {
        ...
        let args = CliArgs::parse_from(["crew-acp", "--private-key", &"0".repeat(64)]);
    ...
    // --- CREW_ACP_ALLOWED_RESPOND_TO gate ---
    ...
    fn allowed_respond_to_rejects_invalid_mode_string() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("invalid value in CREW_ACP_ALLOWED_RESPOND_TO"),
    ...
    //
    // These tests exercise the actual wiring: CREW_ACP_ALLOWED_RESPOND_TO in the
