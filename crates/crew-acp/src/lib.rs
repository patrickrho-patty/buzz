...
/// name (e.g., `crew-acp --verbose models`) are not supported.
...
/// Timeout for `crew-acp authenticate`. Browser-based vendor auth can require
...
async fn check_sibling_via_profile(
    ...
    for tag in tags {
        ...
        match crew_sdk::nip_oa::verify_auth_tag(&tag_json, &agent_pk) {
            Ok(_) => {
...
/// keyed to `OBSERVER_MAX_PLAINTEXT_LEN` in crew-core/observer.rs:25) is what actually
...
/// already-small frame. Reusing that string would mean changing crew-core's
/// `encrypt_observer_payload` signature or adding a parallel encrypt path; both
/// are out of this change's scope (crew-core stays untouched). The clean `&mut`
...
async fn publish_relay_observer_event(
    ...
    let builder = match crew_sdk::build_agent_observer_frame(
        owner_pubkey_hex,
        agent_pubkey_hex,
...
fn handle_relay_observer_control_event(
    ...
    if let Err(e) = crew_core::verify_event(&event) {
...
async fn tokio_main() -> Result<()> {
    ...
    {
        tracing::info!("crew-acp: setup payload present, entering setup-listener mode");
        return setup_mode::run_setup_listener(config, payload).await;
    }

    tracing::info!("crew-acp starting: {}", config.summary());
    ...
    loop {
        ...
        let pool_event: Option<PoolEvent> = {
            ...
            tokio::select! {
                ...
                crew_event = relay.next_event() => {
                    let _ = result_rx; // end split borrow before relay handling
                    match crew_event {
                        Some(crew_event) => {
                            let kind_u32 = crew_event.event.kind.as_u16() as u32;

                            if kind_u32 == KIND_MEMBER_ADDED_NOTIFICATION
                                || kind_u32 == KIND_MEMBER_REMOVED_NOTIFICATION
                            {
                                let ch = crew_event.channel_id;
                                let ts = crew_event.event.created_at.as_secs();
                                let eid = crew_event.event.id.to_hex();
                            ...
                            if config.ignore_self && crew_event.event.pubkey.to_hex() == pubkey_hex {
                                tracing::debug!(channel_id = %crew_event.channel_id, "dropping self-authored event");
                                continue;
                            }
                            ...
                            if is_shutdown {
                                ...
                                if let Some(owner) = owner {
                                    if crew_event.event.pubkey.to_hex() == *owner {
                                        tracing::info!(
                                            channel_id = %crew_event.channel_id,
                                            sender = %crew_event.event.pubkey.to_hex(),
                                ...
                                if let Some(owner) = owner_cache.get() {
                                    if crew_event.event.pubkey.to_hex() == *owner {
                                        ...
                                        if fired {
                                            ...
                                            let invalidated = pool.invalidate_channel_sessions(crew_event.channel_id);
                                            tracing::info!(
                                                channel_id = %crew_event.channel_id,
                                                invalidated,
                            ...
                            {
                                let author = crew_event.event.pubkey.to_hex();
                                // DM hardening: resolve channel type (fail-closed
                                // to DM) so allowlist/anyone modes cannot be
                                // exercised by non-owner authors inside DMs.
                                let is_dm =
                                    is_dm_channel(crew_event.channel_id, &ctx.channel_info).await;
                                ...
                                if !allowed {
                                    tracing::debug!(
                                        channel_id = %crew_event.channel_id,
                                        author = %crew_event.event.pubkey.to_hex(),
                            ...
                            let matched = filter::match_event(&crew_event.event, crew_event.channel_id, &rules, &pubkey_hex).await;
                            let prompt_tag = match matched {
                                Some(m) => m.prompt_tag,
                                None => {
                                    tracing::debug!(channel_id = %crew_event.channel_id, kind = crew_event.event.kind.as_u16(), "event matched no rule — dropping");
                                    continue;
                                }
                            };
                            ...
                            let event_for_steer = crew_event.event.clone();
                            let prompt_tag_for_steer = prompt_tag.clone();
                            let accepted = queue.push(QueuedEvent {
                                channel_id: crew_event.channel_id,
                                event: crew_event.event,
                            ...
                            if accepted && queue.is_channel_in_flight(crew_event.channel_id) {
...
fn try_native_steer(
    ...
    // `queue::format_prompt` uses internally for `[Crew event: …]`
...
fn handle_prompt_result(
    ...
    // which live in crew-agent's session state and are what `llm: (model) …`
...
/// `crew-acp auth-methods` — spawn an adapter, initialize it, print authMethods.
...
/// `crew-acp authenticate` — invoke one adapter-owned auth method.
...
mod build_mcp_servers_tests {
    ...
    fn test_config() -> Config {
        Config {
            ...
            config_path: std::path::PathBuf::from("./crew-acp.toml"),
...
mod observer_payload_trim_tests {
    ...
    fn test_multi_block_prompt_retains_every_section_header_after_elision() {
        ...
        // {text: "[Agent Memory — core]…"}, … {text: "[Crew event: …]…<huge>"}].
        ...
        // [Crew event] header fell into the elided middle and the count collapsed
        // to 1).
        let sections = [
        ...
        let event_block = texts
            .iter()
            .find(|t| t.starts_with("[Crew event: @mention]"))
            .unwrap();
        assert!(
