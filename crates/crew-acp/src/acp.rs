...
pub struct AcpClient {
    ...
    ///
    /// Both goose and crew-agent emit `session_info_update` with this field;
    ...
    /// run clears (goose/crew-agent emit `activeRunId: null` at end of turn).
    ...
    /// Usage tracker for goose/crew-agent's cumulative notification format.
...
/// `_meta.goose.activeRunId`. Emitted by goose and crew-agent only.
...
impl AcpClient {
    ...
    ///   (ACP protocol v2, crew-agent, goose unused).
    ...
    ///
    /// Both goose and crew-agent emit `session_info_update`; other agents
    ...
    /// Consume per-turn usage for NIP-AM publishing. Goose/crew-agent is an
    ...
    /// Notify the usage tracker that crew-acp just spawned a new session.
    ///
    ...
    /// Must be called only when crew-acp created the session via `session/new`;
    ...
    fn handle_session_update(&mut self, msg: &serde_json::Value) -> bool {
        ...
        match update_type {
            ...
            "session_info_update" => {
                // Both goose and crew-agent emit `session_info_update` with
                ...
                // alongside it at the params level. Goose and crew-agent both
...
///
/// - **`Field`** — bare `systemPrompt` field (ACP protocol v2, crew-agent).
...
mod tests {
    ...
    async fn spawn_named_and_read_child_env(
        ...
        let dir = std::env::temp_dir().join(format!("crew-acp-env-probe-{}", uuid::Uuid::new_v4()));
    ...
    /// `SessionInfoUpdate` schema), matching what goose and crew-agent
    /// emit on the wire.
    ...
    fn capture_path(name: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join("crew-acp-steer-capture");
    ...
    /// Test 1b: no `_meta` at all (goose, crew-agent, any older adapter) must
