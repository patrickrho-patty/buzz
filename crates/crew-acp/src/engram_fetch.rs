...
use crew_core::engram::{conversation_key, d_tag, select_head, validate_and_decrypt, Body};
...
///
/// Wording is from Tyler's brief: "No core memory found. Use `crew mem`
/// to create a core memory. Ask your user about yourself."
pub const ONBOARDING_NUDGE: &str = "No core memory found. \
Use `crew mem set core \"…\"` to create one (it will hold your identity, \
...
async fn fetch_core_body(
    ...
    let d = d_tag(&k_c, crew_core::engram::CORE_SLUG);
...
mod tests {
    use super::*;
    use crew_core::engram::{build_event, Body};
    use serde_json::json;
