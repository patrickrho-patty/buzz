...
//!
//! Proves the security claim of Crew's membership-derived mesh admission:
...
//!      same builder calls Crew desktop makes with a resolved member roster.
...
//! corrupt each other — exactly how Crew runs it in production anyway (one
//! desktop = one node).
//!
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    let scratch = std::env::temp_dir().join(format!("crew-mesh-admission-{}", std::process::id()));
