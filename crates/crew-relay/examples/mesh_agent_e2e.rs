...
//!   P4 agentic tool use     — agent + crew-dev-mcp writes a file on disk.
//!
...
//! `crew-agent` binary with the exact env vars the relay-mesh preset ships.
//!
...
async fn run() -> anyhow::Result<()> {
    ...
    // mode that broke relay-mesh agents when crew-agent's default 32768
    // budget met a 32k-context model). 150k output: passes crew-agent's own
    ...
    // P4: agentic tool use via crew-dev-mcp — write a real file inside the
    ...
    let prompt = format!(
        "Use your developer tools to create {marker_name} in the current working directory containing exactly the text CREW_OK (no quotes, no newline commentary). Then confirm."
    );
    let mcp = vec![("dev".to_string(), repo_bin("crew-dev-mcp")?)];
    let (r, marker) =
        agent_chat_with_marker(&base, "mesh", None, &prompt, &mcp, &marker_name).await;
    let file_ok = std::fs::read_to_string(&marker)
        .map(|c| c.contains("CREW_OK"))
        .unwrap_or(false);
    match r {
        Ok(text) => record(
            "P4 agentic tool use",
...
/// Spawn the real crew-agent with relay-mesh preset env and drive one ACP
...
async fn agent_chat_in_isolated_home(
    ...
    let agent = match repo_bin("crew-agent") {
        Ok(agent) => agent,
    ...
    if let Some(value) = max_output_tokens {
        command.env("CREW_AGENT_MAX_OUTPUT_TOKENS", value);
    }
