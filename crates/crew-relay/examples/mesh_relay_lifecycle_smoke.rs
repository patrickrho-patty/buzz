...
//!      crew-relay (kind:13534 roster via crew-admin); a third is not.
...
fn orchestrate() -> anyhow::Result<()> {
    ...
    // MEMBERSHIP: A and B become relay members via crew-admin (publishes the
    ...
    for (label, keys) in [("A", &member_a), ("B", &member_b)] {
        let status = Command::new(&admin)
            ...
            .status()?;
        anyhow::ensure!(status.success(), "crew-admin add-member {label} failed");
        eprintln!(
