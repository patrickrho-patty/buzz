...
use chrono::Utc;
use crew_core::kind::KIND_STREAM_MESSAGE;
use crew_core::tenant::CommunityId;
use crew_workflow::action_sink::{ActionSink, ActionSinkError};
...
impl ActionSink for RelayActionSink {
    fn send_message(
        ...
        Box::pin(async move {
            ...
            let tenant = crew_core::tenant::TenantContext::resolved(community_id, host);
            ...
            let channel = state
                ...
                .await
                .map_err(|e| match &e {
                    crew_db::DbError::ChannelNotFound(_) | crew_db::DbError::NotFound(_) => {
            ...
            //    - `crew:workflow` tag prevents recursive workflow triggering
            ...
            let thread_meta = Some(match &thread_meta_owned {
                ...
                None => crew_db::event::ThreadMetadataParams {
...
mod integration_tests {
    ...
    //!   `cargo test -p crew-relay --lib workflow_sink -- --ignored`
    use super::*;
    use crew_core::channel::{ChannelType, ChannelVisibility, MemberRole};
    use crew_db::CreateCommunityWithOwnerResult;
    use std::sync::Arc;
