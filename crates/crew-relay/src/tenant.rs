...
//! implementation that queries the `communities` table lives in `crew-db`
//! (`Db::resolve_host`); the relay depends on the trait, not the query, so the
//! binding is testable without a database.

use crew_core::tenant::{normalize_host, CommunityId, TenantContext};
...
/// [`crew_core::tenant::normalize_host`] — [`bind_community`] guarantees that,
...
pub async fn bind_deployment_community<R: HostResolver>(
    ...
    bind_community(resolver, &crew_core::tenant::relay_url_authority(relay_url)).await
}
...
///
/// This is a thin re-export of [`crew_core::tenant::relay_url_authority`]: the
/// canonical implementation lives in `crew-core` so the relay seam *and* the
/// `crew-admin` CLI derive a byte-identical authority (same port/IPv6 handling).
pub use crew_core::tenant::relay_url_authority;

/// Production [`HostResolver`]: the relay resolves hosts against the durable
/// `communities` host map in Postgres.
///
/// This is the *only* place the relay couples the row-zero seam to crew-db. The
/// trait keeps `bind_community` and every call site database-free and testable;
/// this impl is the thin adapter from crew-db's `lookup_community_by_host`
...
impl HostResolver for crew_db::Db {
    type Error = crew_db::DbError;
...
mod tests {
    ...
    mod redteam_attack2 {
        ...
        /// `cargo test -p crew-relay --include-ignored
        ...
        /// trims to empty (`crew-core::tenant::normalize_host_empty_stays_empty`),
        ...
        async fn whitespace_only_raw_host_fails_closed_even_if_db_has_empty_host_row() {
            ...
            let err = bind_community(&r, "   ").await.expect_err(
                "Inv_RowZero: whitespace-only raw_host normalizes to empty \
                 (see crew-core::tenant::normalize_host) and carries no \
                 community evidence",
            );
            assert!(
                matches!(err, BindError::UnmappedHost),
