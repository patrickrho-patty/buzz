//! Relay startup wiring for the inter-relay mesh (`CREW_MESH` seam).
//!
//! [`boot_mesh`] is the ONLY place the relay constructs mesh machinery. It
//! returns `None` — and touches nothing — when `CREW_MESH=off`, so mesh-off
//! deployments stay byte-identical to a relay built before this module
//! existed. When enabled, it:
//!
//! 1. binds the iroh endpoint on `CREW_MESH_BIND_ADDR` (boot-unique keypair =
...
use crew_relay_mesh::endpoint::MeshEndpoint;
use crew_relay_mesh::gossip::GossipRecord;
use crew_relay_mesh::registry::{ReadyRecord, ReadyRegistry};
use crew_relay_mesh::{
...
///   either the `CREW_MESH_DEMO_ECHO` consumer (testbed evidence runs: echo
...
/// `CREW_MESH_DEMO_ECHO` consumer: echo every validated `Data` frame back to
...
const PROTO_VERSION: u16 = crew_relay_mesh::WIRE_VERSION as u16;
...
/// `CREW_MESH_ADVERTISE_ADDR` (explicit, classic-LB shapes) →
...
fn advertise_addrs(endpoint: &MeshEndpoint) -> Vec<String> {
    if let Ok(addr) = std::env::var("CREW_MESH_ADVERTISE_ADDR") {
...
/// Boot the mesh, or return `None` when `CREW_MESH=off`.
///
...
/// sets `CREW_MESH=on` wants the mesh or wants to know why not; silently
...
pub async fn boot_mesh(
    ...
    if !config.mesh.enabled {
        tracing::info!("mesh disabled (CREW_MESH is not 'on') — single-instance behavior");
        return Ok(None);
    }
    ...
    crew_relay_mesh::runtime::spawn_registry_heartbeat(
        registry.clone(),
        ready_record,
...
mod tests {
    ...
    /// CREW_MESH=off must be a hard no-op: no endpoint bind, no Redis write,
    ...
    /// Blocker fix (Wren review of 8b077fdb): absent `CREW_MESH`, the mesh is
    /// OFF — an env-untouched image upgrade must not bind or write Redis.
    #[test]
    fn mesh_defaults_off_when_env_absent() {
        // `Config::from_env` in the test env has no CREW_MESH set unless a
        // caller exported it; assert the fail-safe reading.
        if std::env::var("CREW_MESH").is_ok() {
        ...
        assert!(!config.mesh.enabled, "CREW_MESH absent must mean mesh off");
    }
    ...
    fn session_hello(sender: RuntimeId, profile: Profile) -> StreamHello {
        StreamHello {
            sender,
            role: crew_relay_mesh::StreamRole::Session {
                fenced: FencedHeader {
    ...
    async fn wired_datagram_consumer_shares_the_handle_fence() {
        ...
        impl crew_relay_mesh::RelayPeerTransport for NoopTransport {
