...
use crew_auth::{generate_challenge, AuthContext, LimitType};
...
async fn handle_active_connection(
    ...
    for removed in state.sub_registry.remove_connection(conn.conn_id) {
        if removed.scope.is_global() {
            state
                .pubsub
                .release_topic(&conn.tenant, crew_pubsub::EventTopic::Global)
                .await;
        }
        for &channel_id in removed.scope.channel_ids() {
            state
                .pubsub
                .release_topic(&conn.tenant, crew_pubsub::EventTopic::Channel(channel_id))
                .await;
        }
    }
