...
use crew_core::filter::{filters_match, reader_authorized_for_event};
...
async fn process_match_batch(state: &AppState, batch: crew_db::push::ClaimedMatchBatch) {
    ...
    let mut wakes: Vec<crew_db::push::WakeRequest> = Vec::new();
    for job in &batch.jobs {
        ...
        match match_job(job, &context) {
            ...
            Err(e) => {
                ...
                if job.attempt >= crew_db::push::MAX_MATCH_ATTEMPTS {
    ...
    match state.db.enqueue_push_wakes(community, &wakes).await {
        ...
        Err(e) => {
            ...
            for (event_id, attempt) in pending {
                if attempt >= crew_db::push::MAX_MATCH_ATTEMPTS {
                    completed.push(event_id);
                } else {
                    retry.push(event_id);
                }
...
fn match_job(
    job: &crew_db::push::BatchedMatch,
    context: &MatchContext,
) -> anyhow::Result<Vec<crew_db::push::WakeRequest>> {
    ...
    for lease in &context.leases {
        ...
        wakes.push(crew_db::push::WakeRequest {
...
fn push_filter_authorized_for_event(
    ...
    if crew_core::kind::event_kind_u32(event) != crew_core::kind::KIND_GIFT_WRAP {
        return true;
    }
        ...
        match state.db.usage_community_hosts().await {
            Ok(communities) => {
                for community in communities {
                    let community = crew_core::CommunityId::from_uuid(community.id);
                    let until = Utc::now() + TimeDelta::seconds(CLAIM_SECS);
...
async fn deliver_one(
    state: &AppState,
    http: &reqwest::Client,
    claimed: crew_db::push::ClaimedWake,
) {
    let outcome = match state
        ...
        .await
    {
        Ok(crew_db::push::RevalidateWakeOutcome::Deliver(wake)) => wake,
        Ok(crew_db::push::RevalidateWakeOutcome::Suppressed) => {
            let _ = state
                .db
...
async fn retry_or_fail(state: &AppState, wake: &crew_db::push::ClaimedWake, delay: i64) {
