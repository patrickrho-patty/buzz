...
use crew_relay_mesh::{FencedHeader, MeshError, Profile, RuntimeId};
use redis::Script;
use uuid::Uuid;
...
impl SessionDirectory {
    ...
    pub fn with_db(pool: deadpool_redis::Pool, db: crew_db::Db) -> Self {
        Self {
            pool,
    ...
    async fn begin_serving_write(
        &self,
        community_id: CommunityId,
    ) -> Result<Option<crew_deletion::ServingWriteGuard>, DirectoryError> {
        match &self.db {
            Some(db) => crew_deletion::acquire_serving_write(db, community_id, "session_directory")
                .await
                .map(Some)
...
impl SessionKeys {
    fn new(community_id: CommunityId, session_id: Uuid) -> Self {
        let base = format!("crew:{}:tunnel:{}", community_id, session_id);
        Self {
...
mod tests {
    ...
    fn key_shape_is_community_scoped_and_separates_counter() {
        ...
        assert_eq!(
            keys.lease,
            format!("crew:{}:tunnel:{}:lease", community(), session())
        );
        assert_eq!(
            keys.generation,
            format!("crew:{}:tunnel:{}:generation", community(), session())
        );
