...
//! - `OTEL_SERVICE_NAME` (default: `crew-relay`; read explicitly — not via SDK detector)
...
pub fn otel_env_filter(configured: Option<&str>) -> EnvFilter {
    EnvFilter::new(configured.unwrap_or("crew_relay=info,crew_datastore=info"))
}
...
/// 3. Hard-coded fallback `crew-relay`.
///
...
/// `crew-relay` default.  We therefore read `OTEL_SERVICE_NAME` explicitly
...
pub fn service_resource() -> Resource {
    // Honor OTEL_SERVICE_NAME when set+non-empty; otherwise use crew-relay.
    ...
    let service_name = std::env::var("OTEL_SERVICE_NAME")
        .ok()
        .filter(|s| !s.is_empty())
        .unwrap_or_else(|| "crew-relay".to_string());
...
mod tests {
    ...
    fn test_service_resource_default_when_env_unset() {
        ...
        assert_eq!(
            ...
            Some("crew-relay"),
            "expected crew-relay fallback when OTEL_SERVICE_NAME is unset"
        );
    }
    ...
    fn test_service_resource_empty_string_falls_back_to_default() {
        ...
        assert_eq!(
            service_name_from(&r).as_deref(),
            Some("crew-relay"),
            "expected crew-relay fallback when OTEL_SERVICE_NAME is empty"
        );
    }
