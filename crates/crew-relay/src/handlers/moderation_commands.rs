...
//!
//! Lane ownership: L6 (Quinn) — plus `crew-cli` `moderation` command group.
...
use crew_db::moderation::NewAction;
...
async fn handle_resolve(
    ...
    let (target_pubkey, target_event_id) = match &report.target {
        crew_db::moderation::ReportTarget::Pubkey(p) => (Some(p.as_slice()), None),
        crew_db::moderation::ReportTarget::Event(e) => (None, Some(e.as_slice())),
        crew_db::moderation::ReportTarget::Blob(_) => (None, None),
    };
...
mod tests {
    ...
    fn resolve_audit_actions_are_allowed_by_db_check_vocabulary() {
        for action in ["dismiss", "escalate", "delete", "kick", "ban", "timeout"] {
            let audit_action = resolution_audit_action(action);
            assert!(
                crew_db::moderation::MODERATION_ACTION_CHECK_VOCAB.contains(&audit_action),
