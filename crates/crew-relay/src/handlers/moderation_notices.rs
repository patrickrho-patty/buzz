...
//!    participant-hash-idempotent DM model (`crew-db/src/dm.rs`).
...
use crew_core::kind::{event_kind_u32, KIND_STREAM_MESSAGE};
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            crew_core::CommunityId::from_uuid(Uuid::new_v4()),
            "example.org",
        )
    }
