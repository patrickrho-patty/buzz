...
use crew_core::tenant::{normalize_host, TenantContext};
use url::{Host, Url};
...
/// present, the owner is (re)bootstrapped via [`crew_db::Db::bootstrap_owner`]
...
pub async fn provision_community(
    ...
    if request.create_only {
        ...
        {
            crew_db::CreateCommunityWithOwnerResult::Created(record) => record,
            crew_db::CreateCommunityWithOwnerResult::HostExists => {
                return Err("community already exists".to_string());
            }
            crew_db::CreateCommunityWithOwnerResult::LimitReached => {
                return Err(
...
mod tests {
    ...
    fn host_valid_bare_domain() {
        assert!(validate_host("acme.communities.crew.xyz").is_ok());
    }
