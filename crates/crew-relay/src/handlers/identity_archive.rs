...
use crew_core::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST, KIND_PROFILE};
...
fn verify_auth_tag_owner(auth_tag_json: &str, target_hex: &str) -> Result<String, String> {
    let target_pubkey =
        PublicKey::from_hex(target_hex).map_err(|e| format!("invalid target pubkey: {e}"))?;
    crew_sdk::nip_oa::verify_auth_tag(auth_tag_json, &target_pubkey)
...
mod tests {
    ...
    async fn test_pool() -> Option<sqlx::PgPool> {
        let url = std::env::var("DATABASE_URL")
            .unwrap_or_else(|_| "postgres://crew:crew_dev@localhost:5432/crew".into());
    ...
    fn auth_tag(owner_keys: &Keys, target_pubkey: &nostr::PublicKey) -> Tag {
        let tag_json = crew_sdk::nip_oa::compute_auth_tag(owner_keys, target_pubkey, "")
            .expect("compute auth tag");
        crew_sdk::nip_oa::parse_auth_tag(&tag_json).expect("parse auth tag")
    }
    ...
    async fn seed_test_community(pool: &sqlx::PgPool) -> crew_core::tenant::TenantContext {
    ...
    async fn archival_snapshot_advances_timestamp_for_rapid_state_replacement() {
        ...
        let archived_snapshot = state
            .db
            .query_events(&EventQuery {
                kinds: Some(vec![crew_core::kind::KIND_IA_ARCHIVED_LIST as i32]),
