...
use crew_db::channel::{MemberRecord, MemberRole};
...
/// The per-fire authority gate in `crew-workflow` is the fail-closed backstop;
...
async fn evict_conn_channel_subscriptions(
    ...
    for update in removed {
        state
            .pubsub
            .release_topic(tenant, crew_pubsub::EventTopic::Channel(channel_id))
            .await;
        if update.removed {
...
///
/// Crew accepts standard deletions for self-authored events, plus the owning
...
async fn actor_owns_any_owner_agent(
    ...
    members: &[crew_db::channel::MemberRecord],
    actor_bytes: &[u8],
) -> anyhow::Result<bool> {
    ...
    match kind {
        9000 => {
            ...
            let requested_role = match role_str {
                Some(ref s) => match s.parse::<crew_db::channel::MemberRole>() {
                    Ok(r) => Some(r),
            ...
            let members = state.db.get_members(tenant.community(), channel_id).await?;
            let actor_role: Option<crew_db::channel::MemberRole> = members
                .iter()
                ...
                .filter(|(m, role)| m.role != role.as_str())
            {
                ...
                if target.role == "owner"
                    && role != crew_db::channel::MemberRole::Owner
        ...
        9002 => {
            ...
            for t in event.tags.iter() {
                if t.kind().to_string() == "name" {
                    match t.content() {
                        Some(v)
                            if !crew_core::channel::canonical_channel_name(v)
                                .trim()
                                .is_empty() => {}
                        _ => {
...
async fn emit_addressable_discovery_event(
    ...
    let min_ts = {
        let existing = state
            .db
            .query_events(&crew_db::event::EventQuery {
                kinds: Some(vec![kind as i32]),
                channel_id: Some(channel_id),
                limit: Some(1),
                ..crew_db::event::EventQuery::for_community(tenant.community())
            })
            .await
            .unwrap_or_default();
        existing
...
async fn store_group_members_event(
    ...
    state: &Arc<AppState>,
    channel_id: Uuid,
    member_snapshot: &mut crew_db::channel::LockedMemberSnapshot,
) -> anyhow::Result<Option<crew_core::StoredEvent>> {
...
async fn dispatch_group_members_event(
    tenant: &TenantContext,
    state: &Arc<AppState>,
    stored: Option<crew_core::StoredEvent>,
    relay_pubkey_hex: &str,
) {
...
pub async fn emit_group_discovery_events(
...

    {
        ...
        // Crew channels always require explicit membership
...
async fn handle_create_group(
    ...
    let channel_type: crew_db::channel::ChannelType = channel_type_str
        .parse()
    ...
    if visibility == crew_db::channel::ChannelVisibility::Open {
        state.invalidate_all_accessible_channels(tenant);
    }
...
async fn handle_join_request(
    ...
    if let Err(e) = emit_membership_notification(
        ...
        &actor_bytes,
        &actor_bytes,
        crew_core::kind::KIND_MEMBER_ADDED_NOTIFICATION,
    )
    .await
    {
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        crew_core::kind::KIND_WORKFLOW_DEF => {
...
async fn handle_git_repo_announcement(
    ...
    use crew_db::git_repo::ReserveOutcome;
...
/// carrying the announcer's pubkey in the `p` tag (crew extension).
...
pub async fn reconcile_nip43_membership_snapshots(state: &Arc<AppState>) -> anyhow::Result<usize> {
    ...
    for community in communities {
        let community_id = crew_core::CommunityId::from_uuid(community.id);
