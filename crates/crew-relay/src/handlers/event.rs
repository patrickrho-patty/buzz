...
use crew_core::event::StoredEvent;
use crew_core::kind::{
...
use crew_core::observer::{
    content_looks_like_nip44, OBSERVER_AGENT_TAG, OBSERVER_FRAME_CONTROL, OBSERVER_FRAME_TAG,
    OBSERVER_FRAME_TELEMETRY,
};
use crew_core::tenant::TenantContext;
use crew_core::verification::verify_event;
use crew_core::CommunityId;
use crew_pubsub::EventTopic;
...
pub async fn filter_fanout_by_access(
    ...
    let matches = if crew_core::kind::is_shared_gated_kind(event_kind_u32(&stored_event.event)) {
        let author = stored_event.event.pubkey.to_bytes();
        matches
            .into_iter()
            .filter(|(conn_id, _)| {
...
pub(crate) async fn fan_out_event_to_local_subscribers(
    ...
    metrics::histogram!("crew_fanout_recipients").record(matches.len() as f64);
    if matches.is_empty() {
        return;
    }
...
pub async fn fan_out_pubsub_event(state: &Arc<AppState>, channel_event: crew_pubsub::ChannelEvent) {
    ...
    let channel_id = match channel_event.topic {
        crew_pubsub::EventTopic::Channel(id) => Some(id),
        crew_pubsub::EventTopic::Global => None,
    };
    ...
    let matches = state.sub_registry.fan_out_scoped(community_id, &stored);
    let matches = filter_fanout_by_access(state, community_id, &stored, matches, None).await;
    metrics::counter!("crew_multinode_fanout_total").increment(1);
    if matches.is_empty() {
        return;
    }
...
pub(crate) async fn dispatch_persistent_event(
    ...
    metrics::counter!("crew_post_commit_dispatch_scheduled_total").increment(1);
...
async fn dispatch_persistent_event_inner(
    ...
    // ingest seam (`crates/crew-relay/src/handlers/ingest.rs`'s
    ...
    let event_json = match serde_json::to_string(&stored_event.event) {
        ...
        Err(e) => {
            ...
            metrics::counter!("crew_post_commit_dispatch_errors_total", "stage" => "serialize")
                .increment(1);
            return 0;
        }
    };
    ...
    let owner_only_kind = kind_u32 == crew_core::kind::KIND_DM_VISIBILITY
        || kind_u32 == crew_core::kind::KIND_AGENT_TURN_METRIC;
...
async fn enqueue_event_created_audit(
    ...
    let audit_entry = crew_audit::NewAuditEntry {
        community_id: tenant.community(),
        action: crew_audit::AuditAction::EventCreated,
...
pub async fn handle_event(event: Event, conn: Arc<ConnectionState>, state: Arc<AppState>) {
    ...
    metrics::counter!("crew_events_received_total", "kind" => kind_str).increment(1);
    ...
    metrics::counter!(
        "crew_community_events_received_total",
    ...
    if kind_u32 == crew_core::kind::KIND_AUTH {
        reject("invalid");
        conn.send(RelayMessage::ok(
            &event_id_hex,
            false,
    ...
    if kind_u32 == KIND_AGENT_OBSERVER_FRAME {
        if !scopes.is_empty() && !scopes.contains(&crew_auth::Scope::MessagesWrite) {
            reject("scope");
            conn.send(RelayMessage::ok(
                &event_id_hex,
                false,
    ...
    if is_ephemeral(kind_u32) {
        ...
        match crew_deletion::store(&state.db)
    ...
    match super::ingest::ingest_event(&state, &conn.tenant, event, ingest_auth).await {
        Ok(result) => {
            if result.accepted {
                // crew_events_stored_total is emitted inside ingest_event()
            ...
            metrics::histogram!("crew_event_processing_seconds")
...
mod tests {
    ...
    async fn observer_frame_rate_limiter_is_scoped_by_community() {
        ...
        let community_a = crew_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xAAAA));
        let community_b = crew_core::tenant::CommunityId::from_uuid(uuid::Uuid::from_u128(0xBBBB));
    ...
    async fn observer_owner_cache_is_scoped_to_community() {
        ...
        let community_a = crew_core::CommunityId::from_uuid(Uuid::new_v4());
        let community_b = crew_core::CommunityId::from_uuid(Uuid::new_v4());
    ...
    mod pubsub_fanout {
        ...
        use crew_core::kind::{KIND_MEMBER_ADDED_NOTIFICATION, KIND_PRESENCE_UPDATE};
        use crew_pubsub::{ChannelEvent, EventTopic};
        ...
        fn register_global_sub(
            ...
            state.conn_manager.register(
                ...
                crew_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn global_presence_pubsub_event_fans_out_to_local_subscribers() {
            ...
            fan_out_pubsub_event(
                &state,
                ChannelEvent {
                    community_id: crew_core::tenant::CommunityId::from_uuid(Uuid::nil()),
        ...
        async fn local_echo_presence_pubsub_event_is_not_delivered_twice() {
            ...
            let community = crew_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn local_echo_suppression_is_scoped_to_its_community() {
            ...
            let community_a = crew_core::tenant::CommunityId::from_uuid(Uuid::new_v4());
            let community_b = crew_core::tenant::CommunityId::from_uuid(Uuid::nil());
        ...
        async fn redis_presence_publish_reaches_second_relay_and_suppresses_origin_echo() {
            ...
            // a relay only PSUBSCRIBEs `crew:{community}:global` after it retains
            ...
            let tenant = crew_core::tenant::TenantContext::resolved(
                crew_core::tenant::CommunityId::from_uuid(Uuid::nil()),
                "test",
            );
            origin
            ...
            // Match crew-pubsub's own Redis round-trip test: give PSUBSCRIBE a
        ...
        async fn audit_records_caller_actor_not_relay_signer_for_relay_signed_event() {
            use crew_core::event::StoredEvent;
            use crew_core::tenant::{CommunityId, TenantContext};
            ...
            // Relay-signed event tagged crew:workflow so workflow triggering is
        ...
        /// call that `crew_audit::service::tests::chains_are_independent_per_community`
    ...
    mod fanout_access {
        ...
        pub(super) async fn test_state_with_redis_url(redis_url: &str) -> Arc<AppState> {
            ...
            let media_storage =
                crew_media::MediaStorage::new(&config.media).expect("media storage");
        ...
        async fn open_channel_event_passes_through_unfiltered() {
            ...
            let community_id = crew_core::tenant::CommunityId::from_uuid(Uuid::nil());
            state
        ...
        async fn author_only_reminder_delivers_to_author_only() {
            ...
            let reminder = EventBuilder::new(
                Kind::Custom(crew_core::kind::KIND_EVENT_REMINDER as u16),
                "{}",
            )
    ...
    //
    //  * `ConnEntry` (`crates/crew-relay/src/state.rs:30-44`) records
    ...
    //  * `SubscriptionRegistry` (`crates/crew-relay/src/subscription.rs:57+`)
    ...
    mod redteam {
        ...
        use crew_core::kind::KIND_PRESENCE_UPDATE;
        use crew_core::StoredEvent;
        use nostr::{EventBuilder, Keys, Kind};
        use tokio::sync::{mpsc, Mutex};
        use tokio_util::sync::CancellationToken;
        ...
        fn register_conn(
            state: &AppState,
            community_id: crew_core::tenant::CommunityId,
            pubkey: Option<Vec<u8>>,
        ) -> Uuid {
            let conn_id = Uuid::new_v4();
            let (tx, _rx) = mpsc::channel(1);
        ...
        async fn channel_less_event_must_drop_recipient_in_different_community() {
            ...
            let community_a = crew_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xAAAA));
            let community_b = crew_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xBBBB));
