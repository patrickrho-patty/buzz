...
use crew_core::filter::filters_match;
use crew_core::kind::{
...
pub async fn handle_req(
    ...
    let mut accessible_channels = if filters_are_nip43_membership_only(&filters) {
        metrics::counter!("crew_req_global_access_resolution_skips_total", "kind" => "13534")
            .increment(1);
        Vec::new()
    } else {
        match state
    ...
    let trace_state = crew_core::PublicKey::from_slice(&pubkey_bytes)
        .ok()
...
/// the response.
const MAX_SEARCH_PAGES: u32 = (crew_db::DEFAULT_MAX_PAGE_LIMIT as u32).div_ceil(SEARCH_PAGE_SIZE);
...
/// [`crew_search::ChannelScope`] enum that the Postgres-FTS search layer takes.
///
...
/// 1-to-1 with the table in [`crew_search::ChannelScope`]'s docs:
///
...
pub(crate) fn build_search_channel_scope_filter(
    ...
    include_global: bool,
) -> Option<crew_search::ChannelScope> {
    use crew_search::ChannelScope;
...
async fn handle_search_req(
    ...
    for filter in filters {
        ...
        let limit = filter
            .limit
            .map(|l| (l as u32).min(crew_db::DEFAULT_MAX_PAGE_LIMIT as u32))
            .unwrap_or(crew_db::DEFAULT_MAX_PAGE_LIMIT as u32);
        ...
        for page in 1..=MAX_SEARCH_PAGES {
            ...
            let search_query = crew_search::SearchQuery {
                ...
                mode: crew_search::SearchMode::FullText,
            };
            ...
            if !hit_ids.is_empty() {
                ...
                let event_map: std::collections::HashMap<[u8; 32], &crew_core::StoredEvent> =
                    events
                        .iter()
...
pub async fn build_event_query_from_filter(
    ...
    _pubkey_bytes: &[u8],
    _state: &AppState,
    community: crew_core::tenant::CommunityId,
) -> EventQuery {
...
pub fn filter_fully_pushable(filter: &Filter) -> bool {
    ...
    let is_nip33_only = filter.kinds.as_ref().is_some_and(|ks| {
        !ks.is_empty()
            && ks
                .iter()
                .all(|k| crew_core::kind::is_parameterized_replaceable(k.as_u16() as u32))
    });
    ...
    !filters.is_empty()
        && filters.iter().all(|filter| {
            filter.kinds.as_ref().is_some_and(|kinds| {
                !kinds.is_empty()
                    && kinds.iter().all(|kind| {
                        kind.as_u16() as u32 == crew_core::kind::KIND_NIP43_MEMBERSHIP_LIST
                    })
            })
        })
}
...
fn filter_to_query_params(
    ...
    let kinds: Option<Vec<i32>> = filter.kinds.as_ref().map(|ks| {
        if ks.is_empty() {
            ...
            // Cast to i32 for Postgres INT column; safe because all Crew kinds fit in i32.
    ...
    let limit = filter
        .limit
        .map(|l| (l as i64).min(crew_db::DEFAULT_MAX_PAGE_LIMIT))
        .unwrap_or(crew_db::DEFAULT_MAX_PAGE_LIMIT);
    ...
    let filter_is_nip33_only = kinds.as_ref().is_some_and(|ks| {
        !ks.is_empty()
            && ks
                .iter()
                .all(|&k| crew_core::kind::is_parameterized_replaceable(k as u32))
    });
...
pub(crate) fn event_visible_to_reader(event: &nostr::Event, requester_pubkey_bytes: &[u8]) -> bool {
    ...
    if !crew_core::filter::reader_authorized_for_event(event, &requester_pubkey_hex) {
        return false;
    }
    true
}
...
mod tests {
    ...
    fn global_queries_push_access_scope_before_limit() {
        ...
        let mut query = EventQuery::for_community(crew_core::tenant::CommunityId::from_uuid(
            uuid::Uuid::new_v4(),
        ));
    ...
    fn req_filter_limit_clamps_to_advertised_nip11_max_limit() {
        ...
        let community = crew_core::tenant::CommunityId::from_uuid(uuid::Uuid::new_v4());
        ...
        assert_eq!(crew_db::DEFAULT_MAX_PAGE_LIMIT, advertised);
    ...
    fn count_fallback_fetches_one_extra_candidate() {
        let mut query =
            EventQuery::for_community(crew_core::tenant::CommunityId::from_uuid(uuid::Uuid::nil()));
    ...
    fn extract_channel_id_from_multi_value_filter_returns_none() {
        ...
        assert_eq!(
            filter_to_query_params(
                ...
                crew_core::tenant::CommunityId::from_uuid(uuid::Uuid::nil()),
            )
            .channel_id,
            None,
    ...
    fn dm_visibility_requires_p_tag_even_with_ids() {
        ...
        let dm_vis = nostr::Kind::Custom(crew_core::kind::KIND_DM_VISIBILITY as u16);
        ...
        let member_notif_ids = Filter::new()
            .kind(nostr::Kind::Custom(
                crew_core::kind::KIND_MEMBER_ADDED_NOTIFICATION as u16,
            ))
            .id(nostr::EventId::from_hex(snapshot_id).unwrap());
    ...
    fn agent_turn_metric_requires_p_tag_even_with_ids() {
        ...
        let metric_kind = nostr::Kind::Custom(crew_core::kind::KIND_AGENT_TURN_METRIC as u16);
        ...
        // `reader_authorized_for_event` (crew-core/src/filter.rs), which gates
    ...
    fn agent_observer_subscription_requires_matching_p_tag() {
        ...
        let missing_p = Filter::new().kind(nostr::Kind::Custom(
            crew_core::kind::KIND_AGENT_OBSERVER_FRAME as u16,
        ));
    ...
    fn restricted_search_scope_excludes_global_results() {
        ...
        match scope {
            crew_search::ChannelScope::Channels(ids) => assert_eq!(ids, vec![channel_id]),
    ...
    fn push_lease_requires_self_author_filter_and_count_fallback() {
        ...
        let own = Filter::new()
            .kind(nostr::Kind::Custom(crew_core::kind::KIND_PUSH_LEASE as u16))
            .author(owner_key);
        let foreign = Filter::new()
            .kind(nostr::Kind::Custom(crew_core::kind::KIND_PUSH_LEASE as u16))
            .author(other_key);
        let bare = Filter::new().kind(nostr::Kind::Custom(crew_core::kind::KIND_PUSH_LEASE as u16));
    ...
    fn mixed_filter_omits_another_authors_push_lease() {
        ...
        let lease = nostr::EventBuilder::new(
            nostr::Kind::Custom(crew_core::kind::KIND_PUSH_LEASE as u16),
            "ciphertext",
        )
    ...
    fn result_gated_explicit_44200_can_match() {
        let f = Filter::new().kind(nostr::Kind::Custom(
            crew_core::kind::KIND_AGENT_TURN_METRIC as u16,
        ));
        assert!(filter_can_match_result_gated_kinds(&f));
    }
    ...
    fn result_gated_explicit_30622_can_match() {
        let f = Filter::new().kind(nostr::Kind::Custom(
            crew_core::kind::KIND_DM_VISIBILITY as u16,
        ));
        assert!(filter_can_match_result_gated_kinds(&f));
    }
