...
//! from `crew-conformance`. See `crates/crew-conformance/src/lib.rs` for
...
///   [`crew_db::Crew::communities_of_channels`]). On a hit, return the
...
fn project_row_community(
    ...
    channel_communities: &std::collections::HashMap<Uuid, crew_core::CommunityId>,
...
/// [`crew_db::Crew::communities_of_channels`] over the distinct
/// channel ids in `rows`.
