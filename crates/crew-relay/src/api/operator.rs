...
use crew_core::{CommunityId, TenantContext};
...
async fn check_operator_replay(
    ...
    match state
        ...
        .try_mark_in_scope(
            ...
            &event_id,
            crew_auth::DEFAULT_REPLAY_TTL_SECS,
        )
        .await
    {
        Ok(true) => Ok(()),
...
///
/// ```json
/// { "host": "acme.communities.crew.xyz", "initial_owner_pubkey": "<hex>" }
/// ```
///
...
pub async fn archive_community(
    ...
    let deployment_host = crew_core::tenant::relay_url_authority(&state.config.relay_url);
...
pub async fn transfer_community(
    ...
    let (status, previous_owner) = match result {
        crew_db::relay_members::TransferResult::Transferred { previous_owner } => {
            ("transferred", previous_owner)
        }
        crew_db::relay_members::TransferResult::AlreadyOwner => ("already_owner", None),
        crew_db::relay_members::TransferResult::NoOwner => {
            return Err(api_error(
                StatusCode::NOT_FOUND,
                "community has no owner to transfer from",
            ));
        }
        crew_db::relay_members::TransferResult::OwnerConflict => {
            return Err(api_error(
                StatusCode::CONFLICT,
                "owner_conflict: the current owner no longer matches expected_owner_pubkey",
            ));
        }
        crew_db::relay_members::TransferResult::LimitReached => {
            return Err(api_error(
                StatusCode::CONFLICT,
...
mod tests {
    ...
    use crew_core::{kind::KIND_NIP43_MEMBERSHIP_LIST, CommunityId};
    ...
    async fn fresh_host_at_owner_limit_returns_limit_reached_conflict() {
        ...
        for _ in 0..crew_db::relay_members::MAX_COMMUNITIES_PER_OWNER {
            let host = format!("community-{}.example", Uuid::new_v4().simple());
            assert_eq!(
