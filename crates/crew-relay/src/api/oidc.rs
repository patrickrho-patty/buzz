...
pub async fn complete(
    ...
    let email = match claims.email.clone() {
        ...
        _ => crate::oidc::fetch_userinfo_email(
            &state,
...
pub async fn whoami(
    ...
    let url =
        super::bridge::nip98_expected_url(&state.config.relay_url, &tenant, "/auth/oidc/whoami");
    let (pubkey, event_id_bytes) = super::bridge::verify_bridge_auth_with_options(
        &headers, "GET", &url, None, true,  // NIP-98 always required
