...
use base64::Engine;
use crew_audit::{AuditAction, NewAuditEntry};
use crew_core::tenant::TenantContext;
use crew_media::{BlobDescriptor, MediaError, UploadAttribution, UploadNetworkInfo};
...
fn should_stream_as_video(sniff: &[u8]) -> bool {
    infer::get(sniff).is_some_and(|kind| kind.mime_type() == "video/mp4")
        || crew_media::looks_like_iso_bmff(sniff)
}
...
fn upload_rate_limited(
    state: &AppState,
    community_id: crew_core::CommunityId,
...
impl FromRequestParts<Arc<AppState>> for AuthenticatedUpload {
    ...
    async fn from_request_parts(
        ...
        crew_media::auth::verify_blossom_auth_event(&auth_event, Some(tenant.host()), 3600)?;
...
/// (`CREW_MEDIA_UPLOAD_RECORDS`). Returns `None` when the feature is off —
...
///   (`CREW_MEDIA_UPLOAD_IP_HEADER`) and validated as a public IP —
...
async fn upload_attribution(
    ...
    let ip = header_value(&cfg.upload_ip_header).and_then(crew_media::parse_public_ip);
    let port = ip.and(header_value(&cfg.upload_port_header).and_then(crew_media::parse_port));
...
fn serving_write_error(error: anyhow::Error) -> MediaError {
    if crew_deletion::ServingWriteGuard::acquisition_is_fenced(&error) {
...
pub async fn upload_blob(
    ...
    let serving_write =
        crew_deletion::acquire_serving_write(&state.db, auth.tenant.community(), "media_upload")
            .await
    ...
    let mut descriptor = serving_write
        .protect(async {
            Ok(if should_stream_as_video(&sniff) {
                ...
                crew_media::process_video_upload(
                    &state.media_storage,
                    &state.config.media,
                    &auth.tenant,
                    &auth.auth_event,
                ...
                if is_image {
                    ...
                    crew_media::process_file_upload(
                        &state.media_storage,
                        &state.config.media,
                        &auth.tenant,
                        &auth.auth_event,
        ...
        .await
        .map_err(|error| {
            if crew_deletion::ServingWriteGuard::is_lease_lost(&error) {
...
async fn authenticate_media_read(
    ...
    crew_media::auth::verify_blossom_get_auth(&auth_event, sha256, Some(tenant.host()), 3600)?;
...
pub(crate) async fn serve_blob_for_tenant(
    ...
    let disposition = if crew_media::serve_inline(&content_type) {
        "inline"
    } else {
        "attachment"
    };
...
async fn resolve_s3_key(
    storage: &crew_media::MediaStorage,
    tenant: &TenantContext,
    sha256_ext: &str,
...
mod tests {
    ...
    fn serving_write_error_taxonomy_separates_fence_from_backend_failure() {
        let fenced = anyhow::Error::from(crew_db::DbError::AccessDenied("fenced".to_string()));
        assert!(matches!(
            serving_write_error(fenced),
            MediaError::CommunityWriteFenced
        ));
        let backend = anyhow::Error::from(crew_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
        assert!(matches!(
    ...
    async fn upload_rate_limiter_is_scoped_by_community() {
        ...
        let community_a = crew_core::CommunityId::from_uuid(Uuid::from_u128(0xAAAA));
        let community_b = crew_core::CommunityId::from_uuid(Uuid::from_u128(0xBBBB));
