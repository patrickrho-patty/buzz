...
use crew_auth::{LimitType, Nip98ReplayGuard, DEFAULT_REPLAY_TTL_SECS};
use crew_core::TenantContext;
...
pub(crate) async fn enforce_http_admission(
    ...
    .await
    {
        ...
        Err(crate::admission::AdmissionError::Exceeded { reset_in_secs }) => {
            metrics::counter!("crew_admission_rejections_total", "transport" => "http", "reason" => "quota").increment(1);
            Err(api_error(
        ...
        Err(crate::admission::AdmissionError::Unavailable) => {
            metrics::counter!("crew_admission_rejections_total", "transport" => "http", "reason" => "unavailable").increment(1);
            Err(api_error(
...
pub(crate) fn verify_bridge_auth_with_options(
        ...
        .and_then(|s| s.strip_prefix("Nostr "))
    {
        ...
        let pubkey = crew_auth::verify_nip98_event(&event_json, url, method, body)
...
fn extract_search_mode(raw: &Value) -> crew_search::SearchMode {
    match raw
        .get("search_mode")
        .or_else(|| raw.get("searchMode"))
        .and_then(Value::as_str)
    {
        Some("prefix") => crew_search::SearchMode::Prefix,
        _ => crew_search::SearchMode::FullText,
    }
}
...
const WINDOW_AUX_KINDS: [u32; 4] = [
    crew_core::kind::KIND_DELETION,
    crew_core::kind::KIND_REACTION,
    crew_core::kind::KIND_NIP29_DELETE_EVENT,
    crew_core::kind::KIND_STREAM_MESSAGE_EDIT,
];
...
async fn handle_channel_window_filter(
    state: &AppState,
    tenant: &crew_core::TenantContext,
    raw: &Value,
    filter: &nostr::Filter,
    accessible_channels: &[uuid::Uuid],
    events: &mut Vec<Value>,
) -> Result<(), (StatusCode, Json<Value>)> {
    use crew_core::kind::{KIND_THREAD_SUMMARY, KIND_WINDOW_BOUNDS};
    ...
    if extension_flag(raw, "include_aux") && !row_ids_hex.is_empty() {
        ...
        for hop_kinds in [&WINDOW_AUX_KINDS[..], &WINDOW_AUX_DELETE_KINDS[..]] {
            let mut aux_query = crew_db::EventQuery::for_community(tenant.community());
...
fn event_in_accessible_channel(se: &crew_core::StoredEvent, accessible: &[uuid::Uuid]) -> bool {
    match se.channel_id {
...
async fn submit_event_authed(
    ...
    let kind_u32 = crew_core::kind::event_kind_u32(&event);
    let auth = IngestAuth::Http {
        pubkey,
        scopes: crew_auth::Scope::all_known(), // Pure Nostr: full scopes, channel access via membership
...
async fn query_events_authed(
    ...
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        ...
        for feed_type in &feed_types {
            ...
            for se in type_events {
                ...
                if !crew_core::filter::reader_authorized_for_event(&se.event, &authed_pubkey_hex) {
                    continue;
                }
    ...
    let mut catchall_queries: Vec<(usize, crew_db::EventQuery)> = Vec::new();
    for (idx, (raw, filter)) in raw_filters.iter().zip(filters.iter()).enumerate() {
        if handled.contains(&idx) {
            continue;
        }
        ...
        match filter_events {
            Ok(stored_events) => {
                for se in stored_events {
                    ...
                    if !crew_core::filter::filters_match(std::slice::from_ref(filter), &se) {
                        continue;
                    }
                    ...
                    .await
                {
                    Ok(stored_events) => {
                        if crate::handlers::req::count_fallback_exceeded(stored_events.len()) {
                            metrics::counter!("crew_count_fallback_rejections_total").increment(1);
                            return Err(api_error(
                                StatusCode::BAD_REQUEST,
...
fn search_hit_accepted(
    filter: &nostr::Filter,
    stored: &crew_core::StoredEvent,
    accessible_channels: &[uuid::Uuid],
    reader_pubkey_hex: &str,
) -> bool {
    if !crew_core::filter::filters_match(std::slice::from_ref(filter), stored) {
        return false;
    }
    ...
    if !crew_core::filter::reader_authorized_for_event(&stored.event, reader_pubkey_hex) {
        return false;
    }
    true
}
...
async fn handle_bridge_search(
    ...
    filters: &[nostr::Filter],
    accessible_channels: &[uuid::Uuid],
    tenant: &crew_core::tenant::TenantContext,
    reader_pubkey_hex: &str,
    pubkey_bytes: &[u8],
) -> Result<Json<Value>, (StatusCode, Json<Value>)> {
    ...
    for (raw, filter) in raw_filters.iter().zip(filters) {
        ...
        let filter_channel_scope =
            if let Some(vs) = filter.generic_tags.get(&h_tag).filter(|vs| !vs.is_empty()) {
                ...
                crew_search::ChannelScope::Channels(valid)
            } else {
                channel_scope.clone()
            };
        ...
        let search_query = crew_search::SearchQuery {
            community: tenant.community(),
            q: search_text,
        ...
        let event_map: std::collections::HashMap<[u8; 32], &crew_core::StoredEvent> = stored_events
            .iter()
...
pub async fn workflow_webhook(
    ...
    let def: crew_workflow::WorkflowDef = serde_json::from_value(workflow.definition.clone())
        .map_err(|e| super::internal_error(&format!("corrupt workflow definition: {e}")))?;

    if !matches!(def.trigger, crew_workflow::TriggerDef::Webhook) {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
    ...
    let mut trigger_ctx = crew_workflow::executor::TriggerContext {
        channel_id: workflow
            .channel_id
            .map(|ch| ch.to_string())
            .unwrap_or_default(),
    ...
    if !workflow.enabled || workflow.status != crew_db::workflow::WorkflowStatus::Active {
    ...
    tokio::spawn(async move {
        let def: crew_workflow::WorkflowDef = match serde_json::from_value(def_value) {
            Ok(d) => d,
            Err(e) => {
                ...
                if let Err(db_err) = db
                    .update_workflow_run(
                        community_id,
                        run_id,
                        crew_db::workflow::RunStatus::Failed,
                        0,
                        &serde_json::json!([]),
                        Some(crew_db::workflow::WorkflowRunFailure {
        ...
        let result = crew_workflow::executor::execute_from_step(
            &engine,
            community_id,
            run_id,
            &def,
...
async fn synthesize_presence(
    ...
    use crew_core::kind::{KIND_PRESENCE_SNAPSHOT, KIND_PRESENCE_UPDATE};
...
fn report_json(r: &crew_db::moderation::ReportRecord) -> Value {
    let (target_kind, target) = match &r.target {
        crew_db::moderation::ReportTarget::Event(id) => ("event", hex::encode(id)),
        crew_db::moderation::ReportTarget::Pubkey(pk) => ("pubkey", hex::encode(pk)),
        crew_db::moderation::ReportTarget::Blob(sha) => ("blob", hex::encode(sha)),
    };
    serde_json::json!({
        "id": r.id,
...
fn action_json(a: &crew_db::moderation::ActionRecord) -> Value {
    serde_json::json!({
        "id": a.id,
...
fn ban_json(b: &crew_db::moderation::BanRecord) -> Value {
    serde_json::json!({
...
mod tests {
    ...
    fn fresh_tenant(host: &str) -> TenantContext {
        TenantContext::resolved(
            crew_core::CommunityId::from_uuid(uuid::Uuid::new_v4()),
            host,
        )
    }
    ...
    fn bridge_search_mode_extension_defaults_to_full_text() {
        assert_eq!(
            ...
            crew_search::SearchMode::FullText
        );
        assert_eq!(
    ...
    async fn nip98_replay_guard_rejects_cross_pod_replay_on_bridge_path() {
        let pool = redis_pool();
        let pod_a = crew_pubsub::RedisNip98ReplayGuard::new(pool.clone());
        let pod_b = crew_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    async fn nip98_replay_guard_rejects_same_pod_same_community_replay() {
        let pool = redis_pool();
        let pod = crew_pubsub::RedisNip98ReplayGuard::new(pool);
    ...
    /// `crew-auth/src/nip98_replay.rs:70-73`).
    ///
    ...
    /// `crew_auth::nip98::tests::make_nip98_event` so the bridge tests don't
    /// reach into crew-auth's test scope.
    ...
    /// `crew moderation reports` / `audit` 401'd on a NIP-98 URL mismatch in
    ...
    fn verify_nip42_rejects_event_signed_for_wrong_communitys_host() {
        ...
        assert!(
            matches!(err, crew_auth::AuthError::RelayUrlMismatch),
    ...
    fn engram_envelope(agent: &Keys, owner_hex: &str) -> crew_core::StoredEvent {
        ...
        let ev = EventBuilder::new(Kind::Custom(30174), "engram body")
            ...
            .expect("sign engram");
        crew_core::StoredEvent::new(ev, None)
    }
    ...
    fn extract_page_offset_sizes_pages_from_clamped_limit() {
        let clamped = crew_db::DEFAULT_MAX_PAGE_LIMIT;
    ...
    fn event_accessible_no_channel() {
        ...
        let ev = EventBuilder::new(Kind::Custom(1), "test")
            .sign_with_keys(&keys)
            .unwrap();
        let se = crew_core::StoredEvent::new(ev, None);
    ...
    fn event_accessible_matching_channel() {
        ...
        let mut se = crew_core::StoredEvent::new(ev, None);
    ...
    fn search_hit_rejects_dm_visibility_for_kindless_ids_third_party() {
        ...
        let ev = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_DM_VISIBILITY as u16), "")
            .tags([d_tag, p_tag])
            .sign_with_keys(&relay)
            .expect("sign snapshot");
        let stored = crew_core::StoredEvent::new(ev.clone(), None);
    ...
    // increments crew_events_rejected_total{transport="http"}.  They are
    ...
    impl Nip98ReplayGuard for AlwaysFreshReplayGuard {
        fn try_mark_in_scope<'a>(
            ...
            Box<dyn std::future::Future<Output = Result<bool, crew_auth::AuthError>> + Send + 'a>,
        > {
    ...
    async fn bridge_handler_test_state() -> Option<Arc<crate::state::AppState>> {
        ...
        let db = crew_db::Db::from_pool(pool.clone());
        let redis_pool = deadpool_redis::Config::from_url(&config.redis_url)
            .create_pool(Some(deadpool_redis::Runtime::Tokio1))
            .ok()?;
        let pubsub = Arc::new(
            crew_pubsub::PubSubManager::new(&config.redis_url, redis_pool.clone())
                .await
                .ok()?,
        );
        let audit = crew_audit::AuditService::new(pool.clone());
        let auth = crew_auth::AuthService::new(config.auth.clone());
        let search = crew_search::SearchService::new(pool.clone());
        let workflow_engine = Arc::new(crew_workflow::WorkflowEngine::new(
            db.clone(),
            crew_workflow::WorkflowConfig::default(),
        ));
        let media_storage = crew_media::MediaStorage::new(&config.media).ok()?;
    ...
    /// Collect crew_events_rejected_total with (transport, reason) labels from
    ...
    /// increment crew_events_rejected_total{transport="http",reason="invalid"}.
    ///
    ...
    fn submit_event_relay_only_kind_increments_http_transport_counter() {
        ...
        let relay_only_event = EventBuilder::new(
            Kind::Custom(crew_core::kind::KIND_NIP43_MEMBERSHIP_LIST as u16),
            "",
        )
