...
) -> Result<Json<Vec<crew_db::admin_moderation::AdminReport>>, ApiError> {
...
async fn report_detail(
    ...
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<crew_db::admin_moderation::AdminReportDetail>, ApiError> {
    authorize(&state, &headers)?;
    state
        .db
        .admin_get_report(id)
...
async fn feedback_detail(
    ...
    headers: HeaderMap,
    Path(id): Path<Uuid>,
) -> Result<Json<crew_db::admin_moderation::AdminFeedback>, ApiError> {
    authorize(&state, &headers)?;
    state
        .db
...
async fn feedback_attachment(
    ...
    let response = crate::api::media::serve_blob_for_tenant(&state, &tenant, &sha256, &headers)
        ...
        .map_err(|error| match error {
            crew_media::MediaError::NotFound => ApiError::not_found(),
            _ => ApiError::internal(),
        })?;
    tracing::info!(
        feedback_id = %feedback.id,
...
fn attachment_url_matches(url: &str, community_host: &str, sha256: &str) -> bool {
    ...
    matches!(url.scheme(), "http" | "https")
        && crew_core::tenant::normalize_host(&authority)
            == crew_core::tenant::normalize_host(community_host)
        && url_hash == sha256
