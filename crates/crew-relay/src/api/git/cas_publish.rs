...
fn record_compaction(
    ...
    metrics::counter!("crew_git_pack_compactions_total", "outcome" => outcome).increment(1);
    metrics::histogram!("crew_git_pack_compaction_seconds", "outcome" => outcome)
        .record(started_at.elapsed().as_secs_f64());
    metrics::histogram!("crew_git_pack_compaction_packs_before").record(packs_before as f64);
    if let Some(packs_after) = packs_after {
        metrics::histogram!("crew_git_pack_compaction_packs_after").record(packs_after as f64);
    }
    if let Some(compacted_bytes) = compacted_bytes {
        metrics::histogram!("crew_git_pack_compaction_bytes").record(compacted_bytes as f64);
    }
}
...
async fn cas_publish_inner(
    ...
    let m_after = if let Some(manifest) = compacted_manifest {
        ...
        if pack_bytes.is_some() && packs_before >= MAX_MANIFEST_PACKS {
            ...
            metrics::counter!("crew_git_pack_compaction_required_failures_total").increment(1);
            return Err(error);
        }
...
mod tests {
    ...
    fn tenant() -> TenantContext {
        TenantContext::resolved(
            crew_core::CommunityId::from_uuid(uuid::Uuid::from_u128(1)),
            "git.example",
        )
    }
