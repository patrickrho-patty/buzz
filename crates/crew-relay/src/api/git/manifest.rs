...
/// crew pins SHA-1 today but the predicate is forward-looking. Used
...
mod tests {
    ...
    fn pointer_writer_is_covered_by_deletion_taxonomy() {
        ...
        let prefixes = crew_media::tenant_prefixes(*community.as_uuid());

        assert!(prefixes.iter().any(|prefix| key.starts_with(prefix)));
        assert!(crew_media::is_tenant_owned_key(*community.as_uuid(), &key));
    }
