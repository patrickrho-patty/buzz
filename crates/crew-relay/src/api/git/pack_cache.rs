...
impl Drop for PopulationPermit<'_> {
    fn drop(&mut self) {
        metrics::gauge!("crew_git_pack_cache_populations_active").decrement(1.0);
    }
}
...
impl GitPackCache {
    ...
    pub async fn materialize_pack(
        ...
        if let Some(entry) = self.lookup(digest) {
            metrics::counter!("crew_git_pack_cache_lookups_total", "result" => "hit").increment(1);
        ...
        metrics::counter!(
            "crew_git_pack_cache_lookups_total",
        ...
        let result = if let Some(entry) = self.lookup(digest) {
            ...
            metrics::histogram!(
                "crew_git_pack_cache_populate_seconds",
                "outcome" => outcome
            )
    ...
    async fn populate(
        ...
        metrics::histogram!("crew_git_pack_cache_population_wait_seconds")
            .record(wait_started_at.elapsed().as_secs_f64());
        metrics::gauge!("crew_git_pack_cache_populations_active").increment(1.0);
        ...
        if self.max_bytes == 0 || total_bytes > self.max_bytes {
            metrics::counter!("crew_git_pack_cache_bypasses_total").increment(1);
        ...
        let mut removed = Vec::new();
        {
            ...
            while state.total_bytes > self.max_bytes {
                ...
                if let Some(record) = state.entries.remove(&digest) {
                    ...
                    metrics::counter!("crew_git_pack_cache_evictions_total").increment(1);
                }
            }
            emit_size_metrics(&state);
        }
...
async fn install_entry(
    ...
    metrics::counter!("crew_git_pack_cache_copy_fallbacks_total").increment(1);
    Ok(())
}
...
fn emit_size_metrics(state: &CacheState) {
    metrics::gauge!("crew_git_pack_cache_bytes").set(state.total_bytes as f64);
    metrics::gauge!("crew_git_pack_cache_entries").set(state.entries.len() as f64);
}
