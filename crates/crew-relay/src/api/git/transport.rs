...
impl axum::extract::FromRequestParts<Arc<AppState>> for GitAuth {
    ...
    async fn from_request_parts(
        ...
        let pubkey =
            crew_auth::nip98::verify_nip98_event(&event_json, &expected_url, &event_method, None)
                .map_err(|e| {
...
async fn git_restriction_state(
    ...
    community: crew_core::CommunityId,
    pubkey: &nostr::PublicKey,
) -> Result<crew_db::moderation::RestrictionState, Response> {
...
fn enforce_git_ban(restriction: &crew_db::moderation::RestrictionState) -> Result<(), StatusCode> {
...
fn enforce_git_ban_cascade(
    agent: &crew_db::moderation::RestrictionState,
    owner: Option<&crew_db::moderation::RestrictionState>,
) -> Result<(), StatusCode> {
...
fn read_role_allows(role: Option<&str>) -> bool {
    match role {
        Some(r) => r.parse::<crew_core::channel::MemberRole>().is_ok(),
        None => false,
    }
}
...
async fn finalize_push_inner(
    ...
    let serving_write = match crew_deletion::acquire_serving_write(
        &state.db,
...
mod track_c_tests {
    ...
    async fn approved_deletion(
        ...
        crew_db::deletion::DeletionRequest,
        crew_db::deletion::ClaimedDeletion,
    ) {
        ...
        let inventory = FrozenInventory {
            ...
            storage: StorageManifest {
                version: 4,
                prefixes: crew_media::tenant_prefixes(*request.community_id.as_uuid())
                    .into_iter()
    ...
    async fn finalize_push_holds_serving_lease_through_post_cas_publication() {
        ...
        assert!(matches!(
            error,
            crew_db::DbError::ServingWritesNotDrained { .. }
        ));
        assert!(!state
            .db
            .deletion_store()
        ...
        let mut query = crew_db::event::EventQuery::for_community(community);
        ...
        assert_eq!(
            state
                ...
                .stage,
            crew_db::deletion::DeletionStage::Fenced
        );
        drop(state);
        pool.close().await;
    }
    ...
    fn git_nip98_rejects_token_signed_for_wrong_community_host() {
        ...
        let err = crew_auth::nip98::verify_nip98_event(&event_json, &expected_for_b, "GET", None)
    ...
    fn git_nip98_accepts_token_signed_for_matching_community_host() {
        ...
        let pubkey =
            crew_auth::nip98::verify_nip98_event(&event_json, &expected_for_a, "GET", None)
...
mod sec005_read_gate_tests {
    ...
    fn durable_ban_denies_git_even_with_otherwise_valid_auth() {
        let restriction = crew_db::moderation::RestrictionState {
            banned: true,
            muted_until: None,
        };
    ...
    fn restriction(banned: bool) -> crew_db::moderation::RestrictionState {
        crew_db::moderation::RestrictionState {
            banned,
            muted_until: None,
        }
    }
    ...
    async fn setup_repo(binding: Binding) -> RepoFixture {
        ...
        db.create_channel_with_id(
            ...
            crew_db::channel::ChannelType::Stream,
            crew_db::channel::ChannelVisibility::Open,
            None,
            &creator_pk,
            None,
        )
        ...
        db.add_member(
            ...
            channel,
            &member_pk,
            crew_core::channel::MemberRole::Member,
            Some(&creator_pk),
        )
        .await
        .expect("member");
    ...
    async fn setup_ban_community() -> (crew_db::Db, crew_core::CommunityId, Vec<u8>) {
        let db = setup_db().await;
    ...
    async fn ban_gate_cascades_to_a_banned_nip_oa_owner() {
        ...
        let auth_tag = crew_sdk::nip_oa::compute_auth_tag(&owner, &agent.public_key(), "kind=9")
            .expect("auth tag");
    ...
    async fn ban_gate_fails_closed_with_503_when_the_store_is_unreachable() {
        ...
        let community = crew_core::CommunityId::from_uuid(uuid::Uuid::new_v4());
        let (status, body) = denial_parts(
