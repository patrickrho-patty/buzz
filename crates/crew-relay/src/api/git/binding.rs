...
//!
//! The `crew-channel` tag on a kind:30617 announcement IS the git ACL: the
...
//!
//! Only the *first* `crew-channel` tag is considered, and it must carry a
...
//! who can append a second `crew-channel` tag would pick the channel.
//!
...
pub enum RepoBinding {
    /// No `crew-channel` tag at all. The announcement author (the only
    /// identity that can rebind — 30617 is keyed by `(author, d)`) may be
    /// offered remediation; everyone else gets the generic denial.
    NotBound,
    /// First `crew-channel` tag carries a valid UUID.
    Bound(Uuid),
    /// First `crew-channel` tag exists but its value is not a UUID.
...
pub fn resolve_repo_binding(event: &nostr::Event) -> RepoBinding {
    let Some(first) = event
        .tags
        .iter()
        .find(|t| t.as_slice().first().map(String::as_str) == Some("crew-channel"))
    else {
...
mod tests {
    ...
    fn extracts_valid_uuid() {
        ...
        let event = announcement(vec![
            ...
            Tag::parse(["crew-channel", &ch.to_string()]).unwrap(),
        ]);
    ...
    fn fails_closed_on_ambiguous_duplicate_bindings() {
        ...
        let valid_first = announcement(vec![
            Tag::parse(["d", "repo"]).unwrap(),
            Tag::parse(["crew-channel", &ch.to_string()]).unwrap(),
            Tag::parse(["crew-channel", &other.to_string()]).unwrap(),
        ]);
