...
//! ## The 412 sharp edge
//!
//! `rust-s3 = "0.37"` is shared across the workspace with `crew-media`. The
...
impl GitStore {
    ...
    ///
    /// Credential selection mirrors [`crew_media::MediaStorage::new`]:
    ...
    pub fn new(
        ...
        bucket_name: &str,
        region: &str,
        addressing_style: crew_media::config::S3AddressingStyle,
        ...
        let bucket = match addressing_style {
            crew_media::config::S3AddressingStyle::Path => bucket.with_path_style(),
            crew_media::config::S3AddressingStyle::Virtual => bucket,
        };
        Ok(Self {
...
mod probe {
    ...
    async fn probe_412_surfacing() {
        if !probe_enabled() {
            eprintln!("skipping: set CREW_GIT_S3_PROBE=1 to run against live MinIO");
            return;
        }
        let st = store();
