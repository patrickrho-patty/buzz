...
//! 4. Otherwise resolves the pusher's channel role via crew-channel binding
...
use crew_core::channel::MemberRole;
use crew_core::git_perms::{
...
pub async fn hook_policy_check(
    ...
    let community = crew_core::CommunityId::from_uuid(community_uuid);
    ...
    let rules = match parse_protection_tags(&tags) {
        Ok(parsed) => {
            ...
            for unknown in &parsed.unknown_rules {
                warn!(repo = %req.repo_id, rule = %unknown, "unknown crew-protect rule (skipped)");
            }
            parsed.rules
        }
        Err(e) => {
    ...
    let channel_id = match crate::api::git::binding::resolve_repo_binding(&repo_event.event) {
        ...
        crate::api::git::binding::RepoBinding::Broken => {
            warn!(repo = %req.repo_id, "hook callback: broken crew-channel binding");
    ...
    let role = if is_repo_owner || is_managed_agent_owner {
        ...
        match channel_id {
            None => {
                warn!(repo = %req.repo_id, "hook callback: no crew-channel binding");
                // Declared cross-component contract — see the const docs in
                // crew-core::git_perms for who consumes the token and why
...
mod tests {
    ...
    fn no_channel_binding_body_satisfies_old_and_new_matchers() {
        assert!(
            GIT_NO_CHANNEL_BINDING_BODY.starts_with(&format!(
                "{}: ",
                crew_core::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN
            )),
    ...
    fn bash_hmac_matches_rust_hmac() {
        ...
        let bash_script = format!(
            r#"
export LC_ALL=C
CREW_REPO_ID="{repo_id}"
CREW_REPO_OWNER="{repo_owner}"
CREW_COMMUNITY_ID="{community_id}"
CREW_PUSHER_PUBKEY="{pusher}"
CREW_HOOK_SECRET="{secret}"
TIMESTAMP="{timestamp}"
...
# Compute HMAC-SHA256
printf '%s' "$HMAC_INPUT" | openssl dgst -sha256 -hmac "$CREW_HOOK_SECRET" -hex 2>/dev/null | sed 's/.*= //'
"#,
            repo_id = repo_id,
...
CREW_REPO_ID="{repo_id}"
REPO_ID_LEN=${{#CREW_REPO_ID}}
HMAC_INPUT="${{REPO_ID_LEN}}:${{CREW_REPO_ID}}|{owner}|{community_id}|{pusher}|"
sort "$HMAC_FILE" | while IFS=' ' read -r ref_name old_oid new_oid is_anc; do
    REF_LEN=${{#ref_name}}
    printf '%s%s%s:%s%s' "$old_oid" "$new_oid" "$REF_LEN" "$ref_name" "$is_anc"
done > "$HMAC_FILE.concat"
...
"#,
    ...
    async fn push_gate_denies_owner_through_broken_binding() {
        ...
        assert!(
            !body.contains(crew_core::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN),
