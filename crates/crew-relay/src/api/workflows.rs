...
async fn authorize_workflow_read(
    ...
    let workflow = state
        ...
        .map_err(|error| match error {
            crew_db::error::DbError::NotFound(_) => {
...
fn run_json(run: &crew_db::workflow::WorkflowRunRecord) -> Value {
    serde_json::json!({
        "id": run.id,
...
fn approval_json(approval: &crew_db::workflow::ApprovalRecord) -> Value {
    serde_json::json!({
...
mod tests {
    ...
    fn approval_wire_does_not_expose_hash_as_token() {
        let approval = crew_db::workflow::ApprovalRecord {
            token: vec![0xab; 32],
            ...
            status: crew_db::workflow::ApprovalStatus::Pending,
