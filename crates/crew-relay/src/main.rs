...
use crew_auth::AuthService;
use crew_core::CommunityId;
use crew_db::{Db, DbConfig};
use crew_pubsub::PubSubManager;
use crew_search::SearchService;

use crew_relay::config::{Config, MAX_DRAIN_JITTER_MS};
use crew_relay::metrics as relay_metrics;
use crew_relay::router::{build_health_router, build_router};
use crew_relay::state::AppState;
use crew_relay::storage_sweep;
use crew_relay::telemetry;
use crew_workflow::WorkflowEngine;
use tokio_util::sync::CancellationToken;

fn crew_auto_migrate_enabled(value: Option<&str>) -> bool {
...
async fn main() -> anyhow::Result<()> {
    ...
    //
    // Build a single shared Resource (service.name=crew-relay by default, overridable
    ...
    let otel_layer = match &tracer_init {
        telemetry::TracerInit::Enabled(p) => {
            ...
            Some(tracing_opentelemetry::layer().with_tracer(p.tracer("crew-relay")))
        }
        _ => None,
    };
    ...
    let deployment_community = {
        let host = crew_relay::tenant::relay_url_authority(&config.relay_url);
        if host.is_empty() {
    ...
    let workflow_config = crew_workflow::WorkflowConfig::default();
    ...
    let media_storage = crew_media::MediaStorage::new(&config.media)
    ...
    if let Some(handle) = crew_relay::mesh_boot::boot_mesh(
        &state.config,
        ...
        .unwrap_or(true)
    {
        ...
        let race_rounds = std::env::var("CREW_GIT_PROBE_ROUNDS")
            ...
            .and_then(|v| v.parse().ok())
            .unwrap_or(3);
        let cfg = crew_relay::api::git::store::ProbeConfig {
            race_width,
            race_rounds,
        };
        tracing::info!(
    ...
    match crew_relay::handlers::side_effects::reconcile_large_channel_member_snapshots(&state).await
    {
    ...
    if config.require_relay_membership {
        match crew_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(&state).await
        {
        ...
        tokio::spawn(async move {
            ...
            loop {
                interval.tick().await;
                match crew_relay::handlers::side_effects::reconcile_nip43_membership_snapshots(
                    &reconcile_state,
                )
                .await
                {
    ...
    if std::env::var("CREW_RECONCILE_CHANNELS").is_ok() {
        ...
        tokio::spawn(async move {
            ...
            let tenant = match crew_relay::tenant::bind_deployment_community(
                &reconcile_state.db,
            ...
            for attempt in 0..24u32 {
                ...
                match crew_relay::handlers::side_effects::reconcile_channel_events(
                    &tenant,
                    &reconcile_state,
                )
                .await
    ...
    let action_sink = Arc::new(crew_relay::workflow_sink::RelayActionSink::new(&state));
    ...
    if state.config.oidc.enabled {
        ...
        tokio::spawn(async move {
            info!(
                interval_secs = crew_relay::oidc::SYNC_INTERVAL.as_secs(),
                "OIDC membership sync loop started"
            );
            crew_relay::oidc::run_sync_loop(sync_state).await;
        });
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for channel in &expired {
                    ...
                    if let Err(e) = crew_relay::handlers::side_effects::emit_system_message(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    ...
                    if let Err(e) = crew_relay::handlers::side_effects::emit_group_discovery_events(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
                    ...
                    crew_relay::handlers::side_effects::evict_all_channel_subscriptions(
                        &tenant,
                        &reaper_state,
                        channel_id,
                    )
    ...
    if state.config.push_gateway_delivery_url.is_some() {
        tokio::spawn(crew_relay::push_runtime::run_matcher(Arc::clone(&state)));
        tokio::spawn(crew_relay::push_runtime::run_delivery_worker(Arc::clone(
            &state,
        )));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                for reminder in due {
                    ...
                    let reminder_tenant = crew_core::tenant::TenantContext::resolved(
    ...
    // (published by other relay instances) and fan out to local WS subscribers.
    {
        ...
        tokio::spawn(async move {
            loop {
                match rx.recv().await {
                    Ok(channel_event) => {
                        crew_relay::handlers::event::fan_out_pubsub_event(
                            &state_for_sub,
                            channel_event,
                        )
                        .await;
            ...
            loop {
                match rx.recv().await {
                    Ok(scoped) => match scoped.command {
                        crew_pubsub::conn_control::ConnControl::DisconnectCommunity => {
                            state_for_conn_ctrl
                                .community_connections
                                .disconnect_community(scoped.community_id);
                        }
                        crew_pubsub::conn_control::ConnControl::DisconnectPubkey {
                            pubkey,
                            event_id,
                            reason,
                        } => {
...
mod env_filter_tests {
    ...
    use crew_relay::telemetry::otel_env_filter;
...
/// (capped at [`crew_relay::config::MAX_DRAIN_JITTER_MS`] = 20s) plus the
...
async fn serve(
    ...
    info!(addr = %config.bind_addr, "crew-relay TCP listening");
    ...
    if let Some(ref uds_path) = config.uds_path {
        ...
        info!(path = %uds_path, "crew-relay UDS listening");
...
fn reminder_to_event(reminder: &crew_db::event::DueReminder) -> nostr::Event {
...
async fn run_usage_metrics_tick(
    state: &AppState,
    emission_scope: &EmissionScope,
    leader: &mut Option<crew_db::UsageMetricsLeader>,
...
async fn run_storage_sweep_tick(
    ...
    storage_sweep::maybe_spawn_sweep(
        ...
        config.timeout,
        async move {
            crew_media::fold_bucket_listing(max_objects, move |token| {
...
mod tests {
    ...
    use super::{
        crew_auto_migrate_enabled, dropped_in_memory_keys, idle_timeout_secs,
    ...
    #[test]
    fn crew_auto_migrate_is_opt_in() {
        assert!(!crew_auto_migrate_enabled(None));
        assert!(!crew_auto_migrate_enabled(Some("")));
        assert!(!crew_auto_migrate_enabled(Some("false")));
        assert!(!crew_auto_migrate_enabled(Some("0")));
        assert!(!crew_auto_migrate_enabled(Some("no")));

        assert!(crew_auto_migrate_enabled(Some("true")));
        assert!(crew_auto_migrate_enabled(Some("TRUE")));
        assert!(crew_auto_migrate_enabled(Some(" 1 ")));
        assert!(crew_auto_migrate_enabled(Some("yes")));
        assert!(crew_auto_migrate_enabled(Some("on")));
    }
