...
/// enforces membership (`CREW_REQUIRE_RELAY_MEMBERSHIP=true`) AND has a
...
///
/// `max_limit` is [`crew_db::DEFAULT_MAX_PAGE_LIMIT`], the same constant the
...
fn relay_limitation(max_message_length: usize) -> RelayLimitation {
    ...
    RelayLimitation {
        ...
        max_filters: Some(10),
        max_limit: Some(crew_db::DEFAULT_MAX_PAGE_LIMIT as u32),
...
impl RelayInfo {
    ...
    pub fn build(
        ...
        Self {
            ...
            push: None,
            software: "https://github.com/block/crew".to_string(),
...
mod tests {
    ...
    fn build_advertises_crew_repository_url() {
        ...
        assert_eq!(info.software, "https://github.com/block/crew");
    }
    ...
    fn auth_required_is_advertised_true() {
        ...
        // `AuthState::Authenticated` (see `crates/crew-relay/src/handlers/`),
