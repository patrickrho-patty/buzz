...
use crew_auth::{AuthService, Nip98ReplayGuard};
use crew_core::tenant::TenantContext;
use crew_core::CommunityId;
use crew_db::Db;
use crew_media::MediaStorage;
use crew_pubsub::cache_invalidation::CacheInvalidation;
use crew_pubsub::conn_control::ConnControl;
use crew_pubsub::rate_limiter::RedisRateLimiter;
use crew_pubsub::{PubSubManager, RedisNip98ReplayGuard};
...
pub(crate) async fn run_registered_community_connection<Check, CheckFuture, Run, RunFuture>(
    ...
    CheckFuture: Future<Output = Result<bool, crew_db::DbError>>,
...
) -> (usize, Vec<(CommunityId, crew_db::DbError)>)
where
...
pub struct AppState {
    ...
    pub audit_tx: Option<mpsc::Sender<crew_audit::NewAuditEntry>>,
    ...
    /// See `crates/crew-conformance/` and `crate::conformance` for the
    /// schema, emitter helpers, and the independent checker.
    pub tracer: Arc<dyn crew_conformance::Tracer>,
...
impl AppState {
    ...
    pub fn new(
        ...
        let (audit_tx, mut audit_rx) = mpsc::channel::<crew_audit::NewAuditEntry>(1000);
        ...
        let state = Self {
            ...
            // `crates/crew-test-client` once those land).
    ...
    pub async fn is_member_cached(
        ...
        channel_id: Uuid,
        pubkey: &[u8],
    ) -> Result<bool, crew_db::DbError> {
        let key = (community_id, channel_id, pubkey.to_vec());
        if let Some(cached) = self.membership_cache.get(&key) {
            metrics::counter!("crew_membership_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("crew_membership_cache_misses_total").increment(1);
    ...
    pub async fn disconnect_community_clusterwide(
        &self,
        tenant: &TenantContext,
    ) -> Result<usize, crew_pubsub::PubSubError> {
        let closed = self
    ...
    pub async fn get_accessible_channel_ids_cached(
        ...
        community_id: CommunityId,
        pubkey: &[u8],
    ) -> Result<Vec<Uuid>, crew_db::DbError> {
        let key = (community_id, pubkey.to_vec());
        if let Some(cached) = self.accessible_channels_cache.get(&key) {
            metrics::counter!("crew_accessible_channels_cache_hits_total").increment(1);
            return Ok(cached);
        }
        metrics::counter!("crew_accessible_channels_cache_misses_total").increment(1);
        let result = self
            .db
    ...
    pub async fn channel_visibility_cached(
        ...
        community_id: CommunityId,
        channel_id: Uuid,
        prefetched: Option<&crew_db::channel::ChannelRecord>,
    ) -> Result<String, crew_db::DbError> {
...
async fn log_audit_entry(audit: &crew_audit::AuditService, entry: crew_audit::NewAuditEntry) {
    let t = std::time::Instant::now();
    if let Err(e) = audit.log(entry).await {
        metrics::counter!("crew_audit_log_errors_total").increment(1);
        tracing::error!("Audit log failed: {e}");
    } else {
        metrics::histogram!("crew_audit_log_seconds").record(t.elapsed().as_secs_f64());
    }
}
...
mod tests {
    ...
    async fn test_state() -> Arc<AppState> {
        ...
        let media_storage = crew_media::MediaStorage::new(&config.media).expect("media storage");
    ...
    fn shared_counter_between_direct_and_fanout() {
        ...
        let conn = ConnectionState {
            conn_id,
            tenant: crew_core::tenant::TenantContext::resolved(
                crew_core::tenant::CommunityId::from_uuid(Uuid::nil()),
    ...
    async fn revalidation_continues_after_one_community_lookup_failure() {
        ...
        let (closed, failures) =
            revalidate_registered_communities(&registry, |community| async move {
                if community == failed {
                    Err(crew_db::DbError::InvalidData(
    ...
    async fn disconnect_pubkey_is_fenced_to_the_banning_community() {
        ...
        let community_a = crew_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xa));
        let community_b = crew_core::tenant::CommunityId::from_uuid(Uuid::from_u128(0xb));
    ...
    async fn drain_all_sends_restart_close_and_cancels_every_conn() {
        ...
        let (mut ctrl_a, cancel_a) = register(crew_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xa),
        ));
        let (mut ctrl_b, cancel_b) = register(crew_core::tenant::CommunityId::from_uuid(
            Uuid::from_u128(0xb),
        ));
