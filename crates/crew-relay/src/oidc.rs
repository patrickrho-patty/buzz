...
pub async fn validate_id_token(state: &AppState, token: &str) -> Result<IdClaims, String> {
    ...
    let header =
        jsonwebtoken::decode_header(token).map_err(|e| format!("jwt header parse: {e}"))?;
...
pub async fn fetch_userinfo_email(state: &AppState, access_token: Option<&str>) -> Option<String> {
...
mod tests {
    ...
    fn code_verifier_has_rfc7636_shape() {
        ...
        assert!(
            v.chars().all(|c| VERIFIER_CHARS.contains(&(c as u8))),
