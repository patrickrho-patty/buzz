...
use crew_core::{filter::filters_match, CommunityId, StoredEvent};
...
impl SubscriptionRegistry {
    ...
    fn register_with_scope(
        ...
        metrics::gauge!("crew_subscriptions_active").increment(1.0);
        ...
        F: FnOnce(),
    {
        ...
        metrics::gauge!("crew_subscriptions_active").decrement(1.0);
    ...
    pub fn remove_connection(&self, conn_id: ConnId) -> Vec<RemovedSubscription> {
        ...
        if let Some((_, conn_subs)) = self.subs.remove(&conn_id) {
            ...
            metrics::gauge!("crew_subscriptions_active").decrement(count as f64);
        }
        removed
    }
    ...
    ///
    /// Used by the usage poller to emit `crew_community_subscriptions{community}`.
...
mod tests {
    ...
    fn test_global_p_kind_index_fan_out_targets_matching_p() {
        ...
        let conn_a = Uuid::new_v4();
        let conn_b = Uuid::new_v4();
        let kind = Kind::Custom(crew_core::kind::KIND_AGENT_OBSERVER_FRAME as u16);
