//! crew-relay-mesh — the inter-relay QUIC mesh.
//!
...
//! mesh-free: when `CREW_MESH=off` or no peers exist, the relay never
...
// seams below; huddle fan-out lives in crew-relay's audio module (Dawn).
...
pub struct MeshConfig {
    /// `CREW_MESH` — `on` (default when replicas can exist) | `off` kill
    /// switch. When off, the relay must behave exactly like single-instance.
    pub enabled: bool,
    /// UDP bind for the iroh endpoint (`CREW_MESH_BIND_ADDR`, default
...
pub enum MeshError {
    ...
    #[error("mesh is disabled (CREW_MESH=off)")]
    Disabled,
    #[error("transport: {0}")]
    Transport(String),
    #[error("redis: {0}")]
