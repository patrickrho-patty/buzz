//! Crew V2 kind number registry.
//!
//! This module is the authoritative source for Crew kind numbers.
...
/// NIP-01: Channel metadata (replaceable). Not used by Crew today.
...
/// Crew custom one-time identity binding proof (ephemeral, not stored).
...
// NEW: Crew command kinds (Pure Nostr plan)
...
/// `crates/crew-search/tests/fts_integration.rs`).
///
...
/// visibility pushdown in `crew-db`.
///
...
/// Crew product feedback submission. Accepted at ingest, sidecarred to the
...
// Crew community moderation commands (mod-signed, processed like 9030-series:
...
/// Crew: Set the workspace profile (icon). Admin/owner-signed command.
...
// Not using NIP-90 kinds (5000–6999) — Crew requires auth chains (depth ≤ 3, breadth ≤ 10).
...
/// or the Crew workspace-profile admin command (9033).
...
/// Returns `true` if `kind` is a Crew command kind that requires transactional execution.
...
/// Safe: all Crew kinds fit in i32 (max 65535 < i32::MAX).
...
// Compile-time: all Crew kind constants fit in nostr's u16-backed Kind.
