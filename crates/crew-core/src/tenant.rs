...
//!
//! These types live in `crew-core` (zero I/O deps) so the DB, auth, pub/sub,
...
/// the `crew-admin` CLI's tenant resolution. All of these must derive the
...
mod tests {
    ...
    fn relay_url_authority_keeps_explicit_nondefault_port() {
        ...
        // crew-admin must all derive `localhost:3000` (NOT bare `localhost`),
