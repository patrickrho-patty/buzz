...
//! The permission model: channel role = repo role; `crew-protect` tags on
...
/// Maximum number of `crew-protect` tags per repo.
...
/// A single protection rule parsed from a `crew-protect` tag on kind:30617.
///
/// Format: `["crew-protect", "<ref-pattern>", "<rule>", ...]`
...
/// Errors from parsing a `crew-protect` tag.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Parse a single `crew-protect` tag into a `ProtectionRule`.
///
/// Tag format: `["crew-protect", "<pattern>", "<rule1>", "<rule2>", ...]`
/// The first element ("crew-protect") should already be stripped — pass
/// the remaining values starting with the pattern.
/// Parse a single `crew-protect` tag (simple API, discards unknown rules).
...
/// Parse a single `crew-protect` tag, returning unknown rules for logging.
...
/// Parse all `crew-protect` tags from a kind:30617 event's tag list.
///
/// Returns an error if any `crew-protect` tag is structurally malformed.
...
pub fn parse_protection_tags(tags: &[Vec<String>]) -> Result<ParsedProtection, RuleParseError> {
    ...
    for tag in tags {
        if tag.first().map(|s| s.as_str()) != Some("crew-protect") {
            continue;
        }
...
/// Built-in default minimum role for an operation when no `crew-protect` tag matches.
