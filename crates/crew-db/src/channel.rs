...
// Re-export the canonical enum definitions from crew-core.
// These live in core (zero I/O deps) so the SDK can share them
// without pulling in sqlx/tokio.
pub use crew_core::channel::{ChannelType, ChannelVisibility, MemberRole};
...
impl LockedMemberSnapshot {
    ...
    pub async fn replace_member_event(
        ...
        channel_id: Uuid,
        event: &nostr::Event,
    ) -> Result<(crew_core::StoredEvent, bool)> {
        ...
        let kind = crew_core::kind::event_kind_i32(event);
        if kind != 39002 {
            return Err(DbError::InvalidData(
        ...
        if let Some((existing_ts, existing_id)) = existing {
            ...
            {
                return Ok((
                    crew_core::StoredEvent::with_received_at(
                        event.clone(),
                        Utc::now(),
                        Some(channel_id),
                        false,
...
pub async fn update_channel(
    ...
    if let Some(name) = updates.name.as_mut() {
        *name = crew_core::channel::canonical_channel_name(name).to_owned();
        if name.is_empty() {
            return Err(DbError::InvalidData("channel name is required".into()));
        }
    }
...
mod tests {
    ...
    const TEST_DB_URL: &str = "postgres://crew:crew_dev@localhost:5432/crew"; // sadscan:disable np.postgres.1 -- local test-only credentials
