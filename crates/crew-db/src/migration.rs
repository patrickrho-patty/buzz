...
mod tests {
    ...
    fn migration_execution_cannot_bypass_schema_destruction_lock() {
        ...
        // exemption honest.
        let push_gateway_exception = crates_dir.join("crew-push-gateway/src/postgres.rs");
        let push_gateway_migrations = crates_dir.join("crew-push-gateway/migrations");
        for entry in
            std::fs::read_dir(&push_gateway_migrations).expect("read push gateway migrations")
        {
            ...
            assert!(
                ...
                "{} defines community-scoped data; its migrator would bypass the \
                 schema/destruction lock and must move under crew-db migrations",
                path.display()
            );
        }
