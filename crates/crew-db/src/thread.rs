...
mod tests {
    ...
    async fn create_test_channel(
    ...
    ) -> crate::error::Result<(crate::channel::ChannelRecord, crew_core::CommunityId)> {
        let id = Uuid::new_v4();
        ...
        crate::channel::get_channel(pool, crew_core::CommunityId::from_uuid(community_id), id)
            .await
            .map(|channel| (channel, crew_core::CommunityId::from_uuid(community_id)))
    }
    ...
    async fn get_thread_metadata_by_event_is_scoped_when_event_id_collides_across_communities() {
        ...
        let community_a = crew_core::CommunityId::from_uuid(community_a);
        let community_b = crew_core::CommunityId::from_uuid(community_b);
