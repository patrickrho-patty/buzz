...
use crew_datastore_tracing::datastore_span;
use sqlx::postgres::{PgConnection, PgPoolOptions};
use sqlx::{Connection, PgPool, QueryBuilder, Row};
use std::time::Duration;
use uuid::Uuid;

use crew_core::{CommunityId, StoredEvent};
...
enum RouteDecision {
    ...
    /// failure emits exactly one `crew_db_route_decision` event per request
...
impl Db {
    ...
    /// request/connection error. crew-db only reads the durable host map.
    ...
    /// fully-formed [`crew_core::tenant::TenantContext`] — host included — to
    ...
    ///
    /// Used by the runtime conformance read-seam emitters in `crew-relay`:
    ...
    pub async fn insert_event_with_serving_write_guard(
        ...
        if crew_core::kind::is_ephemeral(kind_u32) {
    ...
    pub async fn replace_addressable_event(
        ...
        let kind_i32 = crew_core::kind::event_kind_i32(event);
    ...
    pub async fn nip43_membership_snapshot_needs_reconciliation(
        ...
        let snapshot = self
            .query_events(&crate::event::EventQuery {
                kinds: Some(vec![crew_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32]),
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        let kind_i32 = crew_core::kind::KIND_NIP43_MEMBERSHIP_LIST as i32;
    ...
    pub async fn replace_parameterized_event(
        ...
        let read_state_t_tag_count = event
            ...
            .count();
        let is_nip_rs = kind_i32 == crew_core::kind::KIND_READ_STATE as i32
            && d_tag_count == 1
            && has_exact_d_tag
...
mod tests {
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let base = Timestamp::now().as_secs();
        let old = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "old")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign old");
        let new = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "new")
            .tags(tags)
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign new");
    ...
    async fn mesh_status_replacement_keeps_one_physical_row() {
        ...
        for (offset, content) in [(0, "running"), (1, "running-again"), (2, "stopped")] {
            let event = EventBuilder::new(
                Kind::Custom(crew_core::kind::KIND_BOOKMARK_SET as u16),
                content,
            )
            .tags(tags.clone())
    ...
    async fn coordinate_delete_spares_head_newer_than_the_deletion() {
        ...
        let community = CommunityId::from_uuid(make_community(&db.pool).await);
        let keys = Keys::generate();
        let kind = crew_core::kind::KIND_PROJECT as i32;
        let d_tag = "stale-tombstone-project";
        let pubkey = keys.public_key().to_bytes().to_vec();
        let base = Timestamp::now().as_secs();

        let version = |content: &str, offset: u64| {
            EventBuilder::new(Kind::Custom(crew_core::kind::KIND_PROJECT as u16), content)
    ...
    async fn duplicate_nip_rs_discriminator_tags_keep_legacy_retention() {
        ...
        for (case, tags) in [
            ...
            let old = EventBuilder::new(
                Kind::Custom(crew_core::kind::KIND_READ_STATE as u16),
                format!("{case}-old"),
            )
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        let base = Timestamp::now().as_secs();
        let a = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "A")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base))
            .sign_with_keys(&keys)
            .expect("sign A");
        let x = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "X")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 1))
            .sign_with_keys(&keys)
            .expect("sign X");
        let b = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "B")
            .tags(tags.clone())
            .custom_created_at(Timestamp::from(base + 2))
            .sign_with_keys(&keys)
            .expect("sign B");
        let c = EventBuilder::new(Kind::Custom(crew_core::kind::KIND_READ_STATE as u16), "C")
            .tags(tags)
        ...
        async fn legacy_insert(
            ...
            .bind(crew_core::kind::KIND_READ_STATE as i32)
    ...
    async fn test_usage_metrics_lock_has_single_owner_and_releases_on_drop() {
        ...
        // races any live crew-relay on the same database (see #3619).
        ...
        // Same key as production (`crew-relay` USAGE_METRICS_LOCK_KEY) — safe here
    ...
    /// ceiling, not the writer's — `crew_db_read_pool_active / _max` is the
    ...
    /// directly, because `crew_db_route_decision` is emitted by `route_read`
