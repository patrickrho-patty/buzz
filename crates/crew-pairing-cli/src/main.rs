//! `crew-pair` — NIP-AB device pairing interop testing CLI.
//!
//! # Usage
//!
//! ```text
//! crew-pair source --relay wss://relay.example.com [--nsec nsec1...]
//! crew-pair target [--relay wss://relay.example.com]
...
use crew_core::kind::KIND_PAIRING;
use crew_core::pairing::session::PairingSession;
use crew_core::pairing::{
...
async fn cmd_source(relay_url: String, nsec: Option<String>) -> Result<(), CliError> {
    ...
    if !confirmed {
        ...
        if let Some(abort_event) =
            session.abort(crew_core::pairing::types::AbortReason::SasMismatch)?
        {
