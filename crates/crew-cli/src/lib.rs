...
///
/// ```ignore
/// let code = crew_cli::run_from_args(std::env::args()).await;
    ...
    S: Into<std::ffi::OsString> + Clone,
{
    ...
    // features across the workspace and enables *both* ring (from crew-acp/crew-dev-mcp)
    ...
    // crew-dev-mcp delegates to run_from_args, it has already installed ring; the
...
pub enum MessagesCmd {
    ...
    Thread {
        ...
        #[arg(long, required_unless_present = "link", conflicts_with = "link")]
        event: Option<String>,
        /// Canonical crew://message deep link; uses the configured relay and identity
