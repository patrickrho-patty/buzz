//! Desktop-local channel template loading for `crew channels create --template`.
//!
...
//! a crate, since crew-cli and desktop-tauri are independent crates and the
...
const PROD_BUNDLE_IDENTIFIER: &str = "xyz.block.crew.app";
...
/// `<platform-data-dir>/xyz.block.crew.app/templates/channel-templates.json`.
...
fn load_templates(path: &Path) -> Result<Vec<ChannelTemplateRecord>, CliError> {
    if !path.exists() {
        return Err(CliError::NotFound(format!(
            "no channel templates store found at {} (create a template in Crew Desktop first, \
...
mod tests {
    ...
    fn resolve_templates_path_defaults_to_prod_bundle() {
        ...
        assert!(path.ends_with("xyz.block.crew.app/templates/channel-templates.json"));
    }

    #[test]
    fn find_template_matches_case_insensitive() {
        let f = write_store(r#"[{"id":"t1","name":"Crew Team","createdAt":"x","updatedAt":"x"}]"#);
        let t = find_template(f.path(), "crew team").expect("found");
        assert_eq!(t.name, "Crew Team");
    ...
    fn load_templates_parses_full_roster() {
        let f = write_store(
            r##"[{
                "id":"t1","name":"Crew Team","channel_type":"forum","visibility":"private",
