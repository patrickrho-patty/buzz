use crew_core::kind::KIND_MANAGED_AGENT;
use nostr::PublicKey;

use crate::client::{extract_d_tag, normalize_write_response, CrewClient};
use crate::error::CliError;
use crate::validate::validate_hex64;

// TODO(phase-4): Replace raw nostr::EventBuilder usage in cmd_set_presence with crew-sdk builder
...
fn effective_owner(client: &CrewClient) -> String {
    client
        .auth_tag_owner_hex()
        .unwrap_or_else(|| client.keys().public_key().to_hex())
}

fn resolve_owner(client: &CrewClient, owner: Option<&str>) -> Result<Option<String>, CliError> {
    owner
        .map(|owner| {
            if owner == "me" {
...
fn owner_verification(event: &serde_json::Value, expected_owner: &str) -> &'static str {
    ...
    match crew_sdk::nip_oa::verify_auth_tag(&auth_tag_json, &agent_pubkey) {
...
pub async fn cmd_set_profile(
    ...
    let builder = crew_sdk::build_profile(
...
pub async fn cmd_get_presence(client: &CrewClient, pubkeys_csv: &str) -> Result<(), CliError> {
...
pub async fn cmd_set_presence(client: &CrewClient, status: &str) -> Result<(), CliError> {
    let builder = crew_sdk::build_presence_update(status).map_err(crate::validate::sdk_err)?;
    let event = client.sign_event(builder)?;
...
pub async fn cmd_set_status(
    ...
    let builder = crew_sdk::build_user_status(text, emoji).map_err(crate::validate::sdk_err)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{}", normalize_write_response(&resp));
    Ok(())
...
mod tests {
    ...
    fn owner_verification_requires_one_valid_auth_tag_for_requested_owner() {
        ...
        let valid_tag: serde_json::Value = serde_json::from_str(
            &crew_sdk::nip_oa::compute_auth_tag(&owner_keys, &agent_keys.public_key(), "kind=0")
                .unwrap(),
        )
        .unwrap();
        let foreign_tag: serde_json::Value = serde_json::from_str(
            &crew_sdk::nip_oa::compute_auth_tag(
                &foreign_owner_keys,
