...
fn resolve_owner(client: &CrewClient, owner_flag: Option<&str>) -> Result<PublicKey, CliError> {
    ...
    let tag = client.auth_tag_owner_hex().ok_or_else(|| {
        CliError::Usage(
            "owner pubkey required (set CREW_AUTH_TAG with a NIP-OA attestation or pass --owner)"
                .into(),
        )
    })?;
...
/// owner from `--owner` / CREW_AUTH_TAG. Owner-side recovery passes
