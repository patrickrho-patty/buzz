...
fn apply_git_provenance(
    ...
    if let Some(agent_name) = agent_name {
        ...
        let origin_tag = Tag::parse(["crew-origin-agent", agent_name])
            .map_err(|error| CliError::Other(format!("invalid git origin tag: {error}")))?;
        return Ok(builder.tag(origin_tag));
    }
...
mod tests {
    ...
    fn public_channel_origin_uses_h_tag_and_suppresses_agent_name() {
        ...
        assert!(!event
            .tags
            .iter()
            .any(|tag| tag.as_slice().first().map(String::as_str) == Some("crew-origin-agent")));
    }
    ...
    fn private_origin_exposes_only_agent_name() {
        ...
        assert!(event
            .tags
            .iter()
            .any(|tag| tag.as_slice() == ["crew-origin-agent", "Builder"]));
        assert!(!event
            .tags
            .iter()
