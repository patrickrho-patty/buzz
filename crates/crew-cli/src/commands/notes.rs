...
pub async fn fetch_own_note(client: &CrewClient, slug: &str) -> Result<Option<Event>, CliError> {
...
pub async fn fetch_by_slug(client: &CrewClient, slug: &str) -> Result<Vec<Event>, CliError> {
    let filter = serde_json::json!({
        "kinds": [KIND_LONG_FORM],
        "#d": [slug],
        "limit": 50,
...
pub async fn resolve_author(client: &CrewClient, author_flag: &str) -> Result<PublicKey, CliError> {
    if author_flag == "me" {
...
pub async fn cmd_rm(client: &CrewClient, slug: &str) -> Result<(), CliError> {
...
pub async fn dispatch(cmd: crate::NotesCmd, client: &CrewClient) -> Result<(), CliError> {
    use crate::NotesCmd;
    match cmd {
        NotesCmd::Set {
            name,
