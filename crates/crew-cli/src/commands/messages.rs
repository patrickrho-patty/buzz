use crew_sdk::{DeleteMessageOptions, DiffMeta, ThreadRef, VoteDirection};
use nostr::PublicKey;
use uuid::Uuid;
...
///
/// Delegates marker parsing and collapse to [`crew_core::nip10`] (shared with
...
fn find_root_from_tags(tags: &serde_json::Value) -> Option<String> {
    let parts: Vec<Vec<String>> = tags
        ...
        .collect();
    crew_core::nip10::parse_thread_markers_from_parts(parts.iter().map(Vec::as_slice))
        .resolve()
        .map(|(root, _)| root)
}
...
pub async fn cmd_send_message(
    ...
    let builder = match p.kind {
        Some(45001) => {
            crew_sdk::build_forum_post(channel_uuid, &final_content, &mention_refs, &media_tags)
        ...
        None | Some(9) => crew_sdk::build_message(
            channel_uuid,
            &final_content,
            thread_ref.as_ref(),
            &mention_refs,
...
pub async fn cmd_send_diff_message(client: &BuzzClient, p: SendDiffParams) -> Result<(), CliError> {
    ...
    let builder =
        crew_sdk::build_diff_message(channel_uuid, &diff, &diff_meta, thread_ref.as_ref())
...
pub async fn cmd_delete_message(
    ...
    let builder = crew_sdk::build_delete_message_with_options(
        channel_uuid,
        target_eid,
...
pub async fn cmd_edit_message(
    ...
    let builder = crew_sdk::build_edit(channel_uuid, target_eid, content)
...
pub async fn cmd_vote_on_post(
    ...
    let builder = crew_sdk::build_vote(channel_uuid, target_eid, vote_dir)
