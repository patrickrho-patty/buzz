...
pub async fn cmd_list_dms(client: &CrewClient, limit: Option<u32>) -> Result<(), CliError> {
...
pub async fn cmd_open_dm(client: &CrewClient, pubkeys: &[String]) -> Result<(), CliError> {
...
pub async fn cmd_hide_dm(client: &CrewClient, channel_id: &str) -> Result<(), CliError> {
    ...
    let builder =
        EventBuilder::new(Kind::Custom(crew_sdk::kind::KIND_DM_HIDE as u16), "").tags(tags);
    let event = client.sign_event(builder)?;
...
pub async fn cmd_add_dm_member(
    ...
    let builder = crew_sdk::build_dm_add_member(channel_uuid, pubkey).map_err(sdk_err)?;
    let event = client.sign_event(builder)?;
...
pub async fn dispatch(cmd: crate::DmsCmd, client: &CrewClient) -> Result<(), CliError> {
    use crate::DmsCmd;
    match cmd {
