...
use crew_sdk::{GitIssueMeta, GitRepoCoord, GitStatusMeta};
use nostr::Timestamp;
...
pub async fn cmd_create_issue(
    ...
    let builder = with_git_provenance(
        crew_sdk::build_git_issue(&repo, subject, &body, &meta).map_err(sdk_err)?,
    )?;
...
async fn publish_issue_assignment_operation(
    ...
    let builder = match (operation, is_self_service) {
        (IssueAssignmentOperation::Assign, true) => {
            crew_sdk::build_git_issue_assignment_with_prior(
                &repo,
                issue,
                assignees,
                &content,
        ...
        (IssueAssignmentOperation::Unassign, true) => {
            crew_sdk::build_git_issue_unassignment_with_prior(
                &repo,
                issue,
                assignees,
                &content,
        ...
        (IssueAssignmentOperation::Assign, false) => {
            crew_sdk::build_git_issue_assignment(&repo, issue, assignees, &content)
        }
        (IssueAssignmentOperation::Unassign, false) => {
            crew_sdk::build_git_issue_unassignment(&repo, issue, assignees, &content)
        }
    }
...
pub async fn cmd_issue_status(
    ...
    let builder =
        with_git_provenance(crew_sdk::build_git_status(status, &body, &meta).map_err(sdk_err)?)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
    Ok(())
