...
use crew_core::kind::{KIND_MANAGED_AGENT, KIND_TEAM};
...
pub async fn cmd_create_channel(
    ...
    let vis = match visibility {
        "open" => crew_sdk::Visibility::Open,
        "private" => crew_sdk::Visibility::Private,
        _ => unreachable!(),
    };
    let ct = match channel_type {
        "stream" => crew_sdk::ChannelKind::Stream,
        "forum" => crew_sdk::ChannelKind::Forum,
        _ => unreachable!(),
    };
    let builder =
        crew_sdk::build_create_channel(channel_uuid, name, Some(vis), Some(ct), description, ttl)
...
pub async fn cmd_create_channel_from_template(
    ...
    let builder = crew_sdk::build_create_channel(
        channel_uuid,
        name,
        Some(vis),
        Some(ct),
    ...
    if let Some(canvas_template) = template.canvas_template.as_deref() {
        ...
        let canvas_result: Result<(), CliError> = async {
            let builder = crew_sdk::build_set_canvas(channel_uuid, &content)
    ...
    for agent in &resolved.agents {
        let outcome: Result<(), CliError> = async {
            let builder = crew_sdk::build_add_member(
                channel_uuid,
                &agent.pubkey,
                Some(crew_sdk::MemberRole::Bot),
            )
...
pub async fn cmd_update_channel(
    ...
    let builder =
        crew_sdk::build_update_channel(channel_uuid, name, description, visibility, ttl_change)
...
pub async fn cmd_set_channel_topic(
    ...
    let builder = crew_sdk::build_set_topic(channel_uuid, topic)
...
pub async fn cmd_set_channel_purpose(
    ...
    let builder = crew_sdk::build_set_purpose(channel_uuid, purpose)
...
pub async fn cmd_add_channel_member(
    ...
    let typed_role = match role {
        None => None,
        Some("owner") => Some(crew_sdk::MemberRole::Owner),
        Some("admin") => Some(crew_sdk::MemberRole::Admin),
        Some("member") => Some(crew_sdk::MemberRole::Member),
        Some("guest") => Some(crew_sdk::MemberRole::Guest),
        Some("bot") => Some(crew_sdk::MemberRole::Bot),
        Some(other) => {
            return Err(CliError::Usage(format!(
    ...
    let builder = crew_sdk::build_add_member(channel_uuid, pubkey, typed_role)
        .map_err(|e| CliError::Other(format!("build_add_member failed: {e}")))?;
...
pub async fn cmd_remove_channel_member(
    ...
    let builder = crew_sdk::build_remove_member(channel_uuid, pubkey)
...
pub async fn cmd_set_add_policy(client: &BuzzClient, policy: &str) -> Result<(), CliError> {
    ...
    if let Ok(allowed_raw) = std::env::var("CREW_ACP_ALLOWED_CHANNEL_ADD_POLICIES") {
        ...
        if !allowed.is_empty() && !allowed.contains(&policy) {
            return Err(CliError::Usage(format!(
                "channel_add_policy '{policy}' is not permitted on this deployment \
                 (CREW_ACP_ALLOWED_CHANNEL_ADD_POLICIES={allowed_raw})"
            )));
        }
    }
    ...
    let builder = EventBuilder::new(
        Kind::Custom(crew_sdk::kind::KIND_AGENT_PROFILE as u16),
        &content,
    )
    .tags([]);
    let event = client.sign_event(builder)?;
...
mod tests {
    ...
    // --- CREW_ACP_ALLOWED_CHANNEL_ADD_POLICIES gate ---
    ...
    async fn set_add_policy_env_gate_rejects_disallowed_via_full_path() {
        std::env::set_var("CREW_ACP_ALLOWED_CHANNEL_ADD_POLICIES", "owner_only,nobody");
        let client = make_test_client();
        let result = cmd_set_add_policy(&client, "anyone").await;
        std::env::remove_var("CREW_ACP_ALLOWED_CHANNEL_ADD_POLICIES");
