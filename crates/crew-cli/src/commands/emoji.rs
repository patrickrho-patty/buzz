...
use crate::client::{normalize_write_response, CrewClient};
use crate::error::CliError;
use crew_sdk::CustomEmoji;
...
const CUSTOM_EMOJI_SET_D_TAG: &str = crew_sdk::CUSTOM_EMOJI_SET_D_TAG;
...
async fn fetch_own_emoji(client: &CrewClient) -> Result<Vec<CustomEmoji>, CliError> {
...
async fn publish_own_set(client: &CrewClient, emojis: &[CustomEmoji]) -> Result<(), CliError> {
    let builder = crew_sdk::build_custom_emoji_set(emojis)
...
async fn cmd_set(client: &CrewClient, shortcode: &str, url: &str) -> Result<(), CliError> {
...
async fn cmd_rm(client: &CrewClient, shortcode: &str) -> Result<(), CliError> {
    let normalized = crew_sdk::normalize_custom_emoji_shortcode(shortcode)
        .map_err(|e| CliError::Other(format!("invalid shortcode: {e}")))?;
    let mut emojis = fetch_own_emoji(client).await?;
...
pub async fn dispatch(cmd: crate::EmojiCmd, client: &CrewClient) -> Result<(), CliError> {
    use crate::EmojiCmd;
    match cmd {
