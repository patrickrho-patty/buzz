...
/// A single contact entry (CLI-local, not from crew-sdk).
...
pub async fn cmd_publish_note(
    ...
    let builder = crew_sdk::build_note(content, reply_id)
...
pub async fn cmd_set_contact_list(
    ...
    let builder = crew_sdk::build_contact_list(&contacts)
        .map_err(|e| CliError::Other(format!("build error: {e}")))?;
...
pub async fn cmd_get_event(client: &CrewClient, event_id: &str) -> Result<(), CliError> {
...
pub async fn cmd_get_contact_list(client: &CrewClient, pubkey: &str) -> Result<(), CliError> {
    validate_hex64(pubkey)?;
    let filter = serde_json::json!({
        "kinds": [3],
        "authors": [pubkey],
...
pub async fn dispatch(cmd: crate::SocialCmd, client: &CrewClient) -> Result<(), CliError> {
    use crate::SocialCmd;
    match cmd {
