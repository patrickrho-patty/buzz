//! `crew pack` subcommands — local persona pack operations.
//!
...
pub fn cmd_validate(path: &str) -> Result<(), CliError> {
    ...
    let report = crew_persona::validate::validate_pack(pack_dir);

    for diag in &report.diagnostics {
        match diag {
            crew_persona::validate::ValidationDiagnostic::Error(msg) => {
                eprintln!("  ERROR: {msg}");
            }
            crew_persona::validate::ValidationDiagnostic::Warning(msg) => {
                eprintln!("  WARN:  {msg}");
            }
        }
    }
...
pub fn cmd_inspect(path: &str) -> Result<(), CliError> {
    ...
    let pack = crew_persona::resolve::resolve_pack(pack_dir)
