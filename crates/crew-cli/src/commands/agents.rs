use crew_core::kind::KIND_IA_ARCHIVED_LIST;
use crew_sdk::builders::{build_archive_identity_request, build_unarchive_identity_request};
use nostr::PublicKey;
use serde_json::json;
...
pub async fn dispatch(command: AgentsCmd, client: &CrewClient) -> Result<(), CliError> {
    match command {
...
/// Require `CREW_AUTH_TAG` and parse the owner pubkey from it. Used only by
/// the `draft-create` and `draft-update` paths.
fn require_owner(client: &CrewClient) -> Result<PublicKey, CliError> {
    let hex = client
        .auth_tag_owner_hex()
        .ok_or_else(|| CliError::Auth("agent draft requests require CREW_AUTH_TAG".into()))?;
...
/// failure cases without a live `CrewClient` or async runtime.
fn extract_auth(
...
pub(crate) async fn fetch_archived_snapshot(client: &CrewClient) -> Result<Vec<String>, CliError> {
...
mod tests {
    ...
    fn test_client(base_url: &str) -> crate::client::CrewClient {
        let keys = nostr::Keys::generate();
        crate::client::CrewClient::new(base_url.to_string(), keys, None, None).unwrap()
    }
