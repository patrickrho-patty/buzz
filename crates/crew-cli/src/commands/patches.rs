...
use crew_sdk::{GitAppliedPatchRef, GitPatchMeta, GitRepoCoord, GitStatus, GitStatusMeta};
...
pub async fn cmd_send_patch(
    ...
    let builder =
        with_git_provenance(crew_sdk::build_git_patch(&repo, &content, &meta).map_err(sdk_err)?)?;
