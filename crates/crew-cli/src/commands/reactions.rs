...
pub async fn cmd_add_reaction(
    ...
    let builder = if let Some(url) = emoji_url {
        crew_sdk::build_custom_emoji_reaction(target_eid, emoji, url)
            .map_err(|e| CliError::Other(format!("build_custom_emoji_reaction failed: {e}")))?
    } else {
        crew_sdk::build_reaction(target_eid, emoji)
...
pub async fn cmd_remove_reaction(
    ...
    let builder = crew_sdk::build_remove_reaction(reaction_eid)
...
pub async fn cmd_get_reactions(client: &CrewClient, event_id: &str) -> Result<(), CliError> {
    validate_hex64(event_id)?;
    let filter = serde_json::json!({
        "kinds": [7],
        "#e": [event_id]
...
pub async fn dispatch(cmd: crate::ReactionsCmd, client: &CrewClient) -> Result<(), CliError> {
