...
use crew_sdk::{GitPrUpdateMeta, GitPullRequestMeta, GitRepoCoord, GitStatusMeta};
...
pub async fn cmd_open_pr(
    ...
    let builder = with_git_provenance(
        crew_sdk::build_git_pull_request(&repo, &content, &meta).map_err(sdk_err)?,
    )?;
...
pub async fn cmd_update_pr(
    ...
    let builder = with_git_provenance(
        crew_sdk::build_git_pr_update(&repo, &content, &meta).map_err(sdk_err)?,
    )?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
...
pub async fn cmd_pr_status(
    ...
    let builder =
        with_git_provenance(crew_sdk::build_git_status(status, &content, &meta).map_err(sdk_err)?)?;
    let event = client.sign_event(builder)?;
    let resp = client.submit_event(event).await?;
    println!("{resp}");
    Ok(())
