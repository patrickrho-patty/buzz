...
struct Session {
    ...
    /// `_goose/unstable/session/update` usage notification so crew-acp's
...
/// `crew-agent auth <provider>` — run the interactive auth flow for a
...
async fn auth_subcommand(args: &[String]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match provider {
        Some("databricks" | "databricks_v2" | "databricks-v2") => {
            ...
            eprintln!("Authenticated. Token cached under ~/.config/crew-agent/oauth/databricks/.");
            Ok(())
        }
        Some(other) => Err(format!("auth: unknown provider {other:?}").into()),
        None => Err("auth: provider required (try: crew-agent auth databricks)".into()),
    }
}
    ...
    wire::send(
        ...
        wire::ok(
            ...
            json!({
                ...
                "agentInfo": { "name": "crew-agent", "version": env!("CARGO_PKG_VERSION") },
            }),
        ),
    )
    .await;
...
async fn run_prompt(app: Arc<App>, id: Value, params: Value, wire_tx: WireSender) {
    ...
    // BEFORE sending the session/prompt response. crew-acp's UsageTracker
