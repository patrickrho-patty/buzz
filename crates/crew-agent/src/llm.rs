...
/// consent for `databricks-cli` covers crew-agent too.
...
async fn openrouter_post(
    ...
    'attempt: for attempt in 0..MAX_RETRIES {
        ...
        let resp = match http
            .post(url)
            .header("content-type", "application/json")
            .header("HTTP-Referer", "https://github.com/block/crew")
...
mod tests {
    ...
    async fn openrouter_post_sends_attribution_headers() {
        ...
        let header_str = headers
            ...
            .to_lowercase();
        assert!(
            header_str.contains("http-referer: https://github.com/block/crew"),
            "got: {header_str}"
        );
        assert!(
