...
/// one. `expected_run_id` must match the run id crew-agent advertised via
...
/// session use `model_id` instead of the configured `CREW_AGENT_MODEL`.
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn classify(msg: &Value) -> Inbound {
    ...
    match (method, id) {
        ...
        // Bare responses (id present, no method) are unexpected — crew-agent
...
/// method goose uses for custom usage and status events.  Used by crew-agent
/// to emit the `usage_update` payload so crew-acp's `UsageTracker` can treat
/// crew-agent and goose symmetrically.
...
/// exists to prevent.
///
/// All counts are SESSION-cumulative, matching goose, so crew-acp's
...
/// | `contextLimit` | `u64` | `0` — crew-agent has no context limit tracking |
...
pub fn usage_update_payload(
    ...
    let mut update = json!({
        ...
        // contextLimit: 0 (crew-agent has no context limit tracking).
