...
///
/// Set via `CREW_AGENT_THINKING_EFFORT` (`none|minimal|low|medium|high|xhigh|max`).
...
fn resolve_openai_effort(
    ...
    tracing::warn!(
        ...
        "CREW_AGENT_THINKING_EFFORT={} is not supported by this OpenAI model; using nearest supported level",
    ...
    match effort {
        ThinkingEffort::None | ThinkingEffort::Minimal => {
            tracing::warn!(
                ...
                "CREW_AGENT_THINKING_EFFORT={} is not expressible as an Anthropic thinking level; \
        ...
        NormalizationPolicy::OpenaiClampMaxToXhigh => {
            if effort == ThinkingEffort::Max {
                tracing::warn!(
                    ...
                    resolved = "xhigh",
                    model = raw_model,
                    "CREW_AGENT_THINKING_EFFORT=max not confirmed for this DatabricksV2 model; clamping to xhigh"
                );
        ...
        ThinkingMode::ManualBudget => {
            ...
            if budget < MIN_ANSWER_TOKENS {
                tracing::warn!(
                    ...
                    "CREW_AGENT_THINKING_EFFORT: max_output_tokens too small to fit thinking budget + answer headroom; omitting thinking fields"
                );
                return (None, None);
            }
            (
        ...
        ThinkingMode::Adaptive => {
            ...
            if clamped != effort {
                tracing::warn!(
                    ...
                    "CREW_AGENT_THINKING_EFFORT is not available for this model; clamping to highest supported level"
                );
            }
            (
...
///
/// Set via `CREW_AGENT_THINKING_SUMMARY` (`auto|concise|detailed`).
...
/// Parse `CREW_AGENT_THINKING_SUMMARY`. Pure (env-free) for testability.
///
...
pub fn parse_thinking_summary(raw: Option<&str>) -> Result<ThinkingSummary, String> {
    match raw.map(|s| s.trim().to_ascii_lowercase()).as_deref() {
        ...
        Some(other) => Err(format!(
            "config: CREW_AGENT_THINKING_SUMMARY={other} not supported (use auto|concise|detailed)"
        )),
    }
}

/// Parse `CREW_AGENT_THINKING_EFFORT`. Pure (env-free) for testability.
pub fn parse_thinking_effort(raw: Option<&str>) -> Result<Option<ThinkingEffort>, String> {
    match raw.map(|s| s.trim().to_ascii_lowercase()).as_deref() {
        ...
        Some(other) => Err(format!(
            "config: CREW_AGENT_THINKING_EFFORT={other} not supported (use none|minimal|low|medium|high|xhigh|max)"
        )),
    }
}
...
/// Text is governed by the much smaller `CREW_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
/// 10 KB. Tunable via `CREW_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
...
const DEFAULT_SYSTEM_PROMPT: &str =
    "You are crew-agent. Use the provided tools to act. Tool calls are your only output.";
...
pub struct Config {
    ...
    /// Set via `CREW_AGENT_MAX_TOOL_RESULT_TEXT_BYTES`.
    ...
    /// Set via `CREW_AGENT_MAX_HANDOFFS`. Default 10.
    ...
    /// thinking config sent). Set via `CREW_AGENT_THINKING_EFFORT`.
    ...
    /// Set via `CREW_AGENT_THINKING_SUMMARY`. Ignored on Anthropic, Chat
    ...
    /// disable with `CREW_AGENT_PROMPT_CACHING=0`. Consulted on every route that
...
impl Config {
    pub fn from_env() -> Result<Self, String> {
        ...
        let provider = resolve_provider(
            env("CREW_AGENT_PROVIDER").as_deref(),
        ...
        let crew_agent_model = env("CREW_AGENT_MODEL");
        ...
        let (api_key, model, base_url, openai_api) = match provider {
            ...
            Provider::Databricks | Provider::DatabricksV2 => (
                ...
                resolve_model(crew_agent_model.as_deref(), databricks_model.as_deref())
        ...
        let system_prompt = match (env("CREW_AGENT_SYSTEM_PROMPT"), env("CREW_AGENT_SYSTEM_PROMPT_FILE")) {
            (Some(_), Some(_)) => return Err(
                "config: CREW_AGENT_SYSTEM_PROMPT and CREW_AGENT_SYSTEM_PROMPT_FILE are mutually exclusive".into()),
            (Some(s), _) => s,
        ...
        let cfg = Config {
            ...
            openai_api,
            max_rounds: parse_env("CREW_AGENT_MAX_ROUNDS", 0)?,
            max_output_tokens: parse_env("CREW_AGENT_MAX_OUTPUT_TOKENS", 65_536)?,
            max_token_recoveries: parse_env("CREW_AGENT_MAX_TOKEN_RECOVERIES", 3u32)?,
            llm_timeout: Duration::from_secs(parse_env("CREW_AGENT_LLM_TIMEOUT_SECS", 240)?),
            tool_timeout: Duration::from_secs(parse_env("CREW_AGENT_TOOL_TIMEOUT_SECS", 660)?),
            mcp_init_timeout: Duration::from_secs(parse_env(
                "CREW_AGENT_MCP_INIT_TIMEOUT_SECS",
                30,
            )?),
            mcp_max_restart_attempts: parse_env("CREW_AGENT_MCP_RESTART_MAX_ATTEMPTS", 3u32)?,
            mcp_restart_base_ms: parse_env("CREW_AGENT_MCP_RESTART_BASE_MS", 500u64)?,
            mcp_restart_max_ms: parse_env("CREW_AGENT_MCP_RESTART_MAX_MS", 30_000u64)?,
            max_sessions: parse_env("CREW_AGENT_MAX_SESSIONS", usize::MAX)?,
            max_line_bytes: parse_env("CREW_AGENT_MAX_LINE_BYTES", 4 * 1024 * 1024)?,
            max_history_bytes: parse_env("CREW_AGENT_MAX_HISTORY_BYTES", 16 * 1024 * 1024)?,
            max_tool_result_text_bytes: parse_env(
                "CREW_AGENT_MAX_TOOL_RESULT_TEXT_BYTES",
                DEFAULT_TOOL_RESULT_TEXT_BYTES,
            )?,
            max_context_tokens: parse_env("CREW_AGENT_MAX_CONTEXT_TOKENS", 200_000u64)?,
            max_handoffs: parse_env("CREW_AGENT_MAX_HANDOFFS", 10)?,
            max_parallel_tools: parse_env("CREW_AGENT_MAX_PARALLEL_TOOLS", 8usize)?,
            hook_timeout: Duration::from_millis(parse_env("CREW_AGENT_HOOK_TIMEOUT_MS", 2500u64)?),
            stop_max_rejections: parse_env("CREW_AGENT_STOP_MAX_REJECTIONS", 3u32)?,
            require_reply: parse_env("CREW_AGENT_REQUIRE_REPLY", 0u8)? != 0,
            hook_servers: parse_hook_servers_env("MCP_HOOK_SERVERS"),
            hints_enabled: parse_env("CREW_AGENT_NO_HINTS", 0u8)? == 0,
            thinking_effort: parse_thinking_effort(env("CREW_AGENT_THINKING_EFFORT").as_deref())?,
            thinking_summary: parse_thinking_summary(
                env("CREW_AGENT_THINKING_SUMMARY").as_deref(),
            )?,
            prompt_caching: parse_env("CREW_AGENT_PROMPT_CACHING", 1u8)? != 0,
        };
        cfg.validate()?;
        Ok(cfg)
    }
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if self.max_output_tokens < 1 {
            return Err("config: CREW_AGENT_MAX_OUTPUT_TOKENS must be >= 1".into());
        }
        if self.max_context_tokens <= u64::from(self.max_output_tokens) {
            return Err(format!(
                "config: CREW_AGENT_MAX_CONTEXT_TOKENS ({}) must be > CREW_AGENT_MAX_OUTPUT_TOKENS ({}) — the context window must leave room for the response",
                self.max_context_tokens, self.max_output_tokens
            ));
        }
        if self.max_history_bytes < MIN_HISTORY_BYTES {
            return Err(format!(
                "config: CREW_AGENT_MAX_HISTORY_BYTES must be >= {MIN_HISTORY_BYTES}"
            ));
        }
        if self.max_history_bytes < MAX_PROMPT_BYTES {
            return Err(format!(
                "config: CREW_AGENT_MAX_HISTORY_BYTES ({}) must be >= MAX_PROMPT_BYTES ({MAX_PROMPT_BYTES})",
                self.max_history_bytes
            ));
        }
        if self.max_line_bytes < MIN_LINE_BYTES {
            return Err(format!(
                "config: CREW_AGENT_MAX_LINE_BYTES must be >= {MIN_LINE_BYTES}"
            ));
        }
        ...
        {
            return Err(format!(
                "config: CREW_AGENT_MAX_TOOL_RESULT_TEXT_BYTES must be in {MIN_TOOL_RESULT_TEXT_BYTES}..={MAX_TOOL_RESULT_BYTES}"
            ));
        }
        if self.llm_timeout < MIN_TIMEOUT {
            return Err("config: CREW_AGENT_LLM_TIMEOUT_SECS must be >= 1".into());
        }
        if self.tool_timeout < MIN_TIMEOUT {
            return Err("config: CREW_AGENT_TOOL_TIMEOUT_SECS must be >= 1".into());
        }
        if self.mcp_init_timeout < MIN_TIMEOUT {
            return Err("config: CREW_AGENT_MCP_INIT_TIMEOUT_SECS must be >= 1".into());
        }
        if self.max_parallel_tools < 1 {
            return Err("config: CREW_AGENT_MAX_PARALLEL_TOOLS must be >= 1".into());
        }
        if self.mcp_max_restart_attempts < 1 {
            return Err("config: CREW_AGENT_MCP_RESTART_MAX_ATTEMPTS must be >= 1".into());
        }
        if self.mcp_restart_base_ms < 1 {
            return Err("config: CREW_AGENT_MCP_RESTART_BASE_MS must be >= 1".into());
        }
        if self.mcp_restart_max_ms < self.mcp_restart_base_ms {
            return Err(
                "config: CREW_AGENT_MCP_RESTART_MAX_MS must be >= CREW_AGENT_MCP_RESTART_BASE_MS"
                    .into(),
            );
        }
        ...
        if let Some(effort) = self.thinking_effort {
            ...
            {
                return Err(format!(
                    "config: CREW_AGENT_THINKING_EFFORT={} is not valid for Anthropic providers \
...
/// Returns the first present value. `explicit_override` (CREW_AGENT_MODEL,
    ...
    match requested.map(str::trim).filter(|s| !s.is_empty()) {
        Some(raw) => {
            ...
            match normalized.as_str() {
                ...
                _ => Err(format!(
                    "config: CREW_AGENT_PROVIDER={raw} not supported"
                )),
            }
        }
        None => Err(
            "config: CREW_AGENT_PROVIDER is required — set it to your provider (e.g. anthropic, openai, databricks)".into(),
        ),
    }
}
...
mod tests {
    ...
    fn resolve_provider_errors_when_provider_env_absent() {
        // No implicit inference — absent CREW_AGENT_PROVIDER is an error.
        let err = resolve_provider(None, None, None, None).unwrap_err();
        assert!(err.contains("CREW_AGENT_PROVIDER is required"), "{err}");
    }
    ...
    fn resolve_provider_requires_databricks_host_and_model_for_fallback() {
        ...
        // When CREW_AGENT_PROVIDER=databricks, resolve_provider succeeds regardless
    ...
    fn resolve_provider_unsupported_error_preserves_user_casing() {
        ...
        assert!(err.contains("CREW_AGENT_PROVIDER=OpenAIish"));
    }
    ...
    fn parse_thinking_effort_rejects_unknown_value() {
        ...
        assert!(err.contains("CREW_AGENT_THINKING_EFFORT=extreme"), "{err}");
        assert!(
    ...
    fn parse_thinking_summary_rejects_unknown_value() {
        ...
        assert!(err.contains("CREW_AGENT_THINKING_SUMMARY=verbose"), "{err}");
    ...
    fn validate_rejects_none_effort_for_anthropic() {
        ...
        assert!(
            err.contains("CREW_AGENT_THINKING_EFFORT=none"),
    ...
    fn validate_rejects_minimal_effort_for_anthropic() {
        ...
        let err = cfg.validate().unwrap_err();
        assert!(err.contains("CREW_AGENT_THINKING_EFFORT=minimal"), "{err}");
