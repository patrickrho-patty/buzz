...
const REPLY_GUARD_SERVER: &str = "crew-agent";
        ...
        loop {
                ...
                || response.output_tokens.is_some()
            {
                ...
                *self.turn_cache_write_tokens = self
                ...
                // the consumer: crew-acp's UsageTracker advances its committed
...
fn synthetic_hook_id(hook: &str, server: &str, ordinal: u64) -> String {
    format!("crew_hook_{hook}_{server}_{ordinal}")
}
