...
/// `cache_namespace` is the directory under `~/.config/crew-agent/oauth/`
...
pub struct PkceOAuthConfig {
    ...
    /// `~/.config/crew-agent/oauth/<cache_namespace>/`. Production code
...
impl PkceOAuthTokenSource {
    ...
    pub(crate) async fn try_bearer_no_browser(&self) -> Result<String, AgentError> {
        ...
        Err(AgentError::LlmAuth(
            "no cached Databricks token; run `crew-agent auth databricks` first".into(),
        ))
    }
}
...
/// write-path property: a world-readable cache left by an older crew-agent
...
mod tests {
    ...
    async fn test_save_repairs_preexisting_loose_mode_file() {
        ...
        // Simulate a world-readable cache written by an older crew-agent.
    ...
    async fn test_bearer_cache_hit_tightens_preexisting_loose_mode_file() {
        ...
        // A world-readable cache left by an older crew-agent must be tightened
