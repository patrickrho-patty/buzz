...
/// sends `notifications/cancelled` to the MCP server. With crew-dev-mcp,
...
async fn cancel_kills_inflight_tool_via_mcp_notification() {
    // crew-dev-mcp is a separate crate; locate its binary relative to
    // the crew-agent test binary (they share the same target dir).
    let self_bin = std::path::PathBuf::from(env!("CARGO_BIN_EXE_crew-agent"));
    let dev_mcp_bin = self_bin.parent().unwrap().join("crew-dev-mcp");
    ...
    if !dev_mcp_is_executable {
        eprintln!(
            "SKIP: crew-dev-mcp not built at {}; run `cargo build -p crew-dev-mcp` first",
            dev_mcp_bin.display()
        );
        return;
    }
...
/// any MCP server (not just crew-dev-mcp) when a session is cancelled
...
/// in-process guard (`server: "crew-agent"`) at the `_Stop` hook point — the
                ...
                .filter(|m| {
                    m["role"] == "tool"
                        && serde_json::from_str::<Value>(m["content"].as_str().unwrap_or(""))
                            .map(|p| p["hook"] == "_Stop" && p["server"] == "crew-agent")
                            .unwrap_or(false)
                })
                .count()
        })
