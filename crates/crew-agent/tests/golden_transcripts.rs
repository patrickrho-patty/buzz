...
async fn handshake(h: &mut Harness) -> String {
    ...
    assert_eq!(init["result"]["agentInfo"]["name"], "crew-agent");
    assert_eq!(
...
/// crew-agent must emit a `usage_update` notification with `accumulatedInputTokens`
...
/// When the Anthropic parser produces an input-sum overflow, crew-agent must
