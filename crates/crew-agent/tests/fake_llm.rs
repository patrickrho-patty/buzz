//! Integration test: fake LLM HTTP server + crew-agent subprocess.
//!
...
impl Harness {
    async fn spawn(base_url: &str) -> Self {
        ...
        cmd.env("CREW_AGENT_PROVIDER", "openai")
            ...
            .env("CREW_AGENT_MAX_ROUNDS", "4")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .kill_on_drop(true);
        let mut child = cmd.spawn().expect("spawn crew-agent");
...
async fn init_session(h: &mut Harness) -> String {
    ...
    assert_eq!(r["result"]["agentInfo"]["name"], "crew-agent");
...
async fn rejects_oversized_line() {
    ...
    cmd.env("CREW_AGENT_PROVIDER", "openai")
        ...
        .env("CREW_AGENT_MAX_LINE_BYTES", "256")
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
        .kill_on_drop(true);
...
async fn system_prompt_reaches_llm_system_role() {
    ...
    let default_prompt = "You are crew-agent";
    assert!(
...
async fn system_prompt_absent_no_canary() {
    ...
    assert!(
        system_content.contains("You are crew-agent"),
...
/// Wait for the `activeRunId` advert crew-agent emits at prompt start and
...
/// completion_tokens). crew-agent maps these to `accumulatedInputTokens` /
...
/// crew-agent must treat this turn's total as Unknown and poison the cumulative.
...
/// crew-agent must emit `_goose/unstable/session/update` with `sessionUpdate:
...
/// When the provider returns a response with no `usage` block, crew-agent must
...
/// (so token counts are observed), crew-agent must still emit the usage
