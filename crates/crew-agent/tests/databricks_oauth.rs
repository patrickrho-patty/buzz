...
//! exercised manually via the `crew-agent auth databricks` subcommand
//! (see `lib.rs::auth_subcommand`).
//!
//! The second test module (further down) is an ACP-level envelope
//! regression: it spawns the real `crew-agent` binary with
...
use crew_agent::auth::{PkceOAuthConfig, PkceOAuthTokenSource, TokenSource};
...
//
// Boots the real crew-agent binary with `DATABRICKS_TOKEN` set (so the
...
impl AgentHarness {
    ...
    async fn spawn_provider_with_options(
        ...
        cmd.env("CREW_AGENT_PROVIDER", provider)
            ...
            .env("CREW_AGENT_MAX_ROUNDS", "2")
            .env("CREW_AGENT_MAX_SESSIONS", max_sessions.to_string())
...
async fn model_discovery_surfaces_rejected_static_token_as_auth_failure() {
    ...
    use crew_agent::config::{Config, Provider};
    use crew_agent::discover_databricks_models;
