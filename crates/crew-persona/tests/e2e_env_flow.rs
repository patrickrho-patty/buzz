...
use crew_persona::resolve::resolve_pack;
...
"#,
    ...
    assert_eq!(
        env.get("CREW_AGENT_MODEL"),
        Some(&"gpt-4o"),
        "should emit CREW_AGENT_MODEL=gpt-4o"
    );
    assert_eq!(
        env.get("CREW_AGENT_PROVIDER"),
        Some(&"openai"),
        "should emit CREW_AGENT_PROVIDER=openai"
    );

    // Must NOT contain GOOSE_* keys
    assert!(
        !env.contains_key("GOOSE_MODEL"),
        "crew-agent runtime must not emit GOOSE_MODEL"
    );
    assert!(
        !env.contains_key("GOOSE_PROVIDER"),
        "crew-agent runtime must not emit GOOSE_PROVIDER"
    );
}
...
"#,
    ...
    assert!(
        !goose_env.contains_key("CREW_AGENT_MODEL"),
        "goose persona must not emit CREW_AGENT_MODEL"
    );
    assert!(
        !goose_env.contains_key("CREW_AGENT_PROVIDER"),
    ...
    let crew_env: std::collections::HashMap<_, _> = crew
        ...
        .map(|(k, v)| (k.as_str(), v.as_str()))
        .collect();
    assert_eq!(crew_env.get("CREW_AGENT_MODEL"), Some(&"gpt-4o"));
    assert_eq!(crew_env.get("CREW_AGENT_PROVIDER"), Some(&"openai"));
    assert!(
        !crew_env.contains_key("GOOSE_MODEL"),
        "crew-agent persona must not emit GOOSE_MODEL"
    );
    assert!(
        !crew_env.contains_key("GOOSE_PROVIDER"),
        "crew-agent persona must not emit GOOSE_PROVIDER"
    );
}
