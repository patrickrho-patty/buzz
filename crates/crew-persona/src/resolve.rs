...
fn runtime_env_vars(persona: &LoadedPersona) -> Vec<(String, String)> {
    ...
    if let Some(model_str) = &persona.model {
        ...
        match runtime {
            Some("crew-agent") => {
                vars.push(("CREW_AGENT_MODEL".to_owned(), model_id.to_owned()));
                if let Some(p) = provider {
                    vars.push(("CREW_AGENT_PROVIDER".to_owned(), p.to_owned()));
                }
            }
            _ => {
                if let Some(p) = provider {
...
mod tests {
    ...
    #[test]
    fn runtime_env_vars_crew_agent_emits_crew_agent_vars() {
        let mut lp = stub_persona(Some("databricks:goose-claude-4-6-opus"), None, None);
        lp.runtime = Some("crew-agent".to_owned());
        let vars = runtime_env_vars(&lp);
        let map: HashMap<&str, &str> = vars.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
        assert_eq!(map["CREW_AGENT_MODEL"], "goose-claude-4-6-opus");
        assert_eq!(map["CREW_AGENT_PROVIDER"], "databricks");
    ...
    fn runtime_env_vars_goose_emits_goose_vars() {
        ...
        assert!(!map.contains_key("CREW_AGENT_MODEL"));
    }
    ...
    fn runtime_env_vars_crew_agent_model_without_provider() {
        ...
        let vars = runtime_env_vars(&lp);
        let map: HashMap<&str, &str> = vars.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
        assert_eq!(map["CREW_AGENT_MODEL"], "gpt-4o");
        assert!(!map.contains_key("CREW_AGENT_PROVIDER"));
    }
