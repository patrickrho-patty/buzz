...
for deployment in Crew. It is a **superset of the [Open Plugin Spec](https://open-plugin-spec.org)**
...
1. **Portable** — zip file or git repo; no Crew tooling required to inspect
...
4. **Harness-honest** — explicit about what the agent runtime does vs. what crew-acp does
...
and Crew-specific extensions live alongside the OPS fields at the top level. Since the Open
...
```json
{
  ...
  "version": "1.2.0",
  "description": "A four-agent security review team for Crew.",
  "author": "Meadow Engineering",
  "license": "MIT",
  "homepage": "https://github.com/example/meadow-security-team",
  "keywords": ["security", "code-review", "crew"],
  "engines": {
    "crew": ">=0.9.0"
  },
  "personas": [
...
- **Crew**: reads both OPS fields and the Crew-specific fields; `personas` is authoritative.
- **Version negotiation**: `engines.crew` specifies minimum required Crew version; crew-acp
  rejects packs requiring a newer version.
- **Extension mechanism**: Crew-specific fields sit at the top level of `plugin.json` alongside
  OPS fields. No OPS core field is overloaded.
- **`defaults`**: ignored entirely by OPS consumers. crew-acp resolves it at deploy time before
...
# === Behavioral Config (Crew-specific) ===
subscribe:
  - "#security-reviews"
  - "#code-reviews"
triggers:
...
| `display_name` | string | ✅ | Human-readable name shown in Crew UI. |
...
crew-acp assembles the agent's context from two distinct prompt layers before sending each
...
```
[Base]
<platform orientation — injected by crew-acp>
...
The `[Base]` layer is compiled into crew-acp and is **identical for every agent**. It covers:

| Content | Purpose |
|---------|---------|
| Platform identity | Tells the agent it is running inside Crew and what that means |
...
Pack authors do not write or configure the `[Base]` layer — it is maintained by the Crew team
and updated in crew-acp releases.
...
`CREW_ACP_BASE_PROMPT_FILE` to a file path — crew-acp reads it at startup and uses it instead.
...
- That the agent is running inside Crew (covered by `[Base]`)
...
> **Note**: `.agents/skills/` is crew-acp's canonical skill location. The other paths shown
...
$AGENT_CWD/.agents/skills/<skill-name>/SKILL.md   ← crew-acp uses this one
```
...
crew-acp process. crew-acp determines what value to pass as `NewSessionRequest.cwd` in this
order:

1. The `AGENT_CWD` environment variable on the crew-acp process, if set.
2. `std::env::current_dir()` as a fallback.
3. If both fail, crew-acp logs an error and **refuses to start**.
...
is **not overwritten**. This allows operators to pin custom skill versions. crew-acp **must log
...
crew-acp lists available skills in the user message prefix so the agent knows what's available.
...
by the agent runtime. `crew pack validate` warns on skill name mismatches but does not yet
...
crew-acp merges them and passes the result via the ACP protocol — no filesystem placement required.
...
When implemented, all `env` values will be scanned for `${VAR_NAME}`. crew-acp will resolve
from the process environment **before** passing to the agent runtime. Unresolved variables will
cause a startup error.

### Delivery

crew-acp passes the merged config via `NewSessionRequest.mcp_servers`. **No `.mcp.json` is written to the agent's working directory.**
...
in the pack. crew-acp appends it to the persona prompt in the user message prefix.

crew-acp appends `instructions.md` to the persona prompt in the user message prefix (see
...
Hooks are shell commands fired by crew-acp at agent lifecycle points. **Agent runtimes typically have no hook system**
...
Hooks run as the crew-acp user; working directory is `$AGENT_CWD`; agent env vars are available.
...
that must be implemented directly in crew-acp's dispatch loop, not via a hook.
...
`on_stop` fires on normal exit and on handled errors. It **will not fire** if crew-acp crashes
...
**Hooks are NOT agent runtime features.** They are implemented entirely in crew-acp. Bypassing
crew-acp means no hooks fire.
...
Crew conversations. These are all Crew-specific — the agent runtime has no awareness of them. They sit
...
In this spec, **"deploy time"** means when crew-acp loads the pack and constructs per-persona
session configurations — typically at crew-acp process startup. For git-based packs, this occurs
each time crew-acp starts and reads the installed pack directory.

When crew-acp resolves the effective configuration for a persona, it applies this order (highest
wins):

```
1. Operator env vars           — e.g. GOOSE_MODEL, GOOSE_PROVIDER (agent-runtime-specific)
                                 already set in the parent process environment
2. Desktop UI per-agent        — overrides set in the Crew desktop app per-agent settings
...
5. Built-in defaults           — crew-acp's hardcoded fallback values
```

crew-acp resolves levels 3–5 at deploy time (when the pack is loaded and sessions are
constructed). Levels 1–2 are applied at runtime and are outside the pack's control.

**Level 1 — Operator env vars**: If the operator has already set env vars for model, provider, temperature, or context limit in the parent process environment, crew-acp MUST NOT override them with pack/persona values. crew-acp only injects env vars for fields that are NOT already set in the parent environment. This ensures operators can always override pack configuration.

**Implementation**: when constructing the child process environment, crew-acp checks
...
| `temperature` | float | `0.7` | Provider-dependent (typically 0.0–2.0). crew-acp passes through without range validation; `crew pack validate` checks type only (must be a number), not range. | Passed as env var to agent runtime. |
...
**Unknown keys** in `defaults` (in `plugin.json`) are **validation warnings** in `crew pack
...
crew-acp logs a `WARN` and ignores unknown manifest keys, remaining fail-soft:
...
The `#` prefix in `subscribe` entries is a **display convention only**. Channel names in the Crew
relay are stored and queried **without** the `#` prefix. crew-acp strips the leading `#` before
making any relay API calls. `"#security-reviews"` and `"security-reviews"` are equivalent in this
field.

### Env Var Mapping

crew-acp resolves pack defaults and per-persona overrides (precedence levels 3–5) into a single
...
crew-acp translates persona behavioral config fields to agent configuration via environment
...
If `model` is omitted from both the persona frontmatter and `defaults`, crew-acp does not set
`GOOSE_PROVIDER` or `GOOSE_MODEL`, and the agent runtime uses its configured operator default.

> **Implementation note**: `AcpClient::spawn` accepts per-persona env vars via the `extra_env` parameter. crew-acp checks `std::env::var(key)` before injecting each var — if the parent environment already has the key set, injection is skipped (operator precedence, level 1).
...
> Field Schema table lists crew-acp's built-in fallbacks (precedence level 5). If `defaults` is
...
All fields are consumed entirely by crew-acp. None are passed to the agent runtime directly — they are projected as env vars or used by the harness's subscription/dispatch logic.
...
```bash
crew install github:example/meadow-security-team
crew install github:example/meadow-security-team@v1.2.0
crew install git+https://gitlab.example.com/team/pack.git
```
...
A Crew-hosted registry and in-app browser for discovering and installing packs. API-compatible
with OPS registries. Details TBD.

### Installed Pack Location

Installed packs live at `~/.crew/packs/<pack-id>/`. crew-acp reads packs from this location
at agent startup.

### Desktop App Import

The Crew desktop app's **Agents** page does not import persona-pack `.zip` archives or
...
- **Agent teams section → Import**: Accepts `.team.json` or `.team.png` (a `crew-team-snapshot
...
> `crew pack validate` / `crew pack inspect` (Section 11). A snapshot is captured *from* an
...
> `crew pack inspect`'s resolved config as reference.
...
|-----------|----------------|-----------|-------------------|
| Skills | Copy at deploy time (planned) | crew-acp will copy `skills/` → `$AGENT_CWD/.agents/skills/` | ✅ Yes (only one) |
| MCP servers | ACP protocol | `NewSessionRequest.mcp_servers` | ❌ No |
| Persona prompt | User message prefix | `[System]` block prepended to user message text by crew-acp | ❌ No |
| Pack instructions | User message prefix | Appended to `[System]` block in user message text | ❌ No |
| Lifecycle hooks | Harness internal | crew-acp fires shell commands directly | ❌ No |
| Model/provider | Child process env vars | Agent-runtime-specific env vars (e.g. `GOOSE_PROVIDER`, `GOOSE_MODEL`) | ❌ No |
| Behavioral config | Harness internal | crew-acp subscription + dispatch logic | ❌ No |
| Pack defaults (`defaults`) | Harness internal | Resolved at deploy time by crew-acp into per-persona effective config; never passed to the agent runtime directly | ❌ No |

> **Pack defaults are resolved at deploy time**, not at runtime. When crew-acp loads a pack and
...
crew-acp's `format_prompt()` in `queue.rs` prepends a `[System]` block to the **user message
text** before sending it to the agent runtime. This is a **crew-acp feature, not an agent runtime
...
harness-side interpolation is implemented, crew-acp will resolve them from the process
...
Hooks run with crew-acp's privileges — significant attack surface. Only install packs from
trusted sources. Review all hook commands before installing. Consider sandboxing crew-acp
(container, restricted user) for untrusted packs. crew-acp should display hook commands before
...
### From V6 (crew-namespaced) Format

Field mapping from V6 `.persona.md` to current `.persona.md`:

| V6 location | Current location |
|---|---|
| `crew.model` | `model` (top-level frontmatter) |
| `crew.temperature` | `temperature` (top-level frontmatter) |
| `crew.max_context_tokens` | `max_context_tokens` (top-level frontmatter) |
| `crew.subscribe` | `subscribe` (top-level frontmatter) |
| `crew.respond_to` | `triggers` (top-level frontmatter) |
| `crew.thread_replies` | `thread_replies` (top-level frontmatter) |
| `crew.broadcast_replies` | `broadcast_replies` (top-level frontmatter) |
| `plugin.json` → `crew.defaults` | `plugin.json` → `defaults` (top-level) |
| `plugin.json` → `crew.personas` | `plugin.json` → `personas` (top-level) |
| `plugin.json` → `crew.pack_instructions` | `plugin.json` → `pack_instructions` (top-level) |
| `plugin.json` → `crew.mcp_config` | `plugin.json` → `mcp_config` (top-level) |
| `plugin.json` → `crew.hooks_config` | `plugin.json` → `hooks_config` (top-level) |
...
5. Run `crew pack validate ./my-pack`

### Backward Compatibility

The V6 namespaced `crew:` block format is not supported. Only the current flat top-level fields format is accepted. The `respond_to` key is accepted as a legacy alias for `triggers` in both persona frontmatter and `plugin.json` defaults.
...
1. **`session/set_model` as env var alternative**: The ACP runtime implements `on_set_model()` (ACP
   unstable feature). crew-acp could call `session/set_model` after `session/new` to set the
...
2. **`CONTEXT_FILE_NAMES` env var**: The goose agent runtime supports this env var to control which filenames are
   scanned for hints. Should crew-acp set this to include pack-specific filenames? Deferred
   pending use case.
...
`crew pack init` scaffolding; hot reload of skills/instructions; skill marketplace; pack dependencies; agent-to-agent handoff within a pack.
...
| PF-2 | `crew pack validate` CLI: **Implemented.** Schema-validates `plugin.json`; checks `.persona.md` required identity fields; validates behavioral config fields; warns on unknown keys and skill name mismatches. Remaining: verify `skills:` and `hooks:` paths exist; error on `SKILL.md` missing `name:` or `description:`. | `crew-cli` / `crew-admin` |
| PF-3 | Skill collision warning: emit `WARN` when a pack skill is skipped because a skill with the same load key already exists in `.agents/skills/`. | crew-acp skill copy logic |
| PF-4 | `$AGENT_CWD` resolution: determine `NewSessionRequest.cwd` from (1) `AGENT_CWD` env var, (2) `std::env::current_dir()`, (3) error and refuse to start. | crew-acp startup / session init |
| PF-5 | Skill parse failure warning: emit `WARN` when `parse_skill_content` returns `None` (missing `name:`, missing `description:`, or malformed frontmatter). Currently the agent runtime silently skips. crew-acp should pre-validate during skill copy. | crew-acp skill copy logic |
| PF-6 | Per-subprocess env var injection: **Implemented.** `AcpClient::spawn` accepts `extra_env: &[(String, String)]` injected via `Command::env()`. crew-acp checks `std::env::var(key)` before injecting — operator env vars take precedence (level 1). | `crew-acp/src/acp.rs` `AcpClient::spawn()` |
