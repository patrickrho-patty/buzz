...
use crew_core::kind::KIND_NIP43_MEMBERSHIP_LIST;
use crew_core::tenant::{relay_url_authority, TenantContext};
use crew_db::{Db, DbConfig};
use crew_pubsub::{EventTopic, PubSubManager};
...
async fn main() {
    ...
    // Redis TLS connection without this. Mirrors crew-relay's main().
...
async fn publish_membership_list_with_bump(
    ...
    tracing::info!(
        ...
        ts,
        "NIP-43 membership list published by crew-admin"
    );
    Ok(())
}
...
///
/// `crew-admin` runs inside the relay container (`compose exec relay
/// crew-admin …`), so it shares the relay's `RELAY_URL` and resolves the same
...
async fn resolve_admin_tenant(db: &Db) -> Result<TenantContext> {
    ...
    // the helper keeps crew-admin byte-identical to the community startup seeds.
    ...
    let record = db.lookup_community_by_host(&host).await?.ok_or_else(|| {
        anyhow::anyhow!(
            "RELAY_URL host '{host}' is not mapped to a community.\n\
             crew-admin operates on the configured relay's community; ensure the \
...
async fn reconcile_channels(
    ...
    use crew_core::kind::KIND_NIP29_GROUP_ADMINS;
