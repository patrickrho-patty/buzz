...
//!
//! Run with a local PG: `CREW_TEST_DATABASE_URL=postgres://crew:crew_dev@localhost:5432/crew cargo test -p crew-search --tests -- --include-ignored`
//!
...
use crew_search::{ChannelScope, SearchQuery, SearchService};
...
async fn short_kind0_prefix_prioritizes_exact_lexeme_on_a_noisy_page() {
    ...
    let first_page = svc
        .search(&SearchQuery {
            ...
            page: 1,
            per_page: 3,
            mode: crew_search::SearchMode::Prefix,
        })
        .await
...
async fn p_gated_persistent_kinds_have_storage_null_tsvector() {
    ...
    let persistent: Vec<u32> = P_GATED_KINDS
        .iter()
        .copied()
        .filter(|&k| !crew_core::kind::is_ephemeral(k))
        .collect();
    assert!(
        !persistent.is_empty(),
