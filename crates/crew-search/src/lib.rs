#![deny(unsafe_code)]
#![warn(missing_docs)]
//! Crew search — community-scoped Postgres full-text search over Crew events.
//!
...
//! by `crew-db`. The relay refetches canonical events through `crew-db`'s
