...
use crew_conformance::checker::{check_trace, Scenario};
use crew_conformance::transitions::TransitionError;
use crew_conformance::{
...
/// `expected_trace` byte-exactly. Run with `CREW_CONFORMANCE_UPDATE=1`
...
fn assert_fixture_matches(name: &str, expected_trace: &[TraceStep]) {
    ...
    if std::env::var("CREW_CONFORMANCE_UPDATE").is_ok() {
    ...
    let actual = fs::read_to_string(&path).unwrap_or_else(|e| {
        panic!(
            "fixture {} missing or unreadable ({e}); run with \
             CREW_CONFORMANCE_UPDATE=1 to create it",
            path.display()
        )
    });
    ...
    assert_eq!(
        ...
        "committed fixture {} drifted from the typed builder; run with \
         CREW_CONFORMANCE_UPDATE=1 to refresh if the change is intentional",
        name
    );
