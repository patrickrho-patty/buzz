...
/// trace schema.
///
/// This deliberately does NOT reuse `crew_core::CommunityId`. Two reasons:
///
/// 1. **Production fence preservation.** `crew_core::CommunityId` has no
...
impl CommunityLabel {
    /// Wrap a UUID into a community label. Unlike `crew_core::CommunityId`
