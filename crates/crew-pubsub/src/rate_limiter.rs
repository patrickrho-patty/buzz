...
//!
//! Implements the [`RateLimiter`] trait from `crew-auth`.
...
/// `crew:{community}:ratelimit:{pubkey_hex}:{suffix}`. IP keys remain
/// operator-global: `crew:ratelimit:ip:{ip}:conn`. The counter and its TTL are
...
impl RateLimiter for RedisRateLimiter {
    async fn check_and_increment(
        ...
        let key = crew_auth::rate_limit::rate_limit_key(ctx, pubkey, &limit_type);
    ...
    async fn check_ip_connection(
        ...
        let key = crew_auth::rate_limit::ip_rate_limit_key(ip);
        run_rate_limit(&self.pool, &key, window_secs, limit).await
    }
}
