#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `crew-pubsub` — Redis pub/sub fan-out, presence tracking, and typing indicators.
//!
//! # Architecture
//!
//! ```text
...
//!         └── dynamic SUBSCRIBE crew:{community}:channel:{id} / crew:{community}:global
...
pub struct ChannelEvent {
    ...
    pub community_id: crew_core::CommunityId,
...
/// Central pub/sub manager for a Crew relay instance.
...
impl PubSubManager {
    ...
    /// `crew:{community}:channel:{id}` / `crew:{community}:global`, and
