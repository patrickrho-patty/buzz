...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::args().nth(1).as_deref() == Some("--migrate-only") {
        ...
        let pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .await?;
        let runtime_role = std::env::var("CREW_PUSH_RUNTIME_DATABASE_ROLE")?;
        PostgresAuthorityStore::apply_migrations_and_grants(&pool, &runtime_role).await?;
        return Ok(());
    }
    let c = Config::from_env()?;
    let metrics_handle = crew_push_gateway::metrics::install()?;
    ...
    let reaper = tokio::spawn(async move {
        ...
        loop {
            ...
            if reaper_authority
                ...
                .is_err()
            {
                crew_push_gateway::metrics::record_reaper_failure();
