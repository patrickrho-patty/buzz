...
impl PushTransport for ApnsTransport {
    async fn send(
        ...
        let base_url = match profile {
            AppProfile::CrewIosProduction => &self.production_base_url,
            AppProfile::CrewIosSandbox => &self.sandbox_base_url,
        };
        let response = self
            .client
    ...
    async fn real_outbound_http_body_is_the_exact_constant_for_every_attempt() {
        ...
        for (request_id, expires_at, profile, endpoint) in [
            (
                uuid::Uuid::nil(),
                1,
                AppProfile::CrewIosProduction,
                "00".repeat(32),
            ),
            (
                uuid::Uuid::max(),
                i64::MAX,
                AppProfile::CrewIosSandbox,
                "ff".repeat(32),
            ),
        ] {
            assert_eq!(
