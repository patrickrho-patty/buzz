...
//!
//! Defines [`ChannelAccessChecker`] so `crew-auth` can enforce access
//! without depending on `crew-db` directly.
...
///
/// Implemented by the database layer (`crew-db`) in production. The `crew-auth`
