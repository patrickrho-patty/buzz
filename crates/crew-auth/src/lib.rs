#![deny(unsafe_code)]
#![warn(missing_docs)]
//! `crew-auth` — Authentication and authorization for the Crew relay.
//!
//! ## Auth paths
//!
...
///
/// Uses `SHA-256("crew-test-key:{username}")` as the secret key material.
...
pub fn derive_pubkey_from_username(username: &str) -> Result<nostr::PublicKey, AuthError> {
    ...
    let seed = format!("crew-test-key:{username}");
