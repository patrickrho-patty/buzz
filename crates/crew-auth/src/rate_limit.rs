...
//! `crew-relay` / `crew-pubsub`. Fixed-window counter algorithm.
//!
...
///
/// The Redis-backed production implementation lives in `crew-relay` / `crew-pubsub`.
...
mod tests {
    ...
    fn ip_rate_limit_key_format() {
        ...
        assert_eq!(ip_rate_limit_key(&ip), "crew:ratelimit:ip:192.168.1.1:conn");
    }
