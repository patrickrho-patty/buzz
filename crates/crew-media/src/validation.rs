...
pub fn validate_file_content(
    ...
    match infer::get(bytes) {
        Some(kind) => {
            ...
            // audio is rejected until Crew has an explicit sanitizer and
...
/// Returns true when a raw tEXt chunk payload is a Crew snapshot manifest:
...
fn validate_png_metadata_free(bytes: &[u8]) -> Result<(), MediaError> {
    ...
    while i < bytes.len() {
        ...
        let end = i
            ...
            .ok_or(MediaError::InvalidImage)?;
        if &kind == b"tEXt" {
            // Crew agent/team snapshot manifests ride in a single tEXt chunk
