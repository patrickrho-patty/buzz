...
impl FromStr for S3AddressingStyle {
    ...
    fn from_str(value: &str) -> Result<Self, Self::Err> {
        match value {
            ...
            _ => Err(format!(
                "CREW_S3_ADDRESSING_STYLE must be 'path' or 'virtual', got {value:?}"
            )),
        }
    }
}
...
pub struct MediaConfig {
    ...
    /// `CREW_MEDIA_UPLOAD_RECORDS=true`.
    #[serde(default)]
...
impl MediaConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.upload_ip_header.is_some() && !self.upload_records_enabled {
            return Err(
                "CREW_MEDIA_UPLOAD_IP_HEADER is set but CREW_MEDIA_UPLOAD_RECORDS is not \
        ...
        if self.upload_port_header.is_some() && self.upload_ip_header.is_none() {
            return Err(
                "CREW_MEDIA_UPLOAD_PORT_HEADER is set without CREW_MEDIA_UPLOAD_IP_HEADER — \
        ...
        for (name, value) in [
            ("CREW_MEDIA_UPLOAD_IP_HEADER", &self.upload_ip_header),
            ("CREW_MEDIA_UPLOAD_PORT_HEADER", &self.upload_port_header),
        ] {
            if let Some(h) = value {
...
mod tests {
    ...
    fn addressing_style_rejects_unknown_or_ambiguous_values() {
        for invalid in ["", "auto", "PATH", "virtual-hosted"] {
            ...
            assert!(
                error.contains("CREW_S3_ADDRESSING_STYLE must be 'path' or 'virtual'"),
