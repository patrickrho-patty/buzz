...
2. Copy the encoded files into both fixture directories:
   ...
   cp uikit-encoded.png crates/crew-media/tests/fixtures/ios/
   cp uikit-encoded.jpg crates/crew-media/tests/fixtures/ios/
   ```
...
5. Run `cargo test -p crew-media test_ios_uikit` to verify that UIKit's encoded output is rejected and the matching sanitizer output is accepted by the relay contract.
