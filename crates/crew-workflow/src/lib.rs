...
use crew_core::kind::{event_kind_u32, is_workflow_execution_kind, KIND_REACTION};
use crew_core::tenant::CommunityId;
use crew_db::workflow::RunStatus;
use crew_db::Db;
use dashmap::DashMap;
...
pub struct WorkflowEngine {
    ...
    pub(crate) workflow_cache:
        moka::sync::Cache<(CommunityId, Uuid), Arc<Vec<crew_db::workflow::WorkflowRecord>>>,
}
...
/// Build a [`executor::TriggerContext`] from a [`crew_core::StoredEvent`].
///
...
pub fn build_trigger_context(event: &crew_core::StoredEvent) -> executor::TriggerContext {
    let kind_u32 = event_kind_u32(&event.event);
...
/// marker. Delegates to the shared [`crew_core::nip10`] parser so this stays in
...
fn event_is_reply(event: &nostr::Event) -> bool {
    crew_core::nip10::parse_thread_markers(&event.tags)
        .reply
        .is_some()
}
...
fn trigger_matches_event(trigger: &TriggerDef, kind_u32: u32) -> bool {
    use crew_core::kind::{KIND_REACTION, KIND_STREAM_MESSAGE, KIND_STREAM_MESSAGE_DIFF};
    match trigger {
...
"#;
    ...
    fn trigger_matches_stream_message() {
        ...
        assert!(trigger_matches_event(
            &trigger,
            crew_core::kind::KIND_STREAM_MESSAGE
        ));
        assert!(!trigger_matches_event(
            &trigger,
...
"#;
    ...
    fn schedule_trigger_never_matches_events() {
        ...
        assert!(!trigger_matches_event(
            &trigger,
            crew_core::kind::KIND_WORKFLOW_TRIGGERED
        ));
    }
    ...
    fn workflow_execution_kinds_do_not_match_any_trigger() {
        ...
        for kind in crew_core::kind::KIND_WORKFLOW_TRIGGERED
            ..=crew_core::kind::KIND_WORKFLOW_APPROVAL_DENIED
        {
            assert!(
    ...
    fn make_message_event() -> crew_core::StoredEvent {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "hello world")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        crew_core::StoredEvent::new(event, Some(Uuid::new_v4()))
    }

    /// Create a reaction event with an `e` tag pointing to a target message.
    fn make_reaction_event() -> (crew_core::StoredEvent, String) {
    ...
    fn build_trigger_context_is_reply_true_for_threaded_message() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "a threaded reply")
            ...
            .sign_with_keys(&keys)
            .expect("sign");
        let stored = crew_core::StoredEvent::new(event, Some(Uuid::new_v4()));
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_no_channel_id() {
        ...
        let stored = crew_core::StoredEvent::new(event, None);
        let ctx = build_trigger_context(&stored);
    ...
    fn build_trigger_context_ignores_actor_tag() {
        ...
        let event = EventBuilder::new(Kind::Custom(9), "forged actor")
            ...
            .expect("sign");
        let stored = crew_core::StoredEvent::new(event, Some(uuid::Uuid::new_v4()));
    ...
    async fn setup_db() -> crew_db::Db {
        let database_url = std::env::var("CREW_TEST_DATABASE_URL")
            ...
            .unwrap_or_else(|_| {
                let local_test_database = "postgres://crew:crew_dev@localhost:5432/crew"; // sadscan:disable np.postgres.1
    ...
    async fn setup_channel(db: &crew_db::Db, creator: &[u8], member: &[u8]) -> (CommunityId, Uuid) {
        ...
        {
            crew_db::CreateCommunityWithOwnerResult::Created(rec) => rec.id,
        ...
        db.add_member(
            ...
            channel_id,
            member,
            crew_db::channel::MemberRole::Member,
            Some(creator),
        )
        .await
        .expect("add member");
    ...
    fn message_event(channel_id: Uuid) -> crew_core::StoredEvent {
        let keys = nostr::Keys::generate();
        let event = nostr::EventBuilder::new(nostr::Kind::Custom(9), "hello")
            .sign_with_keys(&keys)
            .expect("sign");
        crew_core::StoredEvent::new(event, Some(channel_id))
    }
