...
pub async fn dispatch_action(
    ...
    let serving_write =
        crew_deletion::acquire_serving_write(&engine.db, community_id, "workflow_action")
            .await
            .map_err(|error| {
...
async fn check_ssrf(host: &str, port: u16) -> Result<std::net::IpAddr, WorkflowError> {
    ...
    for ip in &addrs {
        if crew_core::network::is_private_ip(ip) {
...
async fn add_reaction_impl(message_id: &str, emoji: &str) -> Result<JsonValue, WorkflowError> {
    let base_url =
        std::env::var("CREW_RELAY_BASE_URL").unwrap_or_else(|_| "http://localhost:3000".to_owned());
    ...
    if let Ok(token) = std::env::var("CREW_API_TOKEN") {
        req = req.header("Authorization", format!("Bearer {token}"));
    } else if let Ok(pubkey) = std::env::var("CREW_RELAY_PUBKEY") {
...
pub async fn execute_run(
    ...
    engine
        ...
        .update_workflow_run(
            community_id,
            run_id,
            crew_db::workflow::RunStatus::Running,
            0,
            &serde_json::json!([]),
            None,
        )
