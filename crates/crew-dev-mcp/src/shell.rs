...
fn build_bootstrap(cwd: &Path, shell_hint: &str) -> String {
    ...
    format!(
        "Working directory: {}\n\
         Detected stack: {}\n\
         Shell: {shell_hint} (set CREW_SHELL to override) — write command strings in that shell's syntax.\n\
...
pub async fn run(
    ...
    // CREW_PRIVATE_KEY is intentionally inherited — the crew CLI needs it.
...
///
/// The resolver supports `CREW_SHELL=cmd`/`pwsh` (explicit operator overrides
...
/// broken, so the resolver only needs to honor CREW_SHELL. The probe logic and
...
fn resolve_bash(_path_env: &str) -> Result<(PathBuf, String), String> {
    // Honor CREW_SHELL on Unix so power users can opt into zsh or another shell.
    if let Some(raw) = std::env::var_os("CREW_SHELL") {
...
///   1. `CREW_SHELL` env override — explicit operator choice, any shell (cmd,
...
fn resolve_bash(path_env: &str) -> Result<(PathBuf, String), String> {
    ...
    Err(
        "Git for Windows (Git Bash) is required but was not found. Checked \\
         CREW_SHELL, GIT_BASH, bash.exe and git.exe on PATH, the standard Git install locations, \\
         and HKLM/HKCU\\\\SOFTWARE\\\\GitForWindows. Git's \"Cmd\" PATH option adds \\
         Git\\\\cmd\\\\git.exe but not Git\\\\bin\\\\bash.exe; Crew normally derives Git Bash from that git.exe. \\
         Install it from https://git-scm.com/download/win and select \"Git from the command line \\
         and also from 3rd-party software\", then relaunch Crew. You can also set \\
         CREW_SHELL to a shell executable."
            .into(),
    )
}
...
mod windows_resolver_tests {
    ...
    // Process-global env mutation guard: tests that mutate CREW_SHELL,
    ...
    #[test]
    fn crew_shell_override_wins_over_everything() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // CREW_SHELL pointing at a real file must be returned without probing
        // the standard Git-for-Windows locations or PATH.
        let dir = tempdir().expect("tempdir");
        let fake_bash = dir.path().join("my-bash.exe");
        touch(&fake_bash);
        // Temporarily set CREW_SHELL; clean up after the test.
        env::set_var("CREW_SHELL", &fake_bash);
        let result = resolve_bash("");
        env::remove_var("CREW_SHELL");
        let (resolved, _name) = result.expect("CREW_SHELL override should resolve");
        assert_eq!(resolved, fake_bash);
    }

    #[test]
    fn crew_shell_override_skipped_when_path_absent() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // If CREW_SHELL points at a non-existent path the resolver must fall
        // through rather than returning a dead path.
        env::set_var("CREW_SHELL", r"C:\does\not\exist\bash.exe");
        ...
        if let Ok((resolved, _)) = result {
            assert_ne!(
                ...
                r"C:\does\not\exist\bash.exe",
                "non-existent CREW_SHELL must not be returned"
            );
        }
        // An Err is also acceptable (no Git installed on test host).
    }

    /// Explicit CREW_SHELL bare name resolves through PATH and uses NO System32
    /// exclusion — cmd/pwsh live in System32 legitimately.
    #[test]
    fn crew_shell_explicit_bare_name_resolves_from_system32() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        // Simulate cmd.exe living in a dir that would be excluded by the WSL guard.
        // The explicit CREW_SHELL branch must NOT skip System32.
        ...
        // for bash.exe, but must NOT skip it for an explicit CREW_SHELL).
        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        env::set_var("CREW_SHELL", "cmd");
        ...
        let (resolved, name) =
            result.expect("explicit CREW_SHELL=cmd should resolve even from System32-like dir");
    ...
    fn implicit_bash_scan_still_skips_system32() {
        ...
        // Without an explicit CREW_SHELL, the fallback scan must skip it.
        let sys32 = tempdir().expect("sys32");
        touch(&sys32.path().join("bash.exe"));

        let path_env = env::join_paths([sys32.path().to_path_buf()]).expect("join");
        // No CREW_SHELL — trigger the implicit bash fallback scan.
        env::remove_var("CREW_SHELL");
    ...
    /// Verify that constructing SharedState with CREW_SHELL set produces a
    ...
    fn shared_state_bootstrap_hint_matches_resolved_shell() {
        ...
        touch(&fake_pwsh);
        env::set_var("CREW_SHELL", &fake_pwsh);
    ...
    /// F3: CREW_SHELL bare command name (e.g. "pwsh") resolved through PATH.
    /// When pwsh.exe is on PATH, resolve_bash must return it and report "pwsh".
    #[test]
    fn crew_shell_bare_name_resolved_through_path_when_present() {
        let _guard = ENV_MUTEX.lock().unwrap_or_else(|p| p.into_inner());
        let dir = tempdir().expect("tempdir");
        let fake_pwsh = dir.path().join("pwsh.exe");
        touch(&fake_pwsh);
        ...
        env::set_var("CREW_SHELL", "pwsh");
        let result = resolve_bash(path_env.to_str().expect("utf8"));
        env::remove_var("CREW_SHELL");

        let (resolved, name) = result.expect("bare CREW_SHELL=pwsh should resolve from PATH");
    ...
    /// F3: CREW_SHELL bare command name absent from PATH → fall through, do not
