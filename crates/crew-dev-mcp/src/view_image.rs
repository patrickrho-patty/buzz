...
fn server_authority(url: &reqwest::Url) -> Option<String> {
    let authority = crew_core::tenant::relay_url_authority(url.as_str());
...
/// media and `CREW_PRIVATE_KEY` is available; `None` otherwise.
///
...
fn relay_media_get_auth(url: &reqwest::Url) -> Option<String> {
    ...
    let keys = match nostr::Keys::parse(&key) {
        Ok(k) => k,
        Err(e) => {
            tracing::warn!("CREW_PRIVATE_KEY invalid; fetching relay media unauthenticated: {e}");
            return None;
        }
    };
...
/// `CREW_RELAY_URL` + `CREW_PRIVATE_KEY` are configured.
async fn fetch_url(url: &str) -> Result<Vec<u8>, ErrorData> {
    ...
    if auth.is_some() {
        // Do not forward Crew auth headers to a redirect target. reqwest strips
        ...
        if matches!(status.as_u16(), 401 | 403) && !authed {
            return Err(invalid_params(format!(
                "fetch {url} returned HTTP {status} — this relay requires authenticated media \
                 reads; set CREW_PRIVATE_KEY (and CREW_RELAY_URL) to a member identity"
            )));
        }
        return Err(invalid_params(format!(
