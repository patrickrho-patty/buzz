...
impl Shim {
    pub fn install() -> std::io::Result<Self> {
        let dir = tempfile::Builder::new().prefix("crew-dev-mcp-").tempdir()?;
...
fn write_keyfile(shim_dir: &Path, raw: &str) -> Option<KeyInfo> {
    ...
    let keys = match nostr::Keys::parse(raw) {
        ...
        Err(e) => {
            eprintln!(
                "crew-dev-mcp: warning: NOSTR_PRIVATE_KEY is set but invalid ({e}); \
    ...
    if write_keyfile_atomic(&keyfile, raw.as_bytes()).is_err() {
        eprintln!(
            "crew-dev-mcp: warning: failed to write nostr keyfile; git auth/signing disabled"
        );
        return None;
    }
    let keyfile_path = match keyfile.to_str() {
        ...
        None => {
            eprintln!(
                "crew-dev-mcp: warning: tempdir path is not valid UTF-8; git auth/signing disabled"
            );
            return None;
        }
    };
...
///
/// Deliberately distinct from `CREW_ACP_SESSION_TITLE`, which is per-session UI
...
const DISPLAY_NAME_ENV_VAR: &str = "CREW_ACP_DISPLAY_NAME";
...
/// via crew-agent (which clears env), the base is always 0 — composition only
...
mod git_user_name_tests {
    ...
    fn test_build_git_env_uses_display_name_and_leaves_email_on_the_pubkey() {
        ...
        std::env::remove_var("CREW_RELAY_URL");
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_unset() {
        ...
        // CREW_ACP_DISPLAY_NAME lands on the Desktop side.
    ...
    fn test_build_git_env_falls_back_to_npub_when_display_name_is_unusable() {
        ...
        for raw in ["<>", "\u{200B}"] {
            std::env::set_var("CREW_ACP_DISPLAY_NAME", raw);
            let env = build_git_env(&key_info());
            assert_eq!(
