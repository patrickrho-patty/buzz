...
pub const MANAGED_BY: &str = "crew-backend-kubernetes";
...
impl AgentIdentity {
    ...
    pub fn pod_name(&self) -> String {
        format!("crew-agent-{}", &self.pubkey_hex[..12])
    }
    ...
    pub fn secret_name(&self, generation: &str) -> String {
        format!("crew-agent-{}-{}", &self.pubkey_hex[..12], generation)
    }
...
mod tests {
    ...
    fn pod_name_is_deterministic_and_dns_safe() {
        ...
        assert_eq!(
            id.pod_name(),
            format!("crew-agent-{}", &id.pubkey_hex()[..12])
        );
