...
mod tests {
    ...
    fn terminated_pods_and_their_secrets_are_collected_together() {
        let id = identity();
        let now = Utc::now();
        let pod = pod_named(&id, "crew-agent-dead", Some("crew-agent-dead-gen1"));
        let plan = plan(&id, &[pod], &[], always, Some(now));
        assert_eq!(plan.pods, ["crew-agent-dead"]);
        assert_eq!(plan.secrets, ["crew-agent-dead-gen1"]);
    }

    #[test]
    fn live_pods_are_never_collected() {
        let id = identity();
        let pod = pod_named(&id, "crew-agent-live", Some("crew-agent-live-gen1"));
    ...
    fn young_unreferenced_secrets_are_protected() {
        let id = identity();
        let now = Utc::now();
        let fresh = secret_named(&id, "crew-agent-x-gen-inflight", 5, now);
        assert_eq!(
    ...
    fn secrets_older_than_twice_the_deadline_are_collected() {
        ...
        let old = secret_named(
            &id,
            "crew-agent-x-gen-abandoned",
            ORPHAN_SECRET_MIN_AGE_SECS + 1,
            now,
        );
        let plan = plan(&id, &[], &[old], never, Some(now));
        assert_eq!(plan.secrets, ["crew-agent-x-gen-abandoned"]);
    }
    ...
    fn without_a_server_clock_the_orphan_sweep_is_skipped() {
        let id = identity();
        let now = Utc::now();
        let ancient = secret_named(&id, "crew-agent-x-gen-ancient", 10_000_000, now);
    ...
    fn secrets_referenced_by_a_pending_pod_are_protected() {
        let id = identity();
        let now = Utc::now();
        let pending = pod_named(&id, "crew-agent-pending", Some("crew-agent-pending-gen1"));
        let old = secret_named(&id, "crew-agent-pending-gen1", 10_000_000, now);
    ...
    fn secrets_without_a_creation_timestamp_are_skipped() {
        let id = identity();
        let now = Utc::now();
        let mut no_timestamp = secret_named(&id, "crew-agent-x-gen-unknown", 10_000_000, now);
    ...
    fn a_collected_pods_secret_is_listed_once() {
        let id = identity();
        let now = Utc::now();
        let dead = pod_named(&id, "crew-agent-dead", Some("crew-agent-dead-gen1"));
        let its_secret = secret_named(&id, "crew-agent-dead-gen1", 10_000_000, now);
