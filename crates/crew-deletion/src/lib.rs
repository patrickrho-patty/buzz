...
use crew_media::{is_tenant_owned_key, tenant_prefixes, MediaStorage};
use serde::Serialize;
...
pub struct ServingWriteGuard {
    store: DeletionStore,
    lease: crew_db::deletion::ServingWriteLease,
...
impl ServingWriteGuard {
    ...
    pub fn acquisition_is_fenced(error: &anyhow::Error) -> bool {
        matches!(
            error.downcast_ref::<crew_db::DbError>(),
            Some(crew_db::DbError::AccessDenied(_))
        )
    }
    ...
    pub fn lease(&self) -> &crew_db::deletion::ServingWriteLease {
        &self.lease
    }
...
fn is_permanent_error(error: &anyhow::Error) -> bool {
    error.chain().any(|cause| {
        cause.is::<PermanentSource>()
            || matches!(
                cause.downcast_ref::<crew_db::DbError>(),
                Some(crew_db::DbError::DeletionSafety(_))
            )
            || cause
...
async fn run_with_services(command: Command, services: Services) -> Result<i32> {
    match command {
        ...
        Command::Sweep => {
            ...
            let outcome =
                crew_media::sweep_bucket_taxonomy(cap, SWEEP_UNKNOWN_KEY_SAMPLE, move |token| {
...
fn resolve_submit_host(host: Option<&str>, relay_url: Option<&str>) -> Result<String> {
    ...
    let host = crew_core::tenant::relay_url_authority(relay_url);
    if host.is_empty() {
        anyhow::bail!(
...
fn resolve_s3_region(crew_region: Option<String>, aws_region: Option<String>) -> String {
    crew_region
...
fn validate_storage_ownership(request: &DeletionRequest, manifest: &StorageManifest) -> Result<()> {
    crew_db::deletion::validate_storage_manifest(manifest)?;
...
async fn record_stage_failure(
    ...
    match result {
        Ok(()) => Ok(true),
        Err(error) if crew_db::deletion::is_stale_deletion_lease(&error) => Ok(false),
    ...
    match request.stage {
        DeletionStage::Approved => {
            ...
            match services.store.fence(&token).await {
                Ok(_) => {}
                Err(crew_db::DbError::ServingWritesNotDrained {
                    active_count,
                    operations,
                    ..
                }) => {
...
fn default_executor_id() -> String {
    let hostname = std::env::var("HOSTNAME").unwrap_or_else(|_| "crew-admin".to_string());
...
mod tests {
    ...
    fn empty_storage_manifest(community: crew_core::CommunityId) -> StorageManifest {
        StorageManifest {
            version: 4,
    ...
    async fn frozen_inventory_digest_and_storage_ownership_fail_closed() {
        ...
        let foreign_manifest =
            empty_storage_manifest(crew_core::CommunityId::from_uuid(Uuid::new_v4()));
    ...
    fn permanent_failures_are_typed_not_string_classified() {
        ...
        let db_permanent = anyhow::Error::from(crew_db::DbError::DeletionSafety(
            "typed catalog drift".to_string(),
        ));
        let db_transient = anyhow::Error::from(crew_db::DbError::Sqlx(sqlx::Error::PoolTimedOut));
