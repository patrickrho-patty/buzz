...
///
/// Priority: NOSTR_PRIVATE_KEY > CREW_PRIVATE_KEY > git config nostr.keyfile
///
...
fn load_key() -> Result<zeroize::Zeroizing<String>, Error> {
    ...
    if let Ok(mut val) = crew_env_with_legacy("CREW_PRIVATE_KEY") {
        ...
        if val.len() > 128 {
            val.zeroize();
            std::env::remove_var("CREW_PRIVATE_KEY");
            return Err(Error::Fatal(
                "CREW_PRIVATE_KEY exceeds 128-byte size limit".to_string(),
            ));
        }
        let trimmed = val.trim().to_string();
        val.zeroize();
    ...
    let path = git_config("nostr.keyfile").ok_or_else(|| {
        Error::Fatal(
            "no key available: set NOSTR_PRIVATE_KEY, CREW_PRIVATE_KEY, \
...
///
/// Priority per NIP-GS spec: `CREW_AUTH_TAG` env var > `nostr.authtag` git config.
...
fn load_auth_tag() -> Result<Option<(String, String, String)>, Error> {
    ...
    let arr: serde_json::Value = serde_json::from_str(&json_str)
        .map_err(|e| Error::Fatal(format!("CREW_AUTH_TAG is not valid JSON: {e}")))?;
    let arr = arr
        .as_array()
        .ok_or_else(|| Error::Fatal("CREW_AUTH_TAG must be a JSON array".to_string()))?;
    if arr.len() != 4 {
        return Err(Error::Fatal(
            "CREW_AUTH_TAG must have exactly 4 elements".to_string(),
        ));
    }
    if arr[0].as_str() != Some("auth") {
        return Err(Error::Fatal(
            "CREW_AUTH_TAG[0] must be \"auth\"".to_string(),
        ));
    }

    let owner = arr[1]
        .as_str()
        .ok_or_else(|| Error::Fatal("CREW_AUTH_TAG[1] must be a string".to_string()))?
        .to_string();
    let conditions = arr[2]
        .as_str()
        .ok_or_else(|| Error::Fatal("CREW_AUTH_TAG[2] must be a string".to_string()))?
        .to_string();
    let sig = arr[3]
        .as_str()
        .ok_or_else(|| Error::Fatal("CREW_AUTH_TAG[3] must be a string".to_string()))?
        .to_string();
    ...
    if !validate_conditions(&conditions) {
        return Err(Error::Fatal(
            "CREW_AUTH_TAG conditions contain invalid characters".to_string(),
        ));
    }
    ...
    {
        return Err(Error::Fatal(
            "CREW_AUTH_TAG owner must be 64 lowercase hex chars".to_string(),
        ));
    }
    if sig.len() != 128
        || !sig
    ...
    {
        return Err(Error::Fatal(
            "CREW_AUTH_TAG signature must be 128 lowercase hex chars".to_string(),
        ));
    }
        ...
        if !verify_oa(&pk_hex, oa_val) {
            return Err(Error::Fatal(
                "auth tag owner signature (oa[2]) verification failed — \
                 the configured CREW_AUTH_TAG is invalid or stale"
                    .to_string(),
            ));
        }
    }
