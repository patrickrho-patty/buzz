#!/usr/bin/env bash
# Local docker-desktop k8s testbed for the Crew relay mesh.
#
...
if [ "$helm_rc" != 0 ]; then
  ...
  kubectl -n "$NS" describe pods -l app.kubernetes.io/name=crew | tee "$EVID/describe-onfail.txt"
  kubectl -n "$NS" logs -l app.kubernetes.io/name=crew --tail=100 --all-containers | tee "$EVID/logs-onfail.txt"
...
# The bundled MinIO + init pods share app.kubernetes.io/name=crew, so select by
...
RELAY_PODS=$(kubectl -n "$NS" get pods \
  -l "app.kubernetes.io/name=crew,app.kubernetes.io/instance=$RELEASE" \
