...
`CREW_S3_ENDPOINT` plus autogenerated credentials automatically. No external services required. The `quickstart=true` flag is an
...
Store `CREW_S3_ACCESS_KEY=${{Object Storage.ACCESS_KEY_ID}}` and
`CREW_S3_SECRET_KEY=${{Object Storage.SECRET_ACCESS_KEY}}` in the Secret named by
...
`CREW_GIT_CONFORMANCE_PROBE` is enabled (the relay default). A probe failure is
...
autoscaling:
  ...
  websocketMetricName: crew_ws_connections_active
...
`crew_ws_connections_active` gauge as a pod metric with the name in
...
- **Bundled MinIO is eval-only.** The quickstart profile runs an in-cluster
  ...
  `minio.enabled` off and points `s3.endpoint` (or `CREW_S3_*` in
...
- **Minimal-mode is not yet supported.** The relay's `CREW_PUBSUB=local` /
