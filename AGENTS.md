...
This guide is for AI agents contributing to the Crew codebase. It covers
...
Crew spans five repos. This one (`block/crew`) is the OSS source for the relay, desktop, mobile, and CLI. The others handle internal builds and deployment:

| Repo | Purpose |
|------|---------|
| [block/crew](https://github.com/block/buzz) | OSS source — relay, desktop app, mobile app, CLI, agent harness |
| [squareup/buzz-releases](https://github.com/squareup/squareup/buzz-releases) | Buildkite pipelines producing Block-signed macOS + iOS builds with `-block` desktop version suffix |
...
crates/
  # Relay + core
  crew-relay          # WebSocket relay server — main entry point; also hosts git + huddle audio
  crew-core           # Core types, event verification, filter matching, kind registry
  crew-db             # Postgres event store and data access layer
  crew-auth           # Authentication and authorization
  crew-pubsub         # Redis pub/sub fan-out, presence, typing indicators
  crew-search         # Postgres FTS full-text search
  crew-audit          # Hash-chain audit log
  crew-media          # Blossom/S3 media storage
  # Agent surface
  crew-acp            # ACP harness bridging Crew events to AI agents
  crew-agent          # Minimal ACP-compliant agent (non-streaming, tool-calls-as-output)
  crew-dev-mcp        # Developer MCP server — shell + file-edit tools
  crew-persona        # Agent persona packs
  crew-workflow       # YAML-as-code workflow engine (evalexpr conditions)
  # Clients + interop
  crew-pair-relay     # Ephemeral sidecar relay for NIP-AB device pairing
  crew-pairing-cli    # CLI for NIP-AB device pairing interop testing
  ...
  crew-sdk            # Typed Nostr event builders
  crew-admin          # Operator CLI for relay administration
  crew-ws-client      # Shared NIP-42 WebSocket client (connect, auth, publish)
  crew-test-client    # Integration test client and E2E test suite
...
Run `just test` for integration tests if you touched `crew-relay`,
`crew-db`, or `crew-auth` — these require a running Postgres and Redis.
...
- `POST /query` — Nostr REQ filters over HTTP. NIP-50 `search` filters
  are routed to `crew-search` (Postgres FTS) automatically.
...
`crew-core/src/kind.rs`. New features get new kind integers — add them here
...
**Agent-facing operations go in `crew-cli`**: New agent-facing features belong in `crew-cli` — add a subcommand there first, then wire the REST/WebSocket call in `client.rs`. `crew-dev-mcp` (shell + file tools for `crew-agent`) is separate.

**Workflow conditions**: `crew-workflow` uses
...
`crew` is the agent-first CLI. Auth env vars
...
```bash
cargo build --release -p crew-cli
```

Binary location: `./target/release/crew`. Add `./target/release` to `PATH`
or invoke with the full path.

### Deep Links

`crew://message?channel=<uuid>&id=<hex>` links reference a specific message
thread. Pass the link directly to the CLI:

```bash
crew --format compact messages thread --link '<crew://message?...>'
```
...
`crew --format compact channels list`, NOT `crew channels list --format compact`.

See `crates/crew-cli/TESTING.md` for the full live-testing runbook.
...
E2E tests live in `crates/crew-test-client/tests/`:
...
> **Do NOT use `crew upload`, the relay media endpoint, or any third-party
...
gh pr view <pr> --repo block/crew --json comments \
  --jq '.comments[] | select(.body | test("pr-<pr>--")) | {id, url}'
gh api -X DELETE repos/block/crew/issues/comments/<stale-comment-id>
```
...
1. **Kind `39000` for channel metadata, not `41`** — kind 41 is NIP-01 (unused). All kinds defined in `crew-core/src/kind.rs`.
