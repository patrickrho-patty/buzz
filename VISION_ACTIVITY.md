...
**The spine — read constantly.** Message (the agent's voice), Crew relay op (acting on the platform), File-edit (the actual code work), Shell command (the agent's hands), and Tool status & turn lifecycle (the heartbeat). If these are unclear, the feed has failed.
...
- **Semantics over transport.** Render *what the agent did*, not *which API it used*. A message sent through an MCP tool and the same message sent through a shell `crew` command render as the identical card. How the agent reached the relay is plumbing; what it did is the contract.
...
A feed built this way is protocol-honest at its base: any compliant agent's messages, thoughts, tool calls, and turns become first-class items regardless of which tools it runs. The Crew-specific richness — semantic relay cards, the crew-CLI parser, diff rendering — is a layer of enrichment on top, not a requirement underneath. Non-Crew agents get a correct, legible feed; Crew agents get a native one.
