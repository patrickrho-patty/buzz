...
const BOT_ABOUT: &str =
    "A tiny non-AI Crew reference bot that replies to !countdown and countdown-style !fib.";
...
impl Config {
    fn from_env() -> Result<Self> {
        ...
        let bot_keys = Keys::parse(&required_env("CREW_BOT_PRIVATE_KEY")?)
            .context("CREW_BOT_PRIVATE_KEY must be an nsec or hex private key")?;

        let auth_mode =
            std::env::var("CREW_BOT_AUTH_MODE").unwrap_or_else(|_| "standalone".to_string());
                ...
                let tag_json = match std::env::var("CREW_AUTH_TAG") {
                    ...
                    _ => {
                        let owner_keys = Keys::parse(&required_env("CREW_OWNER_PRIVATE_KEY")?)
                            .context("CREW_OWNER_PRIVATE_KEY must be an nsec or hex private key")?;
                        crew_sdk::nip_oa::compute_auth_tag(&owner_keys, &bot_keys.public_key(), "")?
                    }
                };

                let owner = crew_sdk::nip_oa::verify_auth_tag(&tag_json, &bot_keys.public_key())
                    .context("CREW_AUTH_TAG is not valid for CREW_BOT_PRIVATE_KEY")?;
                eprintln!("owner-attested auth tag verified; owner={}", owner.to_hex());
                Some(crew_sdk::nip_oa::parse_auth_tag(&tag_json)?)
            }
            other => {
                bail!("CREW_BOT_AUTH_MODE must be 'standalone' or 'owner-attested', got {other:?}")
            }
        };
...
async fn publish_profile(ws: &mut Ws, config: &Config) -> Result<()> {
    let builder = crew_sdk::builders::build_profile(
...
async fn maybe_reply(
    ...
    let builder = crew_sdk::builders::build_message(
