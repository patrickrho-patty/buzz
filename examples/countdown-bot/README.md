...
A tiny non-AI Crew bot example.

The bot is deliberately boring and algorithmic: it listens to one Crew channel
...
It demonstrates that Crew participants do not have to be LLM agents. Any
...
members list and in Crew's mention autocomplete.
...
```bash
CREW_RELAY_URL=ws://localhost:3000 \
CREW_CHANNEL_ID=<channel-uuid> \
CREW_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
CREW_BOT_AUTH_MODE=standalone \
...
relay. This reuses the same owner-attestation credential path that Crew agents
...
CREW_CHANNEL_ID=<channel-uuid> \
CREW_BOT_PRIVATE_KEY=<bot-nsec-or-hex-secret> \
CREW_OWNER_PRIVATE_KEY=<owner-or-agent-nsec-or-hex-secret> \
CREW_BOT_AUTH_MODE=owner-attested \
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```
...
```bash
CREW_AUTH_TAG='["auth","<owner-pubkey>","","<sig>"]' \
CREW_BOT_AUTH_MODE=owner-attested \
# plus CREW_RELAY_URL, CREW_CHANNEL_ID, CREW_BOT_PRIVATE_KEY
cargo run --manifest-path examples/countdown-bot/Cargo.toml
```

Relay requirements for this path:

- `CREW_REQUIRE_RELAY_MEMBERSHIP=true` on closed relays.
- `CREW_ALLOW_NIP_OA_AUTH=true` so owner-attested non-member bot keys can be
  admitted.
...
- Mention commands require both text like `@Countdown Bot fib 8` and a `p` tag
  for the bot pubkey. The Crew UI adds that tag when the bot is selected from
