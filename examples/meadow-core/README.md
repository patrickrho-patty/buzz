...
A minimal three-agent persona pack for Crew.
...
```bash
# Validate the pack
crew pack validate ./examples/meadow-core

# Inspect resolved config
crew pack inspect ./examples/meadow-core
```
...
app), not persona-pack source. `crew pack inspect` above shows the fully-resolved per-agent
...
`crates/crew-persona/PERSONA_PACK_SPEC.md` for the current import paths.
...
See `crates/crew-persona/PERSONA_PACK_SPEC.md` for the full format reference.
