...
test("invite requires age and legal consent before opening Crew", async ({
  page,
}) => {
  ...
  await page
    .locator("label")
    .filter({
      hasText: "I agree to the Crew Terms of Service and Privacy Policy.",
    })
...
test("invite asks Safari users to choose their Mac download", async ({
  ...
  await expect(openedPage).toHaveURL("https://github.com/block/crew/releases");
...
test("invite download falls back for mobile and non-desktop devices", async ({
  ...
  for (const device of unsupportedDevices) {
    ...
    await expect(
      page.getByRole("link", { name: "Download it now" }),
      device.name,
    ).toHaveAttribute("href", "https://github.com/block/crew/releases");
    await context.close();
  }
});
