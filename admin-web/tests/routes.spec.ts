...
test("report rows render the relay response contract", async ({ page }) => {
  ...
  await page.goto("/reports");
  await expect(page.getByText("design.crew.xyz")).toBeVisible();
...
test("feedback can be searched and filtered by community and time", async ({
    ...
    route.fulfill({
      ...
      body: JSON.stringify([
        ...
        },
        {
          id: "old",
          communityId: "two",
          communityHost: "engineering.crew.xyz",
  ...
  await page.getByLabel("Community").selectOption("design.crew.xyz");
...
test("feedback attachments render from imeta without raw markdown", async ({
  ...
  const imageUrl = `https://design.crew.xyz/media/${"a".repeat(64)}.png`;
  const fileUrl = `https://design.crew.xyz/media/${"b".repeat(64)}.txt`;
  await page.route(`**/api/admin/v1/feedback/${id}`, (route) =>
    route.fulfill({
      contentType: "application/json",
      body: JSON.stringify({
