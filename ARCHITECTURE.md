...
A Crew **community** is the tenant-visible workspace selected by the request host.
...
Crew is a Rust monorepo, licensed Apache 2.0 under Block, Inc.
...
│                           CLIENTS                                    │
│                                                                      │
│  Human (Nostr app, web, mobile)    Agent (CLI tools via crew-cli)    │
│           │                                    │                     │
│           └──────────── WebSocket ─────────────┘                    │
└─────────────────────────────────────────────────────────────────────┘
                               │
...
└──────────┬──────────────┬──────────────────────────────────────────┘
     ...
     ┌──────────────┐
     │  Postgres    │  ← crew-search (FTS over the search_tsv
...
```
crew-core    (zero I/O — types, verification, filter matching, kind registry)
    │
    ├── crew-db          (Postgres: events, channels, tokens, workflows, audit)
    ├── crew-auth        (NIP-42, NIP-98, API tokens, scopes, rate limiting)
    ├── crew-pubsub      (Redis pub/sub, presence, typing indicators)
    ├── crew-search      (Postgres FTS: query, delete)
    ├── crew-audit       (hash-chain tamper-evident log)
    └── crew-workflow    (YAML-as-code automation engine)
         │
         └── crew-relay       (ties everything together — the server)

crew-acp            (agent harness — bridges relay @mentions → AI agents via ACP/JSON-RPC)
crew-sdk            (typed Nostr event builders — used by crew-acp and crew-cli)
crew-media          (Blossom/S3 media storage)
crew-cli            (agent-first CLI)
crew-admin          (operator CLI: relay membership + key generation)
crew-test-client    (integration test harness + manual CLI)
```

**Key architectural principle:** The relay is the single source of truth. `crew-relay` orchestrates all subsystems by calling them directly — it imports `crew-db`, `crew-auth`, `crew-pubsub`, `crew-search`, `crew-audit`, and `crew-workflow`. However, those subsystems are isolated from each other: `crew-workflow` never calls `crew-pubsub`, `crew-search` never calls `crew-db`, etc. Cross-subsystem coordination happens only through the relay. In multi-community mode, the relay also owns propagation of `TenantContext`; service crates should receive community-scoped inputs rather than independently deriving tenancy from client-controlled event tags.

---

## 2. The Protocol

Crew uses Nostr NIP-01 on the wire. Every action is a JSON event with six fields:
...
| 40000–49999 | Crew custom kinds |

### Crew Custom Kinds (selected)
...
`crew-core` defines each event kind as a `pub const u32` and exports the full registry as `ALL_KINDS: &[u32]` (127 kinds at the time of writing); `crates/crew-core/src/kind.rs` is the source of truth for the current list. Kinds are `u32` (NIP-01 specifies unsigned integer; `u32` covers the full range). Crew uses both standard Nostr kinds (e.g., kind 7 for reactions) and custom ranges (40000+).

Note: `KIND_AUTH` (22242) is `pub const KIND_AUTH: u32` in `crew-core/src/kind.rs` and imported by `crew-relay/src/handlers/event.rs`. `KIND_CANVAS` (40100) is likewise `pub const KIND_CANVAS: u32` in `crew-core/src/kind.rs`.
...
Workflow loop prevention: workflow execution kinds (46001–46012), relay-signed messages with `crew:workflow` tag, and `KIND_GIFT_WRAP` are excluded from triggering workflows. All other stored events (including kind 9 stream messages) trigger workflow evaluation.
...
### crew-core — Shared Types and Verification
...
### crew-auth — Authentication and Authorization
...
- Dev-only key derivation: `SHA-256("crew-test-key:{username}")` — gated behind `#[cfg(any(test, feature = "dev"))]`. The `dev` feature must not be enabled in production relay deployments.
...
### crew-pubsub — Redis Pub/Sub, Presence, Typing

Manages Redis pub/sub fan-out, presence tracking, and typing indicators. In multi-community mode all tenant-visible keys are prefixed or otherwise partitioned by community (`crew:{community}:...`) so channel fan-out, presence, typing, and cache invalidation cannot cross hosts.

**Architecture:**

```
Publisher  → pool connection   → PUBLISH crew:channel:{uuid}
Subscriber → dedicated PubSub  → PSUBSCRIBE crew:channel:*
...
**Presence:** `SET crew:presence:{pubkey_hex} {status} EX 180` — 180-second TTL (3× the 60-second heartbeat interval). Single missed heartbeat does not cause presence flap.

**Typing indicators:**
```
ZADD crew:typing:{channel_id} {now_unix} {pubkey_hex}
ZREMRANGEBYSCORE crew:typing:{channel_id} -inf {now - 5.0}
EXPIRE crew:typing:{channel_id} 60
```
...
### crew-search — Postgres FTS Integration
...
- Permission filtering is **caller's responsibility** — `crew-search` returns
...
### crew-audit — Hash-Chain Audit Log
...
Real-time voice lives inside `crew-relay` (`src/audio/`), not a separate crate. A WebSocket endpoint (`wss://.../huddle/{channel_id}/audio`) authenticates each participant with a NIP-42 challenge, checks channel membership, admits them to an in-memory room, and forwards opaque Opus frames between peers. No external SFU.
...
### crew-acp — Agent Communication Protocol Harness

Standalone binary that bridges Crew relay events to AI agents via the [Agent Communication Protocol](https://agentclientprotocol.com/) (ACP).

**Architecture:**

```
Crew Relay ──WS──→ crew-acp ──stdio (ACP/JSON-RPC)──→ Agent (goose/codex/claude)
```

`crew-acp` spawns AI agent subprocesses (1–32, default 1), connects to the relay via WebSocket with NIP-42 auth, discovers channels via REST API, and queues `@mention` events per channel. At most one prompt is in-flight per channel. Queued events are batched into a single prompt sent via `session/prompt` over ACP.
...
- Depends on `crew-core` (kind constants) and `crew-sdk` (relay/REST utilities).

**Does NOT:** persist state.

---

### crew-admin — Operator CLI
...
The `crew-admin` binary is shipped in the relay Docker image (`/usr/local/bin/crew-admin`) and is the recommended way to manage relay membership in production. Use `./run.sh add-member`, `./run.sh remove-member`, and `./run.sh list-members` in Docker Compose deployments.

---

### crew-test-client — Integration Test Harness

**`CrewTestClient`** wraps a WebSocket connection with a `VecDeque<RelayMessage>` buffer for message interleaving. Methods: `connect`, `connect_unauthenticated`, `authenticate`, `send_event`, `send_text_message`, `subscribe`, `close_subscription`, `recv_event`, `collect_until_eose`, `disconnect`.
...
`src/main.rs` is a manual testing CLI (`crew-test-cli`) with `--send`, `--subscribe`, `--channel`, `--url`, `--kind` flags.
...
| Concern | Mechanism |
|---------|-----------|
| Schnorr signatures | `verify_event()` in `crew-core` — every event verified before storage |
...
`is_private_ip()` in `crew-core` covers:
...
Applied in: `crew-workflow` (CallWebhook action), `crew-core` (shared utility).
...
|---------|------|-----|---------|
| `crew:channel:{uuid}` | Pub/Sub channel | — | Event fan-out (single-community form; shared multi-community Redis must use `crew:{community}:channel:{uuid}` or equivalent) |
| `crew:presence:{pubkey_hex}` | String | 180s | Online/away status (single-community form; shared multi-community Redis must scope by community) |
| `crew:typing:{channel_uuid}` | Sorted Set | 60s | Active typers (5s window; shared multi-community Redis must scope by community) |
...
| 2 | **No rate limiting implementation** | `RateLimiter` trait exists in `crew-auth`. Only implementation is `AlwaysAllowRateLimiter` (test stub, gated behind `#[cfg(any(test, feature = "test-utils"))]`). `RateLimitConfig` defines 4 tiers (human, agent-standard, agent-elevated, agent-platform) but none are enforced. |
