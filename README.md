...
| Platform | File |
|---|---|
| macOS (Apple Silicon) | `Crew_<version>_aarch64.dmg` |
| macOS (Intel) | `Crew_<version>_x64.dmg` |
| Linux (x86_64) | `Crew_<version>_amd64.AppImage` or `Crew_<version>_amd64.deb` |
| Windows (x64) | `Crew_<version>_x64-setup_alpha-unsigned.exe` |
...
By default the app connects to `ws://localhost:3000`. To point it at a relay you're running or one someone shared with you, set `CREW_RELAY_URL` before launching, or switch the relay from inside the app. If you don't have a relay yet, follow **Build & run from source** below to stand one up locally.
