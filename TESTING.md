...
`crew-test-client` — those are marked `#[ignore]` and require a running relay:

```bash
# Start a relay first (see below), then:
cargo test -p crew-test-client -- --ignored
```
...
binaries once, run `crew-relay`, and drive it with the `crew` CLI. The
...
> **Already running Crew Desktop?** Desktop uses the same Docker container
> names (`crew-postgres`, `crew-redis`) and the same
...
> (`COMPOSE_PROJECT_NAME=crew-dev docker compose …`).

`just reset` wipes all local data and starts over — **including Crew
...
```bash
cargo build --release -p crew-relay -p crew-cli -p crew-admin
...
```bash
crew-relay                     # release binary from step 2, serves ws://localhost:3000
# alternatives:
# cargo run --release -p crew-relay     # rebuild + run in release
...
> **Already running Crew Desktop (or another relay) on `:3000` / `:8080` /
> `:9102`?** Crew binds three ports — main, health, metrics — and any of
> them can collide. Use a separate terminal per role and export the right
> vars in each:
>
> **In the relay terminal** (before launching `crew-relay`):
> ```bash
...
> overrides — or the CLI will end up talking to Crew Desktop's relay.

> **Ignore `just setup`'s "Next steps" banner.** It still prints
> `just relay` (a debug build). Use `crew-relay` from step 2 here —
> step 2 already built the release binary.

When you're done, stop the relay (Ctrl-C in its terminal). If it's
backgrounded or you lost the terminal: `pkill -f crew-relay`. Leaving
...
CHANNEL=$(crew channels create --name "smoke-$$" --type stream --visibility open | jq -r '.channel_id')
echo "channel: $CHANNEL"

# Send a message and read it back
SEND=$(crew messages send --channel "$CHANNEL" --content "hello from smoke test")
EVENT_ID=$(echo "$SEND" | jq -r '.event_id')
crew messages get --channel "$CHANNEL" --limit 5 | jq .

# Fetch the reply chain for a specific message (empty array on a leaf — that's fine)
crew messages thread --channel "$CHANNEL" --event "$EVENT_ID" | jq .
```
...
The script refuses debug binaries and refuses a `crew` or `crew-admin` resolved
...
follow [`crates/crew-cli/TESTING.md`](crates/crew-cli/TESTING.md).

The relay's HTTP bridge accepts three endpoints — useful if you're testing
a client other than `crew-cli`:
...
`POST /query` with an `#e` filter, or `crew messages thread`.

---

## ACP Harness (optional, end-to-end with a real agent)

`crew-acp` connects an ACP-speaking agent (goose, codex, claude code,
crew-agent) to the relay. The harness listens for events, drives the
...
AGENT_GEN=$(crew-admin generate-key)
...
crew channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member

# 4. Switch to the agent identity and start it.
#    crew-acp wants ws:// (not http://). If you set CREW_RELAY_URL to an
...
> code / crew-agent, set `CREW_ACP_AGENT_COMMAND` and `CREW_ACP_AGENT_ARGS`
> accordingly — see `crates/crew-acp/README.md`. Without these, crew-acp
...
same env. See `crates/crew-acp/README.md` for parallel agents, heartbeats,
respond-to gates, and forum subscriptions.

To exercise deferred ACP startup, add `CREW_ACP_LAZY_POOL=true` before launching
`crew-acp`. The harness should connect, authenticate, subscribe, and publish
...
crew messages send --channel "$CHANNEL" \
...
# `crew messages get` is how you confirm it ran.
crew messages get --channel "$CHANNEL" --limit 5 | jq '.[] | {pubkey, content}'
```

Replies are kind:9 in the same channel; `crew messages thread --channel <id>
...
| `CREW_WEB_DIR`                  | unset (source), `/srv/crew/web` (container) | Directory containing the invite landing bundle; the production container enables it so `/invite/{code}` always works |
...
| `Address already in use` on relay start (os error 48 on macOS, 98 on Linux) | Another relay (or stale process) holding `:3000` / `:8080` / `:9102` (or your override ports) | The panic line names the failing port — read it first. Then `lsof -iTCP:3000,8080,9102 -sTCP:LISTEN` (or your override equivalents). Kill the offender (`pkill -f crew-relay`) or use the port-override block in step 3. If you already overrode and *still* collide, a prior reviewer left a relay running on the same alt ports — kill it or pick fresh ports |
...
| ACP logs `discovered 0 channel(s)` / `no channel subscriptions resolved` | Agent identity isn't a member of any channel | `crew channels add-member --channel "$CHANNEL" --pubkey "$AGENT_PUBKEY" --role member` from another identity |
