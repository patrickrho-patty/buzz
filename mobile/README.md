...
to the **worktree directory name** (`com.patty.crew.<slug>` on iOS,
`com.patty.crew.<slug>` on Android) plus a display-only branch label
...
- `CREW_ANDROID_UPLOAD_KEYSTORE_PATH`: path to a CI-vended keystore file
- `CREW_ANDROID_UPLOAD_KEYSTORE_PASSWORD`
- `CREW_ANDROID_UPLOAD_KEY_ALIAS`
- `CREW_ANDROID_UPLOAD_KEY_PASSWORD`
...
a local upload keystore must set `CREW_ANDROID_RELEASE_SIGNING=external`. That
mode produces an unsigned release bundle and refuses to run if any
`CREW_ANDROID_UPLOAD_*` value is also set.
