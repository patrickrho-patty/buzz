...
scripts/desktop_release.py generate "$version" --base "$base_sha" --repo block/crew
...
git -c user.name='Wes' -c user.email='wesbillman@users.noreply.github.com' \
  commit -s -F "$msg"
scripts/desktop_release.py validate --candidate HEAD --version "$version" --repo block/crew
