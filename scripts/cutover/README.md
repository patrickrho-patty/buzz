...
Boot the 1321 relay with **`CREW_AUTO_MIGRATE=false`**. The schema is already
...
later `CREW_AUTO_MIGRATE=true` boot doesn't try to re-run it), seed its
...
prefer leaving `CREW_AUTO_MIGRATE=false` unless you have a reason to seed.
