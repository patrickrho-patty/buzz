...
# Crew/relay media URLs often render in Crew but fail in GitHub PR markdown
...
if [[ -s "$tmp_matches" ]]; then
  ...
  echo "error: PR markdown contains Crew/relay media URLs that may not render on GitHub:" >&2
  printf '%s\n' "$matches" >&2
  echo >&2
