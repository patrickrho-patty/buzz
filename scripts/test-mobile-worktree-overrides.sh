...
grep -Eq "^APP_DISPLAY_NAME = Crew \([A-Za-z0-9._-]+\)$" "$ios" \
...
if printf '%s\n' "$clean_out" | grep -Eq '(would uninstall|uninstalling).*xyz\.block\.crew\.mobile$'; then
