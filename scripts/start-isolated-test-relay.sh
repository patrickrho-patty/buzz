...
#   docker compose -p crew-harness -f docker-compose.harness.yml down -v
# =============================================================================
set -euo pipefail
...
# ── Backing services (scoped to crew-harness only) ───────────────────────────
...
cargo build --profile "${CARGO_BUILD_PROFILE}" -p crew-relay
ok "Relay built"
