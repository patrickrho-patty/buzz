...
case "$command" in
  candidate)
    ...
    run_url="$(printf '%s\n' "$dispatch_output" | awk '/^https:\/\/github\.com\/block\/crew\/actions\/runs\/[0-9]+$/ { if (found) exit 2; found = $0 } END { if (found) print found }')" || \
    ...
    run_id="${run_url##*/}"
    gh run watch "$run_id" --repo block/crew --exit-status --compact || \
    ...
    printf 'Published %s at origin/main commit %s through crew-release-bot. Use this exact tag in Release Mobile.\n' \
      "$tag" "$main_sha"
    ;;
