...
export GITHUB_REPOSITORY=block/crew
...
grep -Fq -- '-f message=Crew Mobile 1.2.3 release candidate 1' "$GH_CALLS"
...
if GITHUB_REPOSITORY=attacker/crew "$publisher" 1.2.3 1 "$GH_TARGET_SHA" >/dev/null 2>&1; then
