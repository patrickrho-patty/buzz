# Multi-Tenant Crew Relay: A Formal Specification
...
Today a Crew relay *process* is the security boundary: one `DATABASE_URL`, one
...
4. **Unauthenticated global surface.** The NIP-11 relay information document at
   ...
   (`crates/crew-relay/src/nip11.rs:122`) takes only static inputs and
...
**O.WS — WebSocket transport** (`crates/crew-relay/src/protocol.rs:180-215`). The
...
- **`O.REST.BODY`** — JSON response: row content, projection results, and audit
  entries (`crates/crew-audit/src/service.rs:get_entries`) must all be B-labeled.
...
- **(P3)** *NIP-98 mint freshness.* A NIP-98 mint event (kind:27235) is accepted
  ...
  within ±60s of server time (`crew-auth/src/nip98.rs:77-83`,
  `TIMESTAMP_TOLERANCE_SECS = 60`) **and** a seen-set keyed on event id
  (`crew-relay/src/api/bridge.rs::check_nip98_replay`), whose cache TTL (120s,
...
- **P-SIG / A_HASH** are the standard Nostr crypto assumptions; admitted by using
  the audited libraries the rest of Crew uses.
- **P3** is admitted by the NIP-98 handler enforcing *both* timestamp-range
  ...
  2. **Per-pod scope.** The seen-set is `Arc<AppState>`-scoped, not cross-pod, so
     ...
     `deploy/charts/crew/examples/argocd-app.yaml:27` and
     `deploy/charts/crew/examples/flux-helmrelease.yaml:35`) are
...
- **I1 / I4** — every DB entry point takes `TenantContext` and `SET LOCAL
  ...
  (`crates/crew-db/src/channel.rs:545-560`, which unions every open channel in the
...
- **S3 / S4** — the relay keypair becomes a per-community signing key
  ...
  global audit chain (`crates/crew-audit/src/service.rs`) becomes N per-community
...
- **P3 / S2** — the NIP-98 mint freshness obligation the Tamarin model abstracts
  ...
  `crates/crew-auth/src/nip98.rs:77-83` and the event-id seen-set
  `check_nip98_replay` in `crates/crew-relay/src/api/bridge.rs:76-94`, called
...
- **C2.4** — the NIP-11 builder `RelayInfo::build`
  (`crates/crew-relay/src/nip11.rs:122`) must keep its relay-static-only signature
...
- **Redis / subscription refinement** — Redis pub/sub keys, presence keys, typing
  ...
  `crew:{community}:channel:{channel_id}`,
  `crew:{community}:presence:{pubkey}`, and
  `crew:{community}:typing:{channel_id}`. The current unprefixed keys are
...
#### G1 — establishment (`crates/crew-relay/src/handlers/req.rs:79-204`)
...
#### G2 — delivery (`crates/crew-relay/src/handlers/event.rs:59-113`)
