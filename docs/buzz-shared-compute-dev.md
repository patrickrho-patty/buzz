# Crew shared compute: local GUI verification

This runbook verifies the actual desktop path used by the built-in **Fizz** agent:

`Crew Desktop → crew-acp → crew-agent → MeshLLM SDK → local/remote compute`

It does not use a substitute agent harness.

## Before starting

Run from the `block/crew` repository root on the mesh-enabled branch.
...
`~/.crew-dev`, and local Docker volumes; it preserves the installed Crew app's
data, production keyring, and `~/.crew`. The first dev page load also clears
...
If using `mesh-dev-fresh`, the clean window opens at **Welcome to Crew**. Join
the seeded local community before continuing:

1. Click **Join a community**.
2. Use any local name, such as **Local Crew**.
...
Stop only stale Crew/MeshLLM processes shown by that command. Do not leave a
...
take several minutes. Wait for the Crew window to open and for the terminal to
...
Crew may download the model on first use. The model picker ranks models for the
...
resolves to the bundled `crew-agent`. No API key is required.
...
**crew-agent** as the runtime, **Crew shared compute** as the LLM provider,
...
- no current Crew membership snapshot is available (admission fails closed).

## Security boundary

Crew publishes member-signed discovery notes through an ordinary relay-supported
...
tokens in the note. Current Crew membership controls which node identities are
...
MeshLLM—not the Crew relay—carries inference over direct QUIC or its encrypted
...
is an upstream protocol change and is not claimed by the Crew-side checks above.
