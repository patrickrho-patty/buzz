...
**Affected distributions:** Fedora 40+ and any distro shipping Google's Noto Color Emoji in COLRv1 format (`Noto-COLRv1.ttf`). Issues [#2548](https://github.com/block/crew/issues/2548), [#2982](https://github.com/block/crew/issues/2982).
...
**Fix:** Upgrade to the latest AppImage (v0.5.2+). The build container was bumped to `ubuntu:24.04` ([#3602](https://github.com/block/crew/pull/3602)), which ships FreeType 2.13.2. The compiled layout now matches every crash-affected host (FreeType ≥ 2.13), eliminating the ABI mismatch.
...
**Affected hardware:** NVIDIA GPUs (proprietary and nouveau drivers) and AppImage installs on any GPU. Issue [#2338](https://github.com/block/crew/issues/2338).
...
A dedicated fix for RDNA4 detection is being tracked in [#2643](https://github.com/block/crew/issues/2643).
...
4. File a [new issue](https://github.com/block/crew/issues/new) with your distro, GPU, driver version, and the terminal output.
