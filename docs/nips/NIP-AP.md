...
The gate is kind-generic: the relay applies it to every kind in `SHARED_GATED_KINDS` (`crew-core/src/kind.rs`), currently `30175` and the `30178` team-catalog projection described below. The rules and enforcement surfaces are identical for each member kind.
