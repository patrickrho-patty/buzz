...
(***************************************************************************)
(* Formal model of Crew's proposed multi-tenant relay/database isolation.    *)
(*                                                                         *)
...
(*                                                                         *)
(* Source grounding from today's Crew:                                      *)
...
(*   - crates/crew-db/src/event.rs: EventQuery has channel_id/channel_ids    *)
(*     but no community_id; inserts use ON CONFLICT DO NOTHING.              *)
(*   - crates/crew-db/src/channel.rs: get_accessible_channel_ids currently   *)
(*     unions all open channels in the DB; that unscoped variant is the      *)
(*     explicit I1 mutation.                                                 *)
(*   - crates/crew-relay/src/state.rs: process-global AppState/caches today. *)
(*                                                                         *)
