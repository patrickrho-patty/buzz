...
**The manifest pointer is the sole source of truth.** In Crew, a successful
...
- **kind:30618 derived after CAS.** Emission is conditional on
  ...
  the pusher's pubkey rides in a `p` tag (crew extension; NIP-34 does not
...
exists in code at PR #726's tip (`crates/crew-relay/src/api/git/`), with the
