...
[PR #2066](https://github.com/block/crew/pull/2066) and
...
| 5 | Loop circuit breaker | `crew-acp` | backlog ([§2](#2-too-loud-runaway-reply-loop-fixed)) |
| 6 | `!cancel` unreachable from any surface | `crew-acp` + CLI | backlog ([§5](#5-backlog)) |
...
Two rules in `crates/crew-acp/src/base_prompt.md` composed into a perpetual
motion machine:
...
| 5 | Setup-mode nudge ("here's what you still need to configure") | Agent spawns but requirements check fails (e.g. missing API key) | The agent process itself (crew-acp setup-listener mode) |
...
add a mention flag to `crew messages send`. First confirm these were ever
...
- **Rate-limiting incident.** One Welcome agent produced a 42KB log of
  ...
  relay `onboarding.communities.crew.xyz`). A tight retry loop against a quota
  makes every other send in the session fail too — including the kickoff's, one
  of the §3 silent paths. Worth a separate look at crew-acp publish backoff.
