...
Welcome, and thank you for your interest in contributing! Crew is an
...
unacceptable behavior to **conduct@crew-relay.org**.
...
Crew is an agent platform, so AI-assisted PRs are welcome. No need to disclose the tools you used, but you own and must have reviewed the final code. Submissions that are clearly unreviewed may be closed with a pointer here.
...
just reset   # Wipe all dev state and recreate it; installed Crew is preserved
```

Development desktop state uses separate bundle identifiers
(`xyz.block.crew.app.dev` and per-worktree variants), a separate keyring service
(`crew-desktop-dev`), and `~/.crew-dev`. `just reset` removes those dev-only
locations and the local Docker volumes. It does not touch the installed app's
`xyz.block.crew.app` data, `crew-desktop` keyring service, or `~/.crew` nest.
...
End-to-end tests live in `crates/crew-test-client/tests/`:
...
of `crew-core` types, which are shared everywhere).
...
Crew is developed across multiple repositories. This repo (`block/crew`)
...
**External contributors:** Fork `block/crew`, open a PR, and CI runs
...
1. **Define the kind constant** in `crew-core/src/kind.rs`:
...
2. **Define the payload type** in the appropriate module in `crew-core/src/`
...
3. **Register the kind's required scope** in
   `crates/crew-relay/src/handlers/ingest.rs` inside
...
4. **Handle post-storage side effects** by adding a match arm in
   `crates/crew-relay/src/handlers/side_effects.rs` inside
   `handle_side_effects()`:
   ...
   `crates/crew-relay/src/api/` and register it in
   `crates/crew-relay/src/router.rs`.

5. **Persist to the database** — if the event needs to be queryable, add a
   handler in `crew-db/src/` (e.g., `crew-db/src/my_feature.rs`) with
...
8. **Write tests** — add a unit test for payload serialization in
   `crew-core` and an integration test in `crew-test-client` that sends
...
1. **Define the handler** in the appropriate module under
   `crates/crew-relay/src/api/`. Resolve the request tenant before any auth or
   data lookup, use NIP-98 when the endpoint accepts user credentials, and keep
   community scoping explicit.

2. **Register the route** in `crates/crew-relay/src/router.rs` using the
   narrowest path possible. Do not add new `/api/*` compatibility routes unless
   the product decision explicitly calls for one.

3. **Add database queries** in `crew-db/src/` only when the endpoint cannot be
   expressed through the existing event query paths.

4. **Handle errors** using the `api_error()`, `internal_error()`, and
   `not_found()` helpers in `crew-relay/src/api/mod.rs`. Return
   `(StatusCode, Json<Value>)` tuples.

5. **Write tests** with the `crew-test-client` harness in
   `crates/crew-test-client/tests/`, covering auth, community scoping, and the
...
Crew is licensed under the **Apache License, Version 2.0**. See
...
*Thank you for contributing to Crew. Every bug report, documentation fix,
