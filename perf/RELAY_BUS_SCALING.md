# Crew relay bus scaling harness
...
- **new/community-scoped bus:** each pod retains only the server-resolved community topics for which it has local subscribers (`crew:{community_id}:global` or `crew:{community_id}:channel:{channel_id}`).
