...
`crew` CLI on the shell's PATH. No Harbor fork or patch is required.
...
Each trial gets fresh keys and a private Crew channel. The provisioner archives
rather than deletes that channel, leaving the relay/Postgres event timeline
and the per-agent acp/agent logs (downloaded into the trial's `crew/`
artifacts) available for analysis.

### Crew-native tasks

The local [`benchmarks/crew-dataset`](../crew-dataset) suite — a sibling
directory of this harness, not a subdirectory of it — scores Crew product
...
The default condition is `crew-native-solo-luna.yaml` — one solo agent on
...
`anthropic-live.json`. Swap in `crew-native-solo-sonnet.yaml` (no
...
Replace the path with `benchmarks/crew-dataset/create-channel-invite-users`
...
`/logs/artifacts/crew-evidence.json`. The task verifier reads that post-agent
...
cause is written to the trial's `crew/crew-evidence-error.txt`.
...
the tasks whose graded Crew behavior is deliberately absent from
...
stack (`crew-benchmark` compose project — relay :3600, Postgres :5633, secrets
...
relay membership list and opens the Crew desktop app logged in as them, so
...
uv run --project benchmarks/harbor-crew-orchestra/testbed \
    benchmarks/harbor-crew-orchestra/scripts/run_leaderboard.py \
    --dataset terminal-bench/terminal-bench-2-1 \
    --attempts 5 \
    --manifest benchmarks/harbor-crew-orchestra/manifests/<TEAM>.yaml \
    --endpoint-config benchmarks/harbor-crew-orchestra/testbed/endpoints/<ENDPOINTS>.json \
...
```bash
cd benchmarks/harbor-crew-orchestra
