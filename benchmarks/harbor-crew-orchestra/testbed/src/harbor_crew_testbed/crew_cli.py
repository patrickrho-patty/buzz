"""Thin subprocess wrapper over the ``crew`` CLI — the production client path."""
...
class CrewCli:
    ...
    def run(self, *args: str) -> Any:
        """Run a crew subcommand and return its parsed JSON stdout."""
        ...
        try:
            completed = subprocess.run(
                ...
                check=False,
                env={
                    "CREW_RELAY_URL": self._relay_url,
                    "CREW_PRIVATE_KEY": self._secret_key,
                    "CREW_AUTH_TAG": self._auth_tag,
                    "PATH": _path(),
                },
            )
