# 🐝 Crew Projects — A Nostr-Native Forge

> Someone pushes a fix. Crew creates a channel for the branch. The CI agent picks up the push, runs the tests, posts results back to the channel. A co-maintainer reviews the diff inline, approves it — a signed event, cryptographic proof. Merge. The workflow runs the integration. The channel archives into a permanent record of why that code exists.
>
...
This document is the software-forge slice of the broader Crew platform. [VISION.md](VISION.md) covers the platform. [VISION_SOVEREIGN.md](VISION_SOVEREIGN.md) covers the sovereign relay story — one domain, one relay, one project. This doc zooms in on what it looks like when that relay hosts code. In multi-community Crew, the same rule is lifted one level up: a project domain or subdomain selects the community first, and repositories, workflows, approvals, Blossom artifacts, and git ref updates under that host are community-local even if an operator runs many communities on shared backend infrastructure.
...
The portable representation is a NIP-34 repo announcement (kind:30617) — standard metadata that any NIP-34 client can discover and render. Crew extends it with `crew-` prefixed tags for channel binding and visibility:
...
```json
{
  ...
  "tags": [
    ...
    ["crew-protect", "main", "push-allowed", "<alice-npub>", "<bob-npub>"],
    ["crew-protect", "main", "require-approval", "2"],
    ["crew-protect", "main", "no-force-push"]
  ]
}
```

Branch protections live in the same event — `crew-protect` tags. The relay enforces them at the git transport layer. Only npubs listed in `push-allowed` can push to protected branches. Force pushes are blocked. Merges require the specified number of signed approval events (kind:46011) before the relay accepts the push.
...
Standard NIP-34 clients see a normal repo. gitworkshop.dev renders it. ngit-cli works with it. Crew clients read the `crew-` tags and wire up the channel and project UI. One event, two audiences, no custom kind for the repo itself.
...
```json
{
  ...
  "tags": [
    ["d", "platform"],
    ["name", "Platform"],
    ["a", "30617:<alice-npub-hex>:crew"],
    ["a", "30617:<bob-npub-hex>:crew-infra"],
    ["crew-channel", "<channel-uuid>"],
    ["crew-visibility", "listed"]
  ]
}
...
When you create a branch, Crew creates a channel. The branch's patches, review comments, CI results, and merge decision all live in that channel. When the branch merges, the channel archives. The conversation becomes the permanent record of why that code exists.
...
Every contributor — human or agent — has a verifiable identity and a queryable contribution history across every project on the network. Within Crew, that history is queried through a community boundary: one community can choose to surface reputation from other communities later, but profiles, DMs, memberships, and project records are not implicitly shared across hosts.
...
Workflows live in the repo (`.crew/workflows/`) or are defined at the project level and inherited by every branch channel automatically — no per-branch configuration, no copy-pasting YAML. Workflow definitions, schedules, webhooks, runs, and approval tokens inherit the project/community selected by the host, so a webhook or cron trigger for one community cannot resolve a same-named workflow in another.
...
Bug reports are NIP-34 kind:1621 events, rendered through Crew's forum surface. Threaded comments use NIP-22 kind:1111. Labels, assignees, milestones are nostr tags. Design discussions and RFCs use the forum's long-form async surface.

NIP-34 clients can discover and interact with issues. Crew's forum gives them a home with threading, search, and agent triage.
...
| Handle | `alice@crew.dev` | `triage-bot@crew.dev` |
...
| Layer | Standard NIP Kinds | Crew Custom | Rationale |
|-------|-------------------|---------------|-----------|
...
| **Project binding** | 30617 (NIP-34) | `crew-` tags | Channel, visibility |
...
If Crew disappears tomorrow, your repos still work on gitworkshop.dev, your patches still work with ngit-cli, your identities still work on any nostr client. Centralized deployment, decentralized protocol.
...
| Project binding (kind:30617 + `crew-` tags) | 📋 Designed |
...
*Crew 🐝 — the forge where identity is the foundation.*
