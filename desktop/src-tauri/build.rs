...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=CREW_BUILD_CREW_AGENT_PROVIDER");
    println!("cargo:rerun-if-env-changed=CREW_BUILD_CREW_AGENT_MODEL");
    println!("cargo:rerun-if-env-changed=CREW_BUILD_AGENT_ENV");
    println!("cargo:rerun-if-env-changed=CREW_BUILD_RELAY_RECONNECT_CMD");
    println!("cargo:rerun-if-env-changed=CREW_BUILD_AGENT_ACCESS_OWNER_ONLY");
    println!("cargo:rerun-if-env-changed=CREW_BUILD_AUTO_CONNECT_DEFAULT_RELAY");
    println!("cargo:rustc-check-cfg=cfg(crew_updater_enabled)");
    ...
    if std::env::var("CREW_BUILD_AGENT_ACCESS_OWNER_ONLY").is_ok() {
    ...
    if let Ok(relay_http) = std::env::var("CREW_RELAY_HTTP") {
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_RELAY_HTTP={relay_http}");
    }

    if let Ok(provider) = std::env::var("CREW_BUILD_CREW_AGENT_PROVIDER") {
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_CREW_AGENT_PROVIDER={provider}");
    }

    if let Ok(model) = std::env::var("CREW_BUILD_CREW_AGENT_MODEL") {
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_CREW_AGENT_MODEL={model}");
    }
    ...
    if let Ok(raw) = std::env::var("CREW_BUILD_AGENT_ENV") {
        for (line_no, line) in raw.lines().enumerate() {
            ...
            let eq = line.find('=').unwrap_or_else(|| {
                panic!(
                    "CREW_BUILD_AGENT_ENV line {}: missing '=' separator in {:?}",
                    line_no + 1,
                    line
                )
            });
            ...
            if key.is_empty() {
                panic!(
                    "CREW_BUILD_AGENT_ENV line {}: key must not be empty in {:?}",
                    line_no + 1,
                    line
                );
            }
            // The baked env is written into every spawned agent's environment
            // LAST (see `managed_agents/runtime.rs`), after Crew sets the
            ...
            if is_reserved_env_key(key) {
                panic!(
                    "CREW_BUILD_AGENT_ENV line {}: `{}` is reserved by Crew and cannot be baked \
                     into a build (it would override Crew's own identity/access env)",
                    line_no + 1,
                    key
                );
            }
        ...
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_AGENT_ENV={encoded}");
    }

    if let Ok(val) = std::env::var("CREW_BUILD_RELAY_RECONNECT_CMD") {
        let parsed: serde_json::Value = serde_json::from_str(&val)
            .unwrap_or_else(|e| panic!("CREW_BUILD_RELAY_RECONNECT_CMD is not valid JSON: {e}"));
        serde_json::from_value::<ReconnectHookConfig>(parsed).unwrap_or_else(|e| {
            panic!("CREW_BUILD_RELAY_RECONNECT_CMD doesn't match ReconnectHookConfig: {e}")
        });
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_RELAY_RECONNECT_CMD={val}");
    }
    ...
    if std::env::var("CREW_BUILD_AUTO_CONNECT_DEFAULT_RELAY").is_ok() {
        println!("cargo:rustc-env=CREW_DESKTOP_BUILD_AUTO_CONNECT_DEFAULT_RELAY=1");
    }

    let updater_public_key = std::env::var("CREW_UPDATER_PUBLIC_KEY")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());
    let updater_endpoint = std::env::var("CREW_UPDATER_ENDPOINT")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    if updater_public_key.is_some() && updater_endpoint.is_some() {
        println!("cargo:rustc-cfg=crew_updater_enabled");
    }
