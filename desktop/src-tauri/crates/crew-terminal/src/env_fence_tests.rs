...
/// Secrets Crew's own process holds. Sourced from the desktop crate's
...
/// `PATH` is constructed, not inherited, so Crew's Hermit build toolchain
...
fn child_path_is_free_of_buzz_toolchain() {
    ...
    // Stand in for Hermit activation: Crew's own PATH leads with a directory
    ...
    assert!(
        ...
        "a Crew-only executable was reachable from the child shell: {resolved}"
    );
}
...
fn resolve_shell_rejects_a_file_the_user_cannot_execute() {
    ...
    let path = fixture_file("crew-terminal-group-only-exec", "#!/bin/sh\ntrue\n", 0o010);
...
fn resolve_shell_rejects_a_non_executable_file() {
    let path = fixture_file("crew-terminal-not-a-shell", "not a shell", 0o644);
    assert_ne!(resolve_shell(path.to_str()), path.to_str().unwrap());
}
...
/// The child must be told the shell we actually spawned, not the one Crew
