...
/// `CREW_CHANNEL_ID` is always the UUID, so a script has an identifier that no
...
fn channel_id_is_never_the_name() {
    ...
    let id = vars.iter().find(|(k, _)| *k == "CREW_CHANNEL_ID").unwrap();
    assert_eq!(
        id.1, UUID,
...
fn well_formed_key_rejects_the_equals_bypass() {
    for good in ["CREW_CHANNEL", "_UNDERSCORE", "A1"] {
