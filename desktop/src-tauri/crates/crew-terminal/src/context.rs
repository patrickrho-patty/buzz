...
//! shells interpolate into prompts. A `PS1` containing `$CREW_CHANNEL` turns a
...
/// Returns true if `name` is safe to expose as `CREW_CHANNEL`.
///
...
/// The value to expose as `CREW_CHANNEL`: the name when it is safe, otherwise
/// the channel UUID.
...
/// first `=`. A key `CREW_CHANNEL=x` with value `y` lands as
/// `CREW_CHANNEL=x=y`, so `getenv("CREW_CHANNEL")` returns `"x=y"` — a way to
...
///
/// `CREW_CHANNEL` carries the validated display value; `CREW_CHANNEL_ID` is
...
pub fn context_vars(context: &GuiContext) -> Vec<(&'static str, String)> {
    let mut vars = vec![
        ("CREW_CHANNEL_ID", context.channel_id.clone()),
        ("CREW_CHANNEL", channel_display(context).to_owned()),
        ("CREW_NPUB", context.npub.clone()),
        ("CREW_RELAY_URL", context.relay_url.clone()),
        ("CREW_TERM_SESSION", context.session_id.clone()),
        ("CREW_TERM_VERSION", env!("CARGO_PKG_VERSION").to_owned()),
    ];
    // Absent rather than empty when the user is not in a thread: `-n
    // "$CREW_THREAD_ID"` and `${CREW_THREAD_ID+set}` should agree.
    if let Some(thread_id) = &context.thread_id {
        vars.push(("CREW_THREAD_ID", thread_id.clone()));
    }
    vars
}
