...
use crew_terminal::damage::Encoder;
use crew_terminal::fences::Fences;
use crew_terminal::{Action, SharedTerminal, Size, Terminal};
...
fn visible_text(frame: &crew_terminal::damage::Frame) -> Vec<String> {
    frame
        .rows
        .iter()
        .map(|row| {
