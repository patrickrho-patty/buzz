...
fn reconcile_databricks_v1_to_v2_rewrites_v1_provider_on_block_build() {
    ...
    // CREW_AGENT_PROVIDER=databricks_v2). The structured provider field
    ...
    // shadowed by CREW_AGENT_MODEL at spawn time (last-write-wins in Command::env).
    assert!(
...
fn reconcile_databricks_v1_to_v2_preserves_v1_provider_on_oss_build() {
    ...
    write_agents_json(
        ...
        &serde_json::json!([{
            ...
            "provider": "databricks",
            "model": "dbrx-instruct",
            "env_vars": { "CREW_AGENT_PROVIDER": "databricks" }
        }]),
    );
    ...
    assert!(
        records[0]["env_vars"].get("CREW_AGENT_PROVIDER").is_none(),
        "CREW_AGENT_PROVIDER must be stripped even when provider rewrite is disabled"
    );
}
...
fn reconcile_databricks_v1_to_v2_clears_model_on_provider_rewrite() {
    ...
    // via CREW_AGENT_MODEL at spawn time would shadow the baked DATABRICKS_MODEL
...
#[test]
fn reconcile_databricks_v1_to_v2_strips_stale_crew_agent_provider_from_env_vars() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
    ...
    assert!(
        records[0]["env_vars"].get("CREW_AGENT_PROVIDER").is_none(),
        "CREW_AGENT_PROVIDER must be stripped from env_vars"
    );
...
fn reconcile_databricks_v1_to_v2_strips_all_derived_keys_from_env_vars() {
    ...
    assert!(env_vars.get("CREW_AGENT_PROVIDER").is_none());
    assert!(env_vars.get("CREW_AGENT_MODEL").is_none());
...
fn reconcile_databricks_v1_to_v2_handles_multiple_records_block_build() {
    ...
    write_agents_json(
        ...
        &serde_json::json!([
            ...
            },
            {
                "name": "Agent B",
                "provider": "anthropic",
                "env_vars": { "CREW_AGENT_MODEL": "claude-3-5-sonnet" }
            },
            {
                "name": "Agent C",
                "provider": "databricks_v2",
    ...
    assert_eq!(records[0]["provider"], "databricks_v2");
    assert!(records[0]["env_vars"].get("CREW_AGENT_PROVIDER").is_none());
    // B: provider untouched, stale CREW_AGENT_MODEL stripped.
    assert_eq!(records[1]["provider"], "anthropic");
    assert!(records[1]["env_vars"].get("CREW_AGENT_MODEL").is_none());
...
fn reconcile_databricks_v1_to_v2_strips_derived_keys_from_keyless_persona_definition() {
    ...
    write_agents_json(
        ...
        &serde_json::json!([{
            ...
            "env_vars": {
                "CREW_AGENT_PROVIDER": "databricks",
                "CREW_AGENT_MODEL": "goose-claude-4-6-sonnet",
                "DATABRICKS_HOST": "https://dbc.example.com"
            }
        }]),
    );
    ...
    assert!(
        env_vars.get("CREW_AGENT_PROVIDER").is_none(),
        "CREW_AGENT_PROVIDER must be stripped from keyless persona definition env_vars"
    );
    assert!(
        env_vars.get("CREW_AGENT_MODEL").is_none(),
        "CREW_AGENT_MODEL must be stripped from keyless persona definition env_vars"
    );
