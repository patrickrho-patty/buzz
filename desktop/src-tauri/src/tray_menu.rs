...
pub(crate) fn show_main_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    if let Err(error) = window.unminimize() {
        eprintln!("griddle-desktop: failed to restore main window from tray: {error}");
        return;
    }
    if let Err(error) = window.show() {
        eprintln!("griddle-desktop: failed to show main window from tray: {error}");
        return;
    }
    if let Err(error) = window.set_focus() {
        eprintln!("griddle-desktop: failed to focus main window from tray: {error}");
    }
}
...
fn queue_tray_action<R: Runtime>(app: &AppHandle<R>, mut action: TrayAction) {
    ...
    let Ok(mut queue) = state.action_queue.lock() else {
        eprintln!("griddle-desktop: tray action queue is unavailable");
        return;
    };
    ...
    if let Err(error) = app.emit("tray-action-available", ()) {
        eprintln!("griddle-desktop: failed to notify frontend of tray action: {error}");
    }
}
...
pub fn init<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    if let Err(error) = apply_activity_presentation(&tray, activities, recent_activities) {
        eprintln!("griddle-desktop: failed to apply tray menu presentation: {error}");
    }
    mouse_nav::init(app);
    Ok(())
}
