...
//! projection here so the native menu can remain useful while Crew is hidden.
...
/// Builds the standalone Crew bee as a transparent, macOS template image.
///
...
pub(crate) fn show_main_window<R: Runtime>(app: &AppHandle<R>) {
    ...
    if let Err(error) = window.unminimize() {
        eprintln!("crew-desktop: failed to restore main window from tray: {error}");
        return;
    }
    if let Err(error) = window.show() {
        eprintln!("crew-desktop: failed to show main window from tray: {error}");
        return;
    }
    if let Err(error) = window.set_focus() {
        eprintln!("crew-desktop: failed to focus main window from tray: {error}");
    }
}
...
fn queue_tray_action<R: Runtime>(app: &AppHandle<R>, mut action: TrayAction) {
    ...
    let Ok(mut queue) = state.action_queue.lock() else {
        eprintln!("crew-desktop: tray action queue is unavailable");
        return;
    };
    ...
    if let Err(error) = app.emit("tray-action-available", ()) {
        eprintln!("crew-desktop: failed to notify frontend of tray action: {error}");
    }
}
...
/// Installs the persistent Crew tray icon with the initial empty activity menu.
pub fn init<R: Runtime>(app: &AppHandle<R>) -> tauri::Result<()> {
    ...
    if let Err(error) = apply_activity_presentation(&tray, activities, recent_activities) {
        eprintln!("crew-desktop: failed to apply tray menu presentation: {error}");
    }
    mouse_nav::init(app);
    Ok(())
}
...
pub fn take_tray_actions<R: Runtime>(app: AppHandle<R>) -> Result<Vec<TrayAction>, String> {
    ...
    let mut queue = state
        .action_queue
        .lock()
        .map_err(|_| "Crew tray action queue is unavailable".to_string())?;
...
pub fn update_tray_agent_activity<R: Runtime>(
    ...
    let mut activity_items = state
        .activity_items
        .lock()
        .map_err(|_| "Crew tray menu state is unavailable".to_string())?;
            ...
            })
    {
        ...
        let tray = app
            .tray_by_id(TRAY_ID)
            .ok_or_else(|| "Crew tray icon is not available".to_string())?;
