...
/// the binary at compile time via `CREW_BUILD_AGENT_ENV`. The backend readiness
...
///
/// Provider and model arrive as `CREW_AGENT_PROVIDER` / `CREW_AGENT_MODEL`
...
/// record; at spawn `runtime.rs` injects it as `CREW_ACP_EFFORT_LEVEL` and the
