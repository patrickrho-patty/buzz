...
pub async fn decrypt_observer_event(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        crew_core_pkg::observer::decrypt_observer_payload(&keys, &event)
...
pub fn build_observer_control_event(
    ...
    let encrypted =
        crew_core_pkg::observer::encrypt_observer_payload(&keys, &agent_pubkey, &payload)
            .map_err(|error| format!("encrypt observer control failed: {error}"))?;
    let builder = crew_sdk_pkg::build_agent_observer_frame(
        &agent_pubkey_hex,
        &agent_pubkey_hex,
        crew_core_pkg::observer::OBSERVER_FRAME_CONTROL,
        &encrypted,
    )
