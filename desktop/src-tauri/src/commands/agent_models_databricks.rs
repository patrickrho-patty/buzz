...
fn databricks_agent_provider(provider: &str) -> crew_agent_pkg::config::Provider {
    if provider.trim().eq_ignore_ascii_case("databricks_v2")
        || provider.trim().eq_ignore_ascii_case("databricks-v2")
    {
        crew_agent_pkg::config::Provider::DatabricksV2
    } else {
        crew_agent_pkg::config::Provider::Databricks
    }
}
...
pub(super) async fn discover_databricks_models(
    ...
    let config = crew_agent_pkg::config::Config::for_discovery(
    ...
    let entries = match crew_agent_pkg::discover_databricks_models(&config).await {
        Ok(entries) => entries,
        Err(crew_agent_pkg::AgentError::LlmAuth(_)) if should_start_interactive_auth(&api_key) => {
            let _auth = AUTH_GATE.lock().await;
            match crew_agent_pkg::discover_databricks_models(&config).await {
                // A peer sign-in under the gate already succeeded.
                Ok(entries) => entries,
                Err(crew_agent_pkg::AgentError::LlmAuth(_)) => {
                    ...
                    run_interactive_databricks_auth(
                        crew_agent_pkg::authenticate_databricks(&host),
                        AUTH_FLOW_TIMEOUT,
                        &AUTH_COOLDOWNS,
                        &host,
                        &redaction_env,
                    ...
                    .await?;
                    crew_agent_pkg::discover_databricks_models(&config)
                        .await
                        .map_err(|error| {
        ...
        Err(crew_agent_pkg::AgentError::LlmAuth(error)) if !api_key.is_empty() => {
...
where
    Fut: std::future::Future<Output = Result<(), crew_agent_pkg::AgentError>>,
{
