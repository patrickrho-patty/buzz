...
pub(super) fn run(
    ...
    // Observe PATH changes and binaries added after Crew launched.
...
fn failure(
    ...
    Some(InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "The installer finished, but Crew still could not use {runtime_id} (observed: {observed})."
        ),
        exit_code: None,
        hint: Some(
            "Crew requires the vendor CLI executable, not only its desktop app. If the CLI was installed while Crew was open, restart Crew and check again."
                .to_string(),
        ),
    })
}
