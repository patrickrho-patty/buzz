...
fn managed_node_unsupported_step() -> InstallStepResult {
    InstallStepResult {
        ...
        stdout: String::new(),
        stderr: format!(
            "Crew does not provide a managed Node.js runtime for {}-{} yet",
            std::env::consts::OS,
            std::env::consts::ARCH
        ),
        exit_code: None,
        hint: Some(
            "Install Node.js from https://nodejs.org, restart Crew, then click Install again."
                .to_string(),
        ),
    }
}
...
fn managed_node_install_hint() -> String {
    "Crew could not install its private Node.js runtime. Check your network and app-data directory permissions, then click Install again.".to_string()
}
...
///
/// Only a Crew-managed shim (path under `managed_prefix`) with an orphaned
...
pub(super) fn ensure_managed_node_runtime_blocking() -> Result<(), Box<InstallStepResult>> {
    ...
    let Some(root) = crate::managed_agents::crew_managed_node_root() else {
        return Err(Box::new(managed_node_failed_step(
            "failed to resolve Crew app-data directory for private Node.js runtime".to_string(),
        )));
    };
...
/// Guidance text shown when the Crew-private npm prefix is not available.
fn managed_npm_prefix_hint() -> String {
    "Crew could not create its private Node tools directory. Check app-data directory permissions, restart Crew, then click Install again.".to_string()
}
...
pub(super) fn managed_npm_command(command: &str) -> Result<Option<String>, Box<InstallStepResult>> {
    ...
    let Some(prefix) = crate::managed_agents::crew_managed_npm_prefix() else {
        return Err(Box::new(InstallStepResult {
            ...
            success: false,
            stdout: String::new(),
            stderr: "failed to resolve Crew app-data directory for private npm prefix".to_string(),
            exit_code: None,
    ...
    if let Err(error) = std::fs::create_dir_all(&prefix) {
        return Err(Box::new(InstallStepResult {
            ...
            stdout: String::new(),
            stderr: format!(
                "failed to create Crew private npm prefix '{}': {error}",
                prefix.display()
            ),
            exit_code: None,
            hint: Some(managed_npm_prefix_hint()),
...
pub(super) fn npm_eacces_hint(stderr: &str, _command: &str) -> Option<String> {
    if stderr.contains("EACCES: permission denied") || stderr.contains("npm error EACCES") {
        Some(
            "npm could not write to Crew's private Node tools directory. Check app-data directory permissions, restart Crew, then click Install again."
                .to_string(),
        )
    } else {
        None
