...
#[test]
fn crew_mesh_join_uses_the_same_live_member_from_every_other_node() {
    let targets = vec![
    ...
    assert_eq!(
        crew_mesh_join_targets(targets.clone(), "member-b")
            .into_iter()
            .next()
    ...
    assert_eq!(
        crew_mesh_join_targets(targets, "member-c")
            .into_iter()
            .next()
            .map(|target| target.endpoint_addr),
        Some("addr-a".to_string())
...
#[test]
fn crew_mesh_bootstrap_member_does_not_dial_itself() {
    let targets = vec![
        reported_target("member-b", "model-b", "addr-b"),
        reported_target("member-a", "model-a", "addr-a"),
    ];
    ...
    assert_eq!(
        crew_mesh_join_targets(targets, "MEMBER-A")
            .into_iter()
            .next(),
...
#[test]
fn crew_mesh_join_ignores_targets_without_a_validated_reporter() {
    let targets = vec![
...
#[test]
fn crew_mesh_join_keeps_other_device_with_the_same_member_key() {
    ...
    assert_eq!(
        crew_mesh_join_targets(vec![self_target, other_device], "owner-self")
            .into_iter()
            .next()
            .map(|target| target.endpoint_addr),
...
#[test]
fn crew_mesh_name_is_stable_and_does_not_expose_the_relay() {
    let first = crew_mesh_name_for_relay("WSS://EXAMPLE.COM/");
    let second = crew_mesh_name_for_relay("wss://example.com:443/some/path?ignored=yes");
    let other_relay = crew_mesh_name_for_relay("wss://other.example.com");
