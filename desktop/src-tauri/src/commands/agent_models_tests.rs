...
fn saved_agent_model_discovery_uses_record_snapshot_for_definition_less_agent() {
    let record: crate::managed_agents::ManagedAgentRecord = serde_json::from_str(
        r#"{
            ...
            "env_vars": {
                ...
                "CREW_PRIVATE_KEY": "must-not-leak"
            },
    ...
    assert!(!discovery.env.contains_key("CREW_PRIVATE_KEY"));
...
fn effective_discovery_provider_prefers_the_explicit_provider() {
    ...
    assert_eq!(
        effective_discovery_provider(Some("anthropic"), Some("CREW_AGENT_PROVIDER"), &env)
            .as_deref(),
        Some("anthropic")
    );
}
...
fn effective_discovery_provider_recovers_baked_provider_when_record_has_none() {
    ...
    for provider in [None, Some(""), Some("   ")] {
        assert_eq!(
            effective_discovery_provider(provider, Some("CREW_AGENT_PROVIDER"), &env).as_deref(),
            Some("databricks_v2"),
...
/// what the developer happens to have exported (e.g. `CREW_AGENT_PROVIDER`).
const UNSET_PROVIDER_VAR: &str = "CREW_TEST_UNSET_DISCOVERY_PROVIDER";
...
const UNSET_CREDENTIAL: &str = "CREW_TEST_UNSET_DISCOVERY_CREDENTIAL";
...
fn effective_discovery_provider_reads_the_runtimes_own_env_var() {
    // goose keys its provider off GOOSE_PROVIDER, so a CREW_AGENT_PROVIDER in
...
fn update_request_turn_timeout_parses_for_wire_compat() {
    ...
    // (CREW_ACP_TURN_TIMEOUT is deprecated and ignored by the harness). That
...
fn openrouter_draft_agent_model_discovery_derives_provider_env() {
    ...
    assert_eq!(
        merged.get("CREW_AGENT_PROVIDER").map(String::as_str),
        Some("openrouter"),
...
fn draft_agent_model_discovery_env_omits_provider_when_absent() {
    ...
    assert!(
        !merged.contains_key("CREW_AGENT_PROVIDER"),
...
/// `CREW_PRIVATE_KEY` proves a reserved key cannot ride in on a harness
...
fn draft_agent_model_discovery_env_layers_all_three_tiers_in_order() {
    ...
    let definition_env = BTreeMap::from([
        ...
        ("CREW_PRIVATE_KEY".to_string(), "must-not-leak".to_string()),
    ]);
    ...
    // Tier 1 (floor): `Some("openrouter")` derives CREW_AGENT_PROVIDER.
    let merged = draft_agent_model_discovery_env(
        "crew-agent",
        Some("openrouter"),
        &definition_env,
    ...
    let expected: &[(&str, Option<&str>)] = &[
        ...
        ("CREW_AGENT_PROVIDER", Some("openai")),
        // Tier 3 over tier 2.
