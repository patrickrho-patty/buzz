...
fn transcode_to_mp4_with_cancellation(
    ...
    let output = std::env::temp_dir().join(format!("crew-transcode-{}.mp4", uuid::Uuid::new_v4()));
...
fn transcode_heic_to_jpeg(
    ...
    // UUID-based temp path — unique across concurrent uploads.
    let output = std::env::temp_dir().join(format!("crew-heic-{}.jpg", uuid::Uuid::new_v4()));
...
fn extract_poster_frame_with_cancellation(
    ...
    let output = std::env::temp_dir().join(format!("crew-poster-{}.jpg", uuid::Uuid::new_v4()));
        ...
        || std::fs::metadata(&output).map_or(true, |m| m.len() == 0)
    {
        if !result.status.success() {
            let stderr = String::from_utf8_lossy(&result.stderr);
            eprintln!("crew-desktop: poster seek-to-1s failed, trying first frame: {stderr}");
        }
        let _ = std::fs::remove_file(&output);
        ...
        if !fallback.status.success() || !output.exists() {
            ...
            eprintln!("crew-desktop: poster frame extraction failed: {stderr}");
            let _ = std::fs::remove_file(&output);
...
pub(super) fn transcode_and_extract_poster_with_cancellation(
    ...
    let poster_bytes =
        match extract_poster_frame_with_cancellation(&transcoded, &ffmpeg_path, cancellation) {
            ...
            Err(e) => {
                eprintln!("crew-desktop: poster extraction failed (non-fatal): {e}");
                None
            }
        };
...
mod tests {
    ...
    fn test_transcode_to_mp4_drops_source_metadata() {
        ...
        let source =
            std::env::temp_dir().join(format!("crew-metadata-test-{}.mp4", uuid::Uuid::new_v4()));
    ...
    fn test_transcode_heic_round_trip() {
        ...
        let heic_path =
            std::env::temp_dir().join(format!("crew-test-{}.heic", uuid::Uuid::new_v4()));
