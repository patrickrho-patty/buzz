...
/// Query available models from an agent via `crew-acp models --json`.
///
...
pub async fn discover_agent_models(
    ...
    // Crew shared compute discovery must not depend on the local OpenAI ingress: that
        ...
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        ...
        .await
        .map_err(|error| format!("Crew shared compute model discovery failed: {error}"))?;
        ...
        if availability.models.is_empty() {
            return Err(availability.reason.unwrap_or_else(|| {
                "No live Crew shared compute models are available".to_string()
            }));
        }
    ...
    if input.provider.as_deref().map(str::trim)
        == Some(crate::managed_agents::RELAY_MESH_PROVIDER_ID)
    {
        return Err("Crew shared compute is not available in this build".to_string());
    }
...
/// Normalize raw `crew-acp models --json` output into a typed DTO for the frontend.
///
