//! Startup readiness for Crew shared compute.
//!
...
fn mesh_readiness_failure_message(
    ...
    match failure {
        MeshReadinessFailure::CatalogNeverSynced => format!(
            "Crew shared compute connected to the serving member but could not sync \
        ...
        MeshReadinessFailure::RoutingNeverCompleted => format!(
            "Crew shared compute found \"{model_id}\" on a serving member but inference \
