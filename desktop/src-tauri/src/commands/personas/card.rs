...
/// The Crew card frame template — Tyler's gold-honeycomb base. Generation
...
pub fn list_agent_cards(app: AppHandle) -> Result<Vec<ArchivedCardMeta>, String> {
    ...
    for entry in entries.flatten() {
        ...
        let Ok(meta) = serde_json::from_str::<ArchivedCardMeta>(&content) else {
            ...
            eprintln!(
                "crew-desktop: card-archive: skipping malformed sidecar {}",
                path.display()
            );
            continue;
        };
...
pub(crate) fn build_card_instructions(
    ...
    format!(
        r#"You are designing one premium collectible trading card for the Crew agent "{agent_name}".

Input image 1 is the official Crew card frame template (gold honeycomb border, dark interior, name banner top, hex badge top-right, text box lower third). Input image 2 is the agent's avatar — study its exact art style: medium, pixel grid if any, palette, shading, background motifs.
...
pub async fn mint_agent_card(
    ...
    if let Err(e) = archive_minted_card(&app, &id, &display_name, &minted, &final_bytes) {
        eprintln!("crew-desktop: card-archive: failed to archive minted card: {e}");
    }
