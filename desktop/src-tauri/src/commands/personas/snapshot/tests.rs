...
fn import_wrong_format_string_fails_closed() {
    ...
    snapshot.format = "not-crew-agent-snapshot".to_string();
    let bytes = encode_snapshot_json(&snapshot).unwrap();
