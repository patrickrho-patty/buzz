...
//!
//! When the build-time env var `CREW_BUILD_RELAY_RECONNECT_CMD` is set (internal
...
pub fn relay_reconnect_hook_configured() -> bool {
    option_env!("CREW_DESKTOP_BUILD_RELAY_RECONNECT_CMD").is_some()
}

#[tauri::command]
pub async fn relay_reconnect_hook() -> Result<(), String> {
    let Some(config_str) = option_env!("CREW_DESKTOP_BUILD_RELAY_RECONNECT_CMD") else {
