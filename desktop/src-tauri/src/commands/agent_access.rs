...
/// probe in the product crate makes it impossible for crew-releases to validate
...
mod tests {
    #[test]
    #[ignore = "requires CREW_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY"]
    fn compiled_policy_matches_expected() {
        let expected = std::env::var("CREW_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY")
            .expect("CREW_TEST_EXPECTED_AGENT_ACCESS_OWNER_ONLY must be set")
            .parse::<bool>()
