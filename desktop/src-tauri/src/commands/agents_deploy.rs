...
pub(super) struct DeployProjections {
    ...
    /// as `launch.policy_env["CREW_ACP_AGENTS"]`.
...
pub(super) fn build_launch_block(
    ...
    policy_env.insert("CREW_ACP_RELAY_OBSERVER".into(), "true".into());
    policy_env.insert("CREW_ACP_LAZY_POOL".into(), "true".into());
    policy_env.insert(
        "CREW_ACP_AGENTS".into(),
        crate::managed_agents::acp_agents_value(&descriptor.command, record.parallelism),
    );

    if let Some(value) = effective_prompt {
        policy_env.insert("CREW_ACP_SYSTEM_PROMPT".into(), value.to_string());
    }
    if let Some(value) = effective_model {
        ...
        // first and skips the CREW_ACP_MODEL catalog-switch path that would
        // introduce a second startup authority). All other runtimes use
        // CREW_ACP_MODEL, which the harness reads into desired_model at spawn.
    ...
    // path in runtime.rs. The harness reads CREW_ACP_EFFORT_LEVEL into
    ...
    if let Some(ref value) = record.effort_level {
        policy_env.insert("CREW_ACP_EFFORT_LEVEL".into(), value.clone());
    }
    if let Some(value) = record.idle_timeout_seconds {
        policy_env.insert("CREW_ACP_IDLE_TIMEOUT".into(), value.to_string());
    }
    if let Some(value) = record.max_turn_duration_seconds {
        policy_env.insert("CREW_ACP_MAX_TURN_DURATION".into(), value.to_string());
    }
    ...
    {
        policy_env.insert("CREW_ACP_TEAM_INSTRUCTIONS".into(), value);
    }

    // B5 remote parity: when a canonical effort_level is persisted, strip
    // CREW_ACP_EFFORT_LEVEL from launch.env so it cannot shadow the canonical
    ...
    // launch, ALWAYS strip BOTH CREW_ACP_MODEL and ANTHROPIC_MODEL from
    ...
    // launch.env, a user CREW_ACP_MODEL would introduce a second startup
    ...
    let is_claude = runtime.map(|r| r.id == "claude").unwrap_or(false);
    let strip_key = |k: &str| {
        (record.effort_level.is_some() && k.eq_ignore_ascii_case("CREW_ACP_EFFORT_LEVEL"))
            || (is_claude
                && (k.eq_ignore_ascii_case("CREW_ACP_MODEL")
...
/// descriptor as `launch.policy_env["CREW_ACP_AGENTS"]`. Access is projected from
...
pub(super) fn deploy_payload_json(
    ...
    serde_json::json!({
        ...
        // launch.policy_env["CREW_ACP_AGENTS"] — the two are always consistent.
...
mod tests {
    ...
    fn launch_block_preserves_descriptor_and_spawn_policy() {
        ...
        assert_eq!(launch["policy_env"]["CREW_ACP_LAZY_POOL"], "true");
        assert_eq!(launch["policy_env"]["CREW_ACP_RELAY_OBSERVER"], "true");
        assert_eq!(
            launch["policy_env"]["CREW_ACP_TEAM_INSTRUCTIONS"],
            "Coordinate"
        );
        assert_eq!(launch["policy_env"]["CREW_ACP_SESSION_TITLE"], "Agent Name");
        assert_eq!(launch["policy_env"]["CREW_ACP_DISPLAY_NAME"], "Agent Name");
        assert_eq!(launch["policy_env"]["CREW_ACP_SYSTEM_PROMPT"], "prompt");
        // goose runtime: model goes via CREW_ACP_MODEL (non-claude path).
        assert_eq!(launch["policy_env"]["CREW_ACP_MODEL"], "model");
        assert!(
            launch["policy_env"]["ANTHROPIC_MODEL"].is_null(),
            "goose must NOT receive ANTHROPIC_MODEL"
        );
        assert_eq!(launch["policy_env"]["CREW_ACP_IDLE_TIMEOUT"], "17");
        assert_eq!(launch["policy_env"]["CREW_ACP_MAX_TURN_DURATION"], "23");
        assert_eq!(launch["policy_env"]["CREW_ACP_AGENTS"], "4");
    ...
    fn launch_block_claude_runtime_uses_anthropic_model_not_crew_acp_model() {
        ...
        assert!(
            launch["policy_env"]["CREW_ACP_MODEL"].is_null(),
            "claude remote must NOT receive CREW_ACP_MODEL"
        );
    }

    /// F2: remote Claude launch must mirror local A1 — ALWAYS strip BOTH
    /// CREW_ACP_MODEL and ANTHROPIC_MODEL from launch.env (tier 2), so the
    /// canonical model in policy_env (tier 1) is the sole authority. Since
    /// launch.env later-wins over policy_env, a user CREW_ACP_MODEL would add a
    ...
    fn launch_block_claude_strips_both_model_keys_from_launch_env() {
        ...
        let descriptor = EffectiveHarnessDescriptor {
            ...
            args: vec![],
            env: BTreeMap::from([
                ("CREW_ACP_MODEL".to_string(), "user-sonnet".to_string()),
        ...
        assert!(launch["policy_env"]["CREW_ACP_MODEL"].is_null());
        // Both model keys are stripped from launch.env — neither can later-win.
        assert!(
            launch["env"]["CREW_ACP_MODEL"].is_null(),
            "user CREW_ACP_MODEL must be stripped from launch.env for claude"
        );
        assert!(
    ...
    fn launch_block_claude_strips_model_keys_even_without_canonical() {
        ...
        assert!(
            launch["env"]["CREW_ACP_MODEL"].is_null(),
            "user CREW_ACP_MODEL must be stripped even without a canonical model"
        );
        assert!(
            launch["env"]["ANTHROPIC_MODEL"].is_null(),
    ...
    /// model authority stripping is Claude-specific (CREW_ACP_MODEL is the
    ...
    fn launch_block_non_claude_preserves_user_model_env() {
        ...
        let descriptor = EffectiveHarnessDescriptor {
            command: "goose".into(),
            args: vec![],
            env: BTreeMap::from([("CREW_ACP_MODEL".to_string(), "user-model".to_string())]),
        };
        let launch =
        ...
        assert_eq!(launch["policy_env"]["CREW_ACP_MODEL"], "model");
        assert_eq!(launch["env"]["CREW_ACP_MODEL"], "user-model");
    }

    #[test]
    fn launch_block_claude_runtime_injects_effort_level_when_set() {
        // I-4: remote parity — record.effort_level → CREW_ACP_EFFORT_LEVEL in policy_env.
        ...
        let launch = build_launch_block(&record, &descriptor, &[], None, None, "owner-hex");
        assert_eq!(
            launch["policy_env"]["CREW_ACP_EFFORT_LEVEL"], "high",
            "claude remote must receive CREW_ACP_EFFORT_LEVEL when effort_level is set"
        );
    }

    #[test]
    fn launch_block_does_not_inject_effort_level_when_absent() {
        // I-4: no CREW_ACP_EFFORT_LEVEL in policy_env when record.effort_level is None.
        ...
        let launch = build_launch_block(&record, &descriptor, &[], None, None, "owner-hex");
        assert!(
            launch["policy_env"]["CREW_ACP_EFFORT_LEVEL"].is_null(),
            "policy_env must NOT contain CREW_ACP_EFFORT_LEVEL when effort_level is None"
        );
    }

    /// B5 remote parity: when a canonical effort_level is persisted, a conflicting
    /// user-supplied CREW_ACP_EFFORT_LEVEL in descriptor.env must NOT shadow it.
    ...
    fn launch_block_canonical_effort_strips_user_env_collision() {
        ...
        let descriptor = EffectiveHarnessDescriptor {
            ...
            env: BTreeMap::from([("CREW_ACP_EFFORT_LEVEL".to_string(), "low".to_string())]),
        };
        let launch = build_launch_block(&record, &descriptor, &[], None, None, "owner-hex");
        ...
        assert!(
            launch["env"]["CREW_ACP_EFFORT_LEVEL"].is_null(),
            "user CREW_ACP_EFFORT_LEVEL must be stripped from launch.env when canonical is present"
        );
    }

    /// B5 remote parity: when no canonical effort is persisted (effort_level is
    /// None), a user-supplied CREW_ACP_EFFORT_LEVEL in descriptor.env survives
    ...
    fn launch_block_user_effort_env_survives_when_no_canonical_value() {
        ...
        assert_eq!(
            launch["env"]["CREW_ACP_EFFORT_LEVEL"], "low",
            "user-supplied effort must survive in launch.env when no canonical value"
        );
    }

    /// OpenClaw descriptor: `launch.policy_env["CREW_ACP_AGENTS"]` must be "5"
    ...
    fn launch_block_openclaw_over_cap_policy_env_is_capped() {
        ...
        assert_eq!(
            launch["policy_env"]["CREW_ACP_AGENTS"],
            crate::managed_agents::parallelism::OPENCLAW_MAX_PARALLELISM.to_string(),
            "launch.policy_env[CREW_ACP_AGENTS] must be capped at {} for OpenClaw, not 10",
            crate::managed_agents::parallelism::OPENCLAW_MAX_PARALLELISM
        );
    }

    /// Uncapped harness (goose): `launch.policy_env["CREW_ACP_AGENTS"]` passes
    ...
    fn launch_block_goose_policy_env_is_not_capped() {
        ...
        assert_eq!(
            launch["policy_env"]["CREW_ACP_AGENTS"], "8",
            "goose: policy_env[CREW_ACP_AGENTS] must pass through requested value 8"
        );
    }
    ...
    /// OpenClaw. Both `launch.policy_env["CREW_ACP_AGENTS"]` and the legacy
    ...
    fn deploy_payload_json_stale_goose_record_live_openclaw_descriptor_both_capped() {
        ...
        assert_eq!(
            launch["policy_env"]["CREW_ACP_AGENTS"],
            cap.to_string(),
            "launch.policy_env[CREW_ACP_AGENTS] must be capped at {cap} for live OpenClaw descriptor"
        );
        assert_eq!(
    ...
    fn deploy_payload_json_stale_openclaw_record_live_goose_descriptor_both_uncapped() {
        ...
        assert_eq!(
            launch["policy_env"]["CREW_ACP_AGENTS"],
            "4",
            "launch.policy_env[CREW_ACP_AGENTS] must pass through requested 4 for live Goose descriptor"
        );
        assert_eq!(
    ...
    fn deploy_payload_json_explicit_openclaw_override_both_capped() {
        ...
        assert_eq!(
            launch["policy_env"]["CREW_ACP_AGENTS"],
            cap.to_string(),
            "launch.policy_env[CREW_ACP_AGENTS] must be {cap} for explicit OpenClaw override"
        );
        assert_eq!(
            payload["parallelism"], cap,
            "legacy top-level parallelism must match launch.policy_env — both must be {cap}"
