...
/// Read the workspace owner pubkey without holding the lock. Used to populate `CREW_ACP_AGENT_OWNER`
...
pub async fn create_managed_agent(
    ...
    // The harness has its own validator (crew-acp/src/config.rs) but we want
    ...
    let auth_tag = {
        ...
        // Bridge nostr 0.37 → 0.36 (crew-sdk) via hex round-trip.
        ...
        let tag = crew_sdk_pkg::nip_oa::compute_auth_tag(&compat_owner, &compat_agent, "")
