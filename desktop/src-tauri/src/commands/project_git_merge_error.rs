...
impl From<String> for ProjectPullRequestMergeError {
    fn from(message: String) -> Self {
        // Relay push-policy denial for a repo with no `crew-channel` binding.
        // The stable token is declared in `crew-core::git_perms`
        ...
        if message.contains(crew_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN) {
            return Self::new(
                crew_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_TOKEN,
                "This repository is not bound to a channel, so the relay cannot \
                 authorize pushes. Bind it with: crew repos bind --id <repo> \
...
mod tests {
    ...
    fn no_channel_binding_denial_converts_to_structured_code() {
        ...
        let remote_stderr = format!(
            ...
            crew_core_pkg::git_perms::GIT_NO_CHANNEL_BINDING_BODY
        );
        ...
        assert!(error.message.contains("crew repos bind"));
