        ...
        fn did_receive_notification_response(
            ...
            if &*response.actionIdentifier() == unsafe { UNNotificationDefaultActionIdentifier } {
                if let Some(target) = target_from_response(response) {
                    ...
                    {
                        eprintln!(
                            "crew-desktop: failed to emit macOS notification activation: {error}"
                        );
                    }
                }
            }
            ...
            // malformed notifications that Crew intentionally ignores.
...
pub(crate) fn init(app: &AppHandle) -> tauri::Result<()> {
    if !is_bundled_application() {
        ...
        eprintln!(
            "crew-desktop: macOS notifications disabled because the process is not running from an app bundle"
        );
        return Ok(());
    }
...
fn ensure_bundled_application() -> Result<(), String> {
    if is_bundled_application() {
        ...
        Err(
            "macOS notifications are unavailable when Crew is not running from an app bundle"
                .to_string(),
        )
    }
}
...
fn queue_activation(target: serde_json::Value) {
    ...
    let Ok(mut queue) = queue.lock() else {
        eprintln!("crew-desktop: macOS notification activation queue is unavailable");
        return;
    };
...
mod tests {
    ...
    fn requires_the_executable_to_use_the_app_bundle_layout() {
        assert!(is_application_bundle_layout(
            ...
            Path::new("/Applications/Crew.app/Contents/MacOS/crew-desktop"),
        ));
        assert!(!is_application_bundle_layout(
        ...
        assert!(!is_application_bundle_layout(
            Path::new("/Applications/Crew.app"),
            Path::new("/Applications/Other.app/Contents/MacOS/crew-desktop"),
        ));
    }
