...
//!
//! Crew never called `Builder::menu()`, so Tauri installed `Menu::default()`
...
//!
//! That default cannot express Crew's context-dependent behavior:
//!
//! 1. `CloseRequested` on the main window is intercepted in `lib.rs` and turned
//!    into hide-to-tray. Cmd+W should take that path in normal Crew mode.
//! 2. macOS resolves a menu key equivalent before the webview receives any key
//!    event, so Crew Term could never bind Cmd+W to "close this terminal tab"
...
//! Cmd+W conditionally instead: Crew Term consumes it in capture phase while
...
/// Installs Crew's menu, replacing the `Menu::default()` Tauri would otherwise
