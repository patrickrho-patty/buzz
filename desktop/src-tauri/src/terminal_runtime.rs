...
use crew_terminal::context::{context_vars, GuiContext};
use crew_terminal::damage::{Frame, Style};
use crew_terminal::{Fences, SharedTerminal, Size, Terminal, Viewport};
...
impl crew_terminal::lifecycle::DrainingReader for ReaderThread {
...
struct Session {
    ...
    reader: Option<Box<dyn crew_terminal::lifecycle::DrainingReader + Send>>,
...
impl Session {
    ...
    fn shutdown(mut self) {
        ...
        if let Some(reader) = self.reader.take() {
            #[cfg(unix)]
            {
                let _ = crew_terminal::lifecycle::shutdown_draining(&mut self.child, reader);
            }
            #[cfg(not(unix))]
            {
...
pub(crate) fn terminal_attach(
    ...
    let shell = {
        #[cfg(unix)]
        {
            crew_terminal::shell::resolve_shell(std::env::var("SHELL").ok().as_deref())
        }
        #[cfg(windows)]
        {
            crew_terminal::shell::resolve_shell(std::env::var("ComSpec").ok().as_deref())
        }
    };
    let mut command = CommandBuilder::new_default_prog();
    crew_terminal::env_fence::fence_env(
        &mut command,
        &crew_terminal::path::user_shell_path(),
        &shell,
    );
    ...
    std::thread::spawn(move || {
        while let Ok(action) = actions.recv() {
            let presentation = match &action {
                crew_terminal::Action::Title(title) => Some(TerminalMessage::Title(title.clone())),
                crew_terminal::Action::ResetTitle => Some(TerminalMessage::ResetTitle),
                crew_terminal::Action::Bell => Some(TerminalMessage::Bell),
                _ => None,
            };
            ...
            if let Some(reply) = crew_terminal::listener::reply(
                action,
                size.cols,
                size.rows,
                size.pixel_width,
    ...
    let reader_handle = std::thread::spawn(move || {
        ...
        let mut encoder = crew_terminal::damage::Encoder::new();
        let mut snapshot_encoder = crew_terminal::damage::Encoder::new();
        loop {
...
mod tests {
    use super::*;
    use crew_terminal::damage::{CursorFrame, RowFrame, Span};

    fn publication(spans: Vec<Span>) -> Publication {
        Publication {
            subscription_id: SubscriptionId::new(),
            sequence: 7,
            frame: crew_terminal::damage::Frame {
                rows: vec![RowFrame {
                    line: 3,
                    wrapped: true,
                    spans,
