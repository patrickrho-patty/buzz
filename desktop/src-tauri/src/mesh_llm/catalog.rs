...
/// Crew-curated tier picks. These are the models we know survive the agent
...
const CURATED_LARGE_DESCRIPTION: &str =
    "Gemma 4 26B MoE (4B active) — Crew default for 64GB+ machines";
...
/// The Crew-curated recommendation for a machine's rated memory capacity.
...
/// Convert Crew's pre-0.74 curated package aliases into the canonical model
...
pub struct MeshCatalogEntry {
    ...
    /// Crew-curated pick — known to survive the agent harness. Curated
...
fn build_catalog(
    ...
    // The compiled MODEL_CATALOG does not know the Crew large pick; it
