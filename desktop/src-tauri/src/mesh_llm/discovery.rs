...
/// Crew membership, rather than device liveness, is the trust boundary.
...
pub fn availability_from_events(events: Vec<nostr::Event>) -> MeshAvailability {
    if events.is_empty() {
        return MeshAvailability::unavailable("Crew shared compute status is not published yet");
    }
    let Some(members) = latest_membership_list(&events) else {
        return MeshAvailability::unavailable(
            "Crew shared compute is waiting for the current member roster",
        );
    };
    ...
    if !saw_valid_status {
        return MeshAvailability::unavailable("Crew shared compute status is malformed");
    }
    ...
    MeshAvailability {
        reason: if available {
            ...
            Some("no Crew shared compute serving members are available".to_string())
        },
        models,
        serve_targets,
    }
