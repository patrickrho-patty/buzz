...
pub fn backfill_standalone_agents(app: &tauri::AppHandle) {
    ...
    match backfill_standalone_agents_in_dir(&base_dir) {
        ...
        Ok(backfilled) => {
            eprintln!(
                "crew-desktop: standalone-backfill: {backfilled} agents linked to manufactured definitions"
            );
        }
        Err(e) => eprintln!("crew-desktop: standalone-backfill: {e}"),
    }
}
...
fn backfill_standalone_agents_in_dir(base_dir: &Path) -> Result<usize, String> {
    ...
    for record in all.iter_mut().filter(|r| needs_backfill(r)) {
        ...
        if existing_slugs.contains(&record.pubkey) {
            eprintln!(
                "crew-desktop: standalone-backfill: slug collision for agent {} — skipped; \
        ...
        let Some(persona_view) = view_source.to_definition_view() else {
            eprintln!(
                "crew-desktop: standalone-backfill: agent {} produced no persona view — skipped",
                record.pubkey
            );
            continue;
        };
