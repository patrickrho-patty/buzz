...
// nostr 0.36 alias — required for cross-version bridging with crew-sdk.
...
///
/// `crew-sdk` uses `nostr 0.36` while the desktop crate uses `nostr 0.37`. Cross-version
...
fn build_profile_event(
    ...
    let builder = if let Some(tag_json) = auth_tag_json {
        ...
        crew_sdk_pkg::nip_oa::verify_auth_tag(tag_json, &compat_pubkey)
        ...
        let compat_tag = crew_sdk_pkg::nip_oa::parse_auth_tag(tag_json)
...
mod tests {
    ...
    /// `crew_sdk_pkg::nip_oa::compute_auth_tag` expects nostr 0.36 types.
    ...
    fn make_valid_auth_tag(agent_keys: &nostr::Keys) -> String {
        ...
        crew_sdk_pkg::nip_oa::compute_auth_tag(&owner_keys, &agent_compat_pubkey, "")
