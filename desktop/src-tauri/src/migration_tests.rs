...
fn canonical_dev_data_dir_replaces_last_component() {
    ...
    assert_eq!(
        canonical,
        PathBuf::from("/Users/me/Library/Application Support/xyz.patty.crew.app.dev")
    );
}
...
fn setup_sync_layout() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    let worktree = parent.path().join("xyz.patty.crew.app.dev.my-branch");
    let main_instance = parent.path().join("xyz.patty.crew.app.dev.main");
...
fn seed_up_migrates_sibling_file_to_canonical_then_symlinks() {
    ...
    let sibling = canonical
        .parent()
        .unwrap()
        .join("xyz.patty.crew.app.dev.main");
...
fn canonical_dev_data_dir_returns_self_for_canonical_instance() {
    ...
    // testing only.
    let current = PathBuf::from("/Users/me/Library/Application Support/xyz.patty.crew.app.dev");
