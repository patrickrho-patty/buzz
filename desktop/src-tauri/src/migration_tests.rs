...
fn canonical_dev_data_dir_replaces_last_component() {
    let current =
        PathBuf::from("/Users/me/Library/Application Support/xyz.patty.griddle.app.dev.my-branch");
    let canonical = canonical_dev_data_dir(&current).unwrap();
    assert_eq!(
        canonical,
        PathBuf::from("/Users/me/Library/Application Support/xyz.patty.griddle.app.dev")
    );
}
...
fn legacy_app_data_dir_maps_release_identifier() {
    let current = PathBuf::from("/Users/me/Library/Application Support/xyz.patty.griddle.app");
...
fn setup_sync_layout() -> (tempfile::TempDir, PathBuf, PathBuf) {
    ...
    let worktree = parent.path().join("xyz.patty.griddle.app.dev.my-branch");
    let main_instance = parent.path().join("xyz.patty.griddle.app.dev.main");
...
fn seed_up_migrates_sibling_file_to_canonical_then_symlinks() {
    ...
    let sibling = canonical
        .parent()
        .unwrap()
        .join("xyz.patty.griddle.app.dev.main");
...
fn reconcile_mcp_commands_sets_canonical_for_buzz_agent() {
    ...
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    let records = read_agents_json(dir.path());
    assert_eq!(records[0]["mcp_command"], "crew-dev-mcp");
}
...
fn reconcile_mcp_commands_handles_mixed_agents() {
    ...
    assert_eq!(records[3]["mcp_command"], "crew-dev-mcp");
}
...
fn reconcile_mcp_commands_sees_team_dir_runtime_edit_same_launch() {
    ...
    // (goose → crew-agent) into personas.json. The reader runs AFTER, so it
    // must derive the NEW crew-agent mcp_command without a second launch.
    write_personas_json(
        dir.path(),
        &serde_json::json!([{"id": "p1", "runtime": "crew-agent"}]),
    );
    reconcile_mcp_commands_in_file(&dir.path().join("agents/managed-agents.json"));
    assert_eq!(
        read_agents_json(dir.path())[0]["mcp_command"],
...
fn reconcile_mcp_commands_honors_explicit_override_over_persona() {
    ...
    // runtime: persona is goose (no mcp) but the pin is crew-agent, so the
    // reconcile sets the crew-agent mcp_command.
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
