...
const CREW_AGENT_AVATAR_URL: &str =
    "https://raw.githubusercontent.com/block/crew/refs/heads/main/crates/crew-agent/crew-agent.png";
...
const KNOWN_ACP_RUNTIMES: &[KnownAcpRuntime] = &[
    KnownAcpRuntime {
        ...
        cli_install_hint: "Crew talks to Goose through the Goose CLI.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "Crew talks to Claude Code through the Claude Code CLI.",
        adapter_install_hint: "Crew talks to the Claude Code CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/claude-agent-acp.",
    ...
    KnownAcpRuntime {
        ...
        cli_install_hint: "Crew talks to Codex through the Codex CLI.",
        adapter_install_hint: "Crew talks to the Codex CLI through an ACP adapter. Install it with: npm install -g @agentclientprotocol/codex-acp.",
    ...
    KnownAcpRuntime {
        ...
        adapter_install_commands: &[],
        cli_install_instructions_url: "https://github.com/block/crew",
        adapter_install_instructions_url: "https://github.com/block/crew",
        cli_install_hint: "Ships with the Crew desktop app.",
        adapter_install_hint: "",
        skill_dir: None,
...
///
/// Consults the Crew-managed shim dir (a filesystem stat, never a spawn) and
...
///
/// Crew rewrites these catalog commands to an app-private npm prefix before
...
pub fn missing_command_message(command: &str, role: &str) -> String {
    ...
    format!(
        "{role} `{command}` was not found. Make sure it is installed and on your PATH. Antivirus software can quarantine bundled binaries — if that happened, restore the file or reinstall Crew. (Source builds: see TESTING.md.)"
    )
}
...
/// The oldest `codex-acp` version supported by Crew managed agents.
///
/// Older 1.x adapters are detected successfully, but can still bundle a Codex runtime
/// that does not reliably give `crew` CLI subprocesses outbound relay access.
///
...
fn discover_acp_runtime_phase1(runtime: &'static KnownAcpRuntime, force: bool) -> PartialEntry {
    ...
    // node_required now means Crew cannot provide npm for this platform.
    // On supported desktop platforms, Crew downloads a private Node/npm
