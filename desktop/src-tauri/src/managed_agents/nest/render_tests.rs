...
fn test_render_keeps_agent_with_legacy_foreign_relay_pin() {
    ...
    elsewhere.relay_url = "wss://defunct.communities.crew.xyz".to_string();
...
fn test_upsert_managed_section_with_markers() {
    ...
    fs::write(
            &file,
            "# Header\n\nsome content\n\n<!-- BEGIN CREW MANAGED — regenerated automatically, do not edit below -->\nold section\n<!-- END CREW MANAGED -->\n\nafter\n",
        )
        .unwrap();
    ...
    assert!(result.contains("<!-- BEGIN CREW MANAGED"));
    assert!(result.contains("<!-- END CREW MANAGED -->"));
...
fn test_upsert_managed_section_without_markers() {
    ...
    let begin_pos = result.find("<!-- BEGIN CREW MANAGED").unwrap();
...
fn test_upsert_marker_in_code_block() {
    ...
    fs::write(
        &file,
        "# Header\n\n    <!-- BEGIN CREW MANAGED — some indented marker -->\n\nReal content here\n",
    )
    .unwrap();
    ...
    assert!(
        result.contains("    <!-- BEGIN CREW MANAGED — some indented marker -->"),
    ...
    let begin_pos = result
        .find("<!-- BEGIN CREW MANAGED — regenerated")
