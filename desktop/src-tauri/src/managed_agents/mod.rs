...
pub(crate) use agent_env::{
    baked_build_env, build_crew_agent_provider_defaults, discovery_env_with_baked_floor,
};
mod backend;
...
/// Returns the Crew nest directory (`~/.crew`) if it exists as a real
...
/// invoked, so that `~/.crew` exists and gets cached.
///
...
pub fn default_agent_workdir() -> Option<std::path::PathBuf> {
    ...
    WORKDIR
        .get_or_init(|| {
            // Prefer ~/.crew if it exists (created by ensure_nest()).
