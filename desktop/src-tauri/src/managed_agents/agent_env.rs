...
//!
//! Internal builds (crew-releases) bake arbitrary `KEY=VALUE` pairs into the
//! binary via `CREW_BUILD_AGENT_ENV` (base64-encoded, newline-delimited).
...
/// subprocesses back to the empty-slot state (via `CREW_ACP_IDLE_POOL_SLEEP`).
...
/// Value for `CREW_ACP_IDLE_POOL_SLEEP`. Idle re-sleep is only meaningful for
...
///
/// Internal builds (crew-releases) bake provider/model defaults and arbitrary
...
pub(crate) fn baked_build_env() -> BTreeMap<String, String> {
    build_env_map(
        option_env!("CREW_DESKTOP_BUILD_CREW_AGENT_PROVIDER"),
        option_env!("CREW_DESKTOP_BUILD_CREW_AGENT_MODEL"),
        option_env!("CREW_DESKTOP_BUILD_AGENT_ENV"),
    )
}
...
fn build_env_map(
    ...
    if let Some(provider) = raw_provider {
        if !provider.is_empty() {
            map.insert("CREW_AGENT_PROVIDER".to_string(), provider.to_string());
        }
    }
    if let Some(model) = raw_model {
        if !model.is_empty() {
            map.insert("CREW_AGENT_MODEL".to_string(), model.to_string());
        }
    }
    ...
    // (see `managed_agents/runtime.rs`), so a baked `CREW_ACP_RESPOND_TO` would
    // otherwise win over the gate Desktop just set.
    map.retain(|key, _| {
        if super::env_vars::is_reserved_env_key(key) {
            eprintln!(
                "crew-desktop: ignoring reserved env var `{key}` from the baked build env"
            );
            return false;
        }
        true
...
pub(crate) fn build_crew_agent_provider_defaults(cmd: &mut std::process::Command) {
...
mod tests {
    use super::{
        baked_build_env, build_crew_agent_provider_defaults, build_env_map,
        discovery_env_with_baked_floor, parse_agent_env_lines,
    };

    #[test]
    fn crew_agent_provider_defaults_empty_in_oss_build() {
        // OSS (and normal test) builds set neither CREW_BUILD_CREW_AGENT_*,
        // so nothing is baked in and no CREW_AGENT_* is injected on spawn.
        ...
        assert!(
            !stdout.contains("CREW_AGENT_PROVIDER="),
            "CREW_AGENT_PROVIDER should not be injected in OSS builds"
        );
        assert!(
            !stdout.contains("CREW_AGENT_MODEL="),
            "CREW_AGENT_MODEL should not be injected in OSS builds"
        );
        assert!(
    ...
    //
    // `build_crew_agent_provider_defaults` must run BEFORE
    ...
    fn baked_defaults_do_not_override_record_provider_written_after() {
        ...
        cmd.env("CREW_AGENT_PROVIDER", "databricks");
        // Simulate what runtime_metadata_env_vars writes from the record (comes after).
        cmd.env("CREW_AGENT_PROVIDER", "anthropic");
        let output = cmd.output().expect("env should run");
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            stdout.contains("CREW_AGENT_PROVIDER=anthropic"),
            "record provider must win over baked default (last-write-wins)"
        );
        assert!(
            !stdout.contains("CREW_AGENT_PROVIDER=databricks"),
    ...
    fn baked_build_env_is_empty_in_oss_build() {
        // In OSS/test builds none of the CREW_DESKTOP_BUILD_* compile-time vars
    ...
    fn build_env_map_provider_and_model_are_mapped() {
        ...
        assert_eq!(
            map.get("CREW_AGENT_PROVIDER").map(String::as_str),
            Some("databricks")
        );
        assert_eq!(
            map.get("CREW_AGENT_MODEL").map(String::as_str),
            Some("my-model")
        );
    }
    ...
    fn build_env_map_empty_provider_is_skipped() {
        ...
        assert!(
            !map.contains_key("CREW_AGENT_PROVIDER"),
    ...
    // `managed_agents/runtime.rs`), after Crew sets the access gates. If a
    // baked reserved key survived here, an internal build packaged with
    // `CREW_ACP_RESPOND_TO=anyone` would answer anyone while the UI shows
