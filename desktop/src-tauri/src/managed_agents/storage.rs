    ...
    match store.probe(&name) {
        ...
        KeyringProbe::Present | KeyringProbe::ReachableButEmpty => {
            match store.write_and_verify(&name, &record.private_key_nsec) {
                ...
                Err(e) => {
                    eprintln!(
                        "griddle-desktop: keyring write for agent {} failed ({e}), keeping inline",
                        record.pubkey
                    );
...
pub(crate) fn backup_invalid_store(path: &Path) {
    ...
    if let Err(e) = fs::copy(path, &backup) {
        eprintln!(
            "griddle-desktop: failed to preserve malformed store {} as {}: {e}",
            path.display(),
            backup.display()
        );
    }
    ...
    for record in records.iter_mut() {
        ...
        if record.private_key_nsec.is_empty() {
            match store.load(&agent_keyring_name(&record.pubkey)) {
                ...
                Ok(None) => {
                    eprintln!(
                        "griddle-desktop: agent {} has no key in JSON or keyring",
                        record.pubkey
                    );
                }
                ...
                Err(e) => {
                    eprintln!(
                        "griddle-desktop: agent {} key unavailable — keyring read failed ({e}); \
...
/// (`"griddle-desktop"`) to the dev service (`"griddle-desktop-dev"`). Only runs
/// in debug builds — release builds never touch `"griddle-desktop"` from this
/// path.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &tauri::AppHandle) {
    if !cfg!(feature = "system-keyring") || keyring_service() != "griddle-desktop-dev" {
        return;
    }
    ...
    let records = match load_agent_store(app) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("griddle-desktop: keyring-dev-migration: cannot read agent store: {e}");
            return;
        }
    };
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring("griddle-desktop");
...
fn copy_agent_keys_between_stores(pubkeys: &[String], src: &impl KeyStore, dst: &impl KeyStore) {
    ...
    let dst_map: HashMap<String, String> = match dst.load_all_readonly() {
        ...
        Ok(None) => HashMap::new(),
        Err(e) => {
            eprintln!("griddle-desktop: keyring-dev-migration: cannot read dev keyring: {e}");
            return;
        }
    };
        ...
        .all(|pubkey| dst_map.contains_key(&agent_keyring_name(pubkey)))
    {
        ...
        match src.load_all_readonly() {
            ...
            Err(e) => {
                eprintln!("griddle-desktop: keyring-dev-migration: cannot read prod keyring: {e}");
                return;
            }
        }
    };
    ...
    if let Err(e) = dst.store_all(&to_write) {
        eprintln!("griddle-desktop: keyring-dev-migration: cannot write to dev keyring: {e}");
        return;
    }

    if copied > 0 {
        eprintln!(
            "griddle-desktop: keyring-dev-migration: copied {copied} agent key(s) from griddle-desktop"
        );
    }
}
...
pub fn delete_agent_key(pubkey: &str) {
    if let Err(e) = try_delete_agent_key(pubkey) {
        eprintln!("griddle-desktop: failed to delete agent {pubkey} key from keyring: {e}");
    }
}
...
pub fn meaningful_agent_error_from_log(path: &Path) -> Option<AgentLogError> {
    ...
    tail.lines().rev().map(str::trim).find_map(|line| {
        ...
        // Legacy format (older crew-acp builds): "Agent reported error: ..."
