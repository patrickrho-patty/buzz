    ...
    match store.probe(&name) {
        ...
        KeyringProbe::Present | KeyringProbe::ReachableButEmpty => {
            match store.write_and_verify(&name, &record.private_key_nsec) {
                ...
                Err(e) => {
                    eprintln!(
                        "crew-desktop: keyring write for agent {} failed ({e}), keeping inline",
                        record.pubkey
                    );
...
pub(crate) fn backup_invalid_store(path: &Path) {
    ...
    if let Err(e) = fs::copy(path, &backup) {
        eprintln!(
            "crew-desktop: failed to preserve malformed store {} as {}: {e}",
            path.display(),
            backup.display()
        );
    }
    ...
    for record in records.iter_mut() {
        ...
        if record.private_key_nsec.is_empty() {
            match store.load(&agent_keyring_name(&record.pubkey)) {
                ...
                Ok(None) => {
                    eprintln!(
                        "crew-desktop: agent {} has no key in JSON or keyring",
                        record.pubkey
                    );
                }
                ...
                Err(e) => {
                    eprintln!(
                        "crew-desktop: agent {} key unavailable — keyring read failed ({e}); \
...
/// (`"crew-desktop"`) to the dev service (`"crew-desktop-dev"`). Only runs
/// in debug builds — release builds never touch `"crew-desktop"` from this
/// path.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &tauri::AppHandle) {
    if !cfg!(feature = "system-keyring") || keyring_service() != "crew-desktop-dev" {
        return;
    }
    ...
    let records = match load_agent_store(app) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("crew-desktop: keyring-dev-migration: cannot read agent store: {e}");
            return;
        }
    };
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring("crew-desktop");
...
fn copy_agent_keys_between_stores(pubkeys: &[String], src: &impl KeyStore, dst: &impl KeyStore) {
    ...
    let dst_map: HashMap<String, String> = match dst.load_all_readonly() {
        ...
        Ok(None) => HashMap::new(),
        Err(e) => {
            eprintln!("crew-desktop: keyring-dev-migration: cannot read dev keyring: {e}");
            return;
        }
    };
        ...
        .all(|pubkey| dst_map.contains_key(&agent_keyring_name(pubkey)))
    {
        ...
        match src.load_all_readonly() {
            ...
            Err(e) => {
                eprintln!("crew-desktop: keyring-dev-migration: cannot read prod keyring: {e}");
                return;
            }
        }
    };
    ...
    if let Err(e) = dst.store_all(&to_write) {
        eprintln!("crew-desktop: keyring-dev-migration: cannot write to dev keyring: {e}");
        return;
    }

    if copied > 0 {
        eprintln!(
            "crew-desktop: keyring-dev-migration: copied {copied} agent key(s) from crew-desktop"
        );
    }
}
...
pub fn delete_agent_key(pubkey: &str) {
    if let Err(e) = try_delete_agent_key(pubkey) {
        eprintln!("crew-desktop: failed to delete agent {pubkey} key from keyring: {e}");
    }
}
