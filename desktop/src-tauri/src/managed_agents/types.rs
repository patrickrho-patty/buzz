...
pub enum AuthStatus {
    ...
    /// This runtime does not have a login step (e.g. goose, crew-agent).
    NotApplicable,
...
impl RespondTo {
    /// CLI/env wire string (matches `crew-acp`'s `--respond-to`).
...
///
/// Rules mirror `crew-acp/src/config.rs::validate_allowlist`:
