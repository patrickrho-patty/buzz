//! Git Bash discovery shared by Doctor and the crew-agent readiness gate.
//!
...
//! Git-for-Windows registry. A Doctor green state therefore means `crew-dev-mcp`
...
/// (including all non-Windows platforms). Honors `CREW_SHELL` (any executable) —
...
///
/// Unlike `resolve_git_bash_path`, this skips `CREW_SHELL` entirely — that override
...
pub(crate) fn resolve_bash_path() -> Option<std::path::PathBuf> {
    #[cfg(windows)]
    {
        ...
        return resolve_git_bash(
            &env.path,
            None, // skip CREW_SHELL — install/login-shell callers require bash
            env.git_bash_override,
            env.system_root,
            env.program_files,
            env.program_files_x86,
...
/// deliberately sourced from `crew_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV`,
...
impl GitBashEnv {
    ...
    fn from_process_with_overrides(overrides: &std::collections::BTreeMap<String, String>) -> Self {
        let values: std::collections::HashMap<_, _> = crew_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV
            .iter()
            .filter_map(|key| {
                overrides
                    .iter()
    ...
    fn from_lookup(mut get: impl FnMut(&str) -> Option<std::ffi::OsString>) -> Self {
        Self {
            path: get("PATH")
                ...
                .to_string_lossy()
                .into_owned(),
            shell_override: get("CREW_SHELL").map(PathBuf::from),
...
/// Resolve `CREW_SHELL` with the same rooted/bare-name semantics as the MCP
...
mod tests {
    ...
    fn test_detector_env_keys_match_agent_shell_resolution_contract() {
        assert_eq!(
            DETECTOR_ENV_KEYS,
            crew_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV,
        ...
        assert_eq!(env.shell_override, Some(PathBuf::from("CREW_SHELL")));
    ...
    // ── Regression: install/login-shell must skip non-bash CREW_SHELL ─────────

    /// When CREW_SHELL=pwsh.exe, `resolve_git_bash` with `shell_override=None`
    ...
    fn test_install_path_skips_buzz_shell_pwsh() {
        ...
        // Readiness gate: CREW_SHELL=pwsh accepted (Doctor green).
        assert_eq!(
            resolve_git_bash(path_str, Some(pwsh.clone()), None, None, None, None, None),
            Some(pwsh),
            "readiness gate must accept CREW_SHELL=pwsh"
        );
        ...
        assert_eq!(
            ...
            Some(bash),
            "install path must skip CREW_SHELL and find bash on PATH"
        );
    }
    ...
    fn test_install_path_skips_buzz_shell_cmd() {
        ...
        // Readiness gate: CREW_SHELL=cmd accepted.
        assert_eq!(
            resolve_git_bash(path_str, Some(cmd.clone()), None, None, None, None, None),
            Some(cmd),
            "readiness gate must accept CREW_SHELL=cmd"
        );
