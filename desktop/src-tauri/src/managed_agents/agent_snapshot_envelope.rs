...
//!
//! Wire contract (agreed with Wren, crew-agent-trading-cards thread):
...
use crew_core_pkg::engram::NIP44_PLAINTEXT_MAX;
...
/// `crew-agent-snapshot` so detection never guesses.
pub const LOCKED_FORMAT: &str = "crew-agent-snapshot-encrypted";
...
/// Typed outer envelope stored (base64 JSON) in the `crew_agent_snapshot`
...
///
/// - `crew-agent-snapshot` → full plain-manifest decode + validation.
/// - `crew-agent-snapshot-encrypted` → size caps, typed envelope parse,
...
mod tests {
    ...
    fn unknown_format_version_scheme_rejected() {
        ...
        let unknown = serde_json::json!({"format": "crew-agent-snapshot-v9", "version": 1});
