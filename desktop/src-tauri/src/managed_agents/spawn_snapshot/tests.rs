...
fn allowlist_ignored_when_mode_is_not_allowlist() {
    // Spawn only sets CREW_ACP_RESPOND_TO_ALLOWLIST in allowlist mode, so
...
fn display_name_edit_changes_snapshot() {
    // The spawn writes CREW_ACP_SESSION_TITLE from display_name-or-name, so a
...
fn display_name_edit_does_not_change_snapshot_under_an_explicit_title_override() {
    ...
    let mut rec = record();
    rec.env_vars
        .insert("CREW_ACP_SESSION_TITLE".into(), "Pinned Title".into());
...
fn title_override_edit_changes_snapshot() {
    ...
    edited
        .env_vars
        .insert("CREW_ACP_SESSION_TITLE".into(), "Other Title".into());
    assert_ne!(
        snapshot(&rec, &[], &[], "wss://ws.example", &Default::default()),
        snapshot(&edited, &[], &[], "wss://ws.example", &Default::default()),
