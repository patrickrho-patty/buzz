...
// carrier of startup effort. `CREW_ACP_EFFORT_LEVEL` is stripped from the
...
/// Look up the `env.CREW_ACP_EFFORT_LEVEL` leaf of a canonical snapshot, if any.
fn effort_env_leaf(canonical: &serde_json::Value) -> Option<&serde_json::Value> {
    canonical
        .get("env")
        .and_then(|env| env.get("CREW_ACP_EFFORT_LEVEL"))
}

/// A record whose user env seeds `CREW_ACP_EFFORT_LEVEL` (the pre-canonical
...
fn record_with_env_effort(value: &str) -> ManagedAgentRecord {
    let mut rec = record();
    rec.env_vars
        .insert("CREW_ACP_EFFORT_LEVEL".into(), value.into());
    rec
}
...
fn effective_effort_prefers_persisted_canonical_over_user_env() {
    ...
    let env = BTreeMap::from([("CREW_ACP_EFFORT_LEVEL".to_string(), "low".to_string())]);
...
fn snapshot_carries_effort_in_field_not_env() {
    ...
    assert_eq!(
        ...
        None,
        "CREW_ACP_EFFORT_LEVEL must be stripped from the snapshot env"
    );
}
...
fn env_only_effort_edit_changes_effort_level_not_env() {
    ...
    // representation and never reintroduces an `env.CREW_ACP_EFFORT_LEVEL`
