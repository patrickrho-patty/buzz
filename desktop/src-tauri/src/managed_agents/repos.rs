...
pub fn resolve_repos_at_boot(nest_root: &Path) -> bool {
    ...
    if let Err(error) = &symlink_result {
        eprintln!("crew-desktop: repos dir setup failed at boot: {error}");
    }
    ...
    if symlink_result.is_ok() {
        match persisted.as_deref() {
            Some(dir) => eprintln!(
                "crew-desktop: repos dir resolved at boot — REPOS symlinked to configured `{dir}`"
            ),
            None => eprintln!(
                "crew-desktop: repos dir resolved at boot — no configured override, REPOS is the default real dir"
            ),
        }
    }
    if !restore {
        eprintln!(
            "crew-desktop: skipping agent restore — configured repos_dir `{}` could not be resolved at boot; will retry on next launch",
...
mod tests {
    ...
    fn validate_repos_dir_rejects_nest_ancestor() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("home").join(".crew");
        fs::create_dir_all(&root).unwrap();
