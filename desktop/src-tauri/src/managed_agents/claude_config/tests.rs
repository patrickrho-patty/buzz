...
/// A1: CREW_ACP_MODEL must NOT be present in the spawned-child env after
...
fn a1_crew_acp_model_absent_anthropic_model_present_after_env_apply() {
    ...
    // CREW_ACP_MODEL must be removed. Command::get_envs returns None for
    // explicitly-removed keys.
    let crew_acp = env_map.get(std::ffi::OsStr::new("CREW_ACP_MODEL"));
    assert!(
        crew_acp.is_none() || crew_acp.unwrap().is_none(),
        "CREW_ACP_MODEL must be absent (or explicitly removed) after A1 policy"
    );
...
fn a1_anthropic_model_removed_when_no_effective_model() {
    ...
    cmd.env("CREW_ACP_MODEL", "claude-3-5-sonnet");
    ...
    assert!(
        crew_acp.is_none() || crew_acp.unwrap().is_none(),
        "CREW_ACP_MODEL must always be absent after A1 policy"
    );
}
...
/// Simulates the defect scenario: descriptor.env wrote CREW_ACP_EFFORT_LEVEL=low,
...
fn b5_canonical_effort_wins_over_user_env_collision() {
    ...
    cmd.env("CREW_ACP_EFFORT_LEVEL", "low");

    // Post-loop canonical application — the fix.
    apply_effort_env(&mut cmd, Some("high"));

    let env_map: std::collections::HashMap<_, _> = cmd.get_envs().collect();
    let effort = env_map.get(std::ffi::OsStr::new("CREW_ACP_EFFORT_LEVEL"));
    assert!(effort.is_some(), "CREW_ACP_EFFORT_LEVEL must be present");
    assert_eq!(
...
/// Simulates: descriptor.env wrote CREW_ACP_EFFORT_LEVEL=low (already in command),
...
fn b5_canonical_effort_injected_when_no_user_collision() {
    let mut cmd = std::process::Command::new("true");
    // No user-supplied CREW_ACP_EFFORT_LEVEL in descriptor.env.
