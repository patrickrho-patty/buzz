...
//! local Claude Code agents. `CREW_ACP_MODEL` is removed from the spawned
//! env so the harness never sees two model authorities simultaneously.
//!
//! B5 contract: `CREW_ACP_EFFORT_LEVEL` is the canonical persisted startup
...
pub const EFFORT_LEVEL_ENV_VAR: &str = "CREW_ACP_EFFORT_LEVEL";
...
/// Apply the B5 effort authority: inject `CREW_ACP_EFFORT_LEVEL` from
/// `effort_level` (or leave it untouched if `None`).
///
/// Must be called after `descriptor.env` is written so the canonical persisted
/// value wins over any user-supplied `CREW_ACP_EFFORT_LEVEL` entry. When
