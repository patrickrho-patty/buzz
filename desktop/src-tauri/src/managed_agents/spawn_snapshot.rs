...
pub(crate) struct SpawnConfigSnapshot {
    /// The ACP harness binary the desktop launches (`crew-acp`).
    ...
    /// `None` when a user env override shadows `CREW_ACP_SESSION_TITLE`: spawn
    ...
    /// `CREW_ACP_RESPOND_TO_ALLOWLIST` only there, so edits to a dormant list
    ...
    /// present, else the user-seeded `CREW_ACP_EFFORT_LEVEL` from the layered
...
/// is absent does a user-supplied `CREW_ACP_EFFORT_LEVEL` from the layered env
...
impl SpawnConfigSnapshot {
    ...
    pub(crate) fn from_inputs(inputs: SpawnConfigInputs<'_>) -> Self {
        ...
        Self {
            ...
            // value is a no-op (no phantom `env.CREW_ACP_EFFORT_LEVEL` add or
