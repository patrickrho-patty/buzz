//! `crew-agent-snapshot v1` — manifest type, encoder, and decoder stubs.
//!
...
pub const FORMAT_DISCRIMINATOR: &str = "crew-agent-snapshot";
...
/// The top-level `crew-agent-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.agent.json` directly, or in the
/// `crew_agent_snapshot` tEXt chunk of a `.agent.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// in the `crew_agent_snapshot` tEXt chunk.
...
/// into a PNG carrying it base64-encoded in the `crew_agent_snapshot` tEXt
...
/// Extract and base64-decode the raw `crew_agent_snapshot` chunk payload
