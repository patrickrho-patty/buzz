...
//!
//! `CREW_BUILD_AGENT_ACCESS_OWNER_ONLY` marks a build whose managed agents may
...
//!   `CREW_ACP_RESPOND_TO` to `owner-only` and pins the independent
//!   `CREW_ACP_ALLOWED_RESPOND_TO=owner-only` guard on every start, whatever
//!   the record says.
...
/// Release packaging sets `CREW_BUILD_AGENT_ACCESS_OWNER_ONLY`; OSS/custom
/// builds do not.
pub(crate) fn owner_only_access_build() -> bool {
    option_env!("CREW_DESKTOP_BUILD_AGENT_ACCESS_OWNER_ONLY").is_some()
}
...
pub(crate) fn build_respond_to_env_with_policy(
    ...
    let mut set = vec![("CREW_ACP_RESPOND_TO", respond_to.as_str().to_string())];
    ...
    if enforced_owner_only {
        ...
        remove.push("CREW_ACP_ALLOWED_RESPOND_TO");
    }
    if respond_to == RespondTo::Allowlist {
        set.push(("CREW_ACP_RESPOND_TO_ALLOWLIST", normalized.join(",")));
    } else {
        remove.push("CREW_ACP_RESPOND_TO_ALLOWLIST");
    }

    if record.auth_tag.is_none() {
        if let Some(owner) = owner_hex {
            set.push(("CREW_ACP_AGENT_OWNER", owner.to_string()));
        } else {
            remove.push("CREW_ACP_AGENT_OWNER");
        }
    } else {
        remove.push("CREW_ACP_AGENT_OWNER");
    }
...
mod tests {
    ...
    fn owner_only_access_enforcement_clamps_local_and_provider() {
        for (label, backend) in [
            ...
            assert_eq!(
                gate_set.get("CREW_ACP_RESPOND_TO").map(String::as_str),
                Some("owner-only"),
