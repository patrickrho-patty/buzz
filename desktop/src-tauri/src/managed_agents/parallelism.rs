...
//
//   * local spawn  — `CREW_ACP_AGENTS` in the child environment
//   * remote deploy — `launch.policy_env["CREW_ACP_AGENTS"]` + legacy field
...
/// Return the value to emit as `CREW_ACP_AGENTS` for a spawn command.
///
...
/// a decimal string ready for `command.env("CREW_ACP_AGENTS", …)`.
///
...
mod tests {
    ...
    /// Legacy OpenClaw record (parallelism 10, above cap): CREW_ACP_AGENTS must be "5".
    #[test]
    fn acp_agents_value_openclaw_above_cap_is_capped() {
        assert_eq!(
            super::acp_agents_value("openclaw", 10),
            "5",
            "CREW_ACP_AGENTS for openclaw with parallelism 10 must be \"5\""
        );
