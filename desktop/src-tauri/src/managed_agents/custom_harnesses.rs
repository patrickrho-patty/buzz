...
fn validate_harness_definition(def: &HarnessDefinition) -> Result<(), String> {
    ...
    // `CREW_ACP_AGENT_ARGS` env transport (clap `value_delimiter = ','` on the
    // crew-acp side), so a literal comma inside one argument would silently
    ...
    // reserved-key bypass exploit (CREW_AUTH_TAG=x forgery shape), rejects
...
fn builtin_ids() -> impl Iterator<Item = &'static str> {
    const TIER1: &[&str] = &["goose", "claude", "codex", "crew-agent"];
...
// spawn time, causing silent fallback to crew-agent.
//
...
// registry, the caller gets a typed error — never a silent crew-agent fallback.
...
mod tests {
    ...
    fn builtin_ids_are_rejected() {
        ...
        for id in &["goose", "claude", "codex", "crew-agent"] {
    ...
    fn validate_rejects_malformed_key_with_equals_sign() {
        // CREW_AUTH_TAG=x is the documented reserved-key bypass shape:
        // the key contains '=' so Command::env would produce
        // `CREW_AUTH_TAG=x=forged` in the child env.
        let mut env = BTreeMap::new();
        env.insert("CREW_AUTH_TAG=x".to_string(), "forged".to_string());
        let def = HarnessDefinition {
    ...
    /// validation boundary — the comma-delimited `CREW_ACP_AGENT_ARGS`
