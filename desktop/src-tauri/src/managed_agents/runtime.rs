...
pub fn spawn_agent_child(
    ...
    command.env("CREW_PRIVATE_KEY", &record.private_key_nsec);
    command.env("CREW_RELAY_URL", &effective_relay_url);
    command.env("CREW_ACP_LAZY_POOL", if lazy { "true" } else { "false" });
    command.env("CREW_ACP_IDLE_POOL_SLEEP", idle_pool_sleep_env(lazy));
    command.env("CREW_ACP_AGENT_COMMAND", &resolved_agent_command);
    command.env("CREW_ACP_AGENT_ARGS", agent_args.join(","));
    match &resolved_mcp_command {
        Some(mcp_cmd) => {
            command.env("CREW_ACP_MCP_COMMAND", mcp_cmd);
        }
        None => {
            command.env("CREW_ACP_MCP_COMMAND", "");
        }
    }
    // Enable MCP hook tools (_Stop, _PostCompact) for agents that need them.
    // Uses "*" because build_mcp_servers() hard-codes the server name to "crew-mcp".
    ...
    // into CREW_ACP_SETUP_PAYLOAD.  crew-acp detects this env var on startup
    // and enters the minimal setup-listener mode instead of the agent pool.
    //
    // SECURITY: CREW_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS so user env
    ...
    let spawned_setup_mode;
    {
        ...
        //   1. CREW_ACP_SETUP_PAYLOAD is in RESERVED_ENV_KEYS, so
        ...
        command.env_remove("CREW_ACP_SETUP_PAYLOAD");
    ...
    // Emit CREW_ACP_IDLE_TIMEOUT only when explicitly set; the harness
    // DEFAULT_IDLE_TIMEOUT_SECS is the single source of truth. The deprecated
    // CREW_ACP_TURN_TIMEOUT pinned agents to a stale default (320s).
    if let Some(idle) = record.idle_timeout_seconds {
        command.env("CREW_ACP_IDLE_TIMEOUT", idle.to_string());
    }

    if let Some(max_dur) = record.max_turn_duration_seconds {
        command.env("CREW_ACP_MAX_TURN_DURATION", max_dur.to_string());
    }
    let acp_n = super::acp_agents_value(effective_command, record.parallelism);
    command.env("CREW_ACP_AGENTS", acp_n);
    command.env("CREW_ACP_MULTIPLE_EVENT_HANDLING", "steer");
    command.env("CREW_ACP_DEDUP", "queue");
    ...
    if let Some(instructions) = &team_instructions {
        command.env("CREW_ACP_TEAM_INSTRUCTIONS", instructions);
    } else {
        command.env_remove("CREW_ACP_TEAM_INSTRUCTIONS");
    }
    ...
    if let Some(prompt) = &effective_prompt {
        command.env("CREW_ACP_SYSTEM_PROMPT", prompt);
    } else {
        command.env_remove("CREW_ACP_SYSTEM_PROMPT");
    }
    ...
    // told the same thing: `CREW_ACP_MODEL=auto` would name a model the mesh
    // never advertises, leaving crew-acp to warn and fall back on every new
    // session while `CREW_AGENT_MODEL` said `mesh`.
    #[cfg(feature = "mesh-llm")]
    ...
    if let Some(model) = acp_model.as_deref() {
        command.env("CREW_ACP_MODEL", model);
    } else {
        command.env_remove("CREW_ACP_MODEL");
    }
    ...
    command.env_remove("CREW_ACP_PRIVATE_KEY");
    command.env_remove("CREW_ACP_API_TOKEN");
    command.env_remove("CREW_API_TOKEN");

    if let Some(ref auth_tag) = record.auth_tag {
        command.env("CREW_AUTH_TAG", auth_tag);
    } else {
        command.env_remove("CREW_AUTH_TAG");
    }
    ...
    // user-supplied CREW_ACP_EFFORT_LEVEL entry, mirroring the A1 model-authority pattern
    ...
    // CREW_ACP_MODEL is removed (live ACP switches only; two authorities in the same env
    // would be ambiguous).
    ...
    command
        .env("CREW_MANAGED_AGENT", current_instance_id(app))
        .env("CREW_MANAGED_AGENT_START_NONCE", &start_nonce);
