...
fn identifier_prefix_does_not_match_longer_id() {
    ...
    let buf = br#""identifier":"xyz.patty.crew.app.dev""#;
    let id = b"xyz.patty.crew.app";
...
fn identifier_prefix_does_not_match_worktree_slug() {
    ...
    let buf = br#""identifier":"xyz.patty.crew.app.dev.my-branch""#;
    let id = b"xyz.patty.crew.app.dev";
    assert!(!super::buffer_contains_identifier(buf, id));
}
...
fn identifier_match_with_null_boundary() {
    ...
    let mut buf = b"CREW_MANAGED_AGENT=xyz.patty.crew.app.dev".to_vec();
    buf.push(0);
...
fn identifier_exact_match_at_end_of_buffer() {
    ...
    let buf = b"xyz.patty.crew.app.dev";
    let id = b"xyz.patty.crew.app.dev";
    assert!(super::buffer_contains_identifier(buf, id));
}
...
fn longer_id_matches_when_short_prefix_also_present() {
    ...
    let mut buf = b"xyz.patty.crew.app".to_vec();
    buf.push(0);
    buf.extend_from_slice(br#""identifier":"xyz.patty.crew.app.dev""#);
    let id = b"xyz.patty.crew.app.dev";
    assert!(super::buffer_contains_identifier(&buf, id));
}
...
fn marker_entry_is_namespaced_by_instance_id() {
    ...
    assert_eq!(
        super::crew_marker_entry("xyz.patty.crew.app"),
        b"CREW_MANAGED_AGENT=xyz.patty.crew.app".to_vec()
    );
    assert_ne!(
        super::crew_marker_entry("xyz.patty.crew.app"),
        super::crew_marker_entry("xyz.patty.crew.app.dev")
    );
}
