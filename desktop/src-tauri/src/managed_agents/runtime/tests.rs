...
fn identifier_prefix_does_not_match_longer_id() {
    ...
    let buf = br#""identifier":"xyz.patty.griddle.app.dev""#;
    let id = b"xyz.patty.griddle.app";
...
fn identifier_prefix_does_not_match_worktree_slug() {
    ...
    let buf = br#""identifier":"xyz.patty.griddle.app.dev.my-branch""#;
...
fn identifier_exact_match_at_end_of_buffer() {
    ...
    let buf = b"xyz.patty.griddle.app.dev";
    let id = b"xyz.patty.griddle.app.dev";
    assert!(super::buffer_contains_identifier(buf, id));
}
...
fn longer_id_matches_when_short_prefix_also_present() {
    ...
    let mut buf = b"xyz.patty.griddle.app".to_vec();
    buf.push(0);
    buf.extend_from_slice(br#""identifier":"xyz.patty.griddle.app.dev""#);
    let id = b"xyz.patty.griddle.app.dev";
    assert!(super::buffer_contains_identifier(&buf, id));
}
