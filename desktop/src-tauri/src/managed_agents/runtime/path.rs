...
/// with only Crew's managed Node dirs — no `curl`, `sh`, or `tar`, which
...
///   1. `managed` — Crew-controlled dirs (highest precedence, e.g. managed Node/npm bins)
...
///   2. Crew-managed npm prefix bin dir — app-private ACP adapter shims
///   3. Crew-managed Node.js bin dir — app-private Node/npm runtime
...
/// guards against, which left managed agents unable to find `crew`. Returns
...
mod tests {
    ...
    fn splits_colon_delimited_shell_path() {
        ...
        let result = build_augmented_path(
            ...
            Some(PathBuf::from("/Applications/Crew.app/Contents/MacOS")),
        ...
        assert!(
            result.contains(":/Applications/Crew.app/Contents/MacOS:"),
            "{result}"
        );
        assert!(
    ...
    fn nvm_bin_inserted_after_local_bin_before_exe_parent() {
        ...
        let nvm = result
            ...
            .unwrap();
        let exe = result
            .find("/Applications/Crew.app/Contents/MacOS")
            .unwrap();
    ...
    /// the inherited process PATH — otherwise the child gets only Crew-managed
