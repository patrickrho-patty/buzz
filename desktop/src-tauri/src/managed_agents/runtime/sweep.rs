...
///    intermediates run in their own process groups (e.g. crew-acp ->
///    node shim -> codex-acp).
#[cfg(target_os = "macos")]
...
///
/// On Linux, `read_link("/proc/<pid>/exe")` returns `…/crew-acp (deleted)`
...
/// binary name (`crew-acp`) proceed to the full exe-path fetch.  This mirrors
...
/// processes but at most a handful of `crew-acp` instances).
#[cfg(target_os = "macos")]
...
fn collect_process_snapshots(harness_name: &str) -> Vec<ProcessSnapshot> {
    ...
    for entry in entries.flatten() {
        ...
        // "crew-acp" is 8 chars so it's always preserved).
...
/// Derive the expected path of the `crew-acp` harness binary next to the
/// current executable. Returns `None` if `current_exe()` fails or has no
/// parent directory.
///
/// In a `.app` bundle: `.../Contents/MacOS/crew-acp`.
/// In a dev checkout: `<target-dir>/debug/crew-acp` or similar.
...
const HARNESS_BINARY_NAME: &str = "crew-acp";

// ── sweep_untracked_bundle_harnesses ─────────────────────────────────────

/// Sweep and kill harness processes that share this bundle's exact `crew-acp`
...
/// where possible) equals `<this bundle>/crew-acp` are candidates. Dev builds
...
mod tests {
    ...
    fn strip_deleted_suffix_does_not_strip_partial_match() {
        ...
        let p = PathBuf::from("/some/path/crew-acp(deleted)");
        assert_eq!(strip_deleted_suffix(p.clone()), p,);
    }
