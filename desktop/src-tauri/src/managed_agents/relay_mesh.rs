...
pub const RELAY_MESH_API_KEY_PLACEHOLDER: &str = "crew-mesh-local";
...
/// (`repair_tool_result_answer`) before it gets that far. Crew translates the
/// stored `auto` here rather than teaching crew-agent anything about meshes.
...
/// Translate the native Crew shared compute provider into the OpenAI-compatible
/// transport understood by crew-agent. These are derived runtime details, not
...
pub fn apply_relay_mesh_env(
    ...
    env.insert("CREW_AGENT_PROVIDER".to_string(), "openai".to_string());
    env.insert("CREW_AGENT_MODEL".to_string(), model.clone());
    env.insert(
    ...
    insert_default_if_unset(env, "CREW_AGENT_MAX_OUTPUT_TOKENS", "4096");
    ...
    insert_default_if_unset(env, "CREW_AGENT_REQUIRE_REPLY", "1");
    // Deliberately no CREW_AGENT_THINKING_EFFORT default: mesh translates
...
pub fn relay_mesh_process_env(
    ...
    for key in [
        ...
        // `CREW_AGENT_REQUIRE_REPLY=0` would be silently overridden back to `1`.
        "CREW_AGENT_REQUIRE_REPLY",
    ] {
...
mod tests {
    ...
    fn native_provider_uses_context_safe_tool_calling_budget() {
        ...
        assert_eq!(
            env.get("CREW_AGENT_MAX_OUTPUT_TOKENS").map(String::as_str),
            Some("4096")
        );
        ...
        // output budget).
        assert_eq!(env.get("CREW_AGENT_THINKING_EFFORT"), None);
    }

    /// Stored `auto` is translated here, so crew-agent receives a plain model
    ...
    fn stored_auto_becomes_the_virtual_mesh_model_on_the_wire() {
        ...
        assert_eq!(
            env.get("CREW_AGENT_MODEL").map(String::as_str),
    ...
    fn native_provider_preserves_explicit_generation_controls() {
        let mut env = BTreeMap::from([
            ...
            ("CREW_AGENT_THINKING_EFFORT".to_string(), "high".to_string()),
        ]);
        apply_relay_mesh_env(
            &mut env,
            Some(RELAY_MESH_PROVIDER_ID),
        ...
        assert_eq!(
            env.get("CREW_AGENT_THINKING_EFFORT").map(String::as_str),
            Some("high")
        );
    }
    ...
    fn native_provider_enables_reply_guard_by_default() {
        ...
        assert_eq!(
            env.get("CREW_AGENT_REQUIRE_REPLY").map(String::as_str),
            Some("1"),
    ...
    fn native_provider_preserves_explicit_reply_guard_opt_out() {
        let mut env = BTreeMap::from([("CREW_AGENT_REQUIRE_REPLY".to_string(), "0".to_string())]);
        apply_relay_mesh_env(
            &mut env,
            Some(RELAY_MESH_PROVIDER_ID),
            Some(RELAY_MESH_AUTO_MODEL_ID),
    ...
    fn non_mesh_provider_leaves_reply_guard_unset() {
        ...
        assert_eq!(
            env.get("CREW_AGENT_REQUIRE_REPLY"),
            None,
    ...
    fn process_env_preserves_explicit_reply_guard_opt_out() {
        let effective_env =
            BTreeMap::from([("CREW_AGENT_REQUIRE_REPLY".to_string(), "0".to_string())]);
