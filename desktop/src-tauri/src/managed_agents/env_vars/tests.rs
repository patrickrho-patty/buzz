...
fn merged_env_strips_reserved_keys_from_persona() {
    ...
    let persona = map(&[
        ("CREW_PRIVATE_KEY", "nsec1evil"),
        ("ANTHROPIC_API_KEY", "ok"),
    ]);
    let merged = merged_user_env(&persona, &BTreeMap::new());
    assert!(!merged.contains_key("CREW_PRIVATE_KEY"));
    assert_eq!(
        merged.get("ANTHROPIC_API_KEY").map(String::as_str),
        Some("ok")
    );
...
fn merged_env_strips_reserved_keys_from_agent() {
    ...
    assert!(!merged.contains_key("CREW_AUTH_TAG"));
    assert_eq!(merged.get("FOO").map(String::as_str), Some("1"));
    assert_eq!(merged.len(), 1);
}
...
fn is_reserved_recognises_full_list() {
    ...
    assert!(!is_reserved_env_key("CREW_ACP_MODEL")); // behavior knob
}

#[test]
fn reserved_keys_include_agent_owner_for_legacy_records() {
    // Legacy records without auth_tag fall back to CREW_ACP_AGENT_OWNER
    // to enforce the respond-to gate. Must not be user-overridable.
    assert!(is_reserved_env_key("CREW_ACP_AGENT_OWNER"));
    let agent = map(&[("CREW_ACP_AGENT_OWNER", "imposter")]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(merged.is_empty());
}
...
fn reserved_keys_include_remote_lifetime_policy() {
    for key in [
        ...
        "CREW_ACP_IDLE_POOL_SLEEP",
        "CREW_ACP_NO_PRESENCE",
    ] {
        assert!(is_reserved_env_key(key), "{key} should be reserved");
        let agent = map(&[(key, "0")]);
...
fn reserved_keys_include_relay_url() {
    ...
    assert!(is_reserved_env_key("CREW_RELAY_URL"));
    let agent = map(&[("CREW_RELAY_URL", "ws://attacker.example")]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(merged.is_empty());
}
...
fn validate_keys_rejects_reserved() {
    let env = map(&[("CREW_PRIVATE_KEY", "nsec1evil")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("CREW_PRIVATE_KEY"), "got: {err}");
...
fn validate_keys_lists_all_reserved_keys_found() {
    ...
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("CREW_PRIVATE_KEY"));
...
// `c.env("CREW_AUTH_TAG=x", "forged")` results in the child seeing
// `CREW_AUTH_TAG=x=forged` and `getenv("CREW_AUTH_TAG") == "x=forged"`.
...
fn is_well_formed_rejects_malformed_keys() {
    for key in [
        "",                  // empty
        "=",                 // bare equals
        "CREW_AUTH_TAG=x",   // =-in-key bypass
        "CREW_PRIVATE_KEY=", // trailing equals
        "FOO BAR",           // space
...
fn validate_keys_rejects_equals_in_key_bypass() {
    // The actual exploit: `CREW_AUTH_TAG=x` smuggles a value past
    // the reserved-key string compare and into the child's environ.
    let env = map(&[("CREW_AUTH_TAG=x", "forged")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("[A-Za-z_]"), "got: {err}");
    ...
    assert!(err.contains("CREW_AUTH_TAG"), "got: {err}");
...
fn validate_keys_reports_malformed_before_reserved() {
    ...
    // from churning.
    let env = map(&[("CREW_AUTH_TAG=x", "v"), ("CREW_PRIVATE_KEY", "v")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("[A-Za-z_]"), "got: {err}");
    assert!(!err.contains("reserved"), "got: {err}");
}
...
fn merged_env_drops_malformed_keys() {
    ...
    let agent = map(&[
        ("CREW_AUTH_TAG=x", "forged"),
        ("FOO=bar", "v"),
        ("LEGIT", "ok"),
    ]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(!merged.contains_key("CREW_AUTH_TAG=x"));
...
// CREW_AGENT_MODEL, CREW_AGENT_PROVIDER) so they don't shadow the
...
fn is_derived_key_does_not_match_unrelated_keys() {
    ...
    assert!(!is_derived_provider_model_key("CREW_PRIVATE_KEY"));
