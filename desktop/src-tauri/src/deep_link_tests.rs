...
fn parse_entity_deep_link_rejects_malformed_and_non_canonical_links() {
    ...
    for raw in [
        ...
        "crew://repo?owner=nope&d=buzz-world".to_owned(),
        format!("crew://repo?owner={owner}&d=.hidden"),
        format!("crew://repo?owner={owner}&d=has%20space"),
        format!("crew://pr?owner={owner}&d=buzz-world"),
        format!("crew://pr?id=short&owner={owner}&d=buzz-world"),
        // Coordinate links take no event id.
        format!("crew://repo?id={event_id}&owner={owner}&d=buzz-world"),
        // Non-canonical: unknown param, duplicate param, path, fragment.
        format!("crew://repo?owner={owner}&d=buzz-world&relay=wss%3A%2F%2Fx.example"),
        format!("crew://repo?owner={owner}&owner={owner}&d=buzz-world"),
        // Unknown tab value, duplicate tab, and tab on an event link.
        format!("crew://repo?owner={owner}&d=buzz-world&tab=overview"),
        format!("crew://repo?owner={owner}&d=buzz-world&tab=prs&tab=prs"),
        format!("crew://repo?owner={owner}&d=buzz-world&tab=files&commit={event_id}"),
        format!("crew://repo?owner={owner}&d=buzz-world&tab=commits&commit=short"),
        format!("crew://pr?id={event_id}&owner={owner}&d=buzz-world&tab=prs"),
        format!("crew://repo/extra?owner={owner}&d=buzz-world"),
        format!("crew://repo?owner={owner}&d=buzz-world#top"),
        // Not an entity host.
        format!("crew://message?owner={owner}&d=buzz-world"),
    ] {
        assert!(
...
fn pending_entity_links_dedupe_launch_and_open_callbacks() {
    let queue = PendingEntityDeepLinks::default();
    let href = "crew://project?owner=aa&d=buzz".to_owned();
...
fn parse_add_community_deep_link_extracts_relay_and_name() {
    let url = Url::parse(
        "crew://add-community?relay=wss%3A%2F%2Facme.communities.buzz.xyz&name=Acme%20Team&ignored=value",
    )
    .unwrap();
...
fn parse_nostr_bind_deep_link_rejects_missing_challenge_id() {
    let url = Url::parse("crew://nostr-bind?nonce=ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi01234567&verification_code=123456&audience=buzz%3Anostr-identity&action=bind_nostr_identity&protocol=buzz-nostr-identity&version=1&origin=https%3A%2F%2Fexample.com&expires_at=2999-01-01T00%3A00%3A00Z&return=clipboard").unwrap();
    assert!(parse_nostr_bind_deep_link(&url).is_err());
}
