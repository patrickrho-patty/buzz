...
//!
//! Mirrors the crew-sdk builder patterns but uses nostr 0.37 API
...
use crew_core_pkg::kind::{KIND_IA_ARCHIVE_REQUEST, KIND_IA_UNARCHIVE_REQUEST};
...
/// Maximum content size — matches crew-sdk (64 KiB).
const MAX_CONTENT_BYTES: usize = 64 * 1024;

/// Maximum mention count — matches crew-sdk.
const MAX_MENTIONS: usize = 50;

/// Maximum emoji length in characters — matches crew-sdk.
...
pub fn build_create_channel(
    ...
    let name = crew_sdk_pkg::canonical_channel_name(name);
...
pub fn build_update_channel(
    ...
    let name = name.map(crew_sdk_pkg::canonical_channel_name);
