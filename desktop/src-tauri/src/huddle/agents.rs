...
The channel UUID in the current `[Context]` block is the live huddle channel. Only messages sent with `crew messages send` to that current Context channel are spoken aloud, in the order sent; everything else you produce is silent.
        ...
        match submit_event(add_parent, state).await {
            ...
            Err(e) => {
                ...
                if active_after_error {
                    ...
                    eprintln!("crew-desktop: add agent to parent channel failed: {e}");
                    (false, Some(e))
                }
            }
        }
...
mod tests {
    ...
    fn voice_mode_guidelines_pin_spoken_reply_as_first_tool_call() {
        ...
        assert!(guidelines.contains("crew messages send` to that current Context channel"));
