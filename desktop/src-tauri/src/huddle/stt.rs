...
fn stt_worker(
    ...
    let mut resampler = match Fft::<f32>::new(48_000, 16_000, 1024, 2, 1, FixedSync::Input) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("crew-desktop: STT resampler init failed: {e}");
            return;
        }
    };
    ...
    if !tokens_path.exists() || !model_path.exists() {
        eprintln!(
            "crew-desktop: STT model not found at {} — STT disabled",
            model_dir.display()
        );
    ...
    let recognizer = match OfflineRecognizer::create(&cfg) {
        Some(r) => r,
        None => {
            eprintln!("crew-desktop: OfflineRecognizer::create returned None — STT disabled");
            drain_until_shutdown(audio_rx, &shutdown);
            return;
        }
    };
...
fn resample_chunk(resampler: &mut rubato::Fft<f32>, chunk_48k: &[f32]) -> Vec<f32> {
    ...
    let input = match InterleavedSlice::new(chunk_48k, 1, chunk_48k.len()) {
        Ok(a) => a,
        Err(e) => {
            eprintln!("crew-desktop: STT resample input error: {e}");
            return Vec::new();
        }
    };
    ...
    match resampler.process(&input, 0, None) {
        ...
        Err(e) => {
            eprintln!("crew-desktop: STT resample error: {e}");
            Vec::new()
        }
    }
}
...
fn flush_to_stt(
    ...
    if !has_enough_voiced_audio(voiced_frames) {
        eprintln!(
            "crew-desktop: STT dropped short VAD segment ({voiced_frames}/{MIN_VOICED_FRAMES} voiced frames)"
        );
        return;
    }
...
fn send_transcript(text: String, text_tx: &tokio_mpsc::Sender<String>) {
    if !text.is_empty() {
        if let Err(e) = text_tx.blocking_send(text) {
            eprintln!("crew-desktop: STT text channel closed: {e}");
        }
    }
}
