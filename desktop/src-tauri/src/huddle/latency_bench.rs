...
fn baseline_stt_fake_llm_tts_first_audio() {
    ...
    let wav_path = std::env::var("CREW_BENCH_WAV").expect("set CREW_BENCH_WAV");
    ...
    let llm_delay_ms: u64 = std::env::var("CREW_BENCH_LLM_MS")
        .ok()
    ...
    let turns: usize = std::env::var("CREW_BENCH_TURNS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(6);
