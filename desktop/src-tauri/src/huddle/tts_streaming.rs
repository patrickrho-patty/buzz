...
//!
//! `CREW_TTS_STREAMING=1` streams PCM deltas out of Pocket as they are
//! generated instead of waiting for the full first-chunk synthesis.
//! `CREW_TTS_EMIT_FRAMES` tunes the delta size in Flow LM frames (80 ms of
...
/// when `CREW_TTS_STREAMING=1`, `None` for the production batch path.
pub(super) fn streaming_emit_frames() -> Option<usize> {
    std::env::var("CREW_TTS_STREAMING")
        .is_ok_and(|v| v == "1")
        .then(|| {
            std::env::var("CREW_TTS_EMIT_FRAMES")
                .ok()
