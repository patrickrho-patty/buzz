...
//!
//! Mental model:
//!   app launch → start_stt_download (background) → ~/.crew/models/parakeet-tdt-ctc-110m-en/
//!   app launch → start_tts_download (background) → ~/.crew/models/pocket-tts/
//!   STT pipeline → is_stt_ready() → stt_model_dir() → run inference
//!   TTS pipeline → is_tts_ready() → tts_model_dir() → run synthesis
//!
//! Models are downloaded once and cached. A version manifest (`.crew-model-manifest`)
...
//! `~/.crew/models/moonshine-tiny/` is removed best-effort once the new STT
...
const MANIFEST_FILENAME: &str = ".crew-model-manifest";
...
(https://github.com/k2-fsa/sherpa-onnx); Crew ships this conversion
unmodified.
...
#[derive(Clone)]
struct ModelSlot {
    dir_name: &'static str,                  // subdir under ~/.crew/models/
    ...
    fn recover_interrupted_install(&self, models_dir: &Path) {
        ...
        if self.is_ready(models_dir) {
            if let Err(error) = std::fs::remove_dir_all(&backup_dir) {
                eprintln!(
                    "crew-desktop: could not remove stale {} backup: {error}",
                    self.dir_name
                );
            }
            return;
        ...
        if final_dir.exists() {
            if let Err(error) = std::fs::remove_dir_all(&final_dir) {
                eprintln!(
                    "crew-desktop: could not remove incomplete {} install: {error}",
                    self.dir_name
                );
                return;
            }
        ...
        if let Err(error) = std::fs::rename(&backup_dir, &final_dir) {
            eprintln!(
                "crew-desktop: could not restore interrupted {} install: {error}",
                self.dir_name
            );
        }
    }
        ...
        Fut: std::future::Future<Output = Result<(), String>> + Send,
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = download_fn(http_client).await {
                eprintln!("crew-desktop: {name} download failed: {e}");
...
impl ModelManager {
    /// Create a new `ModelManager` rooted at `~/.crew/models/`.
    ///
    /// Returns `None` if the home directory cannot be resolved.
    pub fn new() -> Option<Self> {
        let models_dir = dirs::home_dir()?.join(".crew").join("models");
        let manager = Self {
            models_dir,
    ...
    pub fn start_tts_download(&self, http_client: reqwest::Client) {
        if let Err(error) = voice_upgrade::install_vctk_presets_into_v4_model(&self.models_dir) {
            eprintln!(
                "crew-desktop: could not upgrade existing Pocket voices in place: {error}"
            );
        }
        let manager = self.clone();
    ...
    async fn download_stt_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        eprintln!("crew-desktop: downloading STT model from {STT_DOWNLOAD_URL}");
        ...
        .await?;
        eprintln!("crew-desktop: downloaded {bytes} bytes, wrote to disk");
        ...
        eprintln!("crew-desktop: extracting STT archive…");
        ...
        eprintln!(
            "crew-desktop: STT model ready at {}",
    ...
    async fn download_tts_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        for (i, (url, artifact)) in downloads.iter().enumerate() {
            ...
            eprintln!("crew-desktop: downloading Pocket TTS {filename} from {url}");
            ...
            eprintln!("crew-desktop: downloaded {bytes} bytes ({filename}), wrote to disk");
        ...
        eprintln!(
            "crew-desktop: Pocket TTS model ready at {}",
...
///
/// Removes `~/.crew/models/moonshine-tiny/` if present (~70 MB on disk).
...
async fn cleanup_legacy_moonshine_dir(models_dir: &Path) {
    ...
    match tokio::fs::remove_dir_all(&legacy).await {
        Ok(()) => eprintln!(
            "crew-desktop: removed legacy STT model dir {}",
            legacy.display()
        ),
        Err(e) => eprintln!(
            "crew-desktop: could not remove legacy STT model dir {}: {e} \
