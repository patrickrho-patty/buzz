...
pub(super) fn reconcile_selected_voice(
    ...
    match load_voice_style(&requested_path) {
        ...
        Err(_) => {
            eprintln!("griddle-desktop: tts stage=voice_switch status=fallback reason=voice_style");
            ...
            match load_voice_style(&fallback_path) {
                ...
                Err(_) => {
                    eprintln!(
                        "griddle-desktop: tts stage=voice_switch status=failed reason=fallback_voice_style"
                    );
                    false
                }
            }
...
pub(super) fn reconcile_queued_voice(
    ...
    match load_voice_style(&voice_path(model_dir, requested_voice)) {
        ...
        Err(_) => {
            eprintln!(
                "griddle-desktop: tts stage=agent_voice_switch status=fallback reason=voice_style"
            );
...
pub(super) fn handle_cancel_or_shutdown(
    ...
    if shutdown.load(Ordering::Acquire) {
        eprintln!(
            "griddle-desktop: tts stage=cancellation reason=shutdown route_id={}",
    ...
    if cancel.load(Ordering::Acquire) || voice_cancel.load(Ordering::Acquire) {
        ...
        eprintln!(
            "griddle-desktop: tts stage=cancellation reason={} route_id={}",
