...
//! GPU/driver/compositor combinations, so Crew comes up with no window at all
//! and the user has no way to fix it (#2338, upstream tauri#9394).
//!
//! Historically Crew set `WEBKIT_DISABLE_DMABUF_RENDERER=1`, which used to fall
...
/// process is still single threaded, which it is nowhere else in Crew.
///
...
pub fn apply() -> Result<(), String> {
    match plan(
        ...
        Plan::Apply { vars, why } => {
            ...
            eprintln!("crew-desktop: {} — {why}", applied.join(" "));
            Ok(())
        }
        Plan::Leave { why } => {
            eprintln!("crew-desktop: WebKit rendering left as-is — {why}");
            Ok(())
        }
