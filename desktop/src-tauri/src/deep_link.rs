...
/// Parse the query string of a `crew://message?…` URL into the JSON
...
/// Parse the query string of a `crew://join?…` URL into the JSON payload
...
/// Hosts of the `crew://` git-entity links built by
...
/// Validate a `crew://repo|project|pr|issue?…` link and return it verbatim
...
///
/// Workspace tabs addressable by `crew://repo|project` links — mirrors
...
/// Handle an incoming `crew://` deep link URL.
///
/// Currently supports:
/// - `crew://connect?relay=<ws(s)://...>` — emits `deep-link-connect` to the frontend
/// - `crew://repo|project|pr|issue?…` — emits `deep-link-entity` to the frontend
pub(crate) fn handle_deep_link_url(app: &tauri::AppHandle, url_str: &str) {
    ...
    match url.host_str() {
        ...
        Some("join") => {
            // `crew://join?relay=<ws(s)://...>&code=<invite code>` — fired by
        ...
        Some("repo" | "project" | "pr" | "issue") => {
            // `crew://repo|project?owner=<pubkey>&d=<dtag>` and
            // `crew://pr|issue?id=<eventId>&owner=<pubkey>&d=<dtag>` — the
