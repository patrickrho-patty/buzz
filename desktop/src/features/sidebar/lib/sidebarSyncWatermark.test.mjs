...
test("readWatermark: returns 0 when stored value is 0", () => {
  withFreshStorage((ls) => {
    ls.setItem(`crew-sync-watermark.v1:sections:pk:${RELAY_ENCODED}`, "0");
    assert.equal(readWatermark("pk", "sections", RELAY), 0);
  });
});
...
test("readWatermark: returns stored positive integer", () => {
  withFreshStorage((ls) => {
    ls.setItem(
      `crew-sync-watermark.v1:sections:pk:${RELAY_ENCODED}`,
      "1700000000",
    );
...
test("readWatermark: scopes by blobType", () => {
  withFreshStorage((ls) => {
    ls.setItem(`crew-sync-watermark.v1:sections:pk:${RELAY_ENCODED}`, "100");
    ls.setItem(`crew-sync-watermark.v1:sort:pk:${RELAY_ENCODED}`, "200");
