...
test("storageKey: returns expected format with pubkey", () => {
  assert.equal(storageKey("abc123"), "crew-channel-sections.v1:abc123");
});
...
test("storageKey: with relayUrl includes normalized+encoded relay in key", () => {
  ...
  assert.equal(
    key,
    `crew-channel-sections.v1:pk1:${encodeURIComponent(normalizeRelayUrl(relay))}`,
  );
});

test("storageKey: without relayUrl returns legacy pubkey-only key", () => {
  assert.equal(storageKey("pk1"), "crew-channel-sections.v1:pk1");
  assert.equal(storageKey("pk1", undefined), "crew-channel-sections.v1:pk1");
});
