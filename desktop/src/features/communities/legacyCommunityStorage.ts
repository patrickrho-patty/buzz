...
export function applyLegacyCommunityStorage(
  ...
  const currentCommunitiesRaw = storage.getItem(CREW_COMMUNITIES_KEY);
  ...
  if (shouldWriteCommunities && legacyStorage.workspaces) {
    storage.setItem(CREW_COMMUNITIES_KEY, legacyStorage.workspaces);
  }

  const currentActiveCommunityId = storage.getItem(CREW_ACTIVE_COMMUNITY_KEY);
  if (
    legacyStorage.activeWorkspaceId &&
    (!currentActiveCommunityId || shouldWriteCommunities)
  ) {
    storage.setItem(CREW_ACTIVE_COMMUNITY_KEY, legacyStorage.activeWorkspaceId);
  }

  for (const completion of legacyStorage.onboardingCompletions) {
    const key = `${CREW_ONBOARDING_COMPLETION_STORAGE_KEY_PREFIX}${completion.pubkey}`;
