...
export function HuddleParticipantsControl({
  ...
  const participantDetails = (
    <PopoverContent
      align="end"
      className="crew-huddle-drawer crew-huddle-popover w-72 p-3 text-foreground"
  ...
  const participantStrip = (
    ...
    >
      {visibleIdentities.map((participant) =>
        appearance === "room" ? (
          <div
            className="crew-huddle-participant-tile relative flex w-28 shrink-0 flex-col items-center justify-center gap-1.5 rounded-xl border border-border/70 bg-muted/45 px-3 py-3"
          ...
          <AgentVoiceMenu
            agentPubkey={participant.pubkey}
            contentAlign="start"
            contentClassName="crew-huddle-drawer crew-huddle-popover text-foreground"
            contentSide="top"
            displayName={participant.displayName}
            key={participant.pubkey}
            onRemoveAgent={
        ...
        <PopoverTrigger asChild>
          <Button
            ...
            className={cn(
              ...
              appearance === "room"
                ? "crew-huddle-participant-tile min-h-[6.375rem] min-w-28 rounded-xl border border-border/70 bg-muted/45 text-foreground/70 hover:bg-muted/65 hover:text-foreground"
...
function ParticipantAvatar({
  ...
  const speakerStyle = {
    "--crew-huddle-speaker-opacity":
      speakerLevel > 0.04 ? (0.45 + speakerLevel * 0.55).toFixed(3) : "0",
    "--crew-huddle-speaker-scale": (1.02 + speakerLevel * 0.14).toFixed(3),
  } as React.CSSProperties;

  return (
    <span
      className={cn(
        "crew-huddle-speaking-avatar relative z-0 inline-flex shrink-0 rounded-full",
        sizeClass,
      )}
