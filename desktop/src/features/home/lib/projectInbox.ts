...
/** Returns the canonical Crew Git repository and root event for an Inbox row. */
...
/** Whether a feed event belongs to a Crew Git pull request or issue thread. */
export function isProjectInboxItem(item: FeedItem) {
  return getProjectInboxReference(item) !== null;
}

/** Resolves an Inbox event to the current canonical Crew Git work item. */
