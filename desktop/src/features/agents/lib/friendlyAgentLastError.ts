 ...
 *   crew-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/crew-agent/src/types.rs`.
 *   crew-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped crew-agent prefix or the crew-acp v0 wrap.
  if (
