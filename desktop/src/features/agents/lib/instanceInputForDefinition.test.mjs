...
test("mapping carries the runtime and definition fields", async () => {
  ...
  assert.equal(input.acpCommand, "crew-acp");
...
test("Crew shared compute definition carries native provider and auto model", async () => {
  const input = await buildInstanceInputForDefinition(
    persona({
      runtime: "crew-agent",
      provider: "relay-mesh",
    ...
    { ...gooseRuntime, id: "crew-agent", command: "crew-agent" },
  );
  assert.equal(input.agentCommand, "crew-agent");
...
// ── item-13 regression: crew-agent-first default runtime ─────────────────────
//
// Before this fix, resolveStartRuntimeForDefinition used runtimes[0] (catalog
// order: goose, claude, codex, crew-agent), so an installed goose would beat
// the bundled crew-agent sidecar as the default for runtime-less personas.
// The fix applies the preference order: crew-agent → goose → first available.
...
test("item-13: goose+crew-agent both available — persona with no runtime resolves crew-agent", () => {
  ...
  assert.equal(
    runtime.id,
    "crew-agent",
    "crew-agent must win over catalog-first goose for runtime-less personas",
  );
  assert.deepEqual(warnings, []);
});
