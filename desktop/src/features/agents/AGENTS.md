...
Plan of record: `Crew/Harness-Provider-Model.md` in Morgan's Obsidian vault
...
2. **Effort reads/writes go through the descriptor.** Use the effort
   ...
   `CREW_AGENT_THINKING_EFFORT` literal in UI code. `currentPersistence` is
...
7. **Onboarding setup detects readiness; it does not select defaults.** The
   ...
   requires provider/model/credential config (e.g. crew-agent with no
...
9. **The defaults modal is progressively disclosed.** An unset global config
   starts on the Crew Agent-first deployment fallback and carries that visible
   harness into the next saved edit. The `progressive-defaults` disclosure
   preset therefore begins at Provider for Crew Agent, then reveals Model,
...
11. **Shared agent access names the consequence where it is selected.** The
   ...
   installed `crew-backend-*` provider, and without one `WhereToRunSection`
...
14. **Thinking effort has two surfaces: a local-only WRITE control and a
   ...
   **Cut invariant — live mid-conversation effort machinery was deliberately
   ...
   read from `CREW_ACP_EFFORT_LEVEL` and applies it once at session creation
   (`apply_startup_effort` in `crew-acp/src/pool.rs`); there is no pool-level
