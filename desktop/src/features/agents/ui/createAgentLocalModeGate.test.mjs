...
test("localMode_crewAgent_supportsProviderSelection", () => {
  assert.equal(
    ...
    true,
    "crew-agent must support LLM provider selection",
  );
});
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("crew-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for crew-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("crew-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Crew env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("crew-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("crew-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("crew-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("crew-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("crew-agent", "relay-mesh"),
    "Use agent defaults (Crew shared compute)",
  );
});
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("crew-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("crew-agent", "");
...
// include ANTHROPIC_API_KEY for explicit crew-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("crew-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "crew-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("crew-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("crew-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("crew-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "crew-agent + databricks must list DATABRICKS_HOST as required",
  );
