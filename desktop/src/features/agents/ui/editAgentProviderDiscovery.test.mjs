...
test("editAgent_providerFieldVisible_forBuzzAgent", () => {
  assert.equal(
    ...
    true,
    "crew-agent runtime must expose the provider picker",
  );
});
...
test("editAgent_providerOptions_includesDatabricksV2AndV1OnOSS", () => {
  ...
  const options = getPersonaProviderOptions("", "crew-agent");
...
test("editAgent_providerOptions_includesCurrentIfCustom", () => {
  const options = getPersonaProviderOptions("my-custom-llm", "crew-agent");
  const ids = options.map((o) => o.id);
  assert.ok(
...
test("editAgent_modelClearedOnRuntimeChange", () => {
  const previousRuntime = "crew-agent";
...
test("editAgent_catalogArrival_rederivesRuntimeIdWhenNotTouched", () => {
  ...
  const agentCommand = "/usr/local/bin/crew-agent";
  const catalog = [
    { id: "crew-agent", command: agentCommand, defaultArgs: [] },
...
test("editAgent_noOpSavePreservesProvider_whenCatalogLate", () => {
  ...
  // If the catalog-arrival effect correctly sets selectedRuntimeId = "crew-agent",
  // then llmProviderFieldVisible = true and the provider is preserved on save.
  const selectedRuntimeId = "crew-agent"; // correctly derived after catalog arrival
  const savedProvider = "databricks_v2";
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible once catalog derives crew-agent runtime",
  );
  assert.equal(
    providerUpdate,
    undefined,
...
test("editAgent_runtimeDropdown_pinsHarnessWhenConcreteCatalogRuntimeSelected", () => {
  ...
  const _nextRuntimeId = "crew-agent";
  const catalogRuntime = {
    id: "crew-agent",
    command: "/usr/local/bin/crew-agent",
    defaultArgs: [],
  };
...
test("editAgent_runtimeDropdown_keepsInheritWhenCatalogEntryHasNoCommand", () => {
  ...
  const NO_RUNTIME_DROPDOWN_VALUE = "__none__";
  const nextValue = "crew-agent"; // catalog entry, but adapter missing
  const nextRuntimeId =
    nextValue === NO_RUNTIME_DROPDOWN_VALUE ? "" : nextValue;
  const resolvedRuntimeId = nextRuntimeId || "custom";
  const nextRuntime = { id: "crew-agent", command: null, defaultArgs: [] };
...
test("editAgent_missingRequiredEnvKey_blocksSaveViaValidity", () => {
  ...
  const base = {
    ...
    selectedRuntimeId: "crew-agent",
    inheritHarness: false,
    agentCommand: "crew-agent",
...
test("editAgent_inheritedAgentRuntimeSwitch_producesConsistentCommandProviderPair", () => {
  ...
  //
  // After fix: selecting crew-agent sets inheritHarness=false, so agentCommandUpdate
  // resolves to the crew-agent command, and provider persists consistently.

  // Initial state: inherited Claude agent
  const inheritHarness = false; // after fix: pinned by runtime switch
  const selectedRuntimeCommand = "/usr/local/bin/crew-agent";
  ...
  const selectedRuntimeId = "crew-agent";
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
...
test("editAgent_inheritCheckboxRoundTrip_doesNotPersistProviderOnInheritedRuntime", () => {
  ...
  // → user picks crew-agent (inheritHarness→false, selectedRuntimeId='crew-agent')
  // → user picks databricks_v2
  // → user RE-CHECKS inherit (inheritHarness→true, selectedRuntimeId STAYS 'crew-agent')
  ...
  const runtimes = [
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
    { id: "crew-agent", command: "/usr/local/bin/crew-agent", defaultArgs: [] },
  ];
  const selectedRuntimeId = "crew-agent"; // dropdown state (stale after re-check)
  ...
  const chosenProvider = "databricks_v2"; // chosen while dropdown was crew-agent

  // llmProviderFieldVisible is driven by the live dropdown (crew-agent → true).
  ...
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field is visible (dropdown shows crew-agent) — this is the UX state",
  );
...
test("editAgent_inheritCheckboxRoundTrip_clearsStaleSavedProviderWhenRevertingToInherit", () => {
  // Variant: agent previously had a provider saved (e.g. was pinned to crew-agent
  ...
  const selectedRuntimeId = "crew-agent"; // dropdown state
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
// persona's runtime is crew-agent/Goose legitimately carries a provider
...
test("editAgent_inheritedBuzzAgentProvider_preservedOnNameOnlySave", () => {
  ...
  const agentCommand = "/usr/local/bin/crew-agent"; // inherited crew-agent command
  const runtimes = [
    { id: "crew-agent", command: "/usr/local/bin/crew-agent", defaultArgs: [] },
    { id: "claude", command: "/usr/local/bin/claude", defaultArgs: [] },
  ];
  const selectedRuntimeId = "crew-agent"; // correctly derived by catalog-arrival effect
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
  const savedProvider = "databricks_v2"; // valid provider snapshot
  const currentProvider = "databricks_v2"; // unchanged by user

  // llmProviderFieldVisible (UX) is true since dropdown shows crew-agent.
  const llmProviderFieldVisible =
    runtimeSupportsLlmProviderSelection(selectedRuntimeId);
  assert.equal(
    llmProviderFieldVisible,
    true,
    "provider field must be visible for inherited crew-agent",
  );

  // The effective runtime for submit: inherited → match agentCommand in catalog → crew-agent.
  const llmProviderCanPersistAtSubmit = computeCanPersistAtSubmit({
    inheritHarness,
    agentCommand,
    runtimes,
  ...
  assert.equal(
    llmProviderCanPersistAtSubmit,
    true,
    "inherited crew-agent runtime IS provider-capable — provider must be persistable",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited crew-agent must omit provider (not send null to clear it)",
  );
});
...
test("editAgent_findingE_emptyCatalog_providerOmittedNotCleared", () => {
  ...
  // Inherited crew-agent agent with saved databricks_v2.
  // A name-only save must omit provider (not send null).

  const inheritHarness = true;
  const agentCommand = "crew-agent"; // inherited command (short form)
...
test("editAgent_findingE_commandNullCatalogEntry_providerPreservedByIdMatch", () => {
  // Form 2: catalog loaded, but crew-agent's entry has command:null (adapter
  ...
  const inheritHarness = true;
  const agentCommand = "crew-agent"; // inherited command (short form = runtime id)
  const runtimes = [
    { id: "crew-agent", command: null, defaultArgs: [] }, // adapter missing
  ...
  assert.equal(
    capability,
    "capable",
    "command:null crew-agent entry must resolve to 'capable' via id-based fallback",
  );
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "name-only save on inherited crew-agent (command:null) must omit provider, not clear it",
  );
});
...
test("editAgent_findingE_lockedRuntimeStillClears", () => {
  ...
  const runtimes = [
    { id: "crew-agent", command: "crew-agent", defaultArgs: [] },
    { id: "claude", command: "claude-agent-acp", defaultArgs: [] },
  ];
  const selectedRuntimeId = "crew-agent"; // stale dropdown state (irrelevant)
  const selectedRuntime = runtimes.find((r) => r.id === selectedRuntimeId);
...
test("editAgent_findingE_capableBuzzAgentLoadedCatalog_preservedOnNoOpSave", () => {
  ...
  assert.equal(capability, "capable", "loaded crew-agent must be 'capable'");
  ...
  assert.equal(
    providerUpdate,
    undefined,
    "no-op save on inherited crew-agent (loaded catalog) must omit provider",
  );
});
...
test("editAgent_bugA_inheritedShortName_resolvesViaIdFallback", () => {
  // The core regression: agentCommand is the short name "crew-agent" but the
  // catalog's command is the resolved binary path. Command match fails; id
  // fallback must rescue it.

  const agentCommand = "crew-agent"; // short name stored by effective_agent_command
  const runtimes = [
    {
      id: "crew-agent",
  ...
  assert.equal(
    selectedRuntimeId,
    "crew-agent",
    "short-name agentCommand must resolve to crew-agent id via id-fallback when command path differs",
  );
});
...
test("editAgent_bugA_inheritedShortName_commandMatchStillWinsWhenPresent", () => {
  ...
  const agentCommand = "/usr/local/bin/crew-agent"; // full path (explicit pin)
  const runtimes = [
    {
      id: "crew-agent",
      command: "/usr/local/bin/crew-agent",
...
test("editAgent_bugA_inheritedShortName_discoveryGatePasses", () => {
  // Once selectedRuntimeId is correctly resolved to "crew-agent" via id-fallback,
  ...
  const agentCommand = "crew-agent"; // short name
  const runtimes = [
    {
      id: "crew-agent",
  ...
  assert.equal(
    selectedRuntimeId,
    "crew-agent",
    "id-fallback must resolve selectedRuntimeId to crew-agent",
  );
  assert.ok(
  ...
  assert.equal(
    ...
    true,
    "discovery gate must pass for inherited crew-agent with databricks_v2 provider once id-fallback resolves the runtime",
  );
});
...
test("requiredCredentialEnvKeys: crew-agent + anthropic → ANTHROPIC_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("crew-agent", "anthropic");
  assert.deepEqual(keys, ["ANTHROPIC_API_KEY"]);
});

test("requiredCredentialEnvKeys: crew-agent + openai → OPENAI_COMPAT_API_KEY", () => {
  const keys = requiredCredentialEnvKeys("crew-agent", "openai");
  assert.deepEqual(keys, ["OPENAI_COMPAT_API_KEY"]);
});

test("requiredCredentialEnvKeys: crew-agent + databricks → DATABRICKS_HOST only (no token)", () => {
  const keys = requiredCredentialEnvKeys("crew-agent", "databricks");
...
test("requiredCredentialEnvKeys: crew-agent + databricks_v2 → DATABRICKS_HOST only", () => {
  const keys = requiredCredentialEnvKeys("crew-agent", "databricks_v2");
  assert.deepEqual(keys, ["DATABRICKS_HOST"]);
});
...
test("requiredCredentialEnvKeys: crew-agent + no provider → empty (provider not yet selected)", () => {
  const keys = requiredCredentialEnvKeys("crew-agent", "");
...
//   FALSE-ALLOW: claude pin → inherit crew-agent persona → missing ANTHROPIC_API_KEY
//     → must be BLOCKED (prospective runtime is crew-agent/anthropic, key absent)
...
test("blockSave_inheritTransition_claudePin_toBuzzAgentPersona_missingKey_blocked", () => {
  ...
  // prospectiveRuntimeId resolves to "crew-agent"; providerForRequiredKeys must
  // use the PROSPECTIVE runtime's provider-field visibility (crew-agent supports
  // provider selection) rather than the current locked runtime's suppression.
  const prospectiveRuntimeId = "crew-agent"; // resolved from persona's agentCommand
  ...
  // providerForRequiredKeys must be "anthropic" (crew-agent supports selection)
  ...
  assert.equal(
    missing,
    true,
    "inheriting crew-agent/anthropic persona with no key must BLOCK save (false-allow prevented)",
  );
});
