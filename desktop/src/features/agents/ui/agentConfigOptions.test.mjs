...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "crew-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "crew-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "crew-agent");
});

test("getDefaultPersonaRuntime returns crew-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("crew-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "crew-agent");
});

test("getDefaultPersonaRuntime falls back to goose when crew-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("crew-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither crew-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("crew-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for crew-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("crew-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { CREW_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
...
test("getPersonaModelOptions for crew-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("crew-agent", "anthropic");
...
test("getPersonaModelOptions for crew-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("crew-agent", "");
