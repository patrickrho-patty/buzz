...
test("editAgent_globalProvider_globalApiKey_noPerAgentEnv_notMissing", () => {
  // Setup: crew-agent runtime, provider = anthropic (from global), key in globalEnvVars.
  ...
  assert.ok(
    ...
    "ANTHROPIC_API_KEY must appear in raw required keys for crew-agent/anthropic",
  );
...
test("editAgent_globalProvider_globalApiKeyEmpty_stillMissing", () => {
  // Setup: crew-agent runtime, provider = anthropic (from global), key NOT in globalEnvVars.
...
test("editAgent_globalSatisfiesOneKey_otherKeyStillMissing", () => {
  ...
  const allKeysOpenai = requiredCredentialEnvKeys("crew-agent", "openai");
  const allKeysAnthropic = requiredCredentialEnvKeys("crew-agent", "anthropic");
