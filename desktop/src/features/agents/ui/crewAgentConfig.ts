/**
 * Source-of-truth constants for crew-agent model-tuning configuration knobs.
 *
 ...
 * `./modelCapabilities`; this module owns only the crew-agent env-var keys and
 * the runtime-id guard. Mirrors the `config.rs` ⇄ `model_capabilities.rs` seam
 * in `crates/crew-agent`, where effort resolution is delegated to the manifest.
