...
test("parsePromptText wraps header-less free text in a single Prompt section", () => {
  ...
  // section is a "Crew event", there is no event content to surface, so
  // userText is empty and the title falls through to "Crew event".
  ...
  assert.equal(result.userTitle, "Crew event");
...
test("parsePromptText yields a null pubkey when From has no hex", () => {
  const text = ["[Crew event: note]", "From: Someone", "Content: hi"].join(
    "\n",
  );
...
test("parsePromptText defaults the title to 'Crew event' when no kind is present", () => {
  const text = ["[Crew event]", "Content: x"].join("\n");
  const result = parsePromptText(text);
  assert.equal(result.userTitle, "Crew event");
});
...
test("extractToolIdentity ignores Crew tool names that only appear in file contents", () => {
...
test("extractToolIdentity still recognizes explicit Crew tool fields", () => {
  const identity = extractToolIdentity({
...
test("parseSystemPromptSections pins the realistic Workspace+Base+System+Core harness shape", () => {
  // The real Crew harness emits [Workspace] content before [Base]. The parser
  ...
  const framed = [
    "[Workspace]",
    "You are operating inside the Crew platform.",
    "",
    "[Base]",
...
test("parseSystemPromptSections pins the full Base+System+Core+Canvas harness shape", () => {
  ...
  const framed = [
    ...
    "Fetch current content with: crew canvas get --channel 94a444a4-c0a3-5966-ab05-530c6ddc2301",
  ].join("\n");
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
...
test("parseSystemPromptSections extracts Team Instructions with Core Memory and Channel Canvas (full 5-section shape)", () => {
  ...
  const framed = [
    "[Base]",
    "You are a helpful AI assistant running in Crew.",
    "",
    "[System]",
  ...
  const sections = parseSystemPromptSections(framed);
  assert.deepEqual(sections, [
    { title: "Base", body: "You are a helpful AI assistant running in Crew." },
    {
      title: "System",
