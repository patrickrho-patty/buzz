...
test("inheritedRows_structured_keys_excluded_from_generic_rows", () => {
  // CREW_AGENT_PROVIDER, CREW_AGENT_MODEL, CREW_AGENT_THINKING_EFFORT must
  ...
  const allBaked = [
    ...
    { key: "CREW_AGENT_MODEL", value: "goose-claude-opus-4-8", masked: false },
    { key: "CREW_AGENT_THINKING_EFFORT", value: "medium", masked: false },
    {
      key: "DATABRICKS_HOST",
      value: "https://example.databricks.com/",
      masked: false,
...
// env vars (e.g. CREW_AGENT_MAX_ROUNDS) that are owned by first-class controls
...
test("buildRecord_hidden_tuning_key_unchanged_when_generic_row_edited", () => {
  // Structured field set CREW_AGENT_MAX_ROUNDS to "50"; it lives in value
  ...
  const value = { CREW_AGENT_MAX_ROUNDS: "50", MY_VAR: "old" };
...
test("buildRecord_runtime_switch_new_hiddenKeys_then_generic_edit", () => {
  ...
  //
  // Before switch: agent is crew-agent with CREW_AGENT_MAX_ROUNDS = "50" stored
  ...
  const crewAgentRuntime = {
    ...
    contextLimitEnvVar: "CREW_AGENT_CONTEXT_LIMIT",
  ...
  assert.equal(
    gooseHiddenKeys.includes("CREW_AGENT_MAX_ROUNDS"),
    false,
    "CREW_AGENT_MAX_ROUNDS must not be hidden under Goose descriptors",
  );
  ...
  const valueBeforeSwitch = {
    CREW_AGENT_MAX_ROUNDS: "50",
    GOOSE_MAX_TOKENS: "16384",
    USER_VAR: "original",
  };

  // After the switch to Goose, toRows is reproj with the new (Goose) hidden
  // set. CREW_AGENT_MAX_ROUNDS is no longer hidden → appears as a generic row.
  ...
  assert.ok(
    rowsAfterSwitch.some((r) => r.key === "CREW_AGENT_MAX_ROUNDS"),
...
test("filterBakedGenericRows_numeric_baked_key_excluded_and_placeholder_shown", () => {
  ...
  //
  // The global baked env contains CREW_AGENT_MAX_OUTPUT_TOKENS = "4096"
  ...
  assert.ok(
    numericStructuredKeys.includes("CREW_AGENT_MAX_OUTPUT_TOKENS"),
    "numeric key must appear in structured keys via production helpers",
  );

  // Simulate the baked env: CREW_AGENT_MAX_OUTPUT_TOKENS is baked, plus a
  // non-structured baked var.
  const bakedEnv = [
    { key: "CREW_AGENT_MAX_OUTPUT_TOKENS", value: "4096" },
  ...
  assert.equal(
    genericRows.some((r) => r.key === "CREW_AGENT_MAX_OUTPUT_TOKENS"),
    false,
...
test("buildRecord_clearing_structured_field_allows_placeholder_to_return", () => {
  ...
  //
  // Step 1: value has CREW_AGENT_MAX_ROUNDS = "50" (user set it via the
  //         structured field). CREW_AGENT_MAX_ROUNDS is in hiddenKeys.
  ...
  // After the clear, value no longer contains CREW_AGENT_MAX_ROUNDS.
  ...
  assert.equal(
    "CREW_AGENT_MAX_ROUNDS" in record,
    false,
