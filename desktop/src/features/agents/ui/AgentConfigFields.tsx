...
export function AgentConfigFields({
  ...
  const bakedProvider = React.useMemo(
    () => bakedEnv.find((e) => e.key === "CREW_AGENT_PROVIDER")?.value ?? null,
    [bakedEnv],
  );
  ...
  const bakedEffort = React.useMemo(
    () =>
      bakedEnv.find((e) => e.key === CREW_AGENT_THINKING_EFFORT)?.value ?? null,
    [bakedEnv],
  );
