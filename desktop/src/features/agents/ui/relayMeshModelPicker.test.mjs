...
test("Crew shared compute maps persisted auto to Default and hides custom input", () => {
...
test("Crew shared compute fallback is Default auto while normal providers remain unchanged", () => {
...
test("Crew shared compute keeps Default auto when discovery is empty", () => {
  const state = relayMeshModelPickerState({
    discoveredOptions: [],
    fallbackOptions: fallback,
    isCustomEditing: false,
...
test("Crew shared compute dropdown contains Default plus live models and no custom escape hatch", () => {
