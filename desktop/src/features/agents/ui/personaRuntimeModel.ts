 ...
 * persona runtime string (e.g. `crew-agent`). A catalog lookup then returns
 ...
 *
 * - crew-agent / goose → "capable" (`isProviderCapable`, id-based).
 ...
 * crew-agent/Anthropic). Persisting the local (empty) provider + credential-
 ...
 *
 * MODEL follows the same transition rule. crew-agent/goose readiness requires a
