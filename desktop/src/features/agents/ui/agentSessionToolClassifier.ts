...
const CREW_CLI_ADMIN_VERBS = new Set([
  "archive",
  "unarchive",
  "create",
  "delete",
...
const CREW_CLI_READ_VERBS = new Set([
  "get",
  "list",
  "thread",
  "search",
...
function crewOperationObject(operation: string) {
  ...
  return object ? object.replace(/[-_]+/g, " ") : "Crew";
}
...
function findBuzzCommand(tokens: string[]): CrewCommandRange | null {
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let j = i + 1; j < tokens.length; j++) {
      ...
      if (!CREW_CLI_GROUPS.has(tokens[j])) continue;
      const verbIndex = j + 1;
