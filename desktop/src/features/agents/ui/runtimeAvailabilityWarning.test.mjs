...
function entry(overrides) {
  return {
    ...
    thinkingEnvVar: null,
    installHint:
      "Crew talks to the Amp CLI through the amp-acp adapter. Follow the setup guide to install the adapter so the amp-acp command is on your PATH.",
...
test("not-installed warning includes the install hint", () => {
  ...
  assert.equal(
    warning,
    "Amp is not installed. Crew talks to the Amp CLI through the amp-acp adapter. Follow the setup guide to install the adapter so the amp-acp command is on your PATH.",
  );
});
