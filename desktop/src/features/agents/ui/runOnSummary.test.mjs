...
test("a saved kubernetes config renders labeled scalar rows", () => {
  const summary = summarizeRunOn({
    ...
    id: "kubernetes",
    config: {
      namespace: "crew-agents-x7k2mp",
      image: "ghcr.io/block/crew-sprig@sha256:17facfc7",
      cpu_request: "1",
  ...
  assert.equal(byKey.namespace.value, "crew-agents-x7k2mp");
  ...
  assert.equal(byKey.image.value, "ghcr.io/block/crew-sprig@sha256:17facfc7");
