...
const CREW_WRITE_TOOLS = new Set([
  "send_message",
  "send_diff_message",
  "edit_message",
  "delete_message",
...
const CREW_TOOL_NAMES = new Set([...CREW_READ_TOOLS, ...CREW_WRITE_TOOLS]);

const CREW_TOOL_NAMES_BY_LENGTH = [...CREW_TOOL_NAMES].sort(
  (left, right) => right.length - left.length,
);

const CREW_TOOL_TITLE_ALIASES: Array<[RegExp, string]> = [
...
export function getCrewToolInfo(title: string): CrewToolInfo | null {
  ...
  if (name.includes("workflow") || name === "approve_step") {
    return {
      icon: Workflow,
      label: isRead
        ? "Reads workflow state from Crew."
        : "Updates workflow state in Crew.",
  ...
  if (
    ...
    return {
      icon: Hash,
      label: isRead
        ? "Reads channel context from the Crew relay."
        : "Changes channel state in the Crew relay.",
      tone: isWrite ? "write" : "read",
    };
  }
  if (
    ...
    return {
      icon: Users,
      label: isRead
        ? "Reads Crew identity or presence data."
        : "Updates Crew identity or membership data.",
  ...
  if (name.includes("search") || name === "get_feed") {
    return {
      icon: Search,
      label: "Searches relay-visible Crew history.",
      tone: "read",
    };
  }
  if (
    ...
    return {
      icon: Send,
      label: "Publishes relay-visible Crew activity.",
      tone: "write",
    };
  }
  ...
  return {
    icon: MessageSquare,
    label: isRead ? "Reads from Crew." : "Writes to Crew.",
    tone: isWrite ? "write" : "read",
  };
}
...
export function findCrewToolName(value: string, includeShortNames: boolean) {
  ...
  return (
    CREW_TOOL_NAMES_BY_LENGTH.find(
      (name) =>
...
function findCrewToolAlias(value: string) {
  const normalizedPhrase = value
    ...
    .replace(/\s+/g, " ");
  return (
    CREW_TOOL_TITLE_ALIASES.find(([pattern]) =>
...
export function formatToolTitle(
  ...
  if (CREW_READ_TOOLS.has(name) || CREW_WRITE_TOOLS.has(name)) {
    return name
      .split("_")
