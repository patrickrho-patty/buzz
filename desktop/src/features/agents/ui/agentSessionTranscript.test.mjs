...
test("buildTranscript renders Prompt context + user message for a multi-block session/prompt frame", () => {
  ...
  assert.deepEqual(
    ...
    ["Agent Memory — core", "Context", "Crew event: @mention"],
...
test("buildTranscript keeps read_file activity categorized by the actual tool when output names Crew tools", () => {
  const [item] = toolItems([
    ...
    acpToolUpdate(11, {
      ...
      content: {
        type: "text",
        text: 'const CREW_READ_TOOLS = new Set(["get_feed", "get_event"]);\nconst CREW_WRITE_TOOLS = new Set(["delete_message"]);',
      },
    }),
  ]);
...
test("buildTranscript keeps shell activity categorized by the actual tool when grep output names Crew tools", () => {
  const [item] = toolItems([
    acpToolUpdate(20, {
      sessionUpdate: "tool_call",
...
test("buildTranscript categorizes explicit Crew tool calls for the activity bar", () => {
  const [item] = toolItems([
    acpToolUpdate(30, {
      sessionUpdate: "tool_call",
  ...
  const transcript = buildTranscript([
    {
      ...
      payload: {
        ...
        params: {
          toolCallId: "tool-1",
          title: "Confirm force-with-lease push to block/crew.",
          options: [
...
test("buildTranscript five-section system prompt card is standalone with all sections; CheckCheck context contains only Crew/thread context", () => {
  ...
  //       the session/prompt:context sections (Crew event + Thread context),
      ...
      payload: {
        ...
        params: {
          systemPrompt: [
            ...
            "Fetch current content with: crew canvas get --channel 44444444-4444-4444-4444-444444444444",
          ].join("\n"),
        },
      },
    },
  ...
  // the session/prompt:context item — Crew/thread context only, no system-prompt sections.
  ...
  // Must have Crew event and Thread context sections, NOT Base/System/Team Instructions/Core Memory/Channel Canvas.
  assert.ok(
    contextSectionTitles.some((t) => t.toLowerCase().includes("crew")),
    "prompt context must contain a Crew event section",
  );
  assert.ok(
...
test("buildTranscript session/new via _meta.systemPrompt.append produces identical standalone card as bare systemPrompt field", () => {
  ...
  const SYSTEM_PROMPT = [
    ...
    "Canvas revision (event ID): a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2",
    "Last modified: 2026-07-01T10:00:00Z",
    "Fetch current content with: crew canvas get --channel 55555555-5555-5555-5555-555555555555",
  ].join("\n");
