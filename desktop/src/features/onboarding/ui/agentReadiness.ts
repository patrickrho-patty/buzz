 ...
 * Provider path: the preferred Crew Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "crew-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "crew-agent");
  ...
  if (preferredRuntime.id !== "crew-agent" && preferredRuntime.id !== "goose") {
    return { ready: false };
  }
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "crew-agent" };
    }
  }
