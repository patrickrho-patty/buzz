 ...
 * slot stays inside the `.crew-onboarding-neutral-theme` subtree so
 * `--crew-welcome-chartreuse` and the theme color tokens still resolve for the
 * docked buttons.
 */
export function OnboardingFooterProvider({
  ...
  return (
    <OnboardingFooterTargetContext.Provider value={target}>
      ...
      <div
        aria-hidden
        className="pointer-events-none fixed inset-x-0 bottom-0 z-10 h-36 bg-[linear-gradient(to_top,var(--crew-onboarding-shell-bottom)_35%,transparent)]"
      />
      {backAction ? (
