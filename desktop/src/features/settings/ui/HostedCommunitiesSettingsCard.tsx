...
export function HostedCommunitiesSettingsCard() {
  ...
  const loadAccount = React.useCallback(async () => {
    ...
    if (
      ...
      // Crew identity yet — that's the connect-card empty state, not an error to
      ...
      throw new Error(
        errorMessage(
          ...
          "Could not load the connected Crew identity.",
        ),
      );
    }
  ...
  const connectIdentity = () =>
    run("Connecting Crew identity…", async () => {
      ...
      if (response.error) {
        throw new Error(
          errorMessage(
            response.error,
            response.correlation_id,
            "Could not unpair the Crew identity.",
          ),
        );
      }
      setIdentity(null);
      ...
      if (released.error) {
        throw new Error(
          errorMessage(
            ...
            "Could not release the previously connected Crew identity.",
          ),
        );
      }
      ...
      if (bound.error) {
        ...
        throw new Error(
          bound.error.code === "pubkey_already_bound"
            ? "This device's Crew identity is already reserved by another Builderlab account, so it can't be connected here. Sign in with that account, or transfer the identity there first."
            : errorMessage(
                bound.error,
  ...
  return (
    <section className="space-y-6" data-testid="hosted-communities-settings">
      <SettingsSectionHeader
        ...
        description="Crew works with any relay. This page is only for relay hosting provided by Block — sign in with a Builderlab account to create and manage Block-hosted communities. Builderlab sign-in is used on this page alone."
      />
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-border/70 p-5">
          ...
          >
            Authentication opens in your browser and returns securely to Crew.
          ...
          {!identity ? (
            <div className="rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
              <h3 className="font-medium">
                Link this account to your Crew identity
              </h3>
              <p
                className="mt-2 text-sm text-muted-foreground/70"
                data-settings-subcopy
              >
                This Builderlab account isn&apos;t linked to a Crew identity
                yet. Connect this device&apos;s key to create and own
                communities under it — Crew signs a one-time challenge locally,
            ...
            <div className="rounded-xl border border-amber-500/50 bg-amber-500/5 p-5">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h3 className="font-medium">
                    This account is connected to a different Crew identity
                  </h3>
                  <p
                    className="mt-2 text-sm text-muted-foreground/70"
                    data-settings-subcopy
                  >
                    Your Builderlab account owns communities under another Crew
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border/70 p-4">
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <CheckCircle2 className="h-4 w-4 text-emerald-500" /> Crew
                identity connected
                {identity.npub ? (
...
function UnpairIdentityButton({
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>Unpair this Crew identity?</AlertDialogTitle>
          <AlertDialogDescription>
            Your Builderlab account will no longer be connected to this Crew
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            member. The recipient needs a connected Crew identity first, and
