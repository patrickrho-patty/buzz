...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "crew_custom_emoji";
  const TOKEN_TYPE = "crew_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:crew:` and
    // `http://x:y:crew:` must NOT turn the inner `:crew:` into an image;
