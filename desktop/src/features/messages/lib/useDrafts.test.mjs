...
test("corrupt_localstorage_json_is_silently_ignored", () => {
  ...
  localStorage.setItem("crew-drafts.v1:pubkey-corrupt", "{not-valid-json");
...
test("invalid_draft_entries_in_localstorage_are_skipped", () => {
  ...
  localStorage.setItem("crew-drafts.v1:pubkey-invalid", data);
  clearAllDrafts();
...
test("pre_status_entry_appears_in_getActiveDraftEntries_after_migration", () => {
  ...
  localStorage.setItem(
    "crew-drafts.v1:pubkey-migrate2",
...
test("pre_status_sent_entry_is_dropped_on_read", () => {
  ...
  localStorage.setItem(
    "crew-drafts.v1:pubkey-normalise",
...
function readRawStore(pubkey) {
  const raw = localStorage.getItem(`crew-drafts.v1:${pubkey}`);
...
test("invalid_mention_ref_rejects_corrupt_draft", () => {
  ...
  const now = new Date().toISOString();
  storage.setItem(
    "crew-drafts.v1:corrupt-mention-owner",
    JSON.stringify({
      "chan-corrupt": {
        content: "bad ref",
        selectionStart: 7,
...
test("cross_relay_isolation_via_initDraftStore_without_clearAllDrafts", () => {
  ...
  const v2a = storage.getItem(
    "crew-drafts.v2:wss://relay-a.example.com:pubkey-shared",
  );
  const v2b = storage.getItem(
    "crew-drafts.v2:wss://relay-b.example.com:pubkey-shared",
  );
...
test("legacy_migration_v1_entries_readable_after_scoped_init_v1_key_removed", () => {
  ...
  assert.equal(
    storage.getItem(`crew-drafts.v1:${pk}`),
    null,
    "v1 key must be removed after migration",
  );

  // v2 key must exist.
  const v2 = storage.getItem(`crew-drafts.v2:wss://relay.example.com:${pk}`);
...
test("legacy_migration_second_relay_init_does_not_reimport_deleted_v1", () => {
  ...
  // v1 key is gone.
  assert.equal(storage.getItem(`crew-drafts.v1:${pk}`), null);
...
test("clearAllDrafts_without_reinit_blocks_reads_and_writes_to_previous_bucket", () => {
  ...
  const bucketBefore = storage.getItem(
    "crew-drafts.v2:wss://relay-a.example.com:pubkey-teardown",
  );
  ...
  assert.equal(
    storage.getItem("crew-drafts.v2:wss://relay-a.example.com:pubkey-teardown"),
    bucketBefore,
...
test("failed_v2_flush_during_migration_preserves_v1_key", () => {
  ...
  assert.equal(
    storage.getItem(`crew-drafts.v2:wss://relay.example.com:${pk}`),
    null,
...
test("no_relay_legacy_caller_form_still_reads_writes_v1_key", () => {
  ...
  const raw = JSON.parse(storage.getItem(`crew-drafts.v1:${pk}`));
