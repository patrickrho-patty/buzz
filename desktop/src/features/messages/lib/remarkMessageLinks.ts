/**
 * Remark plugin that detects bare `crew://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `crew://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `crew://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](crew://…)` link.
 *
