...
test("rule fires for a boundary :shortcode: (start of string)", () => {
  const rule = captureRule(["crew"]);
  const { matched, advanced } = runRule(rule, ":crew:", 0);
  assert.equal(matched, true);
  assert.equal(advanced, ":crew:".length);
});

test("rule fires for a :shortcode: preceded by whitespace", () => {
  const rule = captureRule(["crew"]);
  // pos points at the `:` after the space.
  const { matched } = runRule(rule, "hi :crew:", 3);
  assert.equal(matched, true);
});

test("rule does NOT fire when the colon is glued to a word char (not:crew:)", () => {
  const rule = captureRule(["crew"]);
  // pos points at the `:` immediately after `not`.
  const { matched } = runRule(rule, "not:crew:", 3);
  assert.equal(matched, false);
});

test("rule does NOT fire inside a URL-like sequence (http://x:y:crew:)", () => {
  const rule = captureRule(["crew"]);
  const src = "http://x:y:crew:";
  // pos points at the `:` immediately after `y` (a word char).
  const { matched } = runRule(rule, src, src.indexOf(":crew:"));
  assert.equal(matched, false);
});
...
test("rule fires after punctuation boundary (e.g. parenthesis)", () => {
  ...
  const { matched } = runRule(rule, "(:crew:)", 1);
  assert.equal(matched, true);
});
