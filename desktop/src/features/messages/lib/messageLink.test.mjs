...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `crew://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `crew://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `crew://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects crew:// with wrong host", () => {
  const r = parseMessageLink(`crew://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`crew://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`crew://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy crew://message links", () => {
  const r = parseMessageLink(`crew://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches crew://message and legacy crew://message", () => {
  assert.equal(
    isMessageLink(`crew://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`crew://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("crew://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("crew://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `crew://message?channel=${CHANNEL}&id=${MESSAGE}`;
