...
test("crew://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`crew://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message crew:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("crew://channel?foo=bar", handlers);
  assert.deepEqual(external, ["crew://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed crew://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with crew://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("crew://message?nope=1", handlers);
  assert.deepEqual(external, ["crew://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
