...
test("sustained advances persist within each one-second window", () => {
  ...
  try {
    ...
    const contexts = JSON.parse(
      storage.getItem(`crew.channel-read-state.v2:${"5".repeat(64)}`),
    );
...
test("visibility hidden flushes pending local state", () => {
  ...
  try {
    ...
    const contexts = JSON.parse(
      storage.getItem(`crew.channel-read-state.v2:${"2".repeat(64)}`),
    );
...
test("hydrateFromLocalStorage persists immediately", () => {
  ...
  storage.setItem(
    `crew.channel-read-state.v2:${pubkey}`,
...
test("destroyed manager cannot persist after an in-flight fetch resolves", async () => {
  ...
  try {
    ...
    const contexts = JSON.parse(
      storage.getItem(`crew.channel-read-state.v2:${pubkey}`),
    );
    assert.equal(
      contexts["channel-1"],
