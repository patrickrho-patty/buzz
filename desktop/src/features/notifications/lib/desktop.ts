...
type TestWindow = Window & {
  ...
  __CREW_E2E_APP_BADGE_STATE__?: AppBadgeState["kind"];
};
...
export async function listenForDesktopNotificationActions(
  ...
  if (isTauri()) {
    ...
    if (usesMacActivationQueue) {
      // Belt and suspenders for block/crew#3509: the Rust delegate queues the
...
export async function setDesktopAppBadge(state: AppBadgeState): Promise<void> {
  if (typeof window !== "undefined") {
    ...
    testWindow.__CREW_E2E_APP_BADGE_COUNT__ =
      state.kind === "count" ? state.count : 0;
    testWindow.__CREW_E2E_APP_BADGE_STATE__ = state.kind;
  }
...
export async function sendDesktopNotification(
  ...
  // block/crew#5081 — WebKit throws `NotificationError` from the constructor
