...
test("projectRepoHost recognizes a canonical repository on the relay", () => {
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/crew`, ORIGIN), {
    kind: "crew",
  });
});
...
test("projectRepoHost identifies an external repository by host", () => {
  assert.deepEqual(
    projectRepoHost("https://github.com/block/crew.git", ORIGIN),
...
test("projectRepoHost fails closed while either URL is unresolved", () => {
  ...
  assert.deepEqual(projectRepoHost(`${ORIGIN}/git/${OWNER}/crew`, null), {
    kind: "unresolved",
  });
...
test("projectRepoHostForProject recognizes an implicit relay repository", () => {
  assert.deepEqual(
    projectRepoHostForProject(
      { cloneUrls: [], dtag: "crew", owner: OWNER },
      ORIGIN,
    ),
    { kind: "crew" },
  );
...
test("repositoryDisplayPath renders an external repo as host/path without .git", () => {
  assert.equal(
    repositoryDisplayPath(
      {
        cloneUrls: ["https://github.com/block/crew.git"],
        dtag: "crew",
        owner: OWNER,
      },
      ORIGIN,
    ),
    "github.com/block/crew",
  );
});
...
test("repositoryDisplayPath fails closed without a resolvable clone URL", () => {
  assert.equal(
    repositoryDisplayPath({ cloneUrls: [], dtag: "crew", owner: OWNER }, null),
    null,
  );
  assert.equal(
    repositoryDisplayPath(
      { cloneUrls: ["not a URL"], dtag: "crew", owner: OWNER },
      ORIGIN,
    ),
    null,
  );
