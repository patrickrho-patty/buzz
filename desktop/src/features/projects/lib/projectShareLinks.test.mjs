...
test("projectShareLink carries the active workspace tab for both link kinds", () => {
  assert.equal(
    ...
    `crew://project?owner=${OWNER}&d=pollinator&tab=prs`,
  );
  // Legacy projects share as crew://repo and keep the tab too.
  assert.equal(
    projectShareLink({ projectAddress: REPO_ADDRESS }, "issues"),
    `crew://repo?owner=${OWNER}&d=flappy-bee&tab=issues`,
  );
});
...
test("projectShareLink links legacy projects as their backing repository", () => {
  assert.equal(
    ...
    `crew://repo?owner=${OWNER}&d=flappy-bee`,
  );
});
...
test("issue and pull request links carry the event id and repo coordinate", () => {
  assert.equal(
    ...
    `crew://issue?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
  assert.equal(
    pullRequestShareLink({ id: EVENT_ID, repoAddress: REPO_ADDRESS }),
    `crew://pr?id=${EVENT_ID}&owner=${OWNER}&d=flappy-bee`,
  );
});
