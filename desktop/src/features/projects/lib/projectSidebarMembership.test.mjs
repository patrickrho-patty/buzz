...
test("migrates the legacy selected-address array", () => {
  ...
  assert.deepEqual(parsed, {
    version: 1,
    projects: {
      "30621:alice:crew": { selected: true, updatedAt: 0 },
    },
  });
});
...
test("newer removal wins over an older selection", () => {
  const merged = mergeProjectSidebarMembershipStores(
    store({
      "30621:alice:crew": { selected: true, updatedAt: 100 },
    }),
    store({
      "30621:alice:crew": { selected: false, updatedAt: 200 },
    }),
  );
...
test("equal-timestamp conflicts converge with removal winning", () => {
  ...
  const removed = store({
    "30621:alice:crew": { selected: false, updatedAt: 100 },
  });
