...
function buildProjectPatchTemplate({
  ...
  // (d, name, description, crew-channel, crew-visibility, relay hints embedded
  ...
  return {
    ...
    tags: [
      ...repository.eventTags
        .filter((tag) => tag[0] !== "crew-channel")
        .map((tag) => [...tag]),
      ["crew-channel", normalizedChannelId],
    ],
  };
}
...
export function buildAddedRepositoryEventTemplatesFromHead({
  ...
  repositoryTags.push(["crew-channel", normalizedAccessChannelId]);
