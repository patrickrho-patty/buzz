...
async function ready(view) {
  await waitFor(() =>
    assert.ok(view.container.querySelector(".crew-terminal-substrate")),
  );
}
...
test("drag resize batches visual updates and commits state only on release", async () => {
  ...
  assert.equal(substrate.dataset.terminalResizing, "true");
  assert.equal(window.localStorage.getItem("crew-terminal-dock-height"), null);
  ...
  assert.equal(window.localStorage.getItem("crew-terminal-dock-height"), "380");
});
...
test("unmount cancels a queued drag update", async () => {
  ...
  assert.equal(window.localStorage.getItem("crew-terminal-dock-height"), null);
  assert.equal(
    handle.closest(".crew-terminal-substrate").style.height,
    previousHeight,
  );
});
...
async function expectWelcome(view, present) {
  ...
  await waitFor(() =>
    assert.equal(
      view.container.querySelector(".crew-terminal-welcome") !== null,
      present,
    ),
  );
}
...
test("reduced motion keeps the terminal cursor solid", async () => {
  ...
  try {
    ...
    await waitFor(() =>
      assert.equal(
        subject.view.container.querySelector(".crew-terminal-substrate").dataset
          .terminalOwner,
        "terminal",
      ),
    );
