...
before(async () => {
  ...
  dom.window.localStorage.setItem("crew-follow-system", "false");
...
test("opening a tab keeps terminal ownership while its attachment is pending", async () => {
  ...
  attachResolver = () => {};
  fireEvent.click(view.getByLabelText("New Crew Term tab"));
  await waitFor(() => assert.equal(typeof attachResolver, "function"));
  assert.equal(
    substrate.dataset.terminalOwner,
    "terminal",
    "an attaching session must not force the substrate back to Crew",
  );
