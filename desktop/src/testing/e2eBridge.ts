...
declare global {
  interface Window {
    __CREW_E2E__?: E2eConfig;
    __CREW_E2E_WHOAMI_EMAIL__?: string;
    __CREW_E2E_COMMANDS__?: string[];
    __CREW_E2E_COMMAND_PAYLOADS__?: Array<{
      command: string;
      payload: unknown;
    }>;
    ...
    __CREW_E2E_RELEASE_MEDIA_PROXY__?: () => number;
    /** Release mock send events that were stored but withheld from live subscribers. */
    __CREW_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__?: () => number;
    __CREW_E2E_EMIT_MEDIA_UPLOAD_PHASE__?: (input: {
      id: string;
      phase: string;
    }) => Promise<void>;
    __CREW_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__?: (input: {
      id: string;
      sent: number;
      total: number;
    }) => Promise<void>;
    __CREW_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__?: (payload: {
      pubkey: string | null;
      level: number;
    }) => Promise<void>;
    __CREW_E2E_WEBVIEW_ZOOM__?: number;
    __CREW_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?: (input: {
      channelName: string;
      kind?: number;
    }) => boolean;
    __CREW_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__?: (input: {
      ownerPubkey: string;
      kind: number;
    }) => boolean;
    ...
    __CREW_E2E_PREPEND_MOCK_HISTORY__?: (input: {
      channelName: string;
      count: number;
      startIndex?: number;
      lineCount?: number;
    ...
    __CREW_E2E_EMIT_MOCK_TYPING__?: (input: {
      channelName: string;
      createdAt?: number;
      pubkey?: string;
      threadHeadId?: string;
    ...
    __CREW_E2E_EMIT_TAURI_EVENT__?: (
      event: string,
      payload: unknown,
    ) => Promise<void>;
    __CREW_E2E_SET_MOCK_HUDDLE_SNAPSHOT__?: (input: {
      members: MockHuddleMemberSeed[];
      transcriptionEnabled: boolean;
    }) => Promise<void>;
    __CREW_E2E_PUSH_MOCK_FEED_ITEM__?: (item: RawFeedItem) => RawFeedItem;
    /** Replace an existing feed item by id (or push if not found) and fire the updated event. */
    __CREW_E2E_REPLACE_MOCK_FEED_ITEM__?: (
      oldId: string,
      item: RawFeedItem,
    ) => RawFeedItem;
     ...
     */
    __CREW_E2E_EXTRA_PROJECT_EVENTS__?: Array<{
      id: string;
      kind: number;
      pubkey: string;
      created_at: number;
    ...
    __CREW_E2E_PROJECT_MERGE_ERROR__?: {
      code: string;
      message: string;
      recovery: {
    ...
    __CREW_E2E_PROJECT_OWNER_OVERRIDE__?: string;
    /** Project history kinds rejected with CLOSED for aggregate-query tests. */
    __CREW_E2E_REJECT_PROJECT_QUERY_KINDS__?: number[];
    /** Captured aggregate project-history filters for request-count assertions. */
    __CREW_E2E_PROJECT_QUERY_FILTERS__?: MockFilter[];
    /** Optional local repository snapshot returned for project branch tests. */
    __CREW_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__?: unknown;
    /** Optional bounded file contents returned by repository content reads. */
    __CREW_E2E_PROJECT_REPO_FILE_CONTENTS__?: Record<string, string | null>;
    __CREW_E2E_PROJECT_REPO_SYNC_STATUS__?: {
    ...
    __CREW_E2E_SET_RELAY_CONNECTION_STATE__?: (state: ConnectionState) => void;
    __CREW_E2E_GET_RELAY_CONNECTION_STATE__?: () => ConnectionState;
    /** Queue deterministic mock AUTH outcomes, consumed in order. */
    __CREW_E2E_QUEUE_AUTH_RESPONSES__?: (
      responses: Array<{ success: boolean; message: string }>,
    ) => void;
    /** Inject CLOSED into every active mock live subscription. */
    __CREW_E2E_CLOSE_LIVE_SUBSCRIPTIONS__?: (reason: string) => number;
    /** Queue CLOSED responses for channel history REQs. */
    __CREW_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__?: (reasons: string[]) => void;
    __CREW_E2E_SET_STALL_WEBSOCKET_SENDS__?: (stall: boolean) => void;
    __CREW_E2E_DISCONNECT_MOCK_WEBSOCKETS__?: () => number;
    __CREW_E2E_RESTART_MOCK_WEBSOCKETS__?: () => number;
    __CREW_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__?: (
      unavailable: boolean,
    ) => void;
    __CREW_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => number[];
    __CREW_E2E_ACTIVATE_RELAY_RATE_LIMIT__?: (seconds: number) => void;
    __CREW_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__?: () => void;
    __CREW_E2E_SET_MESH__?: (mesh: {
      admitted?: boolean;
    ...
    __CREW_E2E_SEED_ACTIVE_TURNS__?: (input: {
      agentPubkey: string;
      channelId: string;
      turnId: string;
      kind?: "turn_started" | "turn_completed";
    }) => void;
    __CREW_E2E_SEED_OBSERVER_EVENTS__?: (input: {
      agentPubkey: string;
      events: Array<{
        seq: number;
        timestamp: string;
    ...
    __CREW_E2E_EMIT_MOCK_READ_STATE__?: (input: {
      clientId: string;
      contexts: Record<string, number>;
      createdAt: number;
      slotId: string;
    }) => unknown;
    __CREW_E2E_SEED_MOCK_REMINDERS__?: (reminders: RelayEvent[]) => void;
    __CREW_E2E_QUERY_CLIENT__?: {
      invalidateQueries: (filters: { queryKey: readonly unknown[] }) => unknown;
    };
    __CREW_E2E_MD_PARSE_COUNT__?: () => number;
    /**
     ...
     * __CREW_E2E_INVOKE_MOCK_COMMAND__. Call after the mutation to make the
     ...
     * follow up with __CREW_E2E_INVALIDATE_CHANNELS__ to flush the cache.
     *
     ...
     * are held in a queue and not resolved until `__CREW_E2E_RELEASE_GET_EVENT__()`
     ...
     */
    __CREW_E2E_DEFER_GET_EVENT__?: string | null;
...
// When `window.__CREW_E2E_DEFER_GET_EVENT__` is set to a target event ID,
// `handleGetEvent` holds calls for that ID in this queue.  All other event IDs
// continue to resolve immediately.
// `window.__CREW_E2E_RELEASE_GET_EVENT__()` flushes the queue and returns the
...
//
// This entire module (e2eBridge.ts) is loaded only when `window.__CREW_E2E__`
...
function handleUpdateWorkflow(args: {
  ...
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__CREW_E2E__?.mock?.workflowUpdateError;
...
function handleDeleteWorkflow(args: { workflowId: string }) {
  const configuredError = window.__CREW_E2E__?.mock?.workflowDeleteError;
  if (configuredError) throw new Error(configuredError);
...
function handleTriggerWorkflow(args: { workflowId: string }) {
  const workflow = mockWorkflows.find((w) => w.id === args.workflowId);
  if (!workflow) throw new Error(`Workflow ${args.workflowId} not found`);
  const configuredError = window.__CREW_E2E__?.mock?.workflowTriggerError;
  if (configuredError) throw new Error(configuredError);
...
function buildMockProjectEvents(): RelayEvent[] {
  ...
  for (const [projectIndex, seed] of MOCK_PROJECT_SEEDS.entries()) {
    const owner =
      projectIndex === 0
        ? (window.__CREW_E2E_PROJECT_OWNER_OVERRIDE__ ?? seed.owner)
        : seed.owner;
  ...
  const projectOwner =
    window.__CREW_E2E_PROJECT_OWNER_OVERRIDE__ ?? MOCK_PROJECT_SEEDS[0].owner;
  events.push(
    createMockEvent(
      KIND_PROJECT_ANNOUNCEMENT,
      "",
...
function getMockProjectEventStore(): RelayEvent[] {
  if (!mockProjectEventStore) {
    ...
    const extras = window.__CREW_E2E_EXTRA_PROJECT_EVENTS__;
...
async function handleGetEvent(
  ...
  // Defer/release seam: when __CREW_E2E_DEFER_GET_EVENT__ is set to this
  // event's ID, hold this call in the queue until __CREW_E2E_RELEASE_GET_EVENT__()
  ...
  if (
    window.__CREW_E2E_DEFER_GET_EVENT__ &&
    window.__CREW_E2E_DEFER_GET_EVENT__ === args.eventId
  ) {
    // Increment the count only for calls that are actually deferred.
    window.__CREW_E2E_GET_EVENT_CALL_COUNT__ =
      (window.__CREW_E2E_GET_EVENT_CALL_COUNT__ ?? 0) + 1;
...
function sendToMockSocket(args: {
  ...
  if (type === "REQ") {
    ...
    if (
      ...
      window.__CREW_E2E_PROJECT_QUERY_FILTERS__ ??= [];
      window.__CREW_E2E_PROJECT_QUERY_FILTERS__.push(filter);
      const rejectedKinds =
        window.__CREW_E2E_REJECT_PROJECT_QUERY_KINDS__ ?? [];
  ...
  if (type === "EVENT") {
    ...
    if (isMockProjectScopedEvent(event)) {
      ...
      if (
        ...
        window.__CREW_E2E_UNSUPPORTED_PROJECT_ANNOUNCEMENTS__
      ) {
        sendWsText(socket.handler, [
          "OK",
          event.id,
      ...
      const rejectionIndex =
        window.__CREW_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(event.kind) ??
        -1;
      if (rejectionIndex >= 0) {
        window.__CREW_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
          rejectionIndex,
          1,
        );
        sendWsText(socket.handler, [
      ...
      window.__CREW_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
      const failedAckIndex =
        window.__CREW_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(event.kind) ??
        -1;
      if (failedAckIndex >= 0) {
        window.__CREW_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
          failedAckIndex,
          1,
        );
        sendWsText(socket.handler, [
...
export function maybeInstallE2eTauriMocks() {
  ...
  window.__CREW_E2E_LINK_PREVIEW_UPLOAD_STARTS__ = 0;
  window.__CREW_E2E_RELEASE_LINK_PREVIEW_METADATA__ = () => {
    const queued = deferredLinkPreviewMetadataQueue.splice(0);
    for (const release of queued) release();
    return queued.length;
  };
  window.__CREW_E2E_RELEASE_LINK_PREVIEW_UPLOADS__ = () => {
  ...
  window.__CREW_E2E_COMMANDS__ = [];
  window.__CREW_E2E_COMMAND_PAYLOADS__ = [];
  ...
  window.__CREW_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__ = (payload) =>
    emit("huddle-tts-speaker-level", payload);
  window.__CREW_E2E_SIGNED_EVENTS__ = [];
  window.__CREW_E2E_WEBVIEW_ZOOM__ = 1;
  window.__CREW_E2E_EMIT_MEDIA_UPLOAD_PHASE__ = async (input) => {
    await emit("media-upload-phase", input);
  };
  window.__CREW_E2E_EMIT_MEDIA_UPLOAD_PROGRESS__ = async (input) => {
    await emit("media-upload-progress", input);
  };
  window.__CREW_E2E_SET_MOCK_HUDDLE_SNAPSHOT__ = async ({
    members,
  ...
  window.__CREW_E2E_PREPEND_MOCK_HISTORY__ = prependMockHistory;
  window.__CREW_E2E_EMIT_MOCK_TYPING__ = ({
    channelName,
    createdAt,
    pubkey,
    threadHeadId,
  ...
  window.__CREW_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__ = ({ channelName, kind }) => {
    const channel = mockChannels.find(
      (candidate) => candidate.name === channelName,
    );
    if (!channel) {
  ...
  window.__CREW_E2E_HAS_MOCK_OWNER_KIND_SUBSCRIPTION__ = ({
    ownerPubkey,
    kind,
  ...
  window.__CREW_E2E_ACTIVATE_TIMEOUT__ = (expiresAtMs: number) => {
  ...
  window.__CREW_E2E_INVALIDATE_CHANNELS__ = async () => {
    await window.__CREW_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["channels"],
    });
  };
  window.__CREW_E2E_MUTATE_CHANNEL__ = ({
    channelId,
    channelType,
    description,
    removeMemberPubkey,
  ...
  window.__CREW_E2E_GET_EVENT_CALL_COUNT__ = 0;
  window.__CREW_E2E_DEFER_GET_EVENT__ = null;
  deferredGetEventQueue = [];
  deferNextChannelsRead = false;
  deferredChannelsReadResolve = null;
  window.__CREW_E2E_CHANNELS_READ_PENDING__ = 0;
  window.__CREW_E2E_DEFER_NEXT_CHANNELS_READ__ = () => {
    if (deferredChannelsReadResolve) {
      throw new Error("a channel read is already deferred");
    }
    deferNextChannelsRead = true;
  };
  window.__CREW_E2E_RELEASE_CHANNELS_READ__ = () => {
    deferNextChannelsRead = false;
  ...
  window.__CREW_E2E_RELEASE_GET_EVENT__ = () => {
  ...
  window.__CREW_E2E_EMIT_MOCK_READ_STATE__ = ({
    clientId,
    contexts,
    createdAt,
    slotId,
  ...
  window.__CREW_E2E_SET_RELAY_CONNECTION_STATE__ = (state) => {
  ...
  window.__CREW_E2E_GET_RELAY_CONNECTION_STATE__ = () =>
    relayClient.getConnectionState();
  window.__CREW_E2E_QUEUE_AUTH_RESPONSES__ = (responses) => {
    mockAuthResponses.push(...responses);
  };
  window.__CREW_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__ = (reasons) => {
    mockChannelHistoryCloses.push(...reasons);
  };
  window.__CREW_E2E_CLOSE_LIVE_SUBSCRIPTIONS__ = (reason) => {
    let closed = 0;
    for (const socket of mockSockets.values()) {
  ...
  window.__CREW_E2E_SEED_MOCK_REMINDERS__ = (reminders) => {
  ...
  window.__CREW_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__ = (unavailable) => {
    mockWebsocketUnavailable = unavailable;
    if (unavailable) relayWebsocketConnectAttemptStarts.length = 0;
  };
  window.__CREW_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__ = () => [
    ...relayWebsocketConnectAttemptStarts,
  ];
  window.__CREW_E2E_ACTIVATE_RELAY_RATE_LIMIT__ = (seconds) => {
    activateRateLimit(seconds);
  };
  window.__CREW_E2E_RESET_WEBSOCKET_CONNECT_ATTEMPTS__ = () => {
    relayWebsocketConnectAttemptStarts.length = 0;
  };
  window.__CREW_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__ = () => {
  ...
  window.__CREW_E2E_SET_MESH__ = (mesh) => {
  ...
  window.__CREW_E2E_SEED_ACTIVE_TURNS__ = ({
    agentPubkey,
    channelId,
    turnId,
    kind = "turn_started",
  ...
  window.__CREW_E2E_SEED_OBSERVER_EVENTS__ = ({ agentPubkey, events }) => {
  ...
  const handleMockCommand = async (
    ...
    window.__CREW_E2E_COMMANDS__?.push(command);
    const loggedPayload = (() => {
    ...
    window.__CREW_E2E_COMMAND_PAYLOADS__?.push({
      command,
      payload: loggedPayload,
    });
    window.__CREW_E2E_COMMAND_LOG__?.push({ command, payload });
    ...
    switch (command) {
      ...
      case "sign_out":
        ...
        // assert invocation via __CREW_E2E_COMMANDS__ and the pending UI.
        return;
      ...
      case "oidc_whoami":
        return window.__CREW_E2E_WHOAMI_EMAIL__ ?? "";
      case "update_profile":
      ...
      case "get_project_local_repo_snapshot":
        return window.__CREW_E2E_PROJECT_LOCAL_REPO_SNAPSHOT__ ?? null;
      ...
      case "get_project_local_repo_file_content": {
        ...
        return path
          ? (window.__CREW_E2E_PROJECT_REPO_FILE_CONTENTS__?.[path] ?? null)
          : null;
      }
      ...
      case "push_project_local_repository": {
        ...
        const status = window.__CREW_E2E_PROJECT_REPO_SYNC_STATUS__;
      ...
      case "publish_project_owner_announcement": {
        ...
        window.__CREW_E2E_SIGNED_EVENTS__?.push({
          content: event.content,
          createdAt: event.created_at,
          kind: event.kind,
          tags: event.tags,
        ...
        if (
          ...
          const rejectionIndex =
            window.__CREW_E2E_REJECT_PROJECT_EVENT_KINDS__?.indexOf(
              event.kind,
            ) ?? -1;
          if (rejectionIndex >= 0) {
            window.__CREW_E2E_REJECT_PROJECT_EVENT_KINDS__?.splice(
            ...
            window.__CREW_E2E_ACCEPTED_PROJECT_EVENTS__ = acceptedProjectEvents;
            const failedAckIndex =
              window.__CREW_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.indexOf(
                event.kind,
              ) ?? -1;
            if (failedAckIndex >= 0) {
              window.__CREW_E2E_FAIL_PROJECT_EVENT_ACK_KINDS__?.splice(
      ...
      case "sign_project_pull_request_status": {
        ...
        window.__CREW_E2E_SIGNED_EVENTS__?.push(event);
        getMockProjectEventStore().push(event);
        return null;
      }
      ...
      case "merge_project_pull_request": {
        ...
        if (window.__CREW_E2E_PROJECT_MERGE_ERROR__) {
          throw window.__CREW_E2E_PROJECT_MERGE_ERROR__;
        }
      ...
      case "get_channels": {
        ...
        if (deferred) {
          await new Promise<void>((resolve) => {
            ...
            window.__CREW_E2E_CHANNELS_READ_PENDING__ = 1;
          });
        }
        return channels;
      }
      ...
      case "export_agent_snapshot":
        ...
        // Specs assert invocation via __CREW_E2E_COMMANDS__.
        return true;
      ...
      case "upload_media_bytes": {
        ...
        if (input.filename?.startsWith("link-preview-")) {
          window.__CREW_E2E_LINK_PREVIEW_UPLOAD_STARTS__ =
            (window.__CREW_E2E_LINK_PREVIEW_UPLOAD_STARTS__ ?? 0) + 1;
        }
      ...
      case "save_agent_card":
        ...
        // command was invoked via `__CREW_E2E_COMMANDS__`, not the dialog.
        return true;
      ...
      case "plugin:webview|set_webview_zoom":
        window.__CREW_E2E_WEBVIEW_ZOOM__ = (payload as { value: number }).value;
        return;
      case "start_pairing": {
      ...
      case "list_save_subscriptions": {
        ...
        if (!win.__CREW_E2E_IPC_COUNTERS__) {
          win.__CREW_E2E_IPC_COUNTERS__ = {};
        }
        const ipcCounters = win.__CREW_E2E_IPC_COUNTERS__ as Record<
          string,
          number
        >;
  ...
  window.__CREW_E2E_INVOKE_MOCK_COMMAND__ = (command, payload) =>
    handleMockCommand(command, payload ?? null);
  window.__CREW_E2E_EMIT_TAURI_EVENT__ = (event, payload) =>
    emit(event, payload);
