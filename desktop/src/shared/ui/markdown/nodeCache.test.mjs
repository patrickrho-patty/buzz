...
test("customEmoji is keyed by value, not identity", () => {
  clearMarkdownNodeCache();
  const emoji = [{ shortcode: "crew", url: "https://relay/crew.png" }];
  ...
  const second = renderCachedMarkdown({
    ...BASE,
    content: "hi :crew:",
    customEmoji: [{ shortcode: "crew", url: "https://relay/crew.png" }],
  });
  assert.equal(first, second);
  ...
  const third = renderCachedMarkdown({
    ...BASE,
    content: "hi :crew:",
    customEmoji: [{ shortcode: "crew", url: "https://relay/other.png" }],
  });
