...
function AnimatedCountSlot({
  ...
  if (direction === 0 || previous === current) {
    return (
      <span
        className={cn(
          "crew-animated-count__slot",
          isDigit && "crew-animated-count__slot--digit",
        )}
      >
  ...
  if (!isDigit) {
    return (
      <span
        className="crew-animated-count__symbol"
  ...
  return (
    <span
      className="crew-animated-count__slot crew-animated-count__slot--digit"
      data-direction={direction > 0 ? "up" : "down"}
    >
      <span
        className="crew-animated-count__reel"
        key={`${version}-${previous}-${current}`}
      >
        {direction > 0 ? (
          <>
...
export function AnimatedCount({ className, value }: AnimatedCountProps) {
  ...
  return (
    <span className={cn("crew-animated-count", className)}>
      <span className="sr-only">{transition.current}</span>
      <span aria-hidden className="crew-animated-count__motion">
        {slots.map((slot) => (
          <AnimatedCountSlot
      ...
      <span aria-hidden className="crew-animated-count__static">
        {transition.current}
      </span>
    </span>
  );
