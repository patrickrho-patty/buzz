...
function crewDeepLinkUrlTransform(value, key) {
...
function renderMarkdown(content) {
  return renderToStaticMarkup(
    React.createElement(
      ReactMarkdown,
      { urlTransform: crewDeepLinkUrlTransform },
      content,
    ),
  );
}

test("messageLinkUrlTransform: preserves crew://message href", () => {
  const html = renderMarkdown(
    "Click [here](crew://message?channel=abc&id=xyz)",
  );
  // HTML-encoded `&` in attributes is fine — the browser decodes back to `&`.
  assert.match(html, /href="crew:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves crew://message autolink href", () => {
  const html = renderMarkdown("<crew://message?channel=abc&id=xyz>");
  assert.match(html, /href="crew:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves crew://message href with thread", () => {
  const html = renderMarkdown(
    "[link](crew://message?channel=c1&id=m1&thread=t1)",
  );
  assert.match(html, /href="crew:\/\/message\?[^"]*thread=t1"/);
});

test("messageLinkUrlTransform: preserves crew://channel href", () => {
  const html = renderMarkdown(
    "Click [here](crew://channel/580ca78b-9dae-46f3-8854-bd671853ba32)",
  );
  assert.match(
    html,
    /href="crew:\/\/channel\/580ca78b-9dae-46f3-8854-bd671853ba32"/,
  );
});

test("messageLinkUrlTransform: rejects malformed crew://channel href", () => {
  const html = renderMarkdown(
    "Click [here](crew://channel/580ca78b-9dae-46f3-8854-bd671853ba32?extra=true)",
  );
...
test("messageLinkUrlTransform: preserves legacy crew://message href", () => {
  const html = renderMarkdown(
    "Click [here](crew://message?channel=abc&id=xyz)",
  );
  assert.match(html, /href="crew:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
...
test("messageLinkUrlTransform: leaves non-entity crew:// schemes to default", () => {
  // `crew://connect?relay=…` is handled by a different code path (Tauri
  ...
  const html = renderMarkdown(
    "[connect](crew://connect?relay=wss://relay.example)",
  );
  assert.match(html, /href=""/);
});
...
test("crewDeepLinkUrlTransform: preserves crew://project autolink href", () => {
  const projectLink = `crew://project?owner=${OWNER_HEX}&d=onboarding`;
  const html = renderMarkdown(`<${projectLink}>`);
  assert.match(html, /href="crew:\/\/project\?/);
  assert.doesNotMatch(html, /href=""/);
});
...
test("renderEntityLinkAnchor_directEntityLink_returnsAnchorRegardlessOfOrigin", () => {
  ...
  assert.notEqual(
    el,
    null,
    "direct crew://pr link must produce an entity anchor regardless of origin",
  );
});
...
// `remark-gfm`'s autolinker only covers http(s)://, so bare `crew://message`
...
test("remarkMessageLinks: bare crew://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("crew://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
  assert.equal(para.children[0].value, "crew://message?channel=c&id=m");
  assert.equal(para.children[0].data.hName, "message-link");
});

test("remarkMessageLinks: legacy bare crew://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("crew://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
...
test("remarkMessageLinks: mid-sentence URL splits surrounding text", () => {
  const tree = runPlugin(
    paragraph(text("see crew://message?channel=c&id=m here")),
  );
...
test("remarkMessageLinks: two URLs in one text node both replaced", () => {
  const tree = runPlugin(
    paragraph(
      text(
        "first crew://message?channel=a&id=1 then crew://message?channel=b&id=2 done",
      ),
    ),
  );
  const kids = tree.children[0].children;
  ...
  assert.equal(links[0].value, "crew://message?channel=a&id=1");
  assert.equal(links[1].value, "crew://message?channel=b&id=2");
});

test("remarkMessageLinks: trailing sentence punctuation stays outside URL", () => {
  for (const punctuation of [".", ",", ";", ":", "!", "?"]) {
    const tree = runPlugin(
      paragraph(text(`see crew://message?channel=c&id=m${punctuation}`)),
    );
    const kids = tree.children[0].children;
    ...
    assert.equal(kids[1].value, "crew://message?channel=c&id=m", punctuation);
...
test("remarkMessageLinks: URL inside parens keeps closing paren outside", () => {
  const tree = runPlugin(
    paragraph(text("see (crew://message?channel=c&id=m) for details")),
  );
  const kids = tree.children[0].children;
...
test("remarkMessageLinks: non-message crew:// URLs are not matched", () => {
  const original = "crew://connect?relay=wss://x.example";
...
test("remarkMessageLinks: text inside inlineCode is left alone", () => {
    ...
    children: [
      {
        type: "paragraph",
        children: [
          { type: "inlineCode", value: "crew://message?channel=c&id=m" },
        ],
      },
    ],
  };
  ...
  assert.equal(kids[0].value, "crew://message?channel=c&id=m");
});
...
test("bare Crew permalinks render cohesive icon-prefixed chips", () => {
  ...
  const channelLink = `crew://channel/${channelId}`;
  const links = [
    messageLink,
...
test("inline issue and pull-request chips show the repository name without the event hash", () => {
  ...
  assert.match(issueHtml, /data-crew-link-kind="issue"/);
...
test("inline entity chip labels are bounded without truncating their accessible names", () => {
  ...
  const html = renderToStaticMarkup(
    renderEntityLinkAnchor({
      children: null,
      href: `crew://repo?owner=${OWNER_HEX}&d=${longRepository}`,
      onOpenEntityLink: () => {},
      relayOrigin: null,
    }),
  );
...
test("inline message chips omit fetched metadata and the event hash", () => {
  ...
  const markdown = renderCachedMarkdown({
    components: createMarkdownComponents(true, false),
    content: `crew://message?channel=${channelId}&id=${EVENT_HEX}`,
