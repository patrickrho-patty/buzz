...
test("detects known custom emoji-only shortcode messages", () => {
  assert.equal(isEmojiOnlyMessage(":crew:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":crew: :party_parrot:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":Crew:", CUSTOM_EMOJI), true);
});

test("allows mixed unicode and custom emoji", () => {
  assert.equal(isEmojiOnlyMessage("😀 :crew: ❤️", CUSTOM_EMOJI), true);
});
