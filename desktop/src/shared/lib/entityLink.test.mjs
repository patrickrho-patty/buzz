...
test("builders reject invalid identifiers", () => {
  assert.throws(() =>
    buildRepoLink({ owner: "not-a-pubkey", dtag: "crew-world" }),
  );
  assert.throws(() => buildRepoLink({ owner: OWNER, dtag: ".hidden" }));
  assert.throws(() => buildRepoLink({ owner: OWNER, dtag: "a..b" }));
  assert.throws(() =>
    buildPullRequestLink({ id: "short", owner: OWNER, dtag: "crew-world" }),
  );
});
...
test("parseEntityLink round-trips built links", () => {
  ...
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
    value: { type: "pr", id: EVENT_ID, owner: OWNER, dtag: "crew-world" },
  });

  const repoLink = buildRepoLink({ owner: OWNER, dtag: "crew-world" });
  assert.deepEqual(parseEntityLink(repoLink), {
    ok: true,
    value: { type: "repo", owner: OWNER, dtag: "crew-world" },
  });

  const projectLink = buildProjectLink({ owner: OWNER, dtag: "crew-world" });
  assert.deepEqual(parseEntityLink(projectLink), {
    ok: true,
    value: { type: "project", owner: OWNER, dtag: "crew-world" },
  });
});
...
test("commit links select an exact repository commit", () => {
  ...
  assert.equal(
    link,
    `crew://repo?owner=${OWNER}&d=crew-world&tab=commits&commit=${EVENT_ID}`,
  );
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
    value: {
  ...
  assert.deepEqual(
    parseEntityLink(
      `crew://repo?owner=${OWNER}&d=crew-world&tab=files&commit=${EVENT_ID}`,
    ),
...
test("parseEntityLink lowercase-normalizes hex identifiers", () => {
  const parsed = parseEntityLink(
    `crew://issue?id=${EVENT_ID.toUpperCase()}&owner=${OWNER.toUpperCase()}&d=crew-world`,
  );
  assert.deepEqual(parsed, {
    ok: true,
    value: { type: "issue", id: EVENT_ID, owner: OWNER, dtag: "crew-world" },
  });
});
...
test("parseEntityLink rejects malformed links", () => {
  const cases = [
    ...
    [`crew://message?channel=x&id=${EVENT_ID}`, "wrong-host"],
    [`crew://pr?id=${EVENT_ID}&owner=nope&d=repo`, "invalid-owner"],
    [`crew://pr?id=${EVENT_ID}&owner=${OWNER}&d=.hidden`, "invalid-dtag"],
    [`crew://pr?id=${EVENT_ID}&owner=${OWNER}`, "invalid-dtag"],
    [`crew://pr?owner=${OWNER}&d=repo`, "invalid-id"],
    [`crew://issue?id=short&owner=${OWNER}&d=repo`, "invalid-id"],
  ];
...
test("isEntityLink matches entity hosts and excludes message links", () => {
  assert.equal(isEntityLink(`crew://pr?id=${EVENT_ID}`), true);
  assert.equal(isEntityLink(`crew://issue?id=${EVENT_ID}`), true);
...
test("entityLinkProjectRouteId emits the canonical 30617 coordinate route id", () => {
  const parsed = parseEntityLink(
    buildRepoLink({ owner: OWNER, dtag: "crew-world" }),
  );
  assert.ok(parsed.ok);
  assert.equal(
    entityLinkProjectRouteId(parsed.value),
    `30617:${OWNER}:crew-world`,
  );
});

test("entityLinkProjectRouteId routes project links to the 30621 coordinate", () => {
  const parsed = parseEntityLink(
    buildProjectLink({ owner: OWNER, dtag: "crew-world" }),
  );
  assert.ok(parsed.ok);
  assert.equal(
    entityLinkProjectRouteId(parsed.value),
    `30621:${OWNER}:crew-world`,
  );
});
...
test("coordinate links carry an optional workspace tab", () => {
  ...
  assert.equal(link, `crew://project?owner=${OWNER}&d=crew-world&tab=prs`);
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
    value: { type: "project", owner: OWNER, dtag: "crew-world", tab: "prs" },
  });
  ...
  assert.deepEqual(parseEntityLink(repoLink), {
    ok: true,
    value: { type: "repo", owner: OWNER, dtag: "crew-world", tab: "issues" },
  });
  ...
  assert.throws(() =>
    buildRepoLink({ owner: OWNER, dtag: "crew-world", tab: "overview" }),
  );
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=crew-world&tab=overview`),
    { ok: false, reason: "invalid-tab" },
  );
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=crew-world&tab=`),
    { ok: false, reason: "invalid-tab" },
  );
  assert.deepEqual(
    parseEntityLink(
      `crew://pr?id=${EVENT_ID}&owner=${OWNER}&d=crew-world&tab=prs`,
    ),
...
test("isLinkableCoordinate gates coordinates the link format cannot express", () => {
  assert.equal(isLinkableCoordinate(OWNER, "crew-world"), true);
  ...
  assert.equal(isLinkableCoordinate("not-a-pubkey", "crew-world"), false);
});
...
test("parseEntityLink rejects noncanonical extras", () => {
  ...
  assert.deepEqual(
    parseEntityLink(
      `crew://pr/ignored?id=${EVENT_ID}&owner=${OWNER}&d=crew-world`,
    ),
    { ok: false, reason: "unexpected-path" },
  );
  // Fragment — not part of the canonical format.
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=crew-world#section`),
    { ok: false, reason: "unexpected-fragment" },
  );
  // Unknown query parameter — reject to preserve forward-compat posture.
  assert.deepEqual(
    parseEntityLink(
      `crew://repo?owner=${OWNER}&d=crew-world&relay=wss%3A%2F%2Frelay.example`,
    ),
    { ok: false, reason: "unknown-param" },
  );
  // Duplicate required parameter — reject.
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=crew-world&owner=${OWNER}`),
