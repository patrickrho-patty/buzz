...
test("commit links select an exact repository commit", () => {
  ...
  assert.equal(
    link,
    `crew://repo?owner=${OWNER}&d=buzz-world&tab=commits&commit=${EVENT_ID}`,
  );
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
    value: {
  ...
  assert.deepEqual(
    parseEntityLink(
      `crew://repo?owner=${OWNER}&d=buzz-world&tab=files&commit=${EVENT_ID}`,
    ),
...
test("parseEntityLink lowercase-normalizes hex identifiers", () => {
  const parsed = parseEntityLink(
    `crew://issue?id=${EVENT_ID.toUpperCase()}&owner=${OWNER.toUpperCase()}&d=buzz-world`,
  );
...
test("coordinate links carry an optional workspace tab", () => {
  ...
  assert.equal(link, `crew://project?owner=${OWNER}&d=buzz-world&tab=prs`);
  assert.deepEqual(parseEntityLink(link), {
    ok: true,
  ...
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=buzz-world&tab=overview`),
    { ok: false, reason: "invalid-tab" },
  );
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=buzz-world&tab=`),
    { ok: false, reason: "invalid-tab" },
  );
  assert.deepEqual(
    parseEntityLink(
      `crew://pr?id=${EVENT_ID}&owner=${OWNER}&d=buzz-world&tab=prs`,
    ),
...
test("parseEntityLink rejects noncanonical extras", () => {
  ...
  assert.deepEqual(
    parseEntityLink(
      `crew://pr/ignored?id=${EVENT_ID}&owner=${OWNER}&d=buzz-world`,
    ),
    { ok: false, reason: "unexpected-path" },
  );
  // Fragment — not part of the canonical format.
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=buzz-world#section`),
    { ok: false, reason: "unexpected-fragment" },
  );
  // Unknown query parameter — reject to preserve forward-compat posture.
  assert.deepEqual(
    parseEntityLink(
      `crew://repo?owner=${OWNER}&d=buzz-world&relay=wss%3A%2F%2Frelay.example`,
    ),
    { ok: false, reason: "unknown-param" },
  );
  // Duplicate required parameter — reject.
  assert.deepEqual(
    parseEntityLink(`crew://repo?owner=${OWNER}&d=buzz-world&owner=${OWNER}`),
