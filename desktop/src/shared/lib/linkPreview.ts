...
const SUPPORTED_URL_RE =
  /(^|[\s([{<>"'])(https:\/\/[^\s<>"'\]]+|https?:\/\/[^\s<>"'\]]+\/git\/[a-f0-9]{64}\/[^\s<>"'\]]+|crew:\/\/(?:pr|issue|repo|project)\?[^\s<>"'\]]+|(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^\s<>"'\]]+)/gi;
const MARKDOWN_SUPPORTED_LINK_RE =
  /!?\[([^\]\n]+)\]\((https:\/\/[^)\s<>"']+|https?:\/\/[^)\s<>"']+\/git\/[a-f0-9]{64}\/[^)\s<>"']+|crew:\/\/(?:pr|issue|repo|project)\?[^)\s<>"']+|(?:(?:www\.)?github\.com|(?:www\.)?linear\.app|drive\.google\.com|docs\.google\.com)\/[^)\s<>"']+)\)/gi;
 ...
 */
export function crewEntityFallbackTitle(link: ParsedEntityLink): string {
  if (link.type === "repo" || link.type === "project") return link.dtag;
  return `${link.dtag} #${link.id.slice(0, 8)}`;
}

/**
 * Map a `crew://pr|issue|repo|project` deep link onto a preview card. The
...
function parseBuzzGitLink(
  ...
  const match = CREW_GIT_PATH_RE.exec(parsed.pathname);
