...
test("rewriteRelayUrl: matches relay origin case-insensitively (uppercase saved community URL)", async () => {
    ...
    __TAURI_INTERNALS__: {
      invoke(command) {
        ...
        if (command === "get_relay_http_url") {
          ...
          // emits lowercased media URLs (normalize_host in crew-core).
          return Promise.resolve("https://PENDING-SEED.communities.crew.xyz");
        }
        return Promise.reject(new Error(`Unexpected command: ${command}`));
      },
    },
  ...
  try {
    ...
    const relayMediaUrl = `https://pending-seed.communities.crew.xyz/media/${HASH}.png`;
    assert.equal(
