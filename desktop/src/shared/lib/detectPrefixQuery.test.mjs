...
const CHANNELS = ["crew-bugs", "crew dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#crew", CHANNELS), {
    query: "crew",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #crew", CHANNELS), {
    query: "crew",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#crew", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#crew de", CHANNELS), {
    query: "crew de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#crew de` — `#` preceded by `x`, no boundary → no query even though
  // "crew de" is a prefix of a known channel.
  assert.equal(at("#", "x#crew de", CHANNELS), null);
});
