...
test("uses the newest per-profile timestamp for user-label cache buckets", () => {
  ...
  assert.equal(localStorage.getItem("crew-user-labels.v1:all-stale"), null);
  assert.notEqual(localStorage.getItem("crew-user-labels.v1:one-fresh"), null);
});

test("leaves malformed and timestamp-free cache entries untouched", () => {
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:malformed", "not json"],
    ["crew-channels.v1:no-timestamp", JSON.stringify({ payload: "cache" })],
...
test("fallback path: boot-floor setTimeout fires first, then per-slice setTimeout processes entries", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:startup", snapshot(0)],
  ]);
  installWindow(localStorage, {
    setInterval: () => 1,
    clearInterval() {},
  ...
  try {
    ...
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:startup"),
      null,
...
test("scheduler: boot-floor gate, idle callback with SWEEP_IDLE_TIMEOUT_MS, hourly interval, no visibilitychange listener", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:stale", snapshot(now - 14 * DAY_MS)],
  ]);
  installWindow(localStorage, {
  ...
  try {
    ...
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:stale"),
      null,
    ...
    localStorage.setItem(
      "crew-channel-messages.v1:interval",
    ...
    assert.equal(
      localStorage.getItem("crew-channel-messages.v1:interval"),
      null,
    ...
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:vis"),
      null,
...
test("chunked sweep: keys processed across multiple idle slices; stale entries removed only at end", () => {
  ...
  const entries = Array.from({ length: entryCount }, (_, i) => [
    `crew-channel-messages.v1:key-${i}`,
...
test("chunked sweep: keys deleted between snapshot and slice are silently skipped", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:gone", snapshot(now - 14 * DAY_MS)],
    ["crew-channel-messages.v1:fresh", snapshot(now - DAY_MS)],
  ]);
  ...
  try {
    ...
    localStorage.store.delete("crew-channel-messages.v1:gone");
    ...
    // "fresh" key untouched.
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:fresh"),
      null,
    );
  } finally {
    stop();
...
test("chunked sweep: key rewritten fresh before batch removal is not removed", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:target", staleValue],
    ["crew-channel-messages.v1:other", staleValue],
  ]);
  ...
  try {
    ...
    localStorage.setItem("crew-channel-messages.v1:target", freshValue);
    ...
    assert.equal(
      localStorage.getItem("crew-channel-messages.v1:target"),
      freshValue,
      "key rewritten fresh before batch removal must not be removed",
    );
    assert.equal(
      localStorage.getItem("crew-channel-messages.v1:other"),
      null,
...
test("chunked sweep: getItem error on one key does not abort sweep; stale siblings still removed", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:error-key", "value"],
    ["crew-channel-messages.v1:stale-key", snapshot(now - 14 * DAY_MS)],
    ["crew-channel-messages.v1:fresh-key", snapshot(now - DAY_MS)],
  ]);

  // Override getItem so that reading "error-key" throws.
  const realGetItem = localStorage.getItem.bind(localStorage);
  localStorage.getItem = (key) => {
    if (key === "crew-channel-messages.v1:error-key") {
  ...
  try {
    ...
    assert.equal(
      localStorage.getItem("crew-channel-messages.v1:stale-key"),
      null,
      "stale-key must be removed despite error on error-key",
    );
    // Fresh key untouched.
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:fresh-key"),
      null,
...
test("chunked sweep: large values deferred once on zero-budget slices then parsed; converges when all values are large", () => {
  ...
  const localStorage = makeLocalStorage([
    ["crew-channel-messages.v1:large-a", largeStaleValue],
    ["crew-channel-messages.v1:large-b", largeStaleValue],
  ]);
  ...
  try {
    ...
    idleCallbacks[0]({ timeRemaining: () => 0 });
    assert.notEqual(
      localStorage.getItem("crew-channel-messages.v1:large-a"),
      null,
    ...
    assert.equal(
      localStorage.getItem("crew-channel-messages.v1:large-b"),
      null,
