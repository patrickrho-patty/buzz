 ...
 *
 * Only URLs hosted on the Crew relay are rewritten. External Blossom URLs
 ...
 * (the Crew relay), resulting in 404s.
 */
...
/**
 * Cached relay origin (e.g. "https://crew-oss.stage.blox.sqprod.co"),
...
/**
 * The relay origin (e.g. `https://crew-oss.stage.blox.sqprod.co`) if it has
...
/**
 * If `url` is a Blossom media URL hosted on the Crew relay, rewrite it
...
export function rewriteRelayUrl(url: string): string {
  ...
  // was typed with uppercase (e.g. wss://PENDING-SEED.communities.crew.xyz).
  if (cachedRelayOrigin) {
