...
/**
 * Payload emitted by the Rust deep-link handler for `crew://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `crew://join?…` —
 ...
 *
 * When a `crew://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `crew://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `crew://message?…` is handled separately by `listenForNavigationDeepLinks`,
...
/**
 * Register a listener for `deep-link-entity` events — the `crew://` share
