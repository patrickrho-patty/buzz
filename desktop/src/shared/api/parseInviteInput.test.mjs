...
// ---------------------------------------------------------------------------
// crew://join deep link URLs
// ---------------------------------------------------------------------------

test("parseInviteInput_crew_join_with_wss_relay_returns_relay_and_code", () => {
  const result = parseInviteInput(
    "crew://join?relay=wss://relay.example.com&code=abc123",
  );
  assert.deepEqual(result, {
    relayWsUrl: "wss://relay.example.com",
    code: "abc123",
...
test("parseInviteInput_crew_join_with_ws_relay_returns_relay_and_code", () => {
  const result = parseInviteInput(
    "crew://join?relay=ws://localhost:3000&code=testcode",
  );
  assert.deepEqual(result, {
...
test("parseInviteInput_crew_join_with_encoded_relay_param", () => {
  const result = parseInviteInput(
...
test("parseInviteInput_crew_join_rejects_non_ws_relay", () => {
  const result = parseInviteInput(
    "crew://join?relay=https://relay.example.com&code=abc123",
  );
  assert.equal(result, null);
});

test("parseInviteInput_crew_join_rejects_missing_code", () => {
  assert.equal(
    parseInviteInput("crew://join?relay=wss://relay.example.com"),
    null,
  );
});

test("parseInviteInput_crew_join_rejects_missing_relay", () => {
  assert.equal(parseInviteInput("crew://join?code=abc123"), null);
});
...
test("parseInviteInput_rejects_crew_join_with_credentials", () => {
  assert.equal(
    parseInviteInput(
      "crew://user:pass@join?relay=wss://relay.example.com&code=abc123",
    ),
    null,
  );
});

test("parseInviteInput_rejects_crew_join_with_hash", () => {
  assert.equal(
    parseInviteInput(
      "crew://join?relay=wss://relay.example.com&code=abc123#frag",
    ),
    null,
  );
});
...
test("parseInviteInput_crew_join_rejects_credentials_in_nested_relay", () => {
  assert.equal(
    parseInviteInput(
...
test("parseInviteInput_crew_join_rejects_fragment_in_nested_relay", () => {
  assert.equal(
    parseInviteInput(
      "crew://join?relay=wss%3A%2F%2Frelay.example.com%23frag&code=abc",
    ),
    null,
  );
});
