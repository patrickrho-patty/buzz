...
import { migrateCrewStorageKeys } from "@/shared/lib/buzzKeyMigration";
...
function resetDevWebviewStateFromUrl() {
  ...
  // WebKit groups every Crew binary under one disk directory, but storage is
...
function configureDevE2eBridgeFromUrl() {
  ...
  e2eWindow.__CREW_E2E__ ??= { mode: "mock" };
  ...
  window.localStorage.setItem("crew-active-community-id", E2E_COMMUNITY_ID);
...
async function installE2eBridgeIfConfigured() {
  ...
  if (
    ...
    !(window as E2eWindow).__CREW_E2E__
  ) {
    return;
  }
