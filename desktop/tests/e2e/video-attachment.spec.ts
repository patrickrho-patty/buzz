...
// The fresh-profile default is the Crew theme, which pins the neutral accent
// regardless of the stored accent color. Accent-driven review foreground
// assertions must run on a non-Crew theme for the seeded accent to apply.
...
async function installVideoReviewHarness(
  ...
  await page.addInitScript(
    ({ accentColor, themeName }) => {
      if (themeName) {
        window.localStorage.setItem("crew-theme", themeName);
      }
      window.localStorage.setItem("crew-accent-color", accentColor);
