 ...
 *       empty baked env, simulating an OSS install with no CREW_AGENT_PROVIDER).
 *
 ...
 *       runtime (crew-agent) and model discovery runs → model combobox is non-empty.
...
test.describe("agent provider dropdown screenshots", () => {
  ...
  // CREW_AGENT_PROVIDER is baked and hideProviderIds is empty → v1 appears.
  ...
  // default runtime (crew-agent via getDefaultPersonaRuntime) and model
  ...
  test("03-builtin-edit-runtime-seeded", async ({ page }) => {
    ...
    // must have run and selected the app default (crew-agent in the mock catalog).
