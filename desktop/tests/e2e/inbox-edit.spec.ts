...
type MockWindow = Window & {
  ...
  __CREW_E2E_PUSH_MOCK_FEED_ITEM__?: (item: MockFeedItem) => unknown;
};
...
function editCommandCount(page: import("@playwright/test").Page) {
  return page.evaluate(
    () =>
      ((window as MockWindow).__CREW_E2E_COMMAND_PAYLOADS__ ?? []).filter(
...
test("editing an immediate attachment reply preserves its media tags", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockWindow).__CREW_E2E_PUSH_MOCK_FEED_ITEM__ ===
      "function",
  );
  ...
  const releasedEchoes = await page.evaluate(() => {
    const release = (window as MockWindow)
      .__CREW_E2E_RELEASE_SEND_MESSAGE_LIVE_ECHO__;
    if (!release) {
