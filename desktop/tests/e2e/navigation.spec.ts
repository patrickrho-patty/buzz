...
test("mixed Crew permalinks render as chips in the composer", async ({
  ...
  const links = [
    ...
    `crew://repo?owner=${owner}&d=crew-world`,
    `crew://pr?id=${pullRequestId}&owner=${owner}&d=crew-world`,
  ...
  const chips = composerInput.locator('[data-composer-crew-link=""]');
  await expect(chips).toHaveCount(5);
  await expect(chips.nth(0)).toHaveText("general");
  await expect(chips.nth(1)).toHaveText("general");
  await expect(chips.nth(2)).toHaveText("crew-world");
  // PR and issue chips both use repository identity only, matching rendered chips.
  await expect(chips.nth(3)).toHaveText("crew-world");
  await expect(chips.nth(4)).toHaveText("crew-world");
  ...
  await expect(composerInput).not.toContainText("crew://");
});
...
test("message links to visible root messages open the thread panel", async ({
  ...
  const link =
    "crew://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=mock-general-welcome";
  const composerInput = page.getByTestId("message-input");
  ...
  await expect(composerLink).toHaveAttribute("data-crew-link", "");
  await expect(composerLink).toHaveAttribute("title", "Thread in #general");
  await expect(composerInput).not.toContainText("crew://message");
  ...
  await expect(
    messageTooltip.locator('[data-crew-tooltip-metadata-content=""]'),
  ...
  await expect(
    channelTooltip.locator('[data-crew-tooltip-metadata-content=""]'),
  ...
  const linkMenu = page.locator("[data-crew-link-context-menu]");
...
test("direct-message tooltip metadata stays on one physical line", async ({
  ...
  await page
    ...
    .fill(`DM link crew://message?channel=${dmChannelId}&id=${dmMessageId}`);
  await page.getByTestId("send-message").click();
...
test("message links explain when preview metadata is unavailable", async ({
  ...
  await page
    .getByTestId("message-input")
    .fill(
      `Missing preview crew://message?channel=9a1657ac-f7aa-5db0-b632-d8bbeb6dfb50&id=${missingMessageId}`,
    );
  await page.getByTestId("send-message").click();
  ...
  await expect(missingMessageLink).toHaveClass(/crew-link-unavailable/);
