...
test("first channel load paints the first window without waiting for the row-floor top-up", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof window.__CREW_E2E_EMIT_MOCK_MESSAGE__ === "function" &&
      typeof window.__CREW_E2E_PREPEND_MOCK_HISTORY__ === "function",
  );
  ...
  await page.evaluate(() => {
    ...
    window.__CREW_E2E__ = {
      ...window.__CREW_E2E__,
      mock: { ...window.__CREW_E2E__?.mock, channelWindowDelayMs: 5_000 },
    };
  });
...
test("preserves user scroll while older channel history loads", async ({
  ...
  await page.evaluate(() => {
    window.__CREW_E2E__ = {
      ...window.__CREW_E2E__,
      mock: { ...window.__CREW_E2E__?.mock, channelWindowDelayMs: 1_000 },
    };
    (
...
test("does not teleport upward when user abandons fetch by jumping to bottom", async ({
  ...
  await page.evaluate(() => {
    ...
    window.__CREW_E2E_PREPEND_MOCK_HISTORY__?.({
      channelName: "general",
      count: 600,
      lineCount: 3,
    });
...
const REAL_CREW_BUGS_IMAGE_SHA =
  "ff2862080bac3d009f97cad4bb94e6efec328eaaee058a405e854acd49fc1483";
const REAL_CREW_BUGS_IMAGE_URL = `https://sprout-oss.stage.blox.sqprod.co/media/${REAL_CREW_BUGS_IMAGE_SHA}.png`;
const REAL_CREW_BUGS_IMAGE_TAG = [
  "imeta",
  `url ${REAL_CREW_BUGS_IMAGE_URL}`,
  "m image/png",
  `x ${REAL_CREW_BUGS_IMAGE_SHA}`,
  "size 26257",
  "dim 951x244",
  "filename image.png",
] as string[];
...
test("reserves real crew-bugs imeta image height before image loads", async ({
  ...
  await page.evaluate(
    ...
    {
      content: `this setting gets reverted on every update\n![image](${REAL_CREW_BUGS_IMAGE_URL})`,
      extraTags: [REAL_CREW_BUGS_IMAGE_TAG],
    },
  );
...
test("deep-link to a message in older history scrolls and highlights it", async ({
  ...
  const prependedIds: string[] = await page.evaluate(() => {
    ...
    const events = window.__CREW_E2E_PREPEND_MOCK_HISTORY__?.({
      channelName: "general",
      count: 600,
      lineCount: 3,
    });
...
test("older-history fetches never overlap (no concurrent in-flight requests)", async ({
  ...
  await page.evaluate(() => {
    ...
    window.__CREW_E2E__ = {
      ...window.__CREW_E2E__,
      mock: { ...window.__CREW_E2E__?.mock, channelWindowDelayMs: 400 },
    };
  });
...
test("older-history spinner stays visible in viewport while fetching mid-scroll", async ({
  ...
  await page.evaluate(() => {
    ...
    window.__CREW_E2E__ = {
      ...window.__CREW_E2E__,
      mock: { ...window.__CREW_E2E__?.mock, channelWindowDelayMs: 2_000 },
    };
  });
...
test("older-history prepend keeps the reading row fixed (no jump to oldest)", async ({
  ...
  await page.evaluate(() => {
    window.__CREW_E2E__ = {
      ...window.__CREW_E2E__,
      mock: { ...window.__CREW_E2E__?.mock, channelWindowDelayMs: 1_500 },
    };
    (
      window as unknown as { __CHANNEL_WINDOW_INFLIGHT__?: number }
    ).__CHANNEL_WINDOW_INFLIGHT__ = 0;
