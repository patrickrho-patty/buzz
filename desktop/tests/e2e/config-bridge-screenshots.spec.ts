...
test.describe("config bridge screenshots", () => {
  ...
  test("01 — folded config panel", async ({ page }) => {
    ...
    await expect(panel.getByText("Set in Crew")).toHaveCount(0);
  ...
  test("06 — crew-agent empty MCP servers", async ({ page }) => {
    await installMockBridge(page, {
      managedAgents: [
        {
          pubkey: CREW_AGENT_PUBKEY,
          name: "Crew Agent",
          status: "running" as const,
          channelNames: ["agents"],
        },
    ...
    const panel = await openAgentProfileFromChannel(page, "Crew Agent", {
      anchorText: "MCP servers",
      tab: "Runtime",
    });
