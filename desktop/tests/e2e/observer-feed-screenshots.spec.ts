...
async function waitForSeedHook(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () => typeof window.__CREW_E2E_SEED_OBSERVER_EVENTS__ === "function",
    null,
    { timeout: 10_000 },
  );
}
...
async function seedObserverEvents(
  ...
  await page.evaluate(
    ({ pubkey, evts }) => {
      window.__CREW_E2E_SEED_OBSERVER_EVENTS__?.({
        agentPubkey: pubkey,
        events: evts,
      });
    },
...
test.describe("observer feed screenshots", () => {
  ...
  test("11 — first-turn bundle: standalone system-prompt card + Checks ingress (per-turn context only)", async ({
    ...
    // (Crew event / Thread context) — no Base/System/Team Instructions/Core Memory/Channel Canvas sections.
    await seedObserverEvents(page, OBSERVER_AGENT_PUBKEY, [
      {
        seq: 1,
        timestamp: NOW,
    ...
    // Per-turn prompt context (Crew event / Thread context) does NOT appear
    ...
    // (Crew event, Thread context). Base/System/Team Instructions/Core Memory/Channel Canvas must NOT appear.
    ...
    // Only per-turn context sections (Crew event + Thread context) — no system-prompt sections.
    expect(sectionTitles.length).toBe(2);
    expect(sectionTitles[0]).toContain("Crew event");
