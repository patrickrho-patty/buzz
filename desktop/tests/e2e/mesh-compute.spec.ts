...
test("Share compute chooses a model before sharing", async ({ page }) => {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => (window as E2eWindow).__CREW_E2E_COMMANDS__ ?? []),
    )
    .toContain("mesh_start_node");
  await expect
    .poll(() =>
      page.evaluate(
        () => (window as E2eWindow).__CREW_E2E_COMMAND_PAYLOADS__ ?? [],
      ),
    )
    .toContainEqual({
...
test("a consuming client can switch to sharing its saved local model", async ({
  ...
  await page.waitForFunction(
    () => typeof (window as E2eWindow).__CREW_E2E_SET_MESH__ === "function",
  );
  await page.evaluate(() => {
    (window as E2eWindow).__CREW_E2E_SET_MESH__?.({
      nodeState: "running",
      nodeMode: "client",
    });
  });
  ...
  const commands = await page.evaluate(() => ({
    names: (window as E2eWindow).__CREW_E2E_COMMANDS__ ?? [],
    payloads: (window as E2eWindow).__CREW_E2E_COMMAND_PAYLOADS__ ?? [],
  }));
