...
async function setHuddleSnapshot(
  ...
  await page.evaluate(
    async ({ nextMembers, nextTranscriptionEnabled }) => {
      ...
      ).__CREW_E2E_SET_MOCK_HUDDLE_SNAPSHOT__;
      if (!setSnapshot) {
...
test("keeps the drawer open until the huddle is expanded", async ({ page }) => {
  ...
  const gradientUnderlay = page.locator(".crew-theme-gradient-underlay");
  ...
  const huddleShell = page.locator(".crew-huddle-shell");
  await expect(huddleShell).toHaveAttribute("data-huddle-open", "true");
  const huddleBackdrop = page.locator(".crew-huddle-drawer-backdrop");
  await expect(huddleBackdrop).toHaveClass(/crew-huddle-drawer-backdrop-open/);
  const [huddleBackdropColor, huddleDrawerColor] = await Promise.all([
    huddleBackdrop.evaluate(
      (element) => getComputedStyle(element).backgroundColor,
    ),
    page
      .locator(".crew-huddle-drawer")
      .first()
  ...
  await expect
    .poll(() =>
      huddleBackdrop.evaluate((element) => {
        ...
        const appSurface = shell?.querySelector(".crew-huddle-app-surface");
  ...
  const mainContentSurface = page.locator(
    "[data-crew-content-surface]:not([data-crew-content-unframed])",
  );
  ...
  await expect
    ...
    .toBe(1);
  await expect(page.locator(".crew-huddle-shell")).toHaveAttribute(
    "data-huddle-open",
    "false",
  );
...
test("floats the in-app huddle tray over the glass background", async ({
  ...
  const shell = page.locator('.crew-huddle-shell[data-huddle-window="false"]');
  const slot = shell.locator(".crew-huddle-drawer-slot");
  const drawer = slot.locator(":scope > .crew-huddle-drawer");
  const backdrop = shell.locator(".crew-huddle-drawer-backdrop");
...
test("keeps the popped-out huddle dock full-width over glass", async ({
  ...
  const root = page.locator("html");
  const shell = page.locator('.crew-huddle-shell[data-huddle-window="true"]');
  const slot = shell.locator(".crew-huddle-drawer-slot");
  const drawer = slot.locator(":scope > .crew-huddle-drawer");
...
test("shows speaker identity on every huddle chat message", async ({
  ...
  await page.evaluate(() => {
    const threadRoot = window.__CREW_E2E_EMIT_MOCK_MESSAGE__?.({
      channelName: "huddle",
...
test("keeps main-app shortcuts from navigating the huddle room", async ({
  ...
  await page.evaluate(async (channelId) => {
    await window.__CREW_E2E_EMIT_TAURI_EVENT__?.("deep-link-message", {
      channelId,
  ...
  await page.evaluate(async () => {
    await window.__CREW_E2E_EMIT_TAURI_EVENT__?.("deep-link-connect", null);
  });
  await expect
    .poll(() =>
      page.evaluate(async () =>
        window.__CREW_E2E_INVOKE_MOCK_COMMAND__?.(
...
test("animates the responding agent with the shared speaker ring", async ({
  ...
  const agentAvatar = page
    ...
    .locator(".crew-huddle-speaking-avatar")
    .nth(1);
  await expect(agentAvatar).toBeVisible();
  await page.evaluate(async (pubkey) => {
    await window.__CREW_E2E_EMIT_MOCK_HUDDLE_TTS_SPEAKER__?.({
      pubkey,
      level: 0.8,
    });
  }, TEST_IDENTITIES.alice.pubkey);
  await expect
    .poll(() =>
      agentAvatar.evaluate((element) =>
        element.style.getPropertyValue("--crew-huddle-speaker-opacity"),
      ),
    )
    .toBe("0.890");
...
test("assigns distinct agent voices and exposes compact per-agent controls", async ({
  ...
  await expect
    .poll(async () => {
      const state = (await page.evaluate(() =>
        window.__CREW_E2E_INVOKE_MOCK_COMMAND__?.("get_huddle_state"),
      )) as {
  ...
  const assignedVoices = await page.evaluate(async () => {
    const state = (await window.__CREW_E2E_INVOKE_MOCK_COMMAND__?.(
      "get_huddle_state",
    )) as {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const updates = (window.__CREW_E2E_COMMAND_LOG__ ?? []).filter(
...
test("returns the companion transcript to the same channel in the main app", async ({
  ...
  await page.evaluate(async () => {
    await window.__CREW_E2E_INVOKE_MOCK_COMMAND__?.("close_huddle_companion");
  });
...
test("removes an agent from its menu without showing an extra participant control", async ({
  ...
  await expect
    .poll(() =>
      page.evaluate(() =>
        (window.__CREW_E2E_COMMAND_LOG__ ?? []).some(
...
test("starts muted with Push to Talk while preserving manual microphone control", async ({
  ...
  await page.evaluate(async () => {
    await window.__CREW_E2E_EMIT_TAURI_EVENT__?.("ptt-state", true);
  });
  await expect(muteButton).toBeVisible();
  await muteButton.click();
  await expect
  ...
  await page.evaluate(async () => {
    await window.__CREW_E2E_EMIT_TAURI_EVENT__?.("ptt-state", false);
  });
  await expect(unmuteButton).toBeVisible();
