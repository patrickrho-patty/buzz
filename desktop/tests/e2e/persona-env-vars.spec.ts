...
async function invokeTauri<T>(
  ...
  return page.evaluate(
    async ({ command: c, payload: p }) => {
      ...
      const invoke =
        w.__CREW_E2E_INVOKE_MOCK_COMMAND__ ?? w.__TAURI_INTERNALS__?.invoke;
...
test("env vars editor renders in PersonaDialog new-persona form", async ({
  ...
  // Initially empty (no rows — crew-agent with no provider has no required keys).
...
test("persona model options follow the selected LLM provider", async ({
  ...
  await expect(provider).toContainText("Crew Agent (default)");
