...
async function pushMockInboxFeedItems(page: Page, items: MockInboxFeedItem[]) {
  await page.waitForFunction(
    () =>
      typeof (window as Window & { __CREW_E2E_PUSH_MOCK_FEED_ITEM__?: unknown })
        .__CREW_E2E_PUSH_MOCK_FEED_ITEM__ === "function",
  );
  await page.evaluate(
...
async function seedChannelActivity(
  ...
  if (includeAgent) {
    await page.waitForFunction(
      () =>
        typeof (window as Window & { __CREW_E2E_SEED_ACTIVE_TURNS__?: unknown })
          .__CREW_E2E_SEED_ACTIVE_TURNS__ === "function",
    );
    await page.evaluate(
