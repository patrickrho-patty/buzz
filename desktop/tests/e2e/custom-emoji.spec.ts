...
//
// The `:crew:` shortcode lives in a member-authored kind:30030 set
// (d=`crew:custom-emoji`) served by the mock bridge from two distinct
    ...
    .poll(async () =>
      page.evaluate(
        ({ ch }) =>
          ...
          ).__CREW_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({ channelName: ch }) ??
          false,
        { ch: channelName },
      ),
    )
...
async function quickReactionStorageContains(
  ...
  return page.evaluate((selectedEmoji) => {
    for (let index = 0; index < window.localStorage.length; index += 1) {
      ...
      if (!key?.startsWith("crew.quick-reaction-emojis.v1")) continue;
...
test("adding a custom emoji while editing keeps the image after save (Bug 2)", async ({
  ...
  // <img>, not a bare `:crew:`. The pre-fix edit path shipped no emoji tags,
