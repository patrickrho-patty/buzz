...
async function setMockWebsocketSendsStalled(
  ...
  await page.evaluate((shouldStall) => {
    ...
    ).__CREW_E2E_SET_STALL_WEBSOCKET_SENDS__;
    if (!setter) {
...
async function disconnectMockWebsockets(page: import("@playwright/test").Page) {
  const disconnected = await page.evaluate(() => {
    ...
    ).__CREW_E2E_DISCONNECT_MOCK_WEBSOCKETS__;
    if (!disconnect) {
...
async function restartMockWebsockets(page: import("@playwright/test").Page) {
  const restarted = await page.evaluate(() => {
    ...
    ).__CREW_E2E_RESTART_MOCK_WEBSOCKETS__;
    if (!restart)
...
async function setMockWebsocketUnavailable(
  ...
  await page.evaluate((value) => {
    const setUnavailable = window.__CREW_E2E_SET_MOCK_WEBSOCKET_UNAVAILABLE__;
    if (!setUnavailable) {
...
async function activateRelayRateLimit(
  ...
  await page.evaluate((duration) => {
    const activate = window.__CREW_E2E_ACTIVATE_RELAY_RATE_LIMIT__;
    if (!activate) {
...
async function getMockWebsocketConnectAttempts(
  ...
  return page.evaluate(() => {
    const getAttempts = window.__CREW_E2E_GET_WEBSOCKET_CONNECT_ATTEMPTS__;
    return getAttempts?.() ?? [];
  });
}
...
async function queueAuthResponses(
  ...
  await page.evaluate((queued) => {
    const queue = window.__CREW_E2E_QUEUE_AUTH_RESPONSES__;
...
async function closeLiveSubscriptions(
  ...
  const closed = await page.evaluate((message) => {
    const close = window.__CREW_E2E_CLOSE_LIVE_SUBSCRIPTIONS__;
...
async function queueChannelHistoryCloses(
  ...
  await page.evaluate((queued) => {
    const queue = window.__CREW_E2E_QUEUE_CHANNEL_HISTORY_CLOSES__;
    if (!queue) {
...
async function driveConnectionDegraded(
  ...
  await page.evaluate((s) => {
    const setter = (
      window as Window & {
        __CREW_E2E_SET_RELAY_CONNECTION_STATE__?: (state: string) => void;
      }
    ).__CREW_E2E_SET_RELAY_CONNECTION_STATE__;
    if (!setter) {
...
test("stalled early AUTH signing times out and starts a replacement dial", async ({
  ...
  await expect
    .poll(
      () =>
        page.evaluate(() => window.__CREW_E2E_GET_RELAY_CONNECTION_STATE__?.()),
      { timeout: 5_000 },
    )
    .toBe("connected");
});
    ...
    .poll(
      () =>
        page.evaluate(() => {
          const getState = (
            window as Window & {
              __CREW_E2E_GET_RELAY_CONNECTION_STATE__?: () => string;
            }
          ).__CREW_E2E_GET_RELAY_CONNECTION_STATE__;
...
test("sub-2s degraded flap invalidates relay queries on recovery", async ({
  ...
  await page.evaluate(() => {
    const queryClient = window.__CREW_E2E_QUERY_CLIENT__ as
      | {
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (
            window.__CREW_E2E_QUERY_CLIENT__ as unknown as {
              __rawHealInvalidations?: number;
            }
