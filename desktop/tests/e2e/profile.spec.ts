    ...
    .poll(async () => {
      return page.evaluate((channelName) => {
        return (
          ...
          ).__CREW_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({ channelName }) ?? false
        );
      }, channelName);
    })
    .toBe(true);
...
test("notification settings drive the Inbox badge and desktop alerts", async ({
  ...
  async function getAppBadgeCount() {
    return page.evaluate(() => {
      ...
      return win.__CREW_E2E_APP_BADGE_COUNT__ ?? 0;
    });
  }
  ...
  await page.evaluate(() => {
    ...
    win.__CREW_E2E_PUSH_MOCK_FEED_ITEM__?.({
      category: "mention",
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return win.__CREW_E2E_NOTIFICATIONS__?.length ?? 0;
      }),
    )
    .toBe(1);
  ...
  const notifications = await page.evaluate(() => {
    ...
    return win.__CREW_E2E_NOTIFICATIONS__ ?? [];
  });
  ...
  const clickedNotification = await page.evaluate(() => {
    ...
    return win.__CREW_E2E_CLICK_NOTIFICATION__?.(0) ?? false;
  });
