...
// Live gate: boots a REAL crew-relay process, points the app at it, SIGTERMs
...
//
// Requires: CREW_E2E_RELAY_RESTART=1, CREW_E2E_RELAY_BIN, and
// CREW_E2E_DATABASE_URL (plus reachable Redis and media object store, same
// infra as the agents-everywhere live gate).
const enabled = process.env.CREW_E2E_RELAY_RESTART === "1";
...
async function runCli(args: string[], relayUrl: string, privateKey: string) {
  const binary = required("CREW_E2E_CLI_BIN", process.env.CREW_E2E_CLI_BIN);
  const { stdout } = await exec(binary, args, {
    cwd: "..",
    env: {
      ...process.env,
      CREW_AUTH_TAG: "",
      CREW_PRIVATE_KEY: privateKey,
      CREW_RELAY_URL: relayUrl,
    },
  });
  return stdout;
}
...
async function connectionState(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    return win.__CREW_E2E_GET_RELAY_CONNECTION_STATE__?.() ?? "uninstalled";
  });
}
...
test.describe("relay restart live gate", () => {
  test.skip(!enabled, "set CREW_E2E_RELAY_RESTART=1 to run live gate");
  ...
  test("client reconnects after the relay is SIGTERMed and restarted", async ({
    ...
    const spec: RelaySpec = {
      ...
      redisUrl:
        process.env.CREW_E2E_REDIS_RESTART ?? "redis://127.0.0.1:6379/13",
    };
