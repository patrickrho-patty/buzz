...
const enabled = process.env.CREW_E2E_AGENTS_EVERYWHERE === "1";
const relayBin = process.env.CREW_E2E_RELAY_BIN;
const cliBin = process.env.CREW_E2E_CLI_BIN;
const adminBin = process.env.CREW_E2E_ADMIN_BIN;
...
async function run(
  ...
  const { stdout } = await exec(binary, args, {
    cwd: "..",
    env: { ...process.env, CREW_AUTH_TAG: "", ...env },
  });
  return stdout;
}
...
test.describe("agents everywhere live two-relay gate", () => {
  test.skip(!enabled, "set CREW_E2E_AGENTS_EVERYWHERE=1 to run live gate");
    ...
    const specs: [RelaySpec, RelaySpec] = [
      {
        ...
        databaseUrl: required(
          "CREW_E2E_DATABASE_URL",
          process.env.CREW_E2E_DATABASE_URL,
        ),
        redisUrl: process.env.CREW_E2E_REDIS_A ?? "redis://127.0.0.1:6379/11",
      },
      {
        name: "relay-b",
        ports: {
        ...
        redisUrl: process.env.CREW_E2E_REDIS_B ?? "redis://127.0.0.1:6379/12",
      },
    ];
    const harness = await TwoRelayHarness.create(specs);
    try {
      await harness.startRelays(required("CREW_E2E_RELAY_BIN", relayBin));
      const senderOutput = await run(required("CREW_E2E_ADMIN_BIN", adminBin), [
        "generate-key",
      ]);
      const agentOutput = await run(required("CREW_E2E_ADMIN_BIN", adminBin), [
        "generate-key",
      ]);
      ...
      for (const relay of specs) {
        ...
        const senderEnv = {
          CREW_RELAY_URL: relayHttp,
          CREW_PRIVATE_KEY: senderKey,
        };
        const created = JSON.parse(
          await run(
            required("CREW_E2E_CLI_BIN", cliBin),
            [
              "channels",
              "create",
              "--name",
        ...
        await run(
          ...
          { CREW_RELAY_URL: relayHttp, CREW_PRIVATE_KEY: agentKey },
        );
      ...
      for (const { relay } of channels) {
        acpChildren.push(
          await harness.startAcp(
            ...
            agentKey,
            { CREW_ACP_RESPOND_TO: "anyone", CREW_ACP_NO_MEMORY: "true" },
          ),
        );
      }
      ...
      for (const { relay, id } of channels) {
        ...
        await run(
          ...
          { CREW_RELAY_URL: relayHttp, CREW_PRIVATE_KEY: senderKey },
        );
