  ...
  await page.evaluate((seeded) => {
    window.__CREW_E2E_SEED_MOCK_REMINDERS__?.(
      seeded as Parameters<
        NonNullable<typeof window.__CREW_E2E_SEED_MOCK_REMINDERS__>
      >[0],
    );
    window.__CREW_E2E_QUERY_CLIENT__?.invalidateQueries({
      queryKey: ["reminders"],
    });
  }, events);
}
...
const NOTIFICATION_SETTINGS_KEY = `crew-notification-settings.v2:${MOCK_PUBKEY}`;
