...
async function setChannelsReadError(page: Page, error: string | null) {
  await page.evaluate((nextError) => {
    const testWindow = window as Window & {
      __CREW_E2E__?: { mock?: { channelsReadError?: string } };
    };
    ...
    if (nextError === null) {
      delete testWindow.__CREW_E2E__.mock.channelsReadError;
      return;
    }

    testWindow.__CREW_E2E__.mock.channelsReadError = nextError;
  }, error);
}
...
async function setRelayConnectionState(
  ...
  if (state !== "connected") {
    ...
    await page.waitForFunction(() => {
      ...
      return (
        typeof win.__CREW_E2E_SET_RELAY_CONNECTION_STATE__ === "function" &&
        typeof win.__CREW_E2E_GET_RELAY_CONNECTION_STATE__ === "function" &&
        win.__CREW_E2E_GET_RELAY_CONNECTION_STATE__() === "connected"
      );
    });
  } else {
