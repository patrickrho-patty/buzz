...
test("Crew shared compute explains automatic model selection", async ({
  page,
}) => {
  await page.goto("/");
  await page.evaluate(() => {
...
test("create agent persists Crew shared compute with auto model", async ({
  page,
}) => {
...
test("create agent supports parallelism and system prompt overrides", async ({
  ...
  // The crew-agent runtime auto-selects once the ACP runtime catalog loads;
