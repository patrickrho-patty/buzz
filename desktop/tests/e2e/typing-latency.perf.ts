...
test("MEASURE: composer keystroke latency, quiet vs agent-busy channel", async ({
  ...
  await page.evaluate(
    ({ agentCount, typingIntervalMs, messageIntervalMs }) => {
      ...
      const typingTimer = window.setInterval(() => {
        tick += 1;
        w.__CREW_E2E_EMIT_MOCK_TYPING__?.({
          channelName: "agents",
      ...
      const messageTimer = window.setInterval(() => {
        messageIndex += 1;
        w.__CREW_E2E_EMIT_MOCK_MESSAGE__?.({
          channelName: "agents",
  ...
  await page.evaluate(
    ({ agentCount, bufferSize, channelId }) => {
      ...
      for (const agent of w.__OBS_AGENTS__) {
        w.__CREW_E2E_SEED_OBSERVER_EVENTS__?.({
  ...
  await page.evaluate(
    ({ streamCount, prefillLines }) => {
      ...
      for (let stream = 0; stream < streamCount; stream += 1) {
        const target = w.__CREW_E2E_EMIT_MOCK_MESSAGE__?.({
          channelName: "agents",
  ...
  const rootId = await page.evaluate(() => {
    ...
    const root = w.__CREW_E2E_EMIT_MOCK_MESSAGE__?.({
      channelName: "agents",
  ...
  await page.evaluate(
    ({ rootEventId }) => {
      ...
      const replyTimer = window.setInterval(() => {
        replyIndex += 1;
        const reply = w.__CREW_E2E_EMIT_MOCK_MESSAGE__?.({
          channelName: "agents",
