...
test("normal first launch uses the already-persisted identity", async ({
  ...
  const commands = await page.evaluate(
    () =>
      (
        window as Window & {
          __CREW_E2E_COMMAND_PAYLOADS__?: Array<{ command: string }>;
        }
      ).__CREW_E2E_COMMAND_PAYLOADS__ ?? [],
  );
...
test("lost boot offers phone recovery with a single-use QR", async ({
  ...
  await expect(
    page.getByText("Scan this code with a signed-in Crew phone."),
  ).toBeVisible();
  await expect(
...
test("phone recovery uses the desktop pairing card semantics", async ({
  ...
  await expect(qrCode.locator(".crew-qr-cell-reveal").first()).toHaveCSS(
    "animation-name",
    "crew-qr-cell-reveal",
  );
  ...
  await expect(
    card.getByText(
      "This gives this desktop permanent access to your Crew identity. Only continue if you trust it.",
    ),
  ).toBeVisible();
  await expect(
    ...
    .poll(() =>
      page.evaluate(
        () =>
          ...
          ).__CREW_E2E_COMMAND_PAYLOADS__?.some(
