...
async function openNostrBind(
  ...
  await page.waitForFunction(
    () =>
      ...
      ).__CREW_E2E_INVOKE_MOCK_COMMAND__ === "function",
  );
  ...
  await page.addInitScript(
    ({ fail }) => {
      const testWindow = window as Window & {
        __CREW_E2E_CLIPBOARD_TEXT__?: string;
      };
      Object.defineProperty(navigator, "clipboard", {
        ...
        value: {
          writeText: async (text: string) => {
            ...
            testWindow.__CREW_E2E_CLIPBOARD_TEXT__ = text;
          },
        },
      });
    },
...
async function installShakeCounter(page: Page) {
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __CREW_E2E_CODE_SHAKE_CALLS__?: number;
    };
    testWindow.__CREW_E2E_CODE_SHAKE_CALLS__ = 0;
    ...
    Element.prototype.animate = function animate(keyframes, options) {
      if (
        ...
        testWindow.__CREW_E2E_CODE_SHAKE_CALLS__ =
          (testWindow.__CREW_E2E_CODE_SHAKE_CALLS__ ?? 0) + 1;
      }
...
async function shakeCount(page: Page): Promise<number> {
  return page.evaluate(
    () =>
      ...
      ).__CREW_E2E_CODE_SHAKE_CALLS__ ?? 0,
  );
}
