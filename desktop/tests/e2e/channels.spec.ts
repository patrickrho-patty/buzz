...
async function hasOutgoingEventWithContent(
  ...
  return page.evaluate((expectedContent) => {
    return (
      ...
      ).__CREW_E2E_COMMAND_LOG__?.some((entry) => {
...
async function addGenericAgent(
  ...
  return page.evaluate(
    async ({ agentName, channelId }) => {
      ...
      ).__CREW_E2E_QUERY_CLIENT__?.invalidateQueries();
...
test("closes direct message results while opening", async ({ page }) => {
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __CREW_E2E__?: { mock?: { openDmDelayMs?: number } };
    };
    testWindow.__CREW_E2E__ ??= {};
    testWindow.__CREW_E2E__.mock ??= {};
    testWindow.__CREW_E2E__.mock.openDmDelayMs = 1_000;
  });
...
test("opens a sent direct message without waiting for a channel-list refresh", async ({
  ...
  await page.evaluate(() => {
    const testWindow = window as Window & {
      __CREW_E2E__?: { mock?: { channelsReadDelayMs?: number } };
    };
    testWindow.__CREW_E2E__ ??= {};
    testWindow.__CREW_E2E__.mock ??= {};
    testWindow.__CREW_E2E__.mock.channelsReadDelayMs = 3_000;
  });
...
test("profile renders live activity for a viewer-owned relay agent", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__CREW_E2E_SEED_ACTIVE_TURNS__ ===
      "function",
  );
  await page.evaluate(
    ({ agentPubkey, channelId }) => {
      const seedActiveTurns = (window as MockFeedWindow)
        .__CREW_E2E_SEED_ACTIVE_TURNS__;
      if (!seedActiveTurns) {
...
test("channel settings only prompt editors to add an empty description", async ({
  ...
  await page.waitForFunction(
    () =>
      typeof window.__CREW_E2E_MUTATE_CHANNEL__ === "function" &&
      typeof window.__CREW_E2E_INVALIDATE_CHANNELS__ === "function",
  );
  await page.evaluate(
    async ({ generalChannelId, randomChannelId }) => {
      ...
      bridge.__CREW_E2E_MUTATE_CHANNEL__({
...
async function seedHomeInboxMention(
  ...
  await page.waitForFunction(
    () =>
      typeof (window as MockFeedWindow).__CREW_E2E_PUSH_MOCK_FEED_ITEM__ ===
      "function",
  );
  ...
  await page.evaluate(
    ({
      ...
      const pushFeedItem = (window as MockFeedWindow)
        .__CREW_E2E_PUSH_MOCK_FEED_ITEM__;
      if (!pushFeedItem) {
...
test("Inbox All excludes generic channel traffic", async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    ...
    return typeof win.__CREW_E2E_PUSH_MOCK_FEED_ITEM__ === "function";
  });
...
test("Inbox type labels keep the same height with and without a channel chip", async ({
  ...
  await page.evaluate(
    ({
      ...
      const win = window as MockFeedWindow;
      const emitMessage = win.__CREW_E2E_EMIT_MOCK_MESSAGE__;
      const pushFeedItem = win.__CREW_E2E_PUSH_MOCK_FEED_ITEM__;
...
test("Inbox All never lists drafts and unread-only hides reminders", async ({
  ...
  await page.evaluate(
    async ({
      ...
      window.__CREW_E2E_SEED_MOCK_REMINDERS__?.([
        {
          id: reminderId,
...
test("stopping a managed bot in one community leaves its other communities running", async ({
  ...
  // (`seedDefaultCommunity` follows CREW_E2E_RELAY_URL); the agent also has a
  // live runtime in a second community on another relay.
  const activeRelayUrl = (
    process.env.CREW_E2E_RELAY_URL ?? "http://localhost:3000"
  ).replace(/^http/, "ws");
