...
test("machine onboarding: landing, backup, setup docked CTAs", async ({
  ...
  await expect(
    page.getByTestId("backup-options").locator(".crew-card-textured"),
  ).toHaveCount(0);
  ...
  await expect(passwordPanel).not.toHaveClass(/crew-card-textured/);
  ...
  await expect(generatorPopover).not.toHaveClass(/crew-card-textured/);
  await waitForAnimations(page);
