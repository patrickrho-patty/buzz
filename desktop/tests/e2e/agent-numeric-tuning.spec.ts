...
test("delayed_catalog_per_agent_saved_tuning_values_visible_then_structured_controls_appear", async ({
    ...
    managedAgents: [
      {
        ...
        channelNames: ["agents"],
        envVars: {
          CREW_AGENT_MAX_OUTPUT_TOKENS: "4096",
  ...
  // while the catalog hasn't settled: CREW_AGENT_MAX_OUTPUT_TOKENS and
  // CREW_AGENT_MAX_ROUNDS should appear in the env-vars editor.
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="CREW_AGENT_MAX_OUTPUT_TOKENS"]',
    ),
  ).toBeVisible();
  await expect(
    page.locator(
      'input[data-testid="env-vars-key"][value="CREW_AGENT_MAX_ROUNDS"]',
    ),
  ).toBeVisible();
