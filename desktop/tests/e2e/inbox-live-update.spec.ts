...
type MockWindow = Window & {
  ...
  __CREW_E2E_PUSH_MOCK_FEED_ITEM__?: (item: {
  ...
  __CREW_E2E_SCROLL_INTO_VIEW_COUNT__?: number;
  /** When set to an event ID string, get_event calls for that ID are deferred until __CREW_E2E_RELEASE_GET_EVENT__ is called. */
...
async function waitForBridgeReady(page: import("@playwright/test").Page) {
  await page.waitForFunction(() => {
    ...
    return (
      typeof win.__CREW_E2E_EMIT_MOCK_MESSAGE__ === "function" &&
      typeof win.__CREW_E2E_PUSH_MOCK_FEED_ITEM__ === "function" &&
      typeof win.__CREW_E2E_REPLACE_MOCK_FEED_ITEM__ === "function" &&
      typeof win.__CREW_E2E_RELEASE_GET_EVENT__ === "function" &&
      Array.isArray(win.__CREW_E2E_COMMAND_PAYLOADS__)
    );
  });
}
...
async function installScrollSpy(page: import("@playwright/test").Page) {
  await page.evaluate(() => {
    (window as MockWindow).__CREW_E2E_SCROLL_INTO_VIEW_COUNT__ = 0;
    ...
    Element.prototype.scrollIntoView = function (
      ...
      if (this.closest('[data-testid="home-inbox-detail"]')) {
        (window as MockWindow).__CREW_E2E_SCROLL_INTO_VIEW_COUNT__ =
          ((window as MockWindow).__CREW_E2E_SCROLL_INTO_VIEW_COUNT__ ?? 0) + 1;
      }
...
async function clearCommandPayloads(page: import("@playwright/test").Page) {
  await page.evaluate(() => {
    const win = window as MockWindow;
    if (win.__CREW_E2E_COMMAND_PAYLOADS__) {
      win.__CREW_E2E_COMMAND_PAYLOADS__.length = 0;
    }
  });
}
...
async function getLastSendPayload(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    const payloads = (window as MockWindow).__CREW_E2E_COMMAND_PAYLOADS__ ?? [];
...
async function seedNestedAnchor(page: import("@playwright/test").Page) {
  return page.evaluate(
    ({ channelId, currentPubkey, senderPubkey }) => {
      const win = window as MockWindow;
      const emit = win.__CREW_E2E_EMIT_MOCK_MESSAGE__;
      const push = win.__CREW_E2E_PUSH_MOCK_FEED_ITEM__;
...
async function injectNewerSibling(
  ...
  return page.evaluate(
    ({ channelId, currentPubkey, senderPubkey, rootEventId, oldAnchorId }) => {
      const win = window as MockWindow;
      const emit = win.__CREW_E2E_EMIT_MOCK_MESSAGE__;
      const replace = win.__CREW_E2E_REPLACE_MOCK_FEED_ITEM__;
...
test.describe("inbox stable-conversation regressions", () => {
  test("scroll and focused draft preserved; new representative row selected when live sibling displaces anchor", async ({
    ...
    await page.evaluate(
      ({ senderPubkey, rootId }) => {
        const emit = (window as MockWindow).__CREW_E2E_EMIT_MOCK_MESSAGE__;
        if (!emit) return;
        for (let i = 0; i < 30; i++) {
          emit({
            channelName: "general",
    ...
    await page.waitForFunction(() =>
      ((window as MockWindow).__CREW_E2E_COMMAND_PAYLOADS__ ?? []).some(
  ...
  test("cold recovery survives mid-flight feedItems update without being cancelled", async ({
    ...
    //   1. Enable __CREW_E2E_DEFER_GET_EVENT__ so get_event is held in-flight.
    ...
    await page.evaluate(
      ({ targetId }) => {
        (window as MockWindow).__CREW_E2E_DEFER_GET_EVENT__ = targetId;
        (window as MockWindow).__CREW_E2E_GET_EVENT_CALL_COUNT__ = 0;
      },
    ...
    await page.waitForFunction(
      () => (window as MockWindow).__CREW_E2E_GET_EVENT_CALL_COUNT__ === 1,
    );
    ...
    const released = await page.evaluate(
      () => (window as MockWindow).__CREW_E2E_RELEASE_GET_EVENT__?.() ?? 0,
    );
