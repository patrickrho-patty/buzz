...
test("custom personas share with people and keep export separate", async ({
  ...
  await expect(
    exportDialog.getByRole("button", { name: "Send in Crew" }),
  ).toHaveCount(0);
  ...
  await expect(bobChip).not.toHaveClass(/crew-poof-trigger/);
  await waitForAnimations(page);
  ...
  await expect(page.locator(".crew-poof-burst")).toHaveCount(0);
  await page.mouse.up();
