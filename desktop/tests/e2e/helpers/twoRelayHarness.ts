...
export class TwoRelayHarness {
  ...
  static async create(relays: readonly RelaySpec[]) {
    return new TwoRelayHarness(
      await mkdtemp(join(tmpdir(), "crew-ae-e2e-")),
      relays,
    );
  }

  async startRelays(binary = process.env.CREW_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("CREW_E2E_RELAY_BIN is required for the live gate");
    await Promise.all(
  ...
  async restartRelay(name: string, binary = process.env.CREW_E2E_RELAY_BIN) {
    if (!binary)
      throw new Error("CREW_E2E_RELAY_BIN is required for the live gate");
  ...
  async startAcp(
    ...
    const binary = process.env.CREW_E2E_ACP_BIN;
    if (!binary)
      throw new Error("CREW_E2E_ACP_BIN is required for the live gate");
    return this.spawnOwned(name, binary, [], {
      CREW_RELAY_URL: relayWsUrl,
      CREW_PRIVATE_KEY: privateKey,
      CREW_AUTH_TAG: "",
      CREW_ACP_LAZY_POOL: "true",
      CREW_ACP_AGENT_COMMAND: process.execPath,
      CREW_ACP_AGENT_ARGS: resolve("tests/e2e/fixtures/fake-acp-agent.mjs"),
      CREW_E2E_CLI_BIN: process.env.CREW_E2E_CLI_BIN,
      ...extraEnv,
    });
  }
  ...
  private async startRelay(binary: string, relay: RelaySpec) {
    const child = this.spawnOwned(relay.name, binary, [], {
      ...
      CREW_BIND_ADDR: `127.0.0.1:${relay.ports.main}`,
      CREW_HEALTH_PORT: String(relay.ports.health),
      CREW_METRICS_PORT: String(relay.ports.metrics),
