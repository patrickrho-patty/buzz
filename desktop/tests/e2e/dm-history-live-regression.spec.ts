...
const RELAY_HTTP = process.env.CREW_E2E_RELAY_URL ?? "http://localhost:3000";
...
async function cachedMessageIds(page: Page) {
  return page.evaluate((channelId) => {
    const client = window.__CREW_E2E_QUERY_CLIENT__ as unknown as {
      getQueryData: (
...
async function dmLiveRequestCount(page: Page) {
  return page.evaluate((channelId) => {
    let count = 0;
    for (const entry of window.__CREW_E2E_COMMAND_LOG__ ?? []) {
