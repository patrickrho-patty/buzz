...
test("locks viewport rubber-band outside conversation scrollers", async ({
  ...
  // Crew Term consumes wheel gestures as custom scrollback rather than through
...
test("locks horizontal viewport pan everywhere", async ({ page }) => {
  ...
  // space locked even if a future layout places Crew content inside it.
  await page.evaluate(() => {
    const terminal = document.querySelector<HTMLElement>(
      '[data-testid="terminal-wheel-target"]',
    );
    const crewContent = document.createElement("div");
    crewContent.dataset.testid = "concealed-terminal-dead-space";
    terminal?.append(crewContent);
    if (terminal) terminal.dataset.terminalOwner = "crew";
  });
  await expect(
