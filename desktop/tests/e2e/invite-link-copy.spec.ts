  ...
  await page.route("**/api/invites", async (route) => {
    ...
    await route.fulfill({
      ...
      json: {
        ...
        url: "crew://join?relay=wss%3A%2F%2Frelay.example.com&code=qr-download-test",
      },
      status: 200,
    });
  });
...
test("copies a freshly minted invite link from the link field", async ({
  ...
  await expect(page.getByTestId("invite-link-url")).toHaveValue(
    "crew://join?relay=wss%3A%2F%2Frelay.example.com&code=qr-download-test",
  );
  ...
  expect(payload).toEqual({
    text: "crew://join?relay=wss%3A%2F%2Frelay.example.com&code=qr-download-test",
  });
});
...
test("retries a failed invite-link generation", async ({ page }) => {
  ...
  await page.route("**/api/invites", async (route) => {
    ...
    await route.fulfill({
      ...
      json: {
        code: "retry-test",
        expires_at: Math.floor(Date.now() / 1000) + 86_400,
        url: "crew://join?relay=wss%3A%2F%2Frelay.example.com&code=retry-test",
      },
      status: 200,
    });
  });
  ...
  await expect(linkField).toHaveValue(
    "crew://join?relay=wss%3A%2F%2Frelay.example.com&code=retry-test",
  );
