...
//    Both CREW_UPDATER_PUBLIC_KEY and CREW_UPDATER_ENDPOINT are required -
...
const updaterPubkey = process.env.CREW_UPDATER_PUBLIC_KEY;
const updaterEndpoint = process.env.CREW_UPDATER_ENDPOINT;

const missing = [];
if (!updaterPubkey) missing.push("CREW_UPDATER_PUBLIC_KEY");
if (!updaterEndpoint) missing.push("CREW_UPDATER_ENDPOINT");
if (missing.length > 0) {
  console.error(
