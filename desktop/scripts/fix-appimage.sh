...
APP_BIN="$WORKDIR/squashfs-root/usr/bin/griddle-desktop"
if [[ ! -f "$APP_BIN" ]]; then
  echo "Error: app binary usr/bin/griddle-desktop not found — bundler layout changed; update fix-appimage.sh" >&2
  exit 1
fi
if [[ -e "$APP_BIN.bin" ]]; then
  echo "Error: usr/bin/griddle-desktop.bin already exists — shim already installed?" >&2
  exit 1
fi

# The real binary moves aside; griddle-desktop becomes a shim AppRun.wrapped execs.
...
exec -a "griddle-desktop" "$here/griddle-desktop.bin" "$@"
SHIM
chmod +x "$APP_BIN"
