...
APP_BIN="$WORKDIR/squashfs-root/usr/bin/crew-desktop"
if [[ ! -f "$APP_BIN" ]]; then
  echo "Error: app binary usr/bin/crew-desktop not found — bundler layout changed; update fix-appimage.sh" >&2
  exit 1
fi
if [[ -e "$APP_BIN.bin" ]]; then
  echo "Error: usr/bin/crew-desktop.bin already exists — shim already installed?" >&2
  exit 1
fi

# The real binary moves aside; crew-desktop becomes a shim AppRun.wrapped execs.
...
exec -a "crew-desktop" "$here/crew-desktop.bin" "$@"
SHIM
chmod +x "$APP_BIN"
